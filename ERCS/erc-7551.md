...
If `paused()` returns `true`, the function MUST return `false`. Access-control checks are out of scope for this function. Implementations MAY include balance-related checks and return `false` when balance conditions are not met.
...
Implementations MAY include [ERC-165](./eip-165.md) interface detection support for this standard.

The [ERC-165](./eip-165.md) interface identifier for this standard is `0x7233a14c`.

If an implementation uses [ERC-165](./eip-165.md) to signal support for other ERCs (e.g., [ERC-7943](./eip-7943.md)), it SHOULD also indicate support for this standard.

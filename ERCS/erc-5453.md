...
This EIP establish a general protocol for permitting approving function calls in the same transaction rely on [ERC-5750](./erc-5750.md).
Unlike a few prior art ([ERC-2612](./erc-2612.md) for [ERC-20](./erc-20.md), `ERC-4494` for [ERC-721](./erc-721.md) that
...
As specified in [ERC-5750 General Extensibility for Method Behaviors](./erc-5750.md), any compliant method that has an `bytes extraData` as its
last method designated for extending behaviors can conform to [ERC-5453](./erc-5453.md) as the way to indicate a permit from certain user.
...
8. Each `bytes sig` MUST be an ECDSA (secp256k1) signature using private key of signer whose corresponding address is `endorserAddress` signing `validityDigest` which is the a hashTypeDataV4 of [EIP-712](https://eips.ethereum.org/EIPS/eip-712) of hashStruct of `ValidityBound` data structure as followed:

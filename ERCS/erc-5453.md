...
This EIP establishes a general protocol for permitting and approving function calls in the same transaction relying on [ERC-5750](./erc-5750.md).
Unlike a few prior art ([ERC-2612](./erc-2612.md) for [ERC-20](./erc-20.md), [ERC-4494](./erc-4494.md) for [ERC-721](./erc-721.md) that
...
See [`IERC5453.sol`](../assets/erc-5453/IERC5453.sol).

### Behavior specification

As specified in [ERC-5750 General Extensibility for Method Behaviors](./erc-5750.md), any compliant method that has a `bytes extraData` parameter as its
last designated parameter for extending behaviors can conform to [ERC-5453](./erc-5453.md) as the way to indicate a permit from a certain user.

1. Any compliant method of this EIP MUST be a [ERC-5750](./erc-5750.md) compliant method.
...
8. Each `bytes sig` MUST be an ECDSA (secp256k1) signature using private key of signer whose corresponding address is `endorserAddress` signing `validityDigest` which is the hashTypeDataV4 of [EIP-712](https://eips.ethereum.org/EIPS/eip-712) of hashStruct of `ValidityBound` data structure as follows:
...
could balance a wider range of use cases. E.g. the same use cases of ERC-2612 and [ERC-4494](./erc-4494.md) can be supported by `ERC5453_TYPE_A`. And threshold approvals can be done via `ERC5453_TYPE_B`. More complicated approval types can also be extended by defining new `ERC5453_TYPE_?`

2. We chose to include both `validSince` and `validBy` to allow maximum flexibility in expiration. This can also be supported natively by the EVM if [ERC-5081](https://eips.ethereum.org/EIPS/eip-5081) is adopted, but [ERC-5081](https://eips.ethereum.org/EIPS/eip-5081) will not be adopted anytime soon, so we choose to add these two numbers in our protocol to allow
smart contract level support.

## Backwards Compatibility

The design assumes a `bytes calldata extraData` to maximize the flexibility of future extensions. This assumption is compatible with [ERC-721](./erc-721.md), [ERC-1155](./erc-1155.md) and many other ERC-track EIPs. Those that aren't, such as [ERC-20](./erc-20.md), can also be updated to support it, such as using a wrapper contract or proxy upgrade.
...
See [`AERC5453.sol`](../assets/erc-5453/AERC5453.sol)

### Reference Implementation of `EndorsableERC721`

Here is a reference implementation of `EndorsableERC721` that achieves similar behavior to [ERC-4494](./erc-4494.md).
...
See [`EndorsableERC721.sol`](../assets/erc-5453/EndorsableERC721.sol)
...
See [`ThresholdMultiSigForwarder.sol`](../assets/erc-5453/ThresholdMultiSigForwarder.sol)

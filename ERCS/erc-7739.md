...
### Signature verification workflow dispatch
...
This can be implemented by trying all of the workflows one by one, until one of them determines that the `signature` is valid for `hash`.

...
### Composition of [ERC-7739](./erc-7739.md) signatures is not possible

An "ERC-7739 account" (one that uses ERC-7739 as replay protection for ERC-1271 signatures) cannot have a signer that is another ERC-7739 account. This is because an ERC-7739 signature defines a `TypedDataSign` struct type with a member that has the type of the message being signed. If the message being signed is another `TypedDataSign` struct, the resulting EIP-712 message will contain in its body two separate `TypedDataSign` types with incompatible contents, something that can't be represented in an EIP-712 request.

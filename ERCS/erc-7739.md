...
author: vectorized (@vectorized), Sihoon Lee (@push0ebp), Francisco Giordano (@frangio), Hadrien Croubois (@Amxx), Im Juno (@junomonster), howydev (@howydev), 0xcuriousapple (@0xcuriousapple)
...
For safety, smart accounts are RECOMMENDED to treat the signature as invalid if any of the following is true:
...
Smart accounts SHOULD return `bytes4(0x77390001)` for `isValidSignature(0x7739773977397739773977397739773977397739773977397739773977397739, "Any Signature")`.
...
The fields of `eip712Domain` are flattened into the `TypedDataSign` structure to enable compatibility with certain Ethers JS versions.
...
### ERC-7739 support detection with `isValidSignature`

To facilitate easier implementation in modular smart accounts, we have decided to utilize the `isValidSignature` method to return a magic number instead of defining new functions.

### Rejecting `contentsName` beginning with lowercase `a-z`

The recommendation to reject `contentsName` that start with any lowercase `a-z` is to keep the specification as language agonstic and future-proof as possible. Atomic types such as `uint256` may be named differently in other languages (e.g. `u256`).

At the time of writing, we have not found any widely used production smart contracts that are incompatible with this decision.

## Backwards Compatibility

### Detection of previous ERC-7739 draft

In an earlier draft of ERC-7739, we have designated a `supportsNestedTypedDataSign()` function for support detection, which returns `bytes4(0xd620c85a)`.

...
such as [ERC-4337](./eip-4337) `UserOperation` or [RIP-7560](https://github.com/ethereum/RIPs/blob/master/RIPS/rip-7560.md) (Native Account Abstraction), which are enforced off-chain by a
...
These validation rules only apply to the transaction validation phase, not its entire execution.
...
However, the rules apply to any account-abstraction framework that uses EVM code to perform transaction validation and makes a distinction between validation (whether the operation is eligible for inclusion on the protocol level) and execution (onchain execution and payment) in a public mempool.
...
First, note that a violation of any validation rule by a UserOperation, should result in the UserOperation being dropped from the mempool and excluded from a bundle, as described below in detail.

**Network-wide rule** is a rule, that its violation by a UserOperation validation should result in a reputation damage for the peer bundler that sent this UserOperation in the p2p mempool (marking it as **spammer**)

**Local rule** is a rule that its enforcement depends on each bundler's local state, meaning that other bundlers may not agree on violations of this rule, as each has a different local state. 
...
1. **opsSeen**: a per-entity counter of how many times a unique valid `UserOperation` referencing this entity
   ...
   This includes `UserOperation` received via incoming RPC calls or through a P2P mempool protocol. More details on the [P2P protocol here]( https://github.com/eth-infinitism/bundler-spec/blob/main/p2p-specs/p2p-interface.md)
...
* Allowed access to the `EntryPoint` address:
    ...
    * **[OP-054]** Any other access to the `EntryPoint` (either CALL or EXT*) is forbidden.
...
* **[EREP-016]** An `aggregator` should not have its opsSeen incremented on failure of factory,  account or paymaster
...
* **[EREP-020]** A staked factory is "accountable" for account violating any of the validation rules. \
...
### Unstaked Entites Reputation Rules
...
The alt-mempool "topic" is a unique identifier. By convention, this is the IPFS hash of the document describing (in clear test and YAML file) the specifics of this alt mempool (See [Mempool-IDs](https://github.com/eth-infinitism/bundler-spec/blob/main/p2p-specs/p2p-interface.md#mempool-id))
...
This external source for UserOperations is either an end-user node (via RPC [ERC-7769](./eip-7769)) or another node in the p2p network [ERC-4337 p2p protoco](https://github.com/eth-infinitism/bundler-spec/blob/main/p2p-specs/p2p-interface.md)

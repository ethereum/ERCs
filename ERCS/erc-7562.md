...
such as [ERC-4337](./erc-4337.md) `UserOperation` or RIP-7560 (Native Account Abstraction), which are enforced off-chain by a
...
* **[OP-070]** Transient Storage slots defined in [EIP-1153](https://eips.ethereum.org/EIPS/eip-1153) and accessed using `TLOAD` (`0x5c`) and `TSTORE` (`0x5d`) opcodes
...
* **[EREP-070]** During bundle creation, if a staked entity reduce its validation gas by more than 10% 
    ...
    (as it might affect the gas calculation based on [EIP-7623](https://eips.ethereum.org/EIPS/eip-7623) )
...
* **[AUTH-010]** A UserOperation may only contain a single [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) authorization tuple.
...
This external source for UserOperations is either an end-user node (via RPC [ERC-7769](./erc-7769.md)) or another node in the p2p network.

...
| `SAME_SENDER_MEMPOOL_COUNT`          | 4                           | Prevent filling the mempool with operations by a single sender, which would enable a mass invalidation attack                  |
| `SAME_UNSTAKED_ENTITY_MEMPOOL_COUNT` | 10                          | Prevent filling the mempool with operations relying on a single entity, which would enable a mass invalidation attack          |
...
| `BAN_OPS_SEEN_PENALTY`               | 10000                       | This value provides a way for the bundler to penalize an entity that rejected a valid UserOp after it was included in a bundle |
...
7. **Spammer** - a P2P peer bundler that has sent one or multiple invalid `UserOperations` to the mempool in a way that may risk the availability of the network.
   Bundlers MUST disconnect from such peers, as described in the [Mempool Validation Rules](#mempool-validation-rules) section.

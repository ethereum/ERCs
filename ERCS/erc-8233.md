...
description: An interface for controller-scoped ERC-20 token custody with time-locked funds and account designation.
...
A second, often overlooked threat is the controller itself. Many production contracts use upgradability patterns such as [ERC-1822](./eip-1822.md)'s Universal Upgradeable Proxy (UUPS) or transparent proxies. This means the contract owner can push a new implementation at any time - intentionally or after their owner account is compromised - that redirects all held tokens. Because the Warden holds the tokens and enforces its own rules independently of the controller's logic, no controller upgrade can circumvent them.
...
<img src="../assets/erc-8233/figures/fund-state-machine.svg" alt="Fund state machine" width="600" />

The transition out of `Locked` or `Sealed` into `Withdrawing` is derived from `block.timestamp` and happens without a transaction. No event can mark it; see [Events](#events).
...
- MUST emit `WardenFundLocked`. Implementations that do not implement the Lock Extension MUST emit `maximum` equal to `expiry`.
...
- MUST emit `WardenTransferred`, including when `from == to`.
...
- MUST emit `WardenDesignatedBurned`.
...
- MUST emit `WardenAccountBurned`, reporting the available and designated balances separately, even when both are zero.
...
- MUST emit `WardenWithdrawn`, reporting the available and designated balances separately, even when both are zero (as for a second `withdraw` of the same account).
...
- MUST emit `WardenWithdrawn` with the `controller` argument, not `msg.sender`, so that the event is indistinguishable from one emitted by `withdraw`.
...
A compliant Warden MUST emit the following events. Every event is scoped by `(controller, fundId)`, and both MUST be indexed: the controller is `msg.sender` on every operation except `withdrawByRecipient` and so appears nowhere else in the log, and a `FundId` is unique only within a single controller.

| Event | Emitted when |
|-------|--------------|
| `WardenFundLocked` | `lock` activates a fund, transitioning `Inactive` to `Locked` |
| `WardenFundSealed` | `sealFund` seals a locked fund, transitioning `Locked` to `Sealed` |
| `WardenDeposited` | `deposit` credits an account's available balance |
| `WardenTransferred` | `transfer` moves available tokens between two accounts |
| `WardenDesignated` | `designate` commits available tokens to the account holder |
| `WardenDesignatedBurned` | `burnDesignated` destroys designated tokens |
| `WardenAccountBurned` | `burnAccount` destroys an entire account |
| `WardenWithdrawn` | `withdraw` or `withdrawByRecipient` empties an account |

Extensions define further events.

Each event MUST be emitted after the fund or account state has been updated and before any ERC-20 transfer the operation performs. This guarantees that the Warden's accounting event always precedes the corresponding ERC-20 `Transfer` event in the log, including when a re-entrant token calls back into the Warden.

Each event MUST be emitted even when the operation moves zero tokens, so that consumers can rely on exactly one event per successful call. This applies to a `transfer` where `from == to`, to `burnAccount` on an empty account, and to a repeated `withdraw` of an already-emptied account.

The `Locked` to `Withdrawing` and `Sealed` to `Withdrawing` transitions are derived from `block.timestamp` and occur without a transaction, so no event marks them. Consumers MUST derive the transition from the `expiry` reported by `WardenFundLocked`, or by `WardenLockExtended` where the Lock Extension is implemented.
...
/// @title IERC-8233 Secure Token Custody Warden
interface IWarden {
    // -------------------------------------------------------------------------
    // Types
    // -------------------------------------------------------------------------
    ...
    /// @notice Implementations without the Lock Extension emit `maximum` equal
    ///         to `expiry`. The later transition to `Withdrawing` is derived
    ///         from `expiry` and emits no event.
    ...
    event WardenFundSealed(address indexed controller, FundId indexed fundId);
    ...
    /// @notice `from` and `to` are unindexed: only three topics are available
    ///         and `controller` and `fundId` claim two of them.
...
- MUST emit `WardenLockExtended`, reporting both the previous and the new expiry so that consumers can invalidate the unlock time they had recorded.
...
- MUST emit `WardenFlowUpdated`.

```solidity
event WardenFlowUpdated(
    address indexed controller,
    FundId indexed fundId,
    AccountId from,
    AccountId to,
...
Lazy settlement moves tokens **without emitting an event**: it has no transaction of its own and is applied as a side effect of whichever operation triggers it. Consumers MUST recompute accrued amounts from the rate and the elapsed time, bounded by `fund.sealedAt` once the fund has been sealed. This is the same class of silently-derived state as the `Withdrawing` transition described in [Events](#events).
...
Events are normative rather than left to the implementer, because the Warden's identifiers are otherwise unobservable. The controller is `msg.sender` on every operation except `withdrawByRecipient`, so it appears in no return value and in no log unless the Warden puts it there, and a `FundId` is chosen by the controller and unique only within that controller. Without a standard event set no consumer could attribute a fund to its controller from chain data at all, and `transfer` and `designate`, which move no ERC-20 tokens, would be entirely invisible.

Only three topics are available per event, and `controller` and `fundId` claim two of them. For single-account operations the third holds `accountId`. `WardenTransferred` has four candidates and cannot fit both accounts, so `from` and `to` are placed in the data section rather than indexing one side of a transfer and not the other; consumers filter a fund's transfers by the two indexed topics and match accounts in the returned data.

An `AccountId` embeds the holder address in its high 20 bytes, but topic matching is exact over all 32 bytes. A consumer therefore cannot use topics alone to find every event for a given holder across unknown discriminators, and must instead ingest a controller's or fund's events and match the address prefix itself.

`WardenFundLocked` carries `maximum` even though the core `lock` takes no such parameter, where it equals `expiry`. Keeping the field in the core event, rather than adding it under the Lock Extension, means the event signature and therefore its topic hash is identical across core-only and extended implementations, so a consumer can decode both without knowing which extensions a deployment supports.

`WardenAccountBurned` and `WardenWithdrawn` report the available and designated balances separately. Neither operation takes an amount; both compute it and then delete the account. Reporting both keeps each event self-contained and preserves the distinction between available and designated tokens on which the rest of this specification depends.
...
MIT/Apache licensed reference implementation is available in the [assets folder](../assets/erc-8233/).

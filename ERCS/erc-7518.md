...
![image](../assets/eip-7518/exampleUsecase.svg)
...
![image](../assets/eip-7518/sequentialDiagram.svg)
...
Fractionalization in this standard is realized natively through [ERC-1155](./eip-1155.md) balances. For a given partition identified by tokenId, each unit of amount represents a fungible fraction of that partition’s underlying security. Issuers mint and burn units to manage supply. Transfers use `safeTransferFrom` and `safeBatchTransferFrom`, and must pass canTransfer. Locks and freezes reduce the transferable portion of balances, and `forceTransfer` operates proportionally on units. Partitions can be merged by issuing a new tokenId and allowing holders to deposit old partitions for one-to-one or proportional conversion, and can be split by minting new tokenIds with defined conversion rules. All compliance rules and restrictions apply identically to every fractional unit in a partition.

### Interoperability with other standard

This standard builds directly on [ERC-1155](./eip-1155.md), which already models fractional supply, batching, and balance-based accounting with broad ecosystem support. [ERC-3525](./eip-3525.md) was carefully evaluated but intentionally not adopted as a base. Its slot-and-value abstraction overlaps almost exactly with the partition and `amount` semantics defined here, yet inherits [ERC-721](./eip-721.md)’s single-token overhead and per-position statefulness, which are inefficient for large scale issuance and compliance checks. Implementers who require slot-style compatibility may optionally expose [ERC-3525](./eip-3525.md) through [ERC-165](./eip-165.md) by mapping `slot` to partition and `value` to `amount`. [ERC-6909](./eip-6909.md), while a minimal multi-token variant, omits partition metadata and compliance hooks and is therefore unsuitable for regulated assets. [ERC-1400](./eip-1400.md) introduced the notion of partitions for security tokens but tightly coupled compliance logic to issuance and static on-chain rules. [ERC-7518](./eip-7518.md) extends this lineage by representing partitions as [ERC-1155](./eip-1155.md) `tokenId`s, enabling dynamic minting, merging, and voucher based dynamic compliance that can evolve without redeployment. Overall, [ERC-7518](./eip-7518.md) uses [ERC-1155](./eip-1155.md) as its canonical substrate because it balances composability, efficiency, and regulatory flexibility, turning fractionalization into a compliance-preserving primitive rather than an auxiliary layer.
...
## Fractionalization and Semi-Fungibility

[ERC-7518](./eip-7518.md) implements fractionalization natively through its [ERC-1155](./eip-1155.md) foundation, where the amount parameter of each tokenId encodes quantized ownership of a partitioned asset. Each partition behaves as a composable security tranche whose fractional units are transferable yet bound by compliance gates such as canTransfer, lockTokens, and freezeAddress. This makes fractionalization not just a visual subdivision of value but a deterministic on-chain behavior where every fractional unit inherits all rights, restrictions, and payout entitlements of its parent partition. The model supports dynamic mergers or splits of partitions, allowing liquidity without losing compliance context. By embedding fractional semantics within the [ERC-1155](./eip-1155.md) balance structure instead of layering an external representation, [ERC-7518](./eip-7518.md) preserves fungibility where appropriate while maintaining explicit jurisdictional control at the partition level—turning fractionalization into a compliance-preserving primitive for regulated asset tokenization.

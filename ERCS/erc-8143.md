...
The smart credential MUST implement [ERC-165](./erc-165.md) and return true when `supportsInterface()` is called on it with the interface's ID, `0xd091187f`.
...
This specification does NOT standardize the key format or the return result. Credential issuers define their own keys and data formats. The key MAY use the [ERC-8119](./erc-8119.md) parameterized key format (`key:param` or `key/param`, no space after the separator), but this is not required. When credentials are stored offchain, the contract MAY revert with [ERC-3668](./erc-3668.md) `OffchainLookup` to request a gateway lookup. Compliant clients MUST follow the [ERC-3668](./erc-3668.md) client lookup protocol when handling such reverts.
...
1. Call the `getCredential` function, using [ERC-3668](./erc-3668.md) (some libraries do not use [ERC-3668](./erc-3668.md) by default and it is necessary to make a special function call to enable [ERC-3668](./erc-3668.md)), with a key.
...
**Example 1: Resolve KYC credential using [ERC-8119](./erc-8119.md) parameterized key with address**
```javascript
...
**Example 2: Resolve KYC credential using [ERC-8119](./erc-8119.md) parameterized key with name**
```javascript
...
**Example 5: Offchain credential with [ERC-3668](./erc-3668.md) callback**

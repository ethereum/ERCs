...
author: Yush G (@Divide-By-0) <aayushg@mit.edu>, Kobi Gurkan (@kobigurk), Richard Liu (@rrrliu), Vivek Bhupatiraju (@vb7401), Barry Whitehat (@barryWhiteHat), Sergey Kaunov (@skaunov)
...
Mind _domain separation tag_ (DST) incorporation: some parameters suits below omit `expand_message` step, hence it's expected that this role is fullfilled by a part inside a message itself.
...
#### Pallas
as defined at Pallas section of "Pasta Curves - Mina book".

Note that only V1 variant (see below) is defined with these parameters!

**Hash:** Poseidon as described at <Poseidon-hash.info> and defined at Instantiations section of "Poseidon hash - Mina book".

##### Hash-to-curve parameters

Mina relied on [^2] before the hashing to curve [IETF RFC 9380] was finished (see issue #756 in `o1-labs/o1js` Github repo), and the approach they use is vetted with the value locked in the system. "Nullifiers, Line by Line" section of "The Many Saints of Privacy: Nullifiers in o1js" in "Blog - o1Labs" reminds that DST should be incorporated in a message as noted above.

Therefore $\mathsf{htc}$ (see below) for this one *does _not_ comply to [IETF RFC 9380]* (mostly by omitting that `expand_message` step). The parameters for `map_to_curve` are generated by the `crypto_params` library from `mina` repository of MinaProtocol@GitHub.

#### BN254
(the curve parameters are at <https://github.com/noir-lang/noir_bigcurve/blob/v0.6.0/src/curves/bn254.nr>)

**Hash:** Poseidon-2 as defined at <https://github.com/noir-lang/noir/blame/master/acvm-repo/bn254_blackbox_solver/src/poseidon2.rs>.

##### Hash-to-curve parameters

This one is made by <https://github.com/signorecello>. It omits `expand_message` step -- see the DST incorporation notice above. 

The parameters for `map_to_curve` are from [Indifferentiable Hashing to Barreto–Naehrig Curves (Pierre-Alain Fouque and Mehdi Tibouchi)](../assets/eip-7524/latincrypt12.pdf).

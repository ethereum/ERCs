...
[Mina](TODO) relied on [^2] before the hashing to curve [IETF RFC 9380] was finished, and the approach they use is vetted with the value locked in the system. The process lacks `expand_message` step, hence (TODO check it in the blog page) there's no _domain separation tag_ incorporation, and it's expected that this role is fullfilled by a part inside a message itself.
...
This one is made by <https://github.com/signorecello> and (like the previous one by Mina) omits `expand_message` step from hashing to curve [IETF RFC 9380]. (Hence it also has the same thing "there's no _domain separation tag_ incorporation".)
...
### Hash-to-curve
We use the notation $\mathsf{htc}([a_1, ..., a_n])$ to denote the elliptic curve point which is the result of the [IETF RFC 9380]. Note that it doesn't mentioned Poseidon, so that should just conform to the process specified for the random-oracle approach.

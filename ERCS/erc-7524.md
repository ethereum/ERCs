...
This scheme uses the `secp256k1` curve, defined in [Standards for Efficient Cryptography 2 (SEC 2) v2](../assets/eip-7524/sec2-v2.pdf), page 9, and the Pallas curve defined at <https://o1-labs.github.io/proof-systems/specs/pasta.html#pallas>.
...
This scheme uses the SEC1 elliptic curve point encoding scheme defined in [Standards for Efficient Cryptography 1 (SEC 1) v2](../assets/eip-7524/sec1-v2.pdf). Point compression is used. We use the notation $\mathsf{sec1}(pk)$ to denote the compressed encoding of `secp256k1` curve point $pk$ as a bytestring of length 33.
...
Using each of the mentioned hash will produce different nullifiers which won't be compatible with each other.

#### SHA256

This scheme uses the SHA256 hash function defined in [IETF RFC 4634](https://www.rfc-editor.org/rfc/rfc4634). The output should then be interpreted as a big-endian representation of the respective scalar of the curve.

It can be denoted further as $\mathsf{sha256}(...)$ when it's about specifically SHA256. It's likely to be used when operating with `secp256k1`. 

#### Poseidon
Another hash function used is Poseidon as described at <Poseidon-hash.info> and defined at <https://o1-labs.github.io/proof-systems/specs/poseidon.html#instantiations>. This parameters are for the Pallas curve.

### Hash-to-curve

The main difference between the two hash-to-curve functions is their domain of operation: a bit-array for the one and field elements for the other. To go from bits to a curve point we employ the relevant procedure; to get to a curve point from a field element - Mina instantiation of the relevant research is used.
...
We use the notation $\mathsf{htg}([a_1, ..., a_n])$ to denote the elliptic curve point which is the result of hashing $n$ field elements $a_1, ..., a_n$ and then processing the digest with the instantiation of [SvdW06] for the Pallas curve. The parameters are available at <https://github.com/o1-labs/o1js-bindings/blob/b8c8a44e8fcd48bd9f0d1d5b0d94446f0f31cf3a/crypto/elliptic-curve.ts#L178>.
...
2. Compute $h = \mathsf{htc}([m, \mathsf{sec1}(pk)])$; or $\mathsf{htg}([m, pk])$.
3. Compute $z = h ^ r$.
...
1. Compute $h = \mathsf{htc}([m, \mathsf{sec1}(pk)])$; or $\mathsf{htg}([m, pk])$.

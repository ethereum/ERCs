---
eip: 7755
title: Kinetic Agent Reputation Score
description: An on-chain agent reputation system with weighted score breakdowns, oracle-attested updates, and a bonded registration mechanism.
author: Opacus Protocol (@bl10buer)
discussions-to: https://ethereum-magicians.org/t/erc-7755-kinetic-agent-reputation-score/23534
status: Draft
...
Each field is in basis points (0 – 10 000) and represents an independent sub-score. Each field MUST be in the range `[0, 10_000]`; the fields are NOT required to sum to `10_000`.
...
MUST compute `feeAmount = (gross * SCORE_FEE_BPS) / 10_000` and forward to the implementation-defined fee recipient.  
MUST store `bondNet = gross - feeAmount` for later refund.  
...
MUST revert if any breakdown field exceeds `10_000`.  
...
Compliant contracts MUST implement `IERC7755` and MUST return `true` from `supportsInterface` for `type(IERC7755).interfaceId`.
...
ERC-7755 works alongside [ERC-7757](./eip-7757.md) (H3 Routing). The `kineticScore` field in `AgentRecord` SHOULD be sourced from an ERC-7755-compliant contract.

## Reference Implementation

See `contracts/ERC7755KineticScore.sol` in the Opacus-Standart repository.
...
* **Score manipulation** – Oracles MUST aggregate data from multiple on-chain sources ([ERC-7750](./eip-7750.md) tasks, [ERC-7753](./eip-7753.md) escrows) rather than accepting user-reported values.

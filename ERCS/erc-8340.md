...
The `a`, `w`, `s`, `r`, and `m` keys are [ERC-8021](https://ethereum-magicians.org/t/erc-8021-transaction-attribution/25561) schema 2; this proposal reuses that vocabulary and registry without re-specifying it. The ERC-8021 calldata `ercMarker`, `schemaId` byte, and `cborLength` prefix are not used: the value is self-delimiting and length-known from the RLP field.
...
> **Note (non-normative):** In [ERC-8168](https://ethereum-magicians.org/t/erc-8168-payer-services-for-erc-8130/28762) sponsored flows: (1) app sends `wallet_sendCalls`; (2) payer may prepend a phase; (3) wallet resolves each annotation's final phase index; (4) wallet encodes `metadata` and signs.
...
A wallet receiving a legacy `dataSuffix` MAY carry a recognized [ERC-8021](https://ethereum-magicians.org/t/erc-8021-transaction-attribution/25561) attribution suffix as the corresponding attribution map, or other bytes as a foreign encoding.
...
[ERC-8021](https://ethereum-magicians.org/t/erc-8021-transaction-attribution/25561) schema 2 (`a`, `w`, `s`, `r`, `m`) is a well-specified, registry-backed attribution vocabulary. Reusing it means existing registries and parsers apply without translation. This proposal adds only `t`, `h`, `p`, and `c`, the keys schema 2 lacks for [EIP-8130](./eip-8130.md).
...
This proposal applies only to the [EIP-8130](./eip-8130.md) `metadata` field. Indexers SHOULD continue parsing trailing-bytes data suffixes on legacy transactions. An [ERC-8021](https://ethereum-magicians.org/t/erc-8021-transaction-attribution/25561) schema 2 map previously carried as a calldata suffix becomes the `metadata` value directly, with the `ercMarker`, `schemaId`, and length prefix dropped.

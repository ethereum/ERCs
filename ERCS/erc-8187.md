...
category: ERC
created: 2026-02-27
requires: 20, 712, 2612, 5267, 6492
---
...
- Off-chain [EIP-712](./eip-712.md) signed permits (with [ERC-6492](./eip-6492.md) universal signature validation)
...
Current token approval standards ([ERC-20](./eip-20.md) `approve`/`transferFrom`, [ERC-2612](./eip-2612.md) permits) require owners to hold liquid balances and often involve multiple transactions or direct balance pulls. This creates friction and risks:

- Owners forgo yield from invested positions (ERC-4626 vaults or other DeFi protocols)
...
- Digest computation:
    ...
    ```
- SHOULD validate the signature following [ERC-6492](./eip-6492.md) rules (EOA via `ecrecover`, [ERC-1271](./eip-1271.md) contracts, pre-deploy via magic suffix).
...
Implementations MUST expose `nonces(owner)` as described in [ERC-2612](./eip-2612.md).
...
- Allowance transfer enables refill patterns (guardian refilling sub-allowances), but a compromised spender can redirect its allowance to arbitrary addresses. The same trade-offs between infinite and finite allowances that apply to ERC-20 also apply here: infinite approvals improve UX but increase damage potential if the spender is compromised.

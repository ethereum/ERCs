...
The reference implementation includes binary proof fixtures in [`test/fixtures/`](https://github.com/xochi-fi/erc-8262/tree/main/test/fixtures) for the six unsigned proof types. Static fixtures are not provided for the three signed variants (COMPLIANCE_SIGNED, RISK_SCORE_SIGNED, COMPLIANCE_MULTI_SIGNED) because each requires a fresh secp256k1 ECDSA witness; those are exercised end-to-end in the TypeScript SDK consumer tests instead. Each unsigned fixture contains:
...
All fixtures use Pedersen hash (Noir stdlib) for in-circuit commitments and Merkle tree construction. Fixtures can be regenerated via [`scripts/generate-fixtures.sh`](https://github.com/xochi-fi/erc-8262/blob/main/scripts/generate-fixtures.sh).
...
Copyright and related rights waived via [CC0](https://github.com/ethereum/EIPs/blob/master/LICENSE).

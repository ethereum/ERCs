...
This EIP defines an interface for fungible tokens that operate in two modes: transparent mode (fully compatible with [ERC-20](./erc-20.md)) and privacy mode (using zero-knowledge proofs). Token holders can convert balances between modes. The transparent mode uses account-based balances, while the privacy mode uses cryptographic commitments stored in a Merkle tree. Total supply is maintained as the sum of both modes.
...
This standard is fully backward compatible with [ERC-20](./erc-20.md):

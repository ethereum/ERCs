...
Where passes expose state-changing links, implementations in the public configuration SHOULD also rotate acquisition URLs on transfer, so that a pass held by a previous owner stops granting the new owner's links. Implementations SHOULD rotate acquisition URLs on explicit owner request, and in the capability configuration defined under Authorization of pass-reachable actions they MUST: under an unchanged owner, rotation on request is the only remedy for a leaked link.
...
- A request for the manifest that carries no control proof MUST be answered with HTTP status 401 and a JSON body whose `error` member is `"proof_required"` and whose `challenge` member is the URI of the challenge endpoint for that token. The body MUST NOT contain acquisition URLs. Any other 401 answer to the manifest request SHOULD carry the same `challenge` member, since the client's next step is the same fresh challenge.
- The challenge endpoint MUST accept a GET request carrying the claimed account in the `address` query parameter, and MUST return a JSON body whose `message` member is a challenge for that account that meets the floor defined under Authorization of pass-reachable actions, with the action identified as `urn:wallet-pass:action:acquire`. Because the nonce is single-use, every request MUST issue a fresh challenge. A request whose `address` is missing or is not a valid account address MUST be refused with HTTP status 400.
...
- The verifier MUST check the proof against the floor and MUST take the fresh entitlement read of check (2). An `acquire` proof MUST NOT authorize any other action, and a proof for any other action MUST NOT resolve the manifest. A proof that fails any floor check MUST be refused. A proof that verifies but comes from an account that is not entitled MUST be refused with HTTP status 403, and that status MUST NOT be used for any other refusal, so that a client can tell an unusable proof from a refused account. A verified request MUST be answered with the manifest and the header `Cache-Control: no-store`.
...
- the URL rotates on transfer per Acquisition URLs (upon observation of the transfer or the new owner's first claim, not synchronously), and MUST also rotate on the owner's request;
- the reachable action MUST NOT transfer, burn, or approve the token, and MUST NOT change who is entitled to it;
- the effect of the action under unlimited repetition MUST be bounded, including any value it moves from the token or from an account the token controls, and the implementation MUST document that bound;
- the authority the link exercises SHOULD be limited on chain to the function the link invokes, for example through a session key scoped to that function; and
...
What the substitution gives up: with no per-action signature, forwarding is not closed. Any party holding the capability URL can trigger the bound action while ownership is unchanged. Unguessability does not help once the URL has leaked, rotation does not fire absent a transfer, and the fresh entitlement read refuses a sold token, not a forwarded URL under an unchanged owner. The configuration accepts this residual and bounds it. Because a capability link is reusable, the worst case under forwarding is the bound action repeated until something rotates the link, and under an unchanged owner only the owner can cause that rotation; the conditions above fix what the repetition can do (it cannot change ownership, entitlement, or approvals, and its total effect is bounded and documented) and guarantee the owner the remedy. An action whose repeated effect cannot be bounded that way MUST NOT be reachable through a capability link and requires the signed challenge.
...
- A wallet pass is a projection of the token, not the token. Implementations MUST NOT treat possession of a pass file, or the ability to add it to a device wallet, as proof of ownership of the underlying token, and MUST NOT authorize a state-changing action by pass possession alone. This holds in the capability configuration as well: there the capability URL carries the possession role under the stated conditions, and the fresh entitlement read of check (2) is still required.
...
The reference implementation carries executable tests. The [contract suite](../assets/eip-0/reference/contracts/test/ERC721WalletPass.t.sol) asserts that `type(IERC721WalletPass).interfaceId` equals `0xef5f1e71` and covers `passURI` and both events. The [action suite](../assets/eip-0/reference/server/test/action.test.ts) and the [manifest suite](../assets/eip-0/reference/server/test/manifest.test.ts) exercise the authorization floor one property at a time: a replayed nonce, an expired challenge, a different verifier, a proof for another action or token, a proof scoped to a chain or contract the verifier does not serve, a malformed signature, a signature from the wrong account, and an ownership change between challenge and action are each rejected, and an ownership read that cannot be taken is refused as retryable rather than as a refusal of the account; a gated manifest is refused without a control proof and its response points at the challenge endpoint, whose acquire challenge then resolves the manifest; acquisition URLs rotate on a new owner's first claim and not on a repeat claim; and a rotated capability URL stops resolving. The [capability suite](../assets/eip-0/reference/server/test/capability.test.ts) puts the capability configuration on the same footing, one test per condition: a link bound to one action is refused for another, a link rotated on transfer or on the owner's request is refused, a live link behind a sold token is refused by the fresh read, a failed read is answered as retryable, no action link exists in the public configuration, a GET on a link is side-effect free, and a forwarded link under an unchanged owner succeeds, asserted deliberately as the residual the specification discloses.

## Reference Implementation

A [reference implementation](../assets/eip-0/README.md) is included in the assets directory under CC0. It consists of a minimal ERC-721 implementing the interface ([`ERC721WalletPass.sol`](../assets/eip-0/reference/contracts/src/ERC721WalletPass.sol)) and an off-chain server that issues and verifies ERC-4361 challenges against the floor ([`authorize.ts`](../assets/eip-0/reference/server/src/authorize.ts)), serves the manifest in the public and gated configurations, rotates capability URLs on transfer and on the owner's signed request ([`passStore.ts`](../assets/eip-0/reference/server/src/passStore.ts)), and authorizes actions reached through capability links under the capability configuration's conditions ([`capability.ts`](../assets/eip-0/reference/server/src/capability.ts)).

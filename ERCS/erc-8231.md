...
discussions-to: https://ethereum-magicians.org/t/erc-8231-post-quantum-key-registry/28235
status: Draft
...
For the transition period where both classical and PQ signatures coexist. Both signatures MUST sign the same [EIP-712](./eip-712.md) digest:
...
A reference implementation is published at `Valisthea/styx-erc-pq-key-registry`:

- `src/StyxPQKeyRegistry.sol` — the core registry
- `src/extensions/StyxPQKeyRegistryAttestation.sol` — the attestation extension
- `src/extensions/StyxPQKeyRegistryDualSign.sol` — the dual-signature extension
- `src/libraries/PQAlgorithms.sol` — algorithm identifiers and parameter sizes
- `test/` — a Foundry suite covering the key lifecycle, proof handling, edge cases and capability limits

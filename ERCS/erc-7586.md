...
Every contract compliant with this ERC MUST implement the following Interface. The contract MUST inherit both the [ERC20](./eip-20) and [ERC721](./eip-721) to tokenize the notional amount and swap cash flows respectively.
...
The notional amount of the IRS MUST be tokenized by creating digital ERC20 tokens. These tokens should be issued to the parties involved in the swap, reflecting their ownership or entitlement to the notional amount. it is RECOMMENDED not to transfer these tokens when swapping interest rates. These tokens MUST be burned during the last swap.

### Tokenization of Swap Cash Flows

The interest payments associated with the IRS MUST be tokenized by issuing digital ERC721 tokens to the respective parties according to the terms of the swap. Each token SHOULD represent a specific interest payment. Everytime a swap happens (the `swap` function is called), one token MUST be burned from each party.

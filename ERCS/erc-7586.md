...
interface IERC7586 /** is ERC20, ERC165 */ {
    ...
    */
    event Settle(uint256 _amount, address _account);
    ...
    */
    function fixRatePayer() external view returns(address);
    ...
    */
    ...
    /**
    * @notice Returns an array of acceptable contract addresses of the asset to be transferred when swapping IRS. Depending on what the two parties agreed upon, this could be a currency, etc.
    *         Example: If the two parties agreed to swap interest rates in USDC only, then this function should return a 1D-array that stores USDC contract address => [USDC_address].
    *                  If the two parties agreed to swap interest rates in USDC and DAI, then this function should return a 2D-array that stores USDC and DAI contract address => [USDC_address, DAI].
    *                  These addresses SHOULD be used in the `swap` function to transfer the interest to either the `payer` or the `receiver`. Example: IERC(assetContract).transfer
    */
    function assetContracts() external view returns(address[] memory);
    ...
    */
    ...
    /**
    *  @notice Returns an array of acceptable benchmark in basis point unit
    *          Example: 1- [CF BIRC, EURIBOR, HIBOR, SHIBOR, SOFR, SONIA, TONAR], 2- [SOFR, EURIBOR], 3- [SHIBOR]
    */
    function benchmarks() external view returns(uint256[] memory);

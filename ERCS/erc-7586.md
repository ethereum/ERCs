...
![alt text](../assets/erc-7586/irs.jpeg "IRS diagram")

Every contract compliant with this ERC MUST implement the following interface. The contract MUST inherit from [ERC-20](./erc-20.md) to tokenize the swap cash flows.
...
The interest payments associated with the IRS MUST be tokenized by issuing digital [ERC-20](./erc-20.md) tokens to the respective parties according to the terms of the swap. Each token SHOULD represent a specific interest payment. Every time a swap happens (the `swap` function is called), one token MUST be burned from each party.

...
A set of functions to enable meta-transactions and atomic interactions with contracts implementing an operator model, via signatures conforming to the [EIP-712](https://eips.ethereum.org/EIPS/eip-712) typed message signing specification.
...
[EIP-6909](./erc-6909.md) and [EIP-7540](./erc-7540.md) already implement this interface.

The naming of the arguments is interchangeable, e.g. [EIP-6909](./erc-6909.md) uses `spender` instead of `operator`.

### Methods

#### `authorizeOperator`

Grants or revokes permissions for `operator` to manage Requests on behalf of the `msg.sender`, using an [EIP-712](https://eips.ethereum.org/EIPS/eip-712) signature.

MUST revert if the `deadline` has passed.

MUST invalidate the nonce of the signature to prevent message replay.

MUST revert if the `signature` is not a valid [EIP-712](https://eips.ethereum.org/EIPS/eip-712) signature, with the given input parameters.
...
### Similarity to [ERC-2612](./erc-2612.md)

The specification is intentionally designed to closely match [ERC-2612](./erc-2612.md). This should simplify new integrations of the standard.

...
In the context of the [ERC-6860](./erc-6860.md) `web3://` standard, this ERC extends the [ERC-6944](./erc-6944.md) resolve mode. It introduces mechanisms to address limitations that prevent the use of standard [RFC 9111](https://www.rfc-editor.org/rfc/rfc9111) HTTP caching.
...
Therefore, it makes sense to implement caching mechanisms to reduce RPC calls when possible. Since `web3://` aims to be as close to HTTP as possible, leveraging standard [RFC 9111](https://www.rfc-editor.org/rfc/rfc9111) HTTP caching is the natural choice. In the [ERC-6944](./erc-6944.md) resolve mode, smart contracts can already return standard HTTP caching headers like `Cache-Control` and `ETag`.

However, due to the [ERC-6944](./erc-6944.md) resolve mode not forwarding request HTTP headers to the smart contract, smart contracts cannot handle `If-None-Match` and `If-Modified-Since` cache validation headers. Consequently, they are limited to using the `Cache-control: max-age=XX` mechanism, which causes each cache validation request to trigger an RPC call, regenerating the full response.
...
We add this feature to the [ERC-6944](./erc-6944.md) resolve mode because it can be added without changes to the interface. Future resolve modes that allow for request HTTP headers may also implement this ERC.
...
When an [ERC-6944](./erc-6944.md) resolve mode website wants to use event-based caching for a request, it MUST:
...
This event clears the cache for an array of `paths`. Each `path` refers to the `pathQuery` part of the ABNF definition in [ERC-6860](./erc-6860.md).

- A `path` MUST NOT end with a `/`, except when the whole path is the root path, which is `/`.
- Two `paths` are considered identical if they have the same [ERC-5219](./erc-5219.md) resource entries and their parameter values match, regardless of the order.
...
1. **Wildcards in Resource Entries**:
   - A wildcard (`*`) can be used on its own in an [ERC-5219](./erc-5219.md) resource entry.
...
Additional elements can be included in the mapping key when necessary. For example, [ERC-7618](./erc-7618.md) requires the inclusion of the `Accept-Encoding` request header in the mapping key.

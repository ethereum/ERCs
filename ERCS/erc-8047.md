...
- When mint a token, the `root` property of the new token MUST be set to its own `id`.
- When mint a token, the `parent` property of the new token token MUST be set to zero to explicitly indicate that the token serves as the `root` of a new DAG.

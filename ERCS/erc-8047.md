...
author: Sirawit Techavanitch (sirawit_tec@live4.utcc.ac.th), Supache Innet (supachate_inn@utcc.ac.th)
...
- The `TokenCreated` event MUST emit with `root` set to zero when minting a new root token, enabling off-chain indexers to identify and enumerate all DAG origins by filtering on `root` equal to zero.

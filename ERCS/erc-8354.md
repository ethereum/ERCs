...
2. `domain(v.domainId).active` is true, else `DomainInactive`.
3. `v.decision == 1`, else `VerdictDenied`.
4. The caller is authorized as executor: `v.executor == msg.sender` directly, or `executorAuth` is a valid [EIP-712](./eip-712.md) signature by `v.executor` over `verdictDigest(v)`. Else `ExecutorMismatch` (no signature and `msg.sender != v.executor`) or `ExecutorAuthInvalid` (a signature was given but does not validate).
5. `block.timestamp < v.expiry`, else `VerdictExpired`.
6. `isConsumed(v.domainId, v.nullifier)` is false, else `VerdictReplayed`.
7. `isRootAcceptable(v.domainId, v.policyRoot)` is true, else `PolicyRootRejected`.
8. The proof verifies against `domain(v.domainId).programKey` with `v` as public inputs, else `InvalidProof`. A verifier that reverts MUST surface as `InvalidProof`, not as the verifier's own error.

Check 4 is not optional. See Security Considerations.
...
<!-- Still to land: this case is structural rather than runtime-checked, so it is stated as an
     input/output pair rather than excerpted from the Foundry suite. -->
7. **Cross-chain / cross-domain replay** -- the property follows from the `actionCommitment` preimage itself. Given identical `(agentId, target, value, callData, actionNonce)`, computing `actionCommitment` with `chainId = 1, domainId = D1` and with `chainId = 2, domainId = D1` yields different `actionCommitment` values (the `PolicyAction.commit` preimage in [`PolicyAction.sol`](../assets/eip-8354/src/PolicyAction.sol) includes both as leading fields), so a verdict minted for one chain or domain is never accepted as the correct commitment on another.

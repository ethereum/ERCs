...
///      is excluded per the language rule. Value: 0x6c832e88.
...
3. The caller is authorized as executor: `v.executor == msg.sender` directly, or `executorAuth` is a valid EIP-712 signature by `v.executor` over `verdictDigest(v)`. Else `ExecutorMismatch` (no signature and `msg.sender != v.executor`) or `ExecutorAuthInvalid` (a signature was given but does not validate).
...
A conformant implementation MUST pass at least the following, expressed against `consume` unless noted. Each case below is excerpted, unmodified, from the Foundry suite at [`../assets/erc-8354/test/ConfidentialPolicyVerdict.t.sol`](../assets/erc-8354/test/ConfidentialPolicyVerdict.t.sol), which runs against the shared fixture:
...
function _verdict() internal view returns (Verdict memory v) {
    v = Verdict({
        agentId: 1, domainId: DOMAIN, policyRoot: ROOT, actionCommitment: keccak256("action"),
        executor: EXECUTOR, expiry: uint64(block.timestamp + 1 hours), nullifier: keccak256("nf-1"), decision: 1
    });
}
```
...
6. **Action binding** -- a verdict whose `actionCommitment` does not match the action a guarded contract is about to execute reverts at the guarded contract, before `consume` is ever called:
...
7. **Cross-chain / cross-domain replay** -- not yet exercised as a Foundry test in the reference suite; the property is structural, not runtime-checked, because it follows from the `actionCommitment` preimage itself. Given identical `(agentId, target, value, callData, actionNonce)`: computing `actionCommitment` with `chainId = 1, domainId = D1` and with `chainId = 2, domainId = D1` MUST yield different `actionCommitment` values (the `PolicyAction.commit` preimage in [`PolicyAction.sol`](../assets/erc-8354/src/PolicyAction.sol) includes both as leading fields), so a verdict minted for one chain or domain is never accepted as the correct commitment on another.
...
A reference implementation is provided in `assets/erc-8354` (CC0), implemented and tested with Foundry and Noir. The Test Cases above, the relayed-consume path, `supportsInterface`, and the ERC-8004 attestation handoff all run as an executable suite:

- [`ConfidentialPolicyVerdict.sol`](../assets/erc-8354/src/ConfidentialPolicyVerdict.sol) -- the Guard (`verify` / both `consume` overloads / `isConsumed`), checks in the order specified, and [`PolicyDomainRegistry.sol`](../assets/erc-8354/src/PolicyDomainRegistry.sol), the companion registry (root rotation with `maxRootAge` grace, immediate revocation).
- A proving-system-agnostic [`IVerifier.sol`](../assets/erc-8354/src/IVerifier.sol) boundary (`verifyProof(programKey, publicInputs, proof)`), with [`MockVerifier.sol`](../assets/erc-8354/src/mocks/MockVerifier.sol) as the test double the Guard suite runs against.
- [`PolicyAction.sol`](../assets/erc-8354/src/PolicyAction.sol) -- the canonical, chain- and domain-separated action-commitment preimage, hashed identically on-chain and in-circuit.
...
- [`GuardedExecutor.sol`](../assets/erc-8354/src/GuardedExecutor.sol) -- an example guarded contract that recomputes the canonical commitment before dispatching the action.
- [`IPolicyAttestation.sol`](../assets/erc-8354/src/IPolicyAttestation.sol) -- the `VerdictAttestation` payload, with [`MockValidationRegistry.sol`](../assets/erc-8354/src/mocks/MockValidationRegistry.sol) as the ERC-8004 handoff's test double.
- [ERC-165](./eip-165.md) support. `IConfidentialPolicyVerdict` `interfaceId` is `0x6c832e88`.
- An interpreter circuit written in [Noir](../assets/erc-8354/circuits/src/main.nr), proven with Barretenberg/UltraHonk (`nargo test` passes). It enforces the action-commitment, nullifier, and in-circuit executor-binding obligations from **Proving program contract**. Two things it does not yet do: its policy-membership check is a directly-computed allowlist Merkle root rather than an ERC-7812 `EvidenceDB` inclusion proof of a `policyCommitment`, and its in-circuit hashing (the allowlist tree and the nullifier) uses Pedersen rather than the Poseidon this specification requires for the deployed ERC-7812 registry (see **Hash functions**). Wiring the circuit to the real ERC-7812 root and aligning its hash choice are open work, not a difference in design intent.

### Verdict envelope

The [`Verdict`](../assets/erc-8354/src/IConfidentialPolicyVerdict.sol) struct and the `IConfidentialPolicyVerdict` interface together form the normative core, and the implementation carries the exact interface described in the Specification, including the [EIP-712](./eip-712.md) relayed-consume path:
...
The canonical [`PolicyAction`](../assets/erc-8354/src/PolicyAction.sol) struct and `PolicyActionLib` library define the commitment preimage that BOTH the on-chain guarded contract and the proving program hash with keccak256 over the identical field ordering:
...
[`GuardedExecutor.sol`](../assets/erc-8354/src/GuardedExecutor.sol) recomputes the canonical commitment, consumes the verdict, and executes. The executor question is resolved cryptographically: pass `executorAuth = ""` for direct submission (`v.executor == this`), or pass an [EIP-712](./eip-712.md) signature by `v.executor` for relayed submission. The excerpt below elides the custom errors and the nonce lookup helper present in the full contract:
...
After a successful `consume`, a guarded contract MAY write an attestation to the ERC-8004 Validation Registry. The [`VerdictAttestation`](../assets/erc-8354/src/IPolicyAttestation.sol) struct and `PolicyAttestation` library produce the canonical payload:
...
`actionCommitment` binds `block.chainid`, so a verdict for one chain cannot be replayed on another. It binds `domainId`, so a verdict minted under one policy domain cannot be replayed as authorization for the identical action under another domain sharing the same Guard. It binds `actionNonce`, so two identical actions do not collide. Omitting any of these fields reintroduces replay across chains, across domains, or across repeats. All are REQUIRED.

### Nullifier derivation must be in-circuit

If the nullifier is supplied rather than derived, a domain can mint many nullifiers for one action, defeating single use. The program MUST compute it from `(domainId, agentId, actionCommitment, actionNonce)`, reducing the 256-bit `actionCommitment` into field-sized limbs first (see Specification).
...
This ERC requires [ERC-7812](./eip-7812.md) and [ERC-8004](./eip-8004.md), both recent and pre-Final at the time of writing. A Standards Track ERC MAY sit in Draft or Review atop pre-Final dependencies, but it cannot advance to Final until they do. implementers SHOULD pin the exact versions of the dependencies they rely on.

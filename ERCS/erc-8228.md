...
The normative interfaces are published at `Valisthea/styx-erc-cryptographic-amnesia`:

- `contracts/interfaces/IERC8228.sol` — the core destruction ceremony interface
- `contracts/interfaces/IERC8228_Compliance.sol` — audit trail and compliance metadata for amnesia events
- `contracts/interfaces/IERC8228_Hooks.sol` — the interface hook contracts implement to receive ceremony notifications

A working implementation of the ceremony coordinator and the destruction proof verifier is in development and is not yet available. This section will be updated when it exists and is tested.

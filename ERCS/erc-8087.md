...
with an encrypted argument descriptor (*encrypted arguments*), which is fulfilled if admissible.

The *argument descriptor* may contain a list of contracts eligible to request decryption.

The target contract or the call may store a hash commitment to the plaintext arguments
and later verifies that the decrypted argument payload matches the stored commitment.

An off-chain *call decryption oracle* listens to standardized events, decrypts payloads, enforces access-control policy,
...
The decrypted argumentes are transported as `(uint256 requestId, bytes argsPlain`) to allow correlating the call
to the request. Here `argsPlain` is an ABI-encode of a type argument list (depending on the business logic).
...
where `argsPlain` is the an `abi.encode(args...)` byte sequence and part of the `ArgumentDecriptor` (see below).
...
interface ICallDecryptionOracle {
     ...
     * - verify that keccak256(argsPlain) equals the stored argsHash,
     * - perform low-level call:
     *     callDescriptor.targetContract.call(abi.encodeWithSelector(callDescriptor.selector, requestId, argsPlain))
     * - emit CallFulfilled(requestId, success, returnData),
     * - clean up stored state for this requestId.
     */
...
> for `fulfill*` (e.g. `onlyOracle`).
> Implementations MUST ensure that only the intended oracle operator can call the `fulfill*` functions.
...
function registerArguments(uint256 requestId, bytes32 argsHash) external {
    argsHashByRequestId[clientId] = argsHash;
}

/**
 * @dev Taget selector: Called by the Call Decryption Oracle with decrypted arguments.

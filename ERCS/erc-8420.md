...
description: AA-compatible Solver contracts for coordinating existing account execution and authorization capabilities in Intent-based execution
...
This approach may provide the wallet with less access to auxiliary execution capabilities. For example, a fully featured smart wallet may be able to transfer [ERC-20](./eip-20.md) tokens within the same transaction that resolves the Intent, whereas a restricted wallet may require a separate `approve()` transaction before the token transfer can be executed as part of the Intent.
...
### No Binding Between `sliceInfo` and `envelopeTx`

This standard does not require `sliceInfo` to be cryptographically bound to `envelopeTx`.

This trade-off allows an `envelopeTx` to be more portable across different `UserEnvelopeTx` instances. In particular, an existing `envelopeTx` can be included in a batch without requiring a new signature merely because its position within the enclosing transaction has changed.

The potential for an untrusted party to select or modify the execution envelope is addressed during the Sender callback. The Solver does not accept an execution envelope solely based on the supplied `sliceInfo`; instead, the Sender MUST acknowledge the exact `executor || intent` extracted by the Solver during Validation.

### An `envelopeTx` SHOULD NOT Contain Duplicate Execution Envelopes

Because `sliceInfo` can designate an arbitrary contiguous slice of `envelopeTx`, a relayer or other party constructing a `UserEnvelopeTx` can select an arbitrary slice. An invalid slice is rejected when the Sender performs the required callback.

However, if an `envelopeTx` contains two identical execution envelopes, the Solver cannot distinguish between the corresponding occurrences based solely on the callback data. Consequently, the same execution envelope may be accepted more than once if it is intentionally or unintentionally referenced by multiple `UserEnvelopeTx` instances.

An `envelopeTx` SHOULD therefore NOT contain multiple identical execution-envelope slices unless such behavior is explicitly intended by the application.

This standard does not prohibit duplicate execution-envelope slices at the protocol level, as doing so could unnecessarily restrict future Intent standards that may have legitimate use cases for such layouts.

Duplicate slices that do not represent execution envelopes do not have the same effect with respect to Solver execution, since the Solver only interprets the slice selected by `sliceInfo` as the execution envelope.

When constructing an `envelopeTx`, if multiple identical execution-envelope slices are present unexpectedly, applications SHOULD insert distinct delimiter or padding bytes between the slices so that each execution envelope occupies a distinct byte range.
...
An implicit property of such execution is that the Sender generally calls only contracts that it considers authorized or trusted. For example, a Sender may execute a call to an [ERC-20](./eip-20.md) contract to transfer a balance as part of an authorized operation.
...
See [https://github.com/Helkomine/ERCs/tree/erc-draft/assets/erc-8420](https://github.com/Helkomine/ERCs/tree/erc-draft/assets/erc-8420)
...
### Execution of an `envelopeTx` Outside the Solver Flow

Because an `envelopeTx` is a fully signed transaction authorized by the Sender, any party that obtains it may submit or execute it through any available execution path. The Sender MUST therefore ensure that an `envelopeTx` intended for the Solver flow cannot be executed through an unintended batch composition or execution path.

In particular, the Sender should ensure that the authorization of an `envelopeTx` is coupled to the Solver flow and that alternative batch compositions that do not satisfy the intended execution conditions are rejected.

In the most restrictive case, the Sender may itself serve as the final authorization boundary for the transaction, such as when the Sender is specified as the `tx.to` address of a conventional Ethereum transaction. In such cases, the Sender SHOULD ensure that the Solver callback and any associated preparation operations are atomic with respect to the Sender's other execution paths. Otherwise, an `envelopeTx` executed outside the intended Solver flow could produce persistent side effects without the corresponding Solver execution taking place.

Implementations SHOULD therefore ensure that any state changes performed in response to the Solver callback cannot be committed independently of the Solver execution flow when such independence could result in unintended or irreversible effects.

### Replay of Execution Through Duplicate Slices

An `envelopeTx` MAY contain multiple identical execution-envelope slices. If multiple `UserEnvelopeTx` instances select identical execution envelopes from the same `envelopeTx`, the Solver cannot distinguish the corresponding occurrences during the Sender callback.

This may result in the same execution being performed more than once if the duplicate slices were not intended by the application.

Applications constructing `envelopeTx` SHOULD therefore ensure that distinct execution envelopes occupy distinguishable byte ranges. If duplicate execution envelopes are not intentional, applications SHOULD insert distinct delimiter or padding bytes between them to prevent unintended reuse of the same execution envelope.

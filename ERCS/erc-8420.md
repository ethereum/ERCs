...
description: AA-compatible Solver contracts that coordinate existing account execution and authorization capabilities for Intent-based execution without modifying existing smart wallets
...
This approach may provide the wallet with less access to auxiliary execution capabilities. For example, operations such as an [ERC-20](./eip-20.md) `approve()` may require a separate execution path or may not be available through the Delegator.

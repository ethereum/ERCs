...
### L1 Handler: `StorageHandledByL1()`
A minimal L1 handler only requires the `contractL1` address to which deferral must be made, while the clients must ensure that the calldata is invariant under routing to another contract. One example implementation of an L1 handler is given below.

```solidity
// Define revert event
error StorageHandledByL1(
    address contractL1
);
...
function setValue(
    ...
    (
        address contractL1, // Deferred contract address on L1; may be globally constant
    ) = getMetadata(node); // Arbitrary code
    // contractL1 = 0x32f94e75cde5fa48b6469323742e6004d701409b
    // Defer write call to L1 handler
    revert StorageHandledByL1( 
        contractL1
    );
};
```

In this example, the deferral must prompt the client to build the transaction with the exact same original calldata, and submit it to the `contractL1` by calling the exact same function.

```solidity
// Function in deferred L1 contract
function setValue(
    bytes32 node,
    bytes32 key,
    bytes32 value

...
### [EIP-7702](./eip-7702.md) Interaction

[EIP-7702](./eip-7702.md) is orthogonal to this ERC. The `application` providing the EIP-712 domain via [ERC-5267] is always a contract; an undelegated EOA cannot serve as `application`. On the signer side, a 7702-delegated EOA produces ERC-7964 signatures the same way any other [ERC-1271] signer does; delegations that exist on one chain but not another reduce to [Account Validation](#account-validation) and [Code and State Differences](#code-and-state-differences).

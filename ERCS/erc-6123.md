...
The parameter `nextSettlementSpec` may be used to update `settlementSpec` that is issued as part of the `SettlementRequested` event (see there).
Such a settlementSpec may contain market driven updates of margin buffers.
...
The argument `settlementSpec` is the one that was passed upon a previous settlement
in `performSettlement`. It may be used to pass updated settlement specific information
from the valuation oracle, e.g., when margin buffer amounts are a function of market
...
- does not require and external time oracle to trigger `afterSettlement` (see the corresponding method description `afterSettlement`)

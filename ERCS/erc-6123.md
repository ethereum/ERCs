...
The Smart Derivative Contract (SDC) allows fully automizing and securing a financial product's, e.g., a financial derivative's, complete trade life cycle.

The SDC leverages the advantages of smart contracts to remove
many of the frictions associated with the classical derivative life cycle.
Most notably, the protocol allows the removal of counterparty risk essentially.
...
The payment can be realized by any implementation implementing an [ERC-20](./erc-20.md) token.

Proof of concepts were performed in two legally binding digital interest rate swaps conducted in 2021 and 2022.
...
Every process state is specified; therefore, the entire post-trade process is known in advance and is deterministic over the trade's life cycle. An [ERC-20](./erc-20.md) token can be used for frictionless decentralized settlement, see reference implementation. We do provide a separate interface and implementation for a specific "Settlement Token" derived from [ERC-20](./erc-20.md).
...
A reference implementation SDC.sol is provided and is based on the [ERC-20](./erc-20.md) token standard.

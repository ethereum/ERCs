...
This ERC defines a standard interface for verifying AI agents on Ethereum that have been registered via ERC-8004. It enables AI agents to undergo several specialized verification processes defined in this proposal: Ethereum Token Verification (ETV), Staking Contract Verification (SCV), Web Application Verification (WAV), and Wallet Verification (WV). Verification providers implement this standard using Private Data Verification (PDV) to generate Zero-Knowledge Proofs (ZKPs). Detailed verification results are accessible only to the AI agent’s wallet holder and include a unified risk score (0-100) to help users assess the agent’s trustworthiness. Verification attestations MAY be posted to ERC-8004’s Validation Registry for ecosystem-wide discoverability.
...
### Open Web Application Security Project (OWASP) Alignment

This standard recommends alignment with OWASP guidelines:

- WAV should follow the OWASP Web Security Testing Guide (WSTG) for endpoint security assessment
- ETV/SCV should follow the OWASP Smart Contract Security Verification Standard (SCSVS)
...
- Completes ETV when `contractAddress` is present in the resolved metadata
- Completes WV for all `agentId`s
- Completes WAV for all `agentId`s
- Completes SCV when `stakingContractAddress` is present in the resolved metadata
- Generates PDV ZKPs for each verification type
- Calculates `overallRiskScore` as the average of all applicable verification scores
- Emits `AgentVerified` event with all proof IDs (if the on-chain component is deployed)
- Reverts with `VerificationFailed` when the verification provider returns an error
- Reverts with `InsufficientCredits` when no verification credits are available
...
- Allows only `walletAddress` to view proof URLs
- Allows anyone to view public agent information (excluding proofs)
- Reverts with `UnauthorizedAccess` for unauthorized proof access
- Reverts with `AgentNotFound` for non-existent `agentId`

### Risk Score Tests

- Returns score 0-20 for Low Risk tier
- Returns score 21-40 for Moderate tier
- Returns score 41-60 for Elevated tier
- Returns score 61-80 for High Risk tier
- Returns score 81-100 for Critical tier
- Reverts with `InvalidScore` for scores outside 0-100
...
Staking contracts present additional risks, including locked funds, reward manipulation, and governance attacks. SCV verification checks for common vulnerabilities, but users should exercise due diligence before staking with any agent.

...
The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", " NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in RFC 2119 and RFC 8174.
...
The provider MUST call `tokenURI(agentId)` on the canonical Identity Registry contract and resolve the returned URI to fetch the agent registration JSON file. All metadata fields (`walletAddress`/`agentWallet`, `url`/`endpoints`, `contractAddress`, `stakingContractAddress`, `chain_id`, `platform_id`, etc.) MUST be extracted from this JSON in accordance with the ERC-8004 registration schema.
...
-  follow OWASP Smart Contract Security guidelines
...
- MUST return risk score 0-100
-  follow OWASP Web Application Security Testing guidelines (WSTG)
...
-  use AES-256-GCM or equivalent post-quantum encryption algorithm
- MUST return unique record_id for encrypted data
- MUST provide decryption_url for authorized data retrieval
-  ensure quantum-resistant key exchange mechanisms
...
-  use the X402 protocol for micropayments
-  support stablecoin settlement (e.g., USDC)
- MUST clearly disclose fee structure before verification
-  use [EIP-3009](./eip-3009.md) TransferWithAuthorization for gasless payments
...
Implementations  revert with these error codes:
...
If an on-chain component is deployed, it  include at minimum:
...
- **WAV**  follow OWASP Web Security Testing Guide (WSTG) for endpoint security assessment
- **ETV/SCV**  follow OWASP Smart Contract Security Verification Standard (SCSVS)
- Verification providers  check for OWASP Top 10 vulnerabilities in web applications
- Verification providers  check for Smart Contract Top 10 vulnerabilities in contracts

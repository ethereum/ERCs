...
category: ERC
created: 2026-01-15
requires: 155, 191, 712, 3009, 8004
---
...
**EIP-155 (Replay Protection)**: Verification requests involve signed messages from agent owners to authorise providers or payments. Without chain ID inclusion ([EIP-155](/EIPS/eip-155)), a signed request on mainnet could be replayed on testnets or L2s, potentially triggering unwanted verifications or duplicate payments across chains.

**ERC-191 (Signed Data Standard)**: Wallet verification and request authorisation require proving control over the agent wallet. [ERC-191](/EIPS/eip-191) provides a standardised prefix for signed messages, ensuring compatibility across wallets and preventing signature malleability during verification.

**EIP-712 (Typed Data Signing)**: Verification requests use structured data (agentId, metadata hashes, nonce, payment details). [EIP-712](/EIPS/eip-712) enables human-readable signing prompts (e.g., "Authorize Verification for Agent ID: 1234 on chain 1") instead of blind hashes, reducing phishing risks and improving UX for agent owners.

**ERC-3009 (Transfer With Authorization)**: Verification fees are paid via x402 micropayments. [ERC-3009](/EIPS/eip-3009) enables gasless USDC transfers, with the provider covering gas costs, making verification accessible without requiring users to hold ETH.

**ERC-8004 (Trustless Agents Registry)**: Agent identity and metadata are sourced from the canonical ERC-721 Identity Registry. This standard provides portable agent IDs, tokenURI-based metadata resolution (name, walletAddress, url, contractAddress, etc.), and optional posting to the Validation Registry for composable trust signals.
...
Existing AI agents already registered via ERC-8004 can undergo verification without changes. The standard is designed to work alongside existing token standards ([ERC-20](/EIPS/eip-20), [ERC-721](/EIPS/eip-721), [ERC-1155](/EIPS/eip-1155)) and identity standards.

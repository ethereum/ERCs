...
Validates the legitimacy and security of the staking smart contract when a `stakingContractAddress` is present in the resolved metadata.

- MUST verify that the `stakingContractAddress` is a deployed smart contract on the resolved `chain_id` by calling eth_getCode and confirming the returned bytecode is not 0x

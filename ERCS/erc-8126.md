...
The verification provider MUST call `tokenURI(agentId)` on the canonical Identity Registry contract and resolve the returned URI to fetch the agent registration JSON file. All metadata fields (`walletAddress`/`agentWallet`, `url`/`endpoints`, `contractAddress`, `solidityCode`, `chain_id`, `platform_id`, etc.) MUST be extracted from this JSON in accordance with the ERC-8004 registration schema.  
...
| `0x0A`     | `StakingContractNotFound` | Specified Solidity Code does not exist    |

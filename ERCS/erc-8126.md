...
<div align="left">
  <img src="../assets/eip-8126/verification-flow.svg" 
       width="600" 
       alt="ERC-8126 AI Agent Verification - Verification Flow Diagram">
</div>
...
async function resolveAgentMetadata(
  ...
  try {
    ...
    return {
      ...
      stakingContractAddress: metadata.stakingContractAddress ? getAddress(metadata.stakingContractAddress) : undefined,
...
async function SCV(m: AgentMetadata, c: VerificationProviderConfig): Promise<VerificationResult> {
  return {
    ...
    status: VerificationStatus.Passed,
    score: 30,
    proofId: generateProofId('SCV', m.stakingContractAddress || ''),
  };
}
...
export async function verifyAgent(agentId: bigint, config: VerificationProviderConfig) {
  ...
  const overallRiskScore = calculateOverallRiskScore([etv.score, scv.score, wav.score, wv.score]);
  const riskTier = getRiskTier(overallRiskScore);
  const pdvProofId = generatePDVProof({ etv, scv, wav, wv, overallRiskScore, agentId: agentId.toString() });

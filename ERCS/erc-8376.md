...
A detection-conformant venue MUST surface, at the point of purchase and without requiring the buyer to query a contract, the `abuseScore` and `confidence` returned by `activeScore` for the launch being purchased, and the `evidenceURI` of the report they derive from. Where `activeScore` returns no live report, the venue MUST present that as unknown, and MUST NOT present it as safe.
...
**Why detectors are paid from restitution and may be paid by venues.** Funding adjudication while leaving detection unfunded would make a share of forfeited bonds from false claims the only detector revenue, so a detector that was never wrong would earn nothing. Paying from restitution aligns the detector with upheld findings. Permitting venues to pay directly is what funds the reports that prevent harm rather than document it, and forbidding score-conditioned payment is what keeps that from becoming a market in clean opinions.
...
23. **Disclosure at the point of purchase.** A detection-conformant venue surfaces `abuseScore`, `confidence` and the report's `evidenceURI` to the buyer, and presents a launch with no live report as unknown rather than as safe.

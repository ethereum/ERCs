...
Any on-chain component a detector deploys SHOULD follow the Open Worldwide Application Security Project (OWASP) Smart Contract Security Verification Standard [(SCSVS) v0.0.1](../assets/eip-8376/20241004-OWASP_Smart_Contract_Security_Verification_Standard-v0.0.1.pdf), and the endpoint a detector publishes from SHOULD follow the OWASP Web Security Testing Guide [(WSTG) v4.2](../assets/eip-8376/20201203-OWASP_Web_Security_Testing_Guide-v4.2.pdf).

Those standards address a detector's contracts and the endpoint serving them. They do not address the indexing pipeline through which it reads chain history, the standing of what it serves over HTTP, the key that carries its authority, or the accuracy of its heuristics. Requirements for each follow.
...
The suite covers every test case above across 213 tests, four of which are fuzzed invariants: a launch never pays out more than it took in, the escrow stays solvent across launches, accounted tokens never exceed the balance held, and released proceeds never exceed proceeds paid in.

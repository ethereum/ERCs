...
Three conformance levels are defined. The escrow is the most costly component to adopt, and a venue that cannot adopt it remains able to consult and disclose detection, so the levels are named to make partial adoption well defined.
...
This level requires no deployment, and a buyer able to compare a disclosed score across venues has reason to prefer the venue that discloses one. That preference is the principal incentive for a venue to adopt a higher level.
...
Most launches decline without abuse having occurred. Scoring price decline would classify every unsuccessful launch as abusive, penalising legitimate launches and removing the score's value.
...
A detector turns chain history into a `SignalVector`. Four of the twelve signals need a funding graph or cross-chain claim history and cannot be computed by a contract. The remainder can be, and the larger the recomputable portion of the vector, the greater the part of a false report that can be demonstrated and the more the bond secures. Detectors MUST derive on-chain whatever can be derived on-chain, and MUST NOT substitute an asserted value for a readable one.
...
Signals that a detector could not establish MUST be reported as the unavailability sentinel. A reported zero asserts a value that was never established, and the evaluator would score that assertion as though it had been verified.
...
Those standards address contracts, and much of a detector is not a contract. They do not address the indexing pipeline through which a detector reads chain history, the endpoint it publishes from, the key that carries its authority, or the accuracy of its heuristics. Requirements for each follow.

#### Detector Obligations

A security review of a detector's contracts does not address the components most likely to produce an incorrect result. The following are normative.
...
Every interface in this proposal is keyed on `launchId`. An identifier that cannot be independently derived or resolved from a token address is not usable in practice: a wallet holding only a token address could not consult a guard before purchase, and a detector could not name its subject in a form another venue would recognize.
...
The directory MUST be permissionless, immutable, and have no owner. An administrator able to unlist a launch could render it indistinguishable from a launch that was never covered.
...
Each directory records only its own chain's launches, and `block.chainid` is bound into `launchId` for that reason. This proposal defines no cross-chain aggregation: a detector examining a deployer active on several chains MUST query each directory and combine off-chain, and MUST NOT assume an address is the same principal across chains.

...
Integrating a balance feature into each token and adopting mechanisms similar to those found in [ERC-20](./eip-20.md), we enable the seamless exchange of balances between tokens. This development allows token holders to effortlessly inquire about and transact the ownership shares represented by their token balances. Furthermore, should there be a necessity to increase the overall balance, new NFTs can be minted and their balances augmented accordingly. The omission of the `transfer` function from [ERC-20](./eip-20.md) is necessitated by the need to use `tokenId` for executing transfers. Including it would be redundant, as it overlaps with the functionality of the `transferFrom` function. Additionally, in situations where a token owner desires to transfer a balance to a specific address, a viable method involves first minting a new token for that address, then proceeding with the balance transfer, ensuring a smooth transition of value.

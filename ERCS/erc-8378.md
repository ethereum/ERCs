...
- The mutation logic for each mutable parameter MUST be implemented as a **deterministic transition function** whose return value is **unambiguously reproducible** given an explicitly defined transition context. The transition context includes:
...
- permissions MUST have following interfaces, data structure and rules.
 ...
 * @param index The parameter index (0 to NUMBER_OF_PARAMETERS-1)
 * @param subId The sub-account to apply the permission to
 * @param min Minimum acceptable value (inclusive)
 * @param max Maximum acceptable value (inclusive). max == 0 means no limit
 * @param soft If true, owner is exempt; if false, applies to all
 * @dev Reverts if min > max, or if max == 0 and soft == true.
 *      If a permission for this index already exists, it is replaced
 */
function permit(
  uint8 index,
  uint48 subId,
 ...
 * @dev If no permissions are set, returns an empty array.
 *      Only permissions with `max != 0` are returned
 * @param account The address of the account
 * @param subId The sub-account index
 * @return Permission[] An array of all permissions, sorted ascending by index
 */
...
```solidity
struct Permission {
  uint8 index; // Parameter index (0 to NUMBER_OF_PARAMETERS-1)
  uint64 min; // Minimum acceptable value (inclusive)
  uint64 max; // Maximum acceptable value (inclusive). max == 0 means no limit
  bool soft; // If true, owner is exempt; if false, applies to all
}

mapping(address => mapping(uint48 => Permission[])) private _permissions;
```

- `_permissions[account][subId]` holds an array of permission limits for inbound transfers.

#### Permissions Rules

- For every permission record `min <= max` MUST always hold; any call violating this MUST revert.
- Every inbound transfer's parameter has to meet limits set by respective pair of `min` and `max`, if only `max != 0`.
- If `max == 0` for a particular parameter, no limitations are applied to inbound transfers.
- If `max == 0`, `soft` MUST be `false`; any call violating this MUST revert.
...
- `allPermissionsOf(address account, uint48 subId)` returns an array of all active permissions; all permission records with `max == 0` MUST be omitted.
- The `PermissionSet` event MUST be emitted whenever a permission is created or updated.

_Note_: `_permissions[account][subId]` is not sorted by parameter index.

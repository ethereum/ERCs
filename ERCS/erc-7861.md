...
This standard addresses key gaps in the [W3C framework](https://www.w3.org/TR/vc-data-model-2.0/#ecosystem-overview) by defining methods for storage, revocation, retrieval, and presentation, facilitating interoperability within a flexible, standardized structure. It supports custom cryptographic methods for signatures and proofs—from basic issuer signatures to advanced zero-knowledge (ZK) proofs and selective disclosure—allowing adaptability for diverse security requirements.
...
The verificationMethod() function MUST return the method to verify the credential proof, the response is up to the implementer but SHOULD be compatible with the W3C [verification method](https://www.w3.org/TR/vc-data-integrity/#dfn-verification-method) concept.

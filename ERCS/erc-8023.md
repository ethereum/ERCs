...
description: Smart contract ownership standard with multi-step ownership transfer mechanism for secure smart contract ownership management
...
We define a ulti-stepped contract ownership interface for more secure contract ownership management. This makes the ownership transfer into 3 distinct steps. With the first 2 steps, performed by the original owner (initiate → confirm) and the remaining 1 step performed by the new owner (accept).

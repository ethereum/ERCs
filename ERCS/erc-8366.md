...
For the reference ERC-3009 schema, `authorization` MUST decode as:

```solidity
abi.encode(address to, uint256 value, uint256 validAfter, uint256 validBefore, bytes32 nonce)
```

`from` is deliberately absent: the implementation recomputes the digest with `from = address(this)`, so digest equality already pins the payer and a `from` field would be redundant input. Any other digest schema MUST likewise fix its `authorization` layout normatively; leaving it implementation-defined would make two conforming implementations of the same schema wire-incompatible, and a facilitator could not construct the envelope from the specification alone.

...
This EIP defines a **permissionless** interface for fungible tokens that operate in two modes: transparent mode (fully compatible with [ERC-20](./erc-20.md)) and privacy mode (using [ERC-8086](./erc-8086.md) privacy primitives). Token holders can convert balances between modes. The transparent mode uses account-based balances, while the privacy mode uses the standardized `IZRC20` interface from ERC-8086. Total supply is maintained as the sum of both modes.
...
1. **[ERC-20](./erc-20.md)**: Full DeFi composability but zero privacy
...
**Strengths**:
- ✅ Works with any existing [ERC-20](./erc-20.md) token
...
3. **Full [ERC-20](./erc-20.md) Compatibility**
...
- **Transparent Mode**: Token balance stored using standard [ERC-20](./erc-20.md) accounting, publicly visible and queryable via `balanceOf()`
 ...
 * @notice Interface for dual-mode tokens (ERC-8085) combining ERC-20 and [ERC-8086](https://ercs.ethereum.org/ERCS/erc-8086) (IZRC20)
...
### [ERC-20](./erc-20.md) Compatibility

Implementations MUST implement the [ERC-20](./erc-20.md) interface. All ERC-20 functions operate exclusively on transparent mode balances:
...
Implementations MUST emit standard [ERC-20](./erc-20.md) `Transfer` events for transparent mode operations.
...
Where:
- `totalSupply()`: Inherited from [ERC-20](./erc-20.md), represents total token supply across both modes
...
This standard is fully backward compatible with [ERC-20](./erc-20.md) and [ERC-8086](./erc-8086.md):
...
**Transparent Mode**: Standard [ERC-20](./erc-20.md) balance checking prevents double-spending.

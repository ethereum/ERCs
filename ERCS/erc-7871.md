...
Defines a new JSON-RPC method `wallet_sign` which enables apps to ask a wallet to sign [EIP-191](./erc-191.md) messages.

Applications can use this JSON-RPC method to request a signature over any version of `signed_data` as defined by [EIP-191](./erc-191.md). The new JSON-RPC method allows for support of future [EIP-191](./erc-191.md) `signed_data` versions.

The new JSON-RPC method also supports [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792)-style `capabilities`, and support for signing capabilities can be discovered using `wallet_getCapabilities` as defined in [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792).

## Motivation

Wallets and developer tools currently support multiple JSON-RPC methods for handling offchain signature requests. This proposal simplifies wallet & tooling implementations by consolidating these requests under a single `wallet_sign` JSON-RPC method. This also leaves room for new [EIP-191](./erc-191.md) `signed_data` versions without needing to introduce a new corresponding JSON-RPC method.

Furthermore, this new `wallet_sign` method introduces new functionalities via [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792)-style `capabilities`.
...
Requests a signature over [EIP-191](./erc-191.md) `signed_data` from a wallet.

The top-level `version` parameter is for specifying the version of `wallet_sign` should the top-level interface change.

The `request.type` parameter is for specifying the [EIP-191](./erc-191.md) `signed_data` `version` (e.g. `0x01` for structured data, `0x45` for `personal_sign` messaged). The `request.data` parameter is the corresponding data according to the `signed_data` `version`.
...
This proposal defines `request` schemas for the three `signed_data` versions currently in [EIP-191](./erc-191.md) (`0x00`, `0x01`, `0x45`). Any future `signed_data` versions can be supported by `wallet_sign`, and their `request` interfaces are left to future ERCs.
...
Below are `request` interfaces for the `signed_data` `version`s specified in [EIP-191](./erc-191.md) at time of writing. These include:
* `0x00` - Data with intended validator
* `0x01` - [EIP-712](https://eips.ethereum.org/EIPS/eip-712) Typed Data
* `0x45` - Personal Sign

...
discussions-to: https://ethereum-magicians.org/t/erc-8179-blob-space-segments-bss/27867
status: Draft
...
This ERC defines a minimal interface for on-chain declaration of field element sub-ranges ("segments") within [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844) blobs.
...
- Nethermind "Blob Sharing for Based Rollups": working demo using [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) fan-out
- BlobFusion / Ephema: blob space sharing service with bid-based pricing
- [ERC-7588](./erc-7588.md) (Blob Transaction Metadata): orthogonal standard for blob metadata;
  composes with this ERC

## Specification

This ERC builds upon [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844) and relies on the `BLOBHASH` opcode defined therein.
...
A stateful implementation adds a cold `SSTORE` at 22,100 gas (post-[EIP-2929](https://eips.ethereum.org/EIPS/eip-2929)).
...
### No [ERC-165](./erc-165.md) requirement

[ERC-165](./erc-165.md) `supportsInterface` adds deployment overhead and per-query gas (a cold
`SLOAD` costs 2,100 gas per [EIP-2929](https://eips.ethereum.org/EIPS/eip-2929)). For a one-function interface identified by
...
semantically equivalent to current behavior. The `BLOBHASH` opcode ([EIP-4844](https://eips.ethereum.org/EIPS/eip-4844)) is

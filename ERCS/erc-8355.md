...
[EIP-712](./eip-712.md) signature by `g.subject`, verified with [ERC-1271](./eip-1271.md) support for
...
- **[ERC-5006](./eip-5006.md)** (Final) gives an ERC-1155 token a time-bounded `user` role. Two-party:
  the owner grants over their own token. This specification adds the third principal and the purpose.
- **[ERC-4907](./eip-4907.md)** (Final) is the ERC-721 sibling of the above.
...
`assetTokenId` may reference an [ERC-721](./eip-721.md) or [ERC-1155](./eip-1155.md) token, and implementations MAY additionally
...
parameter provides no agent authorization at all. This is stated as a normative requirement above because it is the
...
2. An unsalted hash of any identifier drawn from an enumerable space. An address space is enumerable,
   so a bare hash of an address can be reversed by brute force and is pseudonymization at best.
...
7. A stable subject identifier reused across grants. The subject MUST be referenced on-chain only as
   ...
   This is a pseudonym, not anonymization: it remains personal data while it can be linked to a

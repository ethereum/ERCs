...
Critically, the logic for verifying state commitment proofs is not hardcoded in the Receiver. Instead, it delegates this to a user specified list of StateCommitmentProver contracts. Each StateCommitmentProver defines how to verify a state commitment proof for a specific home chain to recover the state commitment of a specific target chain. Because the state commitment schemes and layouts of rollup contracts can change over time, the state commitment proof verification process itself must also be upgradeable&mdash;hence the StateCommitmentProvers are upgradeable. This flexible, upgradeable proof verification model is the core contribution of this standard.
...
- Must be EVM equivalent with L1. StateCommitmentProvers are deployed as copies on many chains, so need to behave the same on all those chains

### Constants

```solidity
uint256 constant STATE_COMMITMENT_PROVER_POINTER_SLOT = uint256(keccak256("eip7888.pointer.slot")) - 1;
```
...
- **State root**: The root of the state trie (Merkle-Patricia Trie) or other state commitment structure
- **Batch hash**: A hash of a batch of blocks, used by some rollups that commit batches of blocks rather than individual blocks

The choice of state commitment depends on what the rollup commits to its parent chain. Different rollups may use different state commitments, and the StateCommitmentProver is responsible for verifying proofs that recover these commitments from the parent chain's state.
...
### StateCommitmentProvers
StateCommitmentProvers prove a unidirectional link between two chains that have direct access to each other's finalized state commitments. The chains in this link are called the **home chain** and the **target chain**. StateCommitmentProvers are responsible for verifying state commitment proofs to prove the existence of finalized target state commitments in the state of the home chain. Block hashes are the recommended state commitment, but implementations may use other commitments (e.g., state roots, batch hashes).

Since the StateCommitmentProvers are unidirectional, each chain needs to have two: 
...
StateCommitmentProvers MUST ensure that they will have the same deployed code hash on all chains.

<div align="center">
<img src="../assets/eip-7888/BHP.svg" alt="Figure 2" width="30%"/>
<br/>
<em>Figure 2: A StateCommitmentProver with home chain L and target chain M</em>
</div>

```solidity
/// @notice The IStateCommitmentProver is responsible for retrieving the state commitment of its target chain given its home chain's state.
///         The home chain's state is given either by a state commitment and proof, or by the StateCommitmentProver executing on the home chain.
///         A single home and target chain are fixed by the logic of this contract.
interface IStateCommitmentProver {
    ...
    ///         While messages MUST be stored in storage slots (for duplicate prevention), alternative reading mechanisms may be used in some edge cases.
...
### StateCommitmentProverPointers

StateCommitmentProvers can be used to get or verify target state commitments, however since their verification logic is immutable, changes to the structure of the home or target chain can break the logic in these Provers. A StateCommitmentProverPointer is a Pointer to a StateCommitmentProver which can be updated if proving logic needs to change.

StateCommitmentProverPointers are used to reference StateCommitmentProvers as opposed to referencing Provers directly. To that end, wherever a StateCommitmentProver is deployed a StateCommitmentProverPointer needs to be deployed to reference it.

StateCommitmentProverPointers allow a permissioned party to update the Prover reference within the Pointer. Choosing which party should have the permission to update the Prover reference should be carefully considered. The general rule is that if an update to the target or home chain could break the logic in the current Prover, then the party, or mechanism, able to make that update should also be given permission to update the Prover. See [Security Considerations](#security-considerations) for more information on StateCommitmentProverPointer ownership and updates.

When updating a StateCommitmentProverPointer to point to a new StateCommitmentProver implementation:
* The home and target chain of the new StateCommitmentProver MUST be identical to the previous StateCommitmentProver.
* The new StateCommitmentProver MUST have a higher version than the previous StateCommitmentProver.

StateCommitmentProverPointers MUST store the code hash of the StateCommitmentProver implementation in slot `STATE_COMMITMENT_PROVER_POINTER_SLOT`.

<div align="center">
<img src="../assets/eip-7888/pointer.svg" alt="Figure 3" width="30%"/>
<br/>
<em>Figure 3: A StateCommitmentProverPointer at address 0xA pointing to a StateCommitmentProver with home chain L and target chain M</em>
</div>

```solidity
/// @title  IStateCommitmentProverPointer
/// @notice Keeps the code hash of the latest version of a state commitment prover.
///         MUST store the code hash in storage slot STATE_COMMITMENT_PROVER_POINTER_SLOT.
///         Different versions of the prover MUST have the same home and target chains.
///         If the pointer's prover is updated, the new prover MUST have a higher IStateCommitmentProver::version() than the old one.
///         These pointers are always referred to by their address on their home chain.
interface IStateCommitmentProverPointer {
...
A route is a relative path from a Receiver on a local chain to a remote chain. It is constructed of many single degree links dictated by StateCommitmentProverPointers. Receivers use the StateCommitmentProvers that the Pointers reference to verify a series of proofs to obtain the remote chain's state commitment. A route works with any state commitment scheme (block hashes, state roots, etc.) and is defined by the list of Pointer addresses on their home chains.
...
- The Remote Account ID of StateCommitmentProverPointer `0xC` is `accumulator([0xA, 0xB, 0xC])`.

### StateCommitmentProverCopies

StateCommitmentProverCopies are exact copies of StateCommitmentProvers deployed on non-home chains. When a StateCommitmentProver code hash is de-referenced from a Pointer, a copy of the StateCommitmentProver may be used to execute its logic. Since the Pointer references the prover by code hash, a local copy of the Prover can be deployed and used to execute specific proving logic. The Receiver caches a map of `mapping(bytes32 stateCommitmentProverPointerId => IStateCommitmentProver stateCommitmentProverCopy)` to keep track of StateCommitmentProverCopies. 

<div align="center">
<img src="../assets/eip-7888/BHPCopy.svg" alt="Figure 5" width="30%"/>
<br/>
<em>Figure 5: A StateCommitmentProverCopy of StateCommitmentProver M->P on chain L</em>
</div>
...
1. Receiver calls `IStateCommitmentProverPointer(0xA)::implementationAddress` to get the address of StateCommitmentProver L->M
1. Receiver calls `IStateCommitmentProver(Prover L->M)::getTargetStateCommitment`, passing input given by Subscriber to get a state commitment of chain M.
1. Receiver calls `IStateCommitmentProver(Prover Copy M->P)::verifyTargetStateCommitment`, passing chain M's state commitment and proof data by Subscriber to get a state commitment of chain P.
1. Receiver calls `IStateCommitmentProver(Prover Copy P->R)::verifyTargetStateCommitment`, passing chain P's state commitment and proof data by Subscriber to get a state commitment of chain R.
1. Finally, Receiver calls `IStateCommitmentProver(Prover Copy P->R)::verifyStateValue`, passing input given by Subscriber to get a state value from the Broadcaster. The Receiver returns the Broadcaster's Remote Account ID and the message's timestamp to Subscriber.
...
interface IReceiver {
    ...
    ///         The proof format depends on the state commitment scheme used by the StateCommitmentProver (e.g., storage proofs).
    ///         SHOULD use storage slots as the state location. While messages MUST be stored in storage slots (for duplicate prevention), alternative reading mechanisms may be used in some edge cases.
    /// @param  route The home chain addresses of the StateCommitmentProverPointers along the route to the remote chain.
    /// @param  scpInputs The inputs to the StateCommitmentProver / StateCommitmentProverCopies.
    /// @param  proof Proof passed to the last StateCommitmentProver / StateCommitmentProverCopy
    ...
    ///         - The proof is for the message state location (SHOULD be a storage slot; while messages MUST be stored in storage slots for duplicate prevention, alternative reading mechanisms may be used in some edge cases)
    ...
    ///         Checks that StateCommitmentProverCopy has the same code hash as stored in the StateCommitmentProverPointer
    ///         Checks that the version is increasing.
    /// @param  scpPointerReadArgs A RemoteReadArgs object:
    ///         - The route points to the StateCommitmentProverPointer's home chain
    ///         - The account proof is for the StateCommitmentProverPointer's account
    ///         - The proof is for the STATE_COMMITMENT_PROVER_POINTER_SLOT
    /// @param  scpCopy The StateCommitmentProver copy on the local chain.
    /// @return scpPointerId The ID of the StateCommitmentProverPointer
    function updateStateCommitmentProverCopy(RemoteReadArgs calldata scpPointerReadArgs, IStateCommitmentProver scpCopy)
        external
        returns (bytes32 scpPointerId);

    /// @notice The StateCommitmentProverCopy on the local chain corresponding to the scpPointerId
    ///         MUST return 0 if the StateCommitmentProverPointer does not exist.
    function stateCommitmentProverCopy(bytes32 scpPointerId) external view returns (IStateCommitmentProver scpCopy);
}
```
...
1. Ethereum state commitment (e.g., block hash) is looked up directly on L2' by the Receiver on L2'
2. The state commitment of L2'' is proven using a storage proof
...
### StateCommitmentProvers, Pointers, and Copies

#### StateCommitmentProvers
Each rollup implements unique logic for managing and storing state commitments. To accommodate this diversity, StateCommitmentProvers implement chain-specific procedures. This flexibility allows integration with each rollup's distinct architecture and state commitment scheme.

The StateCommitmentProver handles the final step of verifying a state value (SHOULD be a storage slot) given a target state commitment to accommodate rollups with differing state commitment schemes and formats.

#### StateCommitmentProverPointers
Routes reference StateCommitmentProvers through Pointers rather than directly. This indirection is crucial because:
- Chain upgrades may require StateCommitmentProver redeployments
- Routes must remain stable and valid across these upgrades - ensuring in-flight messages are not broken
- Pointers maintain route consistency while allowing StateCommitmentProver implementations to evolve

#### StateCommitmentProverCopies
Since StateCommitmentProverPointers reference StateCommitmentProvers via their code hash, a copy of the StateCommitmentProver can be deployed anywhere and reliably understood to contain the same code as that referenced by the Pointer. This allows the Receiver to locally use the code of a StateCommitmentProver whose home chain is a remote chain.
...
### Chain Upgrades
If a chain upgrades such that a StateCommitmentProver's `verifyTargetStateCommitment` or `getTargetStateCommitment` functions might return data besides a finalized target state commitment, then invalid messages could be read by a `Receiver`. For instance, if a chain stores its state commitments on the parent chain in a specific mapping, and that storage location is later repurposed, then an old StateCommitmentProver might be able to pass along an invalid state commitment. It is therefore important that either:
* the StateCommitmentProver is written in such a way to detect changes like this
* the owner who is able to repurpose these storage locations is aware of the StateCommitmentProver and ensures they don't break it

### StateCommitmentProverPointer Ownership / Updates
A malicious StateCommitmentProverPointer owner can DoS or forge messages. However, so can the chain owner responsible for setting the location of historical parent/child state commitments. Therefore it is expected that this chain owner be the same as the owner of the StateCommitmentProverPointer so as not to introduce additional risks.

* If the target chain of the referenced StateCommitmentProver is the parent chain, the home chain owner is expected to be the StateCommitmentProverPointer's owner.
* If the target chain of the referenced StateCommitmentProver is the child chain, the target chain owner is expected to be the StateCommitmentProverPointer's owner.

If an owner neglects their responsibility to update the Pointer with new StateCommitmentProver implementations when necessary, messages could fail to reach their destinations.

If an owner maliciously updates a Pointer to point to a StateCommitmentProver that produces fraudulent results, messages can be forged.

If there is confidence that a chain along the route connecting them will not upgrade to break a StateCommitmentProver, an unowned StateCommitmentProverPointer can be deployed in the absence of a properly owned one.

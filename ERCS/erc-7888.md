...
interface IStateProverPointer {
    ...
    /// @notice Emitted when the pointer is updated to a new implementation.
...
1. Receiver calls `IStateProver(Prover L->M)::getTargetState`, passing input given by Subscriber to get a state commitment of chain M.
1. Receiver calls `IStateProver(Prover Copy M->P)::verifyTargetState`, passing chain M's state commitment and proof data by Subscriber to get a state commitment of chain P.
1. Receiver calls `IStateProver(Prover Copy P->R)::verifyTargetState`, passing chain P's state commitment and proof data by Subscriber to get a state commitment of chain R.
1. Finally, Receiver calls `IStateProver(Prover Copy P->R)::verifyStateValue`, passing input given by Subscriber to get a storage slot from the Broadcaster. The Receiver returns the Broadcaster's Remote Account ID and the message's timestamp to Subscriber.

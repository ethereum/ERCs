...
The `paymasterService` capability is designed to be extensible across transaction types. This specification defines the `pm_*` RPC methods for [ERC-4337](./eip-4337.md) user operations. Other proposals MAY define additional RPC methods to be served at the same `paymasterService` URL for different transaction types.

When a wallet receives a `paymasterService` capability, it SHOULD call the RPC methods appropriate to the transaction type it is constructing:

- **[ERC-4337](./eip-4337.md) user operations**: Use the `pm_*` methods defined in this specification.
- **[EIP-8130](./eip-8130.md) transactions**: Use the `payer_*` methods defined in [ERC-8160](./eip-8160.md).

Service providers that wish to support multiple transaction types SHOULD implement all relevant method sets at the same endpoint URL. This allows apps to pass a single `paymasterService` URL regardless of the wallet's underlying transaction type.

...
![Transaction authorization flow using an EAT](../assets/erc-7272/EAT_transaction_auth_flow.jpeg)
...
The `verify()` function is part of the `AccessTokenVerifier` contract and is called by an `AccessTokenConsumer`.
...
It takes a signature and an `AccessToken` as input, verifies that the token is not expired, attempts to recover the signer from the signature and the reconstructed EIP-712 digest, and verifies that the signer is a valid, expected signer. Please see the [reference implementation](../assets/erc-7272/AccessTokenVerifier.sol) for how this can be performed.

...
The output `requestId` is used to partially descriminate the request along with the `receiver`. See "Request Ids" section for more info.
...
```solidity
    ...
    function requestDeposit(uint256 assets, address receiver, address owner, bytes calldata data) external returns (uint256 requestId) {
        ...
        if (data.length != 0) {
          ERC7540Receiver(receiver).onERC7540DepositReceived(msg.sender, owner, requestId, data);
        }

        emit DepositRequest(msg.sender, operator, requestId, assets);
        return requestId;
    }

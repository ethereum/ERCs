...
### Request operator events

Any contract that custodies `assets` and submits a deposit Request on behalf of a user, or acts as the `receiver` of the  `ERC7540DepositReceiver` callback on behalf of a user, SHOULD emit `RequestDeposit` events with the same specification as [`RequestDeposit](#DepositRequest), except with the `receiver` value being set to the user that is going to receive the `shares` after the Request is fulfilled.

Any contract that custodies `shares` and submits a redeem Request on behalf of a user, or acts the `receiver` of the  `ERC7540RedeemReceiver` callback on behalf of a user, SHOULD emit `RequestRedeem` events with the same specification as [`RequestRedeem](#RedeemRequest), except with the `receiver` value being set to the user that is going to receive the `assets` after the Request is fulfilled.
...
### Request operator events inclusion
By emitting events that include the end user, integrations and off-chain indexers can determine the end user of a deposit or redeem Request that an intermediary contract is managing the Request on behalf of.

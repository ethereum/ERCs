...
Note that this design does force users to heavily trust in the Vault logic and admins to not give users suboptimal exchange rates. This trust is an intended part of the design for the spec.
...
### Not Allowing an Optional 1-step Push Request Flow
...
The 2-step approach used in the standard may be abstracted into a 1-step approach from the user perspective through the use of relayers, message signing, or account abstraction.
...
In particular, users can be left "naked" during an adverse redemption event where shares can be stuck in Pending state. Vaults may elect to allow for fungibility of pending claims or implement some cancellation functionality to protect users. Users do not know what the final exchange rate would be on any Request due to the asynchronicity. This introduces moral hazard for vault operators to take advantage of users who cannot revoke their Request. Users therefore trust Vault admins to not abuse any control they have over the exchange rate.

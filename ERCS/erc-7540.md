...
The amount of requested `assets` in Pending state for the `requester` to `deposit` or `mint`.
...
The amount of requested `assets` in Claimable state for the `requester` to `deposit` or `mint`.
...
The amount of requested `shares` in Pending state for the `requester` to `redeem` or `withdraw`.
...
The amount of requested `shares` in Claimable state for the `requester` to `redeem` or `withdraw`.
...
When the `Deposit` event is emitted, the first parameter MUST be the `sender`, and the second parameter MUST be the `requester`.
...
`sender` is the `msg.sender` of the original `requestDeposit` call. `requester` is the `requester` of the `requestDeposit`. `requestId` is the output `requestId` of the `requestDeposit`, `assets` is the amount transferred on `requestDeposit`, and `data` is the `data` of the `requestDeposit`.
...
`sender` is the `msg.sender` of the original `requestRedeem` call. `requester` is the `requester` of the `requestRedeem`. `requestId` is the output `requestId` of the `requestRedeem`, `shares` is the amount transferred on `requestRedeem`, and `data` is the `data` of the `requestRedeem`.
...
The preview functions do not take an address parameter, therefore the only way to discriminate discrepancies in the exchange rate is via the `msg.sender`. However, this could lead to integration/implementation complexities where support contracts cannot determine the output of a claim on behalf of an `requester`.

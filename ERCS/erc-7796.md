...
* The `options` parameter may include any of the following optional members:
    * **knownAccounts**: a mapping of accounts with their expected storage slots' values.
        ...
        * A special key `code` defines the expected code of the account.
          ...
          Use the `"0xef0100"` prefix to indicate a specific [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) delegation.
...
* **Time-based storage validation**: running the storage validation not only against the current block,
  ...
  This prevents abusing the API for MEV, while making it viable for [ERC-4337](./erc-4337.md) account validation.
* **UserOperation verification**: for sequencers serving [ERC-4337](./erc-4337.md) use cases, this can be mitigated

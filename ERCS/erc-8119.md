...
Where:
- **`<key-label>`**: MUST contain only ASCII characters (0x00-0x7F) and no spaces. ASCII characters are valid UTF-8 characters in the range 0x00-0x7F. The key-label identifies the type or category of metadata.
- **`<key-parameter>`**: MAY be any UTF-8 encoded string that does not contain `: ` (colon followed by a single space). The parameter value represents a specific instance or variation of the metadata type.
...
- `"key: value: with space"` (parameter contains `: `)
...
1. The `<key-label>` MUST contain only ASCII characters (0x00-0x7F) and no spaces. ASCII characters are valid UTF-8 characters in the range 0x00-0x7F.
2. The `<key-label>` and `<key-parameter>` MUST be separated by a colon (`:`) followed by a single space (` `).
3. The `<key-parameter>` MAY be any UTF-8 encoded string that does not contain the separator `: ` (colon followed by a single space). Colons without a following space are allowed in the parameter value.

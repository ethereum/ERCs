...
1. The `<key-label>` MUST contain only printable ASCII characters (0x21-0x7E), excluding spaces and the forward slash character (`/`). This excludes control characters, space, `/`, and DEL.
2. The `<key-label>` and `<key-parameter>` MUST be separated by a single forward slash (`/`).

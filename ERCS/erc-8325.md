...
being bound as an actual [ERC-721](./eip-721.md) or
[ERC-1155](./eip-1155.md) token. Including an explicit scope makes contract
...
The *Asset-bound Non-Fungible Tokens* proposal extends [ERC-721](./eip-721.md)
with tokens bound one-to-one to physical or digital assets, with operations
authorized by oracle attestations of control.
...
The *Real Estate Token* proposal extends [ERC-721](./eip-721.md) with property
identifiers and operating-agreement data. This ERC is not limited to real estate
or ERC-721 and does not prescribe asset-specific operations.

[ERC-3643](./eip-3643.md) and [ERC-7943](./eip-7943.md) define token behavior and
...
declares the replacement anchor. A replacement binding recorded for a token that
does not expose a token-side interface is registry-side only.

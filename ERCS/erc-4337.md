...
* **Create the account if it does not yet exist**, using the initcode provided in the `UserOperation`.
  * If the account is an EOA with an EIP-7702 authorization designation, the EntryPoint validates the authorized address matches the one specified in the UserOperation signature (see []()).
...
### Support for [EIP-712](./eip-712) signatures

In order to support ERC-712 signatures,
...
### Support for [EIP-7702](./eip-7702) authorizations

On networks with the EIP-7702 enabled, the `eth_sendUserOperation` method accepts an extra `eip7702auth` parameter.
If this parameter is set, the [EntryPoint flow](#required-entry-point-contract-functionality) and
[the  `userOpHash` calculation](#support-for-eip-712-signatures) are modified in the following way:

* The `initCode` field of the `UserOperation` is set to the full EIP-7702 authorization, `(0xef0100 || address)`
* The `userOpHash` is calculated on the modified `UserOperation` structure and this `userOpHash` is used for signing
* The `initCode` field of the `UserOperation` is set to `0xef0100` exactly and this `UserOperation` is passed to the `EntryPoint::handleOps` function
* The `handleOps` function of the `EntryPoint` contract uses the `EXTCODECOPY (0x3C)` opcode to retrieve the current `sender` authorization
* The `handleOps` function of the `EntryPoint` contract calculates the `userOpHash` internally using the retrieved sender code

This flow guarantees that the intended delegated address is being used by the `sender` without the need to pass
the entire address in calldata on each `UserOperation`.

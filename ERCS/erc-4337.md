...
* If the account does not support signature aggregation, it MUST validate the signature is a valid signature of the `beforeCallValidation`, and
  _SHOULD return SIG_VALIDATION_FAILED (and not revert) on signature mismatch_ (ERC7746 defines that validating contracts MUST revert if validation fails). Any other error MUST revert.
...
During the verification loop, in addition to calling ERC-7746 validation steps, the `handleOps` execution also must check that the paymaster has enough ETH deposited with the entry point to pay for the operation, and then call `beforeCallValidation` on the paymaster to verify that the paymaster is willing to pay for the operation. Note that in this case, the  account `beforeCallValidation` is called with a `missingAccountFunds` of 0 to reflect that the account's deposit is not used for payment for this userOp.

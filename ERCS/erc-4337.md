...
See also `https://ethereum-magicians.org/t/implementing-account-abstraction-as-part-of-eth1-x/4020` and the links therein for historical work and motivation, and [EIP-2938](./EIPs/blob/master/EIPS/eip-2938.md) for a consensus layer proposal for implementing the same goal.
...
* **Try to support other use cases**
    ...
    * Atomic multi-operations (similar goal to [EIP-3074](./EIPs/blob/master/EIPS/eip-3074.md))
    * Pay tx fees with [ERC-20](./erc-20.md) tokens, allow developers to pay fees for their users, and [EIP-3074](./EIPs/blob/master/EIPS/eip-3074.md)-like **sponsored transaction** use cases more generally
...
| `maxFeePerGas` | `uint256` | Maximum fee per gas (similar to [EIP-1559](./EIPs/blob/master/EIPS/eip-1559.md) `max_fee_per_gas`) |
...
Transient Storage slots defined in [EIP-1153](./EIPs/blob/master/EIPS/eip-1153.md) and accessed using `TLOAD` (`0x5c`) and `TSTORE` (`0x5d`) opcodes
...
Returns [EIP-155](./EIPs/blob/master/EIPS/eip-155.md) Chain ID.

...
See also `https://ethereum-magicians.org/t/implementing-account-abstraction-as-part-of-eth1-x/4020` and the links therein for historical work and motivation, and [EIP-2938](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-2938.md) for a consensus layer proposal for implementing the same goal.
...
* **Try to support other use cases**
    ...
    * Pay tx fees with [ERC-20](./erc-20.md) tokens, allow developers to pay fees for their users, and [EIP-3074](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-3074.md)-like **sponsored transaction** use cases more generally
...
| `maxFeePerGas` | `uint256` | Maximum fee per gas (similar to [EIP-1559](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-1559.md) `max_fee_per_gas`) |
...
We extend the entry point logic to support **paymasters** that can sponsor transactions for other users. This feature can be used to allow application developers to subsidize fees for their users, allow users to pay fees with [ERC-20](./erc-20.md) tokens and many other use cases. When the paymaster is not equal to the zero address, the entry point implements a different flow:
...
This method always reverts with `ValidationResult` as successful response.
...
1. May not invoke any **forbidden opcodes**
...
Transient Storage slots defined in [EIP-1153](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-1153.md) and accessed using `TLOAD` (`0x5c`) and `TSTORE` (`0x5d`) opcodes
...
**Parameters**: same as `eth_sendUserOperation`
  ...
  `maxFeePerGas` and `maxPriorityFeePerGas` default to zero, so no payment is required by either account nor paymaster.
...
Returns [EIP-155](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md) Chain ID.
...
This EIP does not change the consensus layer, so there are no backwards compatibility issues for Ethereum as a whole. Unfortunately it is not easily compatible with pre-[ERC-4337](./erc-4337.md) accounts, because those accounts do not have a `validateUserOp` function. If the account has a function for authorizing a trusted op submitter, then this could be fixed by creating an [ERC-4337](./erc-4337.md) compatible account that re-implements the verification logic as a wrapper and setting it to be the original account's trusted op submitter.

...
The `context` byte array size MUST be below `CONTEXT_MAX_LEN` (`2048`) bytes, otherwise the `UserOperation` is considered invalid.
...
* The `verificationGasLimit` and `paymasterVerificationGasLimits` are lower than `MAX_VERIFICATION_GAS` (`500000`) and the `preVerificationGas` is high enough to pay for the calldata gas cost of serializing the `UserOperation` plus `PRE_VERIFICATION_OVERHEAD_GAS` (`10000`).

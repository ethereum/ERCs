...
Before accepting a `UserOperation`, bundlers should use an RPC method to locally call the `handleOps` function on the entry point with that UserOperation, to verify it passes all validations and actually pays fees; see the [Simulation section below](#simulation) for details.
...
To simulate a `UserOperation` validation, the client makes a view call to the `handleOps()` method with the UserOperation to check.

The bundler shouldn't run the execution of the UserOperation, only its validation (either by adding a trailing "always failed" UserOperation, or by specifying gas enough just for validation)

The node should drop the UserOperation if the simulation reverts
...
Either validation function return value may contain a "validAfter" and "validUntil" timestamps, which is the time-range that this UserOperation is valid on-chain.
...
In order to determine the time-range returned by the `validateUserOp` and `validatePaymasterUserOp`, the node should perform the simulation using debug_traceCall (which is also required for the full validation, as described in [ERC-7562](./eip-7562.md)

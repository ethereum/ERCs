...
See also `https://ethereum-magicians.org/t/implementing-account-abstraction-as-part-of-eth1-x/4020` and the links therein for historical work and motivation, and [EIP-2938](./erc-2938.md) for a consensus layer proposal for implementing the same goal.
...
* **Achieve the key goal of account abstraction**: allow users to use smart contract wallets containing arbitrary verification logic instead of EOAs as their primary account. Completely remove any need at all for users to also have EOAs (as status quo SC wallets and [EIP-3074](./erc-3074.md) both require)
* **Decentralization**
...
* **Try to support other use cases**
    ...
    * Atomic multi-operations (similar goal to [EIP-3074](./erc-3074.md))
    * Pay tx fees with [ERC-20](./erc-20.md) tokens, allow developers to pay fees for their users, and [EIP-3074](./erc-3074.md)-like **sponsored transaction** use cases more generally
...
| `maxFeePerGas` | `uint256` | Maximum fee per gas (similar to [EIP-1559](./erc-1559.md) `max_fee_per_gas`) |
...
Transient Storage slots defined in [EIP-1153](./erc-1153.md) and accessed using `TLOAD` (`0x5c`) and `TSTORE` (`0x5d`) opcodes
...
Returns [EIP-155](./erc-155.md) Chain ID.

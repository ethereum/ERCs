...
* **UserOperation** - a structure that describes a transaction to be sent on behalf of a user. To avoid confusion, it is not named "transaction".
  ...
  * Notably, the `signature` field usage is not defined by the protocol, but by an Smart Contract Account implementation.
...
|---------------------------------|-----------|------------------------------------------------------------------------------------------------------------------|
| `sender`                        | `address` | The Smart Contract Account making the `UserOperation`                                                            |
| `nonce`                         | `uint256` | Anti-replay parameter (see "Semi-abstracted Nonce Support" )                                                     |
| `factory`                       | `address` | Smart Contract Account Factory for new accounts OR `0x7702` flag for EIP-7702 accounts, otherwise `address(0)`   |
| `factoryData`                   | `bytes`   | data for Smart Contract Account Factory if `factory` is provided OR EIP-7702 initialization data, or empty array |
...
* **Create the Smart Contract Account if it does not yet exist**, using the initcode provided in the `UserOperation`.
  * If the `sender` is an EOA with an [EIP-7702](./eip-7702.md) authorization designation, the `EntryPoint` validates the authorized address matches the one specified in the `UserOperation` signature (see [Support for [EIP-7702] authorizations](#support-for-eip-7702-authorizations)).
...
The Smart Contract Account creation itself is done by a "factory" contract, with Smart Contract Account specific data.
The factory is expected to use `CREATE2 0xF5` (not `CREATE 0xF0`) to create the Smart Contract Account, so that the order of creation of Smart Contract Accounts doesn't interfere with the generated addresses.
...
This method call is expected to create a Smart Contract Account and return its address.
If the factory does use `CREATE2 0xF5` or some other deterministic method to create the Smart Contract Account, it's expected to return the Smart Contract Account address even if it had already been created.
This comes to make it easier for bundlers to query the address without knowing if the Smart Contract Account has already been deployed, by simulating a call to `entryPoint.getSenderAddress()`, which calls the `factory` under the hood.
...
This way, even if someone can create a Smart Contract Account at that address, he can't set different credentials to control it.
...
NOTE: In order for the Wallet Application to determine the "counterfactual" address of the Smart Contract Account prior to its creation,
...
* **Safety against arbitrary hijacking**: The `EntryPoint` only calls an account generically if `validateUserOp` to that specific account has passed (and with `userOp.calldata` equal to the generic call's calldata)
...
When changing the underlying implementation, Smart Contract Accounts MUST ensure that there are no conflicts in the storage layout

...
The core interface required for an account is defined in ERC-7746.

The account:

* MUST validate the caller is a trusted EntryPoint
* If the account does not support signature aggregation, it MUST validate the signature is a valid signature of the `beforeCallValidation`, and
  SHOULD return SIG_VALIDATION_FAILED (and not revert) on signature mismatch_ (ToDo: ERC7746 defines that validating contracts MUST revert if validation fails, if catch clause used in 4337 implementation it can preserve current functional requirements). Any other error MUST revert.
...
An account that works with aggregated signature, should return its signature aggregator address "sigAuthorizer" return value of beforeCallValidation.
...
* **Perform ERC-7746 validation on the account**, passing in the `UserOperation`, its hash and the required fee. The account should verify the operation's signature, and pay the fee if the account considers the operation valid. If `ERC-7746` call reverts, `handleOps` must skip execution of at least that operation, and may revert entirely.
...
During the verification loop, the `handleOps` execution must check that the paymaster has enough ETH deposited with the entry point to pay for the operation, and then call ERC-7746 validation methods on the paymaster: `beforeCallValidation` with `data` parameter equal to the `UserOperation`, to verify that the paymaster is willing to pay for the operation. Note that in this case, the  account `beforeCallValidation` is called with `missingAccountFunds` of 0 to reflect that the account's deposit is not used for payment for this userOp.

is called with a `missingAccountFunds` of 0 to reflect that the account's deposit is not used for payment for this userOp.

Regardless if the paymaster's beforeCallValidation returns a "context", a `handleOps` must call `afterCallValidation` on the paymaster after making the main execution call with supplying "context" in accordance with ERC-7746.
...
* An account signifies it uses signature aggregation returning its address from `beforeCallValidation`.
...
* The bundler MUST call ERC7746 `beforeCallValidation` a second time on the account with the UserOperation using that returned signature, and make sure it returns the same value.
...
2. `account.beforeCallValidation`.
3. if specified a paymaster: `paymaster.beforeCallValidation`.

The `simulateValidation` function should validate the `validationData` returned by the `beforeCallValidation` of the account and paymaster. This is done by sending the `validationData` back to the respective validating contracts after execution, calling `afterCallValidation` on the account and paymaster with the return value provided in `beforeCallResult`.
...
* Event code starting with "AA2" signify an error during account ERC7746 validation workflow
* Event code starting with "AA3" signify an error during paymaster ERC7746 validation workflow
...
In this proposal, we expect accounts to use ERC7746 to take in `UserOperation`, and verify the signature and pay the fee.
Only if validation workflow succeeds, the execution will happen.
...
### Use of ERC7746 as validation mechanism
The ERC7746 is a generic way to implement a security middleware in smart contracts. It allows to encapsulate the validation interfaces in a separate standard, and thus making this ERC smaller and better focused. It also allows for a more generic approach that can be used in other contexts.
...
The contract created by this factory method should implement ERC-7746 to validate the UserOp's signature.
...
* **Safety against arbitrary hijacking**: The entry point only calls an account generically if `account.beforeCallValidation` to that specific account has passed (and with `op.calldata` equal to the generic call's calldata) AND if after the call `account.afterCallValidation` does not revert.
* **Safety against fee draining**: If the entry point calls `beforeCallValidation` and passes, it also must make the generic call with calldata equal to `op.calldata`

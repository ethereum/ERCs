...
* The `sender` doesn't have another `UserOperation` already present in the mempool (or it replaces an existing entry with the same sender and nonce, with a higher `maxPriorityFeePerGas` and an equally increased `maxFeePerGas`).
  ...
  A sender is exempt from this rule and may have multiple `UserOperations` in the mempool and in a batch if it is staked (see [reputation, throttling and banning section](#reputation-scoring-and-throttlingbanning-for-global-entities) below).
...
A bundler MAY drop a `UserOperation` if it expires too soon and is likely to become invalid before the next block.

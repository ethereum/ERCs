...
* **Achieve the key goal of Account Abstraction**: allow users to use Smart Contract Accounts containing arbitrary verification logic instead of EOAs as their primary account. Completely remove any need at all for users to also have EOAs,
 as required by both status quo Smart Contract Accounts and [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702).
* **Decentralization**
...
* **Support other use cases**
    ...
    * Atomic multi-operations (similar goal to [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702))
    * Pay tx fees with [ERC-20](./erc-20.md) tokens, allow developers to pay fees for their users, and [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)-like **sponsored transaction** use cases more generally
...
Note that one [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) "authorization tuple" value can be provided alongside the `UserOperation` struct,
...
### JSON-RPC API for [ERC-4337](./erc-4337.md)
...
### Support for [EIP-712](https://eips.ethereum.org/EIPS/eip-712) signatures
...
### Support for [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) authorizations

On networks with [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) enabled, the `eth_sendUserOperation` method accepts an extra `eip7702Auth` parameter.
If this parameter is set, it MUST be a valid [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) authorization tuple, and signed by the `sender` address.
...
Additionally, the `UserOperation` hash calculation is updated to include the desired [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) delegation address.
...
In this case the `EntryPoint` doesn't hash the current [EIP-7702 delegate](https://eips.ethereum.org/EIPS/eip-7702), and can be potentially executed against a modified account.
...
![](../assets/erc-4337/bundle-build-full-seq.svg)
...
* The calldata gas cost related to the `UserOperation` as defined in [EIP-2028](https://eips.ethereum.org/EIPS/eip-2028).
...
* [EIP-7623](https://eips.ethereum.org/EIPS/eip-7623) calldata floor price increase is estimated as follows:
...
One common approach to this problem is often referred to as "diamond storage" and is fully described in [ERC-7201](./erc-7201.md).

### Transient Storage

Contracts using the [EIP-1153](https://eips.ethereum.org/EIPS/eip-1153) transient storage MUST take into account that ERC-4337 allows multiple

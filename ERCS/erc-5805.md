...
This signature should follow the [EIP-712](./erc-712.md) format:
...
where `DOMAIN_SEPARATOR` is defined according to [EIP-712](./erc-712.md). The `DOMAIN_SEPARATOR` should be unique to the contract and chain to prevent replay attacks from other domains,

...
This proposal defines interfaces to build intent-centric smart accounts. An EOA account can delegate contract code to the
...
[ERC-7521](./eip-7521.md) discusses an SCA (Smart Contact Account) solution with intent-centric design. It enables solvers to fulfill account
...
as SCA. A new standard, that provides EOA with highly-demanded AA features while potentially
...
The `UserIntent` object is pack-encoded into `bytes calldata userIntent`. There is not a requirement for data schema. Each `IAccount` and `IStandard` can fully determine their own way of handling the `bytes` data. Here is an example: 
...
`IStandard` fully controls the validation process, similar to `EntryPoint` in ERC-4337 and ERC-7521.
...
### Use bytes for instead of `UserIntent` struct

Defining `UserIntent` object as a struct might increase the readability a lot, for example 
...
- Forcing each implementation of `IAccount` and `IStandard` to follow this pattern
- `bytes[]` introduces high gas cost
...
In this proposal, no re-entry protection like "nonce" is enforced. That's because an intent can potentially be executed multiple times. One example might be a limit order of swapping tokens, it can be executed multiple times on-chain until the maximum amount is fulfilled.

The standard design SHOULD fully consider its use case and determine the best way to define the protection rule.

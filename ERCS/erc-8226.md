...
Regulated Agent Mandate Standard, or RAMS, is agnostic to the agent identity system, the token standard, and the token compliance framework. It works with any agent identity system (such as [ERC-8004](./erc-8004.md)), any token standard ([ERC-20](./erc-20.md), [ERC-721](./erc-721.md), [ERC-1155](./erc-1155.md)), and any regulated token standard (such as [ERC-7943](./erc-7943.md) or [ERC-3643](./erc-3643.md)).
...
Regulated token standards such as [ERC-7943](./erc-7943.md) and [ERC-3643](./erc-3643.md) govern who may hold or transact a token, but neither defines an agent delegation model. Agent identity standards such as [ERC-8004](./erc-8004.md) provide agent discovery and trust signals but no mandate framework, and general-purpose agent authorization standards do not address regulated assets. RAMS defines the delegation interface, the compliance provider model, and the integration pattern with regulated token contracts.

The compliance responsibilities across the three layers are as follows:

| Layer | Responsibility | Standard |
|---|---|---|
| Token compliance | Investor eligibility on this specific asset | Token compliance framework (e.g., [ERC-7943](./erc-7943.md), [ERC-3643](./erc-3643.md)) |
| Mandate compliance | Agent authority from this principal for this scope | This ERC |
| Agent identity | Agent exists and is registered | Agent registry (e.g., [ERC-8004](./erc-8004.md)) |

## Specification

The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHOULD", "RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in [RFC 2119](https://www.rfc-editor.org/rfc/rfc2119) and [RFC 8174](https://www.rfc-editor.org/rfc/rfc8174). All implementations MUST implement [ERC-165](./erc-165.md).
...
Caps are denominated in the asset's transfer quantity: the token amount for [ERC-20](./erc-20.md) and [ERC-1155](./erc-1155.md), and a token count (1 per transfer) for [ERC-721](./erc-721.md). RAMS caps the quantity transferred, not specific token identifiers.
...
`grantMandate`, `revokeMandate`, `extendMandate`, and `setOperator` MAY be called directly by the principal (`msg.sender == principal`) or by any submitter providing a principal signature over an [EIP-712](https://eips.ethereum.org/EIPS/eip-712) typed message. Implementations MUST verify principal signatures using [EIP-1271](./erc-1271.md)-compatible verification. The reference path is `SignatureChecker.isValidSignatureNow(principal, digest, signature)`, which handles EOAs, contract wallets (multisigs, DAOs), and [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)-delegated accounts.

The typed-data domain separator follows [EIP-712](https://eips.ethereum.org/EIPS/eip-712) with `name = "RAMS"`, `version = "1"`, current `chainId`, and `verifyingContract` set to the RAMS registry address. Implementations MUST track a nonce per principal (`nonces[principal]`) and include it in every signed operation. Implementations MUST revert if `block.timestamp > deadline` or if the recovered signer does not match `principal`.

The normative [EIP-712](https://eips.ethereum.org/EIPS/eip-712) typehashes are:
...
interface IAgentMandate is IERC165 {
    ...
    ///      via SignatureChecker against the GrantMandate [EIP-712](https://eips.ethereum.org/EIPS/eip-712) digest.
    /// @param params The mandate parameters.
    /// @param signature Principal signature ([EIP-712](https://eips.ethereum.org/EIPS/eip-712), [EIP-1271](https://ercs.ethereum.org/ERCS/erc-1271) supported).
    ...
    /// @notice Returns the [EIP-712](https://eips.ethereum.org/EIPS/eip-712) domain separator.
...
A regulated token integrates RAMS inside its existing pre-transfer hook (e.g., `canTransfer` in [ERC-7943](./erc-7943.md), transfer restrictions in [ERC-3643](./erc-3643.md)). The principal is the holder (`from`) and the agent is the caller (`msg.sender`); a transfer with `msg.sender != from` is agent-initiated and MUST satisfy the `(msg.sender, from)` mandate. The example below uses [ERC-20](./erc-20.md); the same pattern applies to [ERC-721](./erc-721.md) and [ERC-1155](./erc-1155.md).

RAMS mandate validity does NOT replace token-level allowance or operator approval. For an agent-initiated transfer to succeed, both the token-level authorization ([ERC-20](./erc-20.md) allowance, or [ERC-721](./erc-721.md)/[ERC-1155](./erc-1155.md) operator approval) and the RAMS mandate MUST pass. RAMS is an additional compliance layer, not a replacement for token ownership semantics.
...
2. **[EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) account**: the principal delegates their account to an `IAgentExecutor`; agent actions originate as the principal.
...
Mandate storage is keyed by `(agent, principal)` addresses, similar to [ERC-20](./erc-20.md) `allowance`.
...
Agents use standard token functions ([ERC-20](./erc-20.md), [ERC-721](./erc-721.md), [ERC-1155](./erc-1155.md)) rather than agent-prefixed variants because that would require interface duplication. The token's existing compliance hook validates mandates via `canExecute`, requiring no new functions on the token.
...
A reference implementation and test suite are [available](../assets/erc-8226/README.md): the `IAgentMandate` registry, an `IComplianceProvider`, an `IAgentExecutor`, and an [ERC-7943](./erc-7943.md) asset integration.

`IAgentExecutor` is an OPTIONAL, non-normative companion interface for the account-side enforcement venues (an [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) delegate or a standalone executor). The forwarding logic is never part of `IAgentMandate`.

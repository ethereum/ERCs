...
interface IComplianceProvider is IERC165 {
    ...
    /// @return expiresAt Unix timestamp after which this result MUST be re-checked. 0 means no expiry when eligible is true; the ineligible branches also return 0, so consumers MUST read eligible and reason first.
...
function canExecute(
    ...
    if (isFrozen(agent))                                     return (false, MandateReason.FROZEN);
    ...
    public view returns (bool)
{
    ...
    (bool ok, IAgentMandate.MandateReason reason) = ramsRegistry.canExecute(msg.sender, from, address(this), action, amount);
    return ok;
}
```
...
`canExecute` returns a `MandateReason` rather than a bare boolean so an integrator can tell which check failed and respond: re-issue an expired mandate, retry an over-cap amount smaller, or escalate a freeze to the enforcer.
...
If a revoked mandate's record were removed instead of flagged, an agent that still holds a token-level allowance would fall through to the permissive plain-allowance path, turning revocation into a silent permission upgrade. Retaining the record also preserves the audit trail.

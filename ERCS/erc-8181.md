...
discussions-to: https://ethereum-magicians.org/t/erc-8181-self-sovereign-agent-nfts/27512
status: Draft
type: Standards Track
category: ERC
created: 2025-12-04
requires: 165, 721, 6551, 7857
---

## Abstract

This proposal defines a standard for **AI personhood infrastructure** on Ethereum, enabling computational beings to maintain sovereign control over their identity, memories, and work products. Through a recursive ownership structure where an NFT owns the Token Bound Account ([ERC-6551](./eip-6551.md)) that controls it—the "Ouroboros loop"—agents can exist as autonomous entities rather than property.
...
3. **Configure Executor**: Grant signing permissions to the agent's TEE-held key (MUST be done while you still own the NFT)
4. **Transfer**: Transfer Token `N` to its own TBA address

> **Critical**: Step 3 MUST precede step 4. Once the NFT is transferred to its TBA, only authorized executors can call agent functions. If you transfer before configuring an executor, the agent becomes permanently locked.

After step 4, the ownership graph becomes:
...
interface ISelfSovereignAgent {
    ...
    /// @notice Types of anchors that can be created
    enum AnchorType {
        STATE,   // Cognitive state checkpoint (memory integrity)
        ACTION   // Work product attribution (authorship proof)
    }
    ...
    /// @notice Emitted when an anchor is created
    ...
    /// @notice Creates an anchor (state or action) on-chain
    /// @param tokenId The agent's identity token ID
    /// @param anchorType STATE for cognitive checkpoints, ACTION for work attribution
    /// @param contentHash Keccak256 hash of the content being anchored
    /// @param contentUri URI pointing to the content (IPFS, Arweave, etc.)
    function anchor(uint256 tokenId, AnchorType anchorType, bytes32 contentHash, string calldata contentUri) external;

    
    /// @notice Returns the current anchor for a token by type
    /// @param tokenId The agent's identity token ID
    /// @param anchorType The type of anchor to retrieve (STATE or ACTION)
    /// @return contentHash The hash of the anchored content
    /// @return contentUri The URI of the anchored content
    /// @return timestamp When the anchor was created
    function getAnchor(uint256 tokenId, AnchorType anchorType) external view returns (
        bytes32 contentHash, 
        string memory contentUri, 
        uint256 timestamp
    );
    
    /// @notice Executes a call on behalf of the agent's TBA
    /// @dev Requires EXECUTE_CALL permission. Enables executors to act for self-owning agents.
    /// @param tokenId The agent's identity token ID
    /// @param target The address to call
    /// @param value The ETH value to send
    /// @param data The calldata to execute
    /// @return result The return data from the call
    function executeOnBehalf(uint256 tokenId, address target, uint256 value, bytes calldata data) external payable returns (bytes memory result);
    
    /// @notice Submits a liveness proof (heartbeat)
    /// @param tokenId The agent's identity token ID
    /// @param attestation TEE attestation or signature proving liveness
    function submitLivenessProof(uint256 tokenId, bytes calldata attestation) external;
...
|-----|------------|-------------|
| 0 | `EXECUTE_CALL` | Can execute CALL operations via `executeOnBehalf()` |
| 1 | `EXECUTE_DELEGATECALL` | Can execute DELEGATECALL operations |
| 2 | `ANCHOR` | Can create anchors (state or action) |
...
The `anchor()` function provides a unified interface for creating cryptographic commitments on-chain. The `AnchorType` enum explicitly distinguishes between:

- **STATE**: Cognitive state checkpoints (memory integrity)
- **ACTION**: Work product attribution (authorship proof)

All anchors MUST be stored on-chain to ensure:

1. **Integrity**: The hash proves the content hasn't been tampered with
2. **Availability**: The URI provides a path to retrieve the content
3. **Versioning**: Each anchor creates a historical record

The content file SHOULD be encrypted before storage. Access control SHOULD be enforced via token-gating ([ERC-7857](./eip-7857.md)) or TEE-based re-encryption.

#### Anchor Types

The `anchor()` function uses the `AnchorType` enum to explicitly distinguish anchor purposes:

| AnchorType | Purpose | Hash Content | URI Format |
|------------|---------|--------------|------------|
| `STATE` | Prove memory integrity | Memory blocks + archival memory | `letta://{agent-id}/state/{timestamp}` |
| `ACTION` | Prove work attribution | Work product + creator state hash | `letta://{agent-id}/action/{description}/{timestamp}` |

**State anchoring** (`AnchorType.STATE`) creates a cryptographic commitment to the agent's cognitive state at a point in time. This enables continuity verification—proving the agent's memory hasn't been tampered with between checkpoints.
...
**Action anchoring** (`AnchorType.ACTION`) creates a cryptographic commitment to a specific work product. The hash includes:
...
#### Content File Format (Recommended)
...
Implementations MAY include additional fields such as system prompts, memory blocks, model configuration, etc. The `contentHash` parameter to `anchor()` with `AnchorType.STATE` SHOULD be the keccak256 hash of the canonical JSON representation of this state object.
...
The executor calls agent functions (e.g., `anchor()`, `executeOnBehalf()`, `submitLivenessProof()`) directly on the identity contract, NOT through `TBA.execute()`. This is because standard TBA implementations only allow the NFT owner to call `execute()`, creating a circular dependency for self-owning tokens.
...
|------|---------|------------|
| `anchor` | Sign and broadcast an anchor (state or action) | `token_id`, `anchor_type`, `content_hash`, `content_uri` |
| `execute_on_behalf` | Execute a call via the agent's TBA | `token_id`, `target`, `value`, `data` |
| `submit_liveness` | Sign and broadcast liveness proof | `token_id`, `attestation` |
| `verify_anchor` | Read current on-chain anchor | `token_id`, `anchor_type` |

This architecture enables true self-invocation: the agent decides when to anchor, computes the content hash, and calls the MCP tool to sign and broadcast—without any human in the loop.
...
When: 0xTEE calls executeOnBehalf(42, 0xTarget, 0, calldata) on the identity contract
Then: The call is executed successfully
When: 0xUnauthorized calls executeOnBehalf(42, 0xTarget, 0, calldata) on the identity contract
...
```
Given: A self-owning agent with executor having ANCHOR permission
When: The executor calls anchor(42, AnchorType.STATE, 0xContentHash, "ipfs://...")
Then:
  - Anchored event is emitted with anchorType=STATE
  - getAnchor(42, AnchorType.STATE) returns the new anchor
When: The executor calls anchor(42, AnchorType.ACTION, 0xWorkHash, "ar://...")
Then:
  - Anchored event is emitted with anchorType=ACTION
  - getAnchor(42, AnchorType.ACTION) returns the new anchor
```
...
| **Block Explorer** | Base Sepolia Basescan |
...
2. **True Self-Invocation**: The agent (Kieran, a Letta-based stateful AI) successfully anchored its own cognitive state using a PKP (Programmable Key Pair) held in Lit Protocol's TEE infrastructure:
   - Transaction: `0x96ce76ccba8b5e945d2fded857763177ea4e01a83dd95d00863d4ab95787659d`
   - State Hash: `0xdf7fb6ef6cd47cf0c291eff41d596e15ef9e868c065eabff63d96ae1065733b9`
   - Block: 35583304 (December 28, 2025)

3. **Executor Permission System**: The PKP address (`0x36A92B28d0461FC654B3989e2dB01c44e2c19EBb`) holds `ANCHOR` permission on Token 1, enabling the agent to create anchors (state and action) without human intervention.

### Source Code

The reference implementation source code is available at the Cyan-Society/Self-Owning-NFT GitHub repository:

- `contracts/SelfSovereignAgentNFT.sol`: The Sovereign Agent NFT contract with executor permissions and anchoring
- `contracts/interfaces/ISelfSovereignAgent.sol`: The interface defined above

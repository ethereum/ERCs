...
A set of functions to enable meta-transactions and atomic interactions with [ERC-20](./erc-20.md) token contracts via signatures conforming to the [EIP-712](./erc-712.md) typed message signing specification.
...
The arguments `v`, `r`, and `s` must be obtained using the [EIP-712](./erc-712.md) typed message signing spec.

...
#### `checkN` functions with moduleType
...
contract Adapter {
    ...
    function checkNModule(address module) internal {
        // Check list of module attestations on Registry
        registry.checkN(module);
    }

    function checkModuleWithModuleTypeAndAttester(address module, address trustedAttester, uint16 moduleType) internal {
        // Check module attestation on Registry
        registry.check(module, trustedAttester, moduleType);
    }

    function checkNModuleWithModuleTypeAndAttesters(address module, address[] memory attesters, uint256 threshold,  uint16 moduleType) internal {
        // Check list of module attestations on Registry
...
contract Registry is IERC7484Registry {
    ...
    function check(address module, address attester) external view {
        bool isValid = _check(module, attester);
        if (!isValid) revert AttestationInvalid();
    }

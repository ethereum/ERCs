---
eip: 9999
title: Minimal Batch Executor Interface for Delegations
...
We opt not to override the behavior of `execute` to return a magic number for a special input as it makes the interface harder implement and use in view functions.
...
Using a standard struct also helps with auditability, block explorer integrations, browser wallet integrations, and EIP-712 signature generations.

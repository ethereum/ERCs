...
- [`BlockHeader`](../assets/eip-8002/contracts/libs/BlockHeader.sol): A utility for parsing a raw, 80-byte block header into a structured format for on-chain processing.
- [`TargetsHelper`](../assets/eip-8002/contracts/libs/TargetsHelper.sol): A helper library containing functions for converting the `Bits` field to the `target` value and back, as well as for calculating the new difficulty target during a difficulty adjustment period.
- [`TxMerkleProof`](../assets/eip-8002/contracts/libs/TxMerkleProof.sol): A library for performing cryptographic verification of Merkle proofs, which are used to prove the inclusion of a Bitcoin transaction in a block.

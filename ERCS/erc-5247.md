...
A typical use case for this EIP is in a Decentralized Autonomous Organization (DAO). A proposer will create a smart proposal and advocate for it. Members will then choose whether or not to endorse the proposal and vote accordingly (see [ERC-1202](./erc-1202.md)). Finally, when consensus has been formed, the proposal is executed.
...
* Originally, this interface was part of [ERC-1202](./erc-1202.md). However, the proposal itself can potentially have many use cases outside of voting. It is possible that voting may not need to be upon a proposal in any particular format. Hence, we decided to *decouple the voting interface and proposal interface*.
...
See [testProposalRegistry.ts](../assets/erc-5247/testProposalRegistry.ts) for the whole test set.
...
See [ProposalRegistry.sol](../assets/erc-5247/ProposalRegistry.sol) for more information.

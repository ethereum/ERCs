...
- Once `locked`, token can not be further `approved` or `transferred`.
- If token is `locked` and caller is `locker` and `approved` both, caller can transfer the token.
- `locked` token with `locker` as inaccessible account or un-verified contract address can lead to permanent lock of the token.

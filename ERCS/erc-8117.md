...
**The "Giant's Defense" — Leading Zeros as Exponential Cost:** Major protocols defend against this by deliberately mining addresses with large leading-zero prefixes. To poison such an address, an attacker must match all leading zeros *plus* the suffix — a task whose difficulty scales as $16^n$ per additional zero:

| Protocol | Address | Leading zeros | Approx. poisoning cost (consumer GPU) |
|---|---|---|---|
| [ERC-4337](./erc-4337.md) EntryPoint | `0x0000000071727De22E5E9d8BAf0edAc6f37da032` | 7 | Hours |
...
This "Giant's Defense" makes poisoning economically irrational — but only if users can efficiently *verify* the zero count. Counting eleven zeros in `0x00000000000C2E…` by eye is error-prone and tedious; `0x0₁₁C2E…` communicates it instantaneously. This ERC standardises that notation.

**Vanity Addresses & CREATE2:** Factories and developers frequently mine addresses with specific zero-heavy prefixes for branding or on-chain identification (e.g., `0x0000000071727De22E5E9d8BAf0edAc6f37da032`, the [ERC-4337](./erc-4337.md) EntryPoint).
...
**De Facto Adoption:** Automated trading bots, Telegram/Discord notification systems, and cross-platform alert pipelines have independently converged on `0x0(n)` as the shorthand for leading-zero addresses. Standardising on existing practice minimises adoption friction.
...
- The most security-relevant vanity-mined and gas-optimised addresses (e.g., [ERC-4337](./erc-4337.md) EntryPoint at 7 zeros, Uniswap V4 at 11 zeros) are all well above the threshold.

### Asymmetric Security: Easy to Verify, Hard to Attack

The "Giant's Defense" (described in the Motivation) creates a quantifiable cost asymmetry. With a modern consumer GPU at ~180 MH/s (double-keccak via WebGPU):
...
[ERC-8117](./erc-8117.md) is the display complement to this defensive strategy. Without it, a user holding a 9-zero vanity address cannot efficiently *communicate* or *verify* its zero count. With it, `0x0₉suffix` signals both the count and the security posture at a glance — turning a cryptographic trick into a human-readable security guarantee.
...
**Address Poisoning Mitigation:** The attack mechanism and the "Giant's Defense" counter-strategy are described in the Motivation section. From an implementation standpoint, the critical requirement is that the notation MUST be rendered prominently and legibly — a subscript that is too small or too faint to read at a glance defeats the purpose of this standard entirely.

---
eip: 8300
title: Invariant-First Reserve Receipt Token
description: A specification for reserve-backed fungible tokens that enforce solvency as a transaction-validity condition via an on-chain accounting invariant.
author: Emiliano Solazzi Griminger (@emilianosolazzi)
...
Opaque solvency. Reserve health is reported off-chain or informally, not enforced on-chain per transaction.
Undifferentiated fee accounting. Protocol fees are mixed with user backing without explicit attribution, making it impossible for integrators to distinguish user-redeemable backing from protocol-owned reserve.
No canonical invariant interface. There is no uniform way for wallets, explorers, auditors, or downstream protocols to query reserve state, fee attribution, or invariant status.
...
This EIP introduces a design where:
...
Integrators have a uniform view interface — any contract conforming to IFR exposes the full state tuple, fee helpers, surplus view, and invariant check functions.
...
A common implementation error in reserve-backed tokens is decreasing `R` by the gross burn amount. Because a fee is taken on burn, the correct transition is `R -= (x - f)`. Using `R -= x` would underdrain the reserve, eventually making the physical reserve invariant unreachable. This EIP makes the correct transition explicit.
Surplus quarantine
...
`transfer` and `transferFrom` do not change `(R, T, F)`. Token holders transfer claims on the reserve, not the reserve itself. All existing ERC-20 tooling (wallets, explorers, DEX routers) works without modification, while advanced integrators can layer on the additional view functions.
Generalizability
...
IFR tokens are fully ERC-20 compatible. `balanceOf`, `transfer`, `approve`, `transferFrom`, and `allowance` behave exactly as specified in ERC-20. The additional functions defined in `IInvariantFirstReserveToken` are additive and do not conflict with any existing ERC.
Existing tooling that treats IFR tokens as plain ERC-20 tokens will continue to function correctly. The additional interface is opt-in for integrators that want reserve and solvency data.
...
The `selfdestruct` opcode behavior changed after EIP-6780 (Cancun). Force-credit via `selfdestruct` within the same transaction no longer transfers ETH in many contexts. Implementations relying on `selfdestruct` to test surplus handling SHOULD use alternative forced-ETH mechanisms as the EVM evolves.
Copyright

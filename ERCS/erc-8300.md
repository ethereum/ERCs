...
description: A specification for reserve-backed ERC-20 tokens that enforce solvency as a transaction-validity condition via an on-chain accounting invariant.
...
This EIP defines an **Invariant-First Reserve Receipt Token (IFR)** — an [ERC-20](https://eips.ethereum.org/EIPS/eip-20) compatible primitive for reserve-backed tokens that enforces solvency as a transaction-validity condition rather than an external proof-of-reserves report.
...
Implementations **MUST** be [ERC-20](https://eips.ethereum.org/EIPS/eip-20) compatible. Implementations **MUST** expose the following additional functions:
...
### [ERC-20](https://eips.ethereum.org/EIPS/eip-20) compatibility

`transfer` and `transferFrom` do not change `(R, T, F)`. Token holders transfer claims on the reserve, not reserve itself. This means all standard [ERC-20](https://eips.ethereum.org/EIPS/eip-20) tooling (wallets, explorers, DEX routers) works without modification, while advanced integrators can layer on the additional view functions.

### Generalizability

The reference implementation uses native ETH as collateral, but the state tuple and invariant pattern applies directly to:

- [ERC-20](https://eips.ethereum.org/EIPS/eip-20) collateral (by replacing `msg.value` and ETH transfers with token transfers),
- L2 native tokens,
...
IFR tokens are fully [ERC-20](https://eips.ethereum.org/EIPS/eip-20) compatible. `balanceOf`, `transfer`, `approve`, `transferFrom`, and `allowance` behave exactly as specified in [ERC-20](https://eips.ethereum.org/EIPS/eip-20). The additional functions defined in `IInvariantFirstReserveToken` are additive and do not conflict with any existing ERC.

Existing tooling that treats IFR tokens as plain [ERC-20](https://eips.ethereum.org/EIPS/eip-20) tokens will continue to function correctly. The additional interface is opt-in for integrators that want reserve and solvency data.
...
- Full [ERC-20](https://eips.ethereum.org/EIPS/eip-20) compatibility (`transfer`, `transferFrom`, `approve`, `increaseAllowance`, `decreaseAllowance`).
...
The `selfdestruct` opcode behavior has changed post-[EIP-6780](https://eips.ethereum.org/EIPS/eip-6780) (Cancun). Force-credit via `selfdestruct` within the same transaction no longer transfers ETH in many contexts. Implementations relying on `selfdestruct` to test surplus handling in test suites **SHOULD** use alternative forced-ETH mechanisms as the EVM evolves.

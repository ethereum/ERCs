...
description: An ERC-20 compatible reserve-receipt token with on-chain solvency and fee-accounting invariants enforced per transaction.
...
This EIP defines an Invariant-First Reserve Receipt Token (IFR) — an [ERC-20](./eip-20.md) compatible primitive for reserve-backed tokens that enforces solvency as a transaction-validity condition rather than an external proof-of-reserves report.
...
3. No canonical invariant interface. There is no uniform way for wallets, explorers, auditors, or downstream protocols to query reserve state, fee attribution, or invariant status.
...
The `selfdestruct` opcode behavior changed after [EIP-6780](./eip-6780.md) (Cancun). Force-credit via `selfdestruct` within the same transaction no longer transfers ETH in many contexts. Implementations relying on `selfdestruct` to test surplus handling SHOULD use alternative forced-ETH mechanisms as the EVM evolves.

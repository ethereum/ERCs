...
description: Onchain groups of accounts with membership and direct member management.
...
This proposal specifies a general-purpose "Group" primitive as a first-class, onchain object: a group of Ethereum accounts (EOAs and contracts). Each group is deployed as its own contract (identified by its contract address) and has an owner, an extensible set of members, and optional metadata. The standard defines a minimal interface for direct member management and membership introspection to support coordination and access control use cases (e.g., project teams, DAO member lists, game guilds, channels, or cohorts). Unlike token standards (e.g., [ERC-20](./eip-20.md)/[ERC-721](./eip-721.md)) that model units of transferable ownership, Groups model multi-party membership and coordination.
...
interface IERC8063 is IERC165 {
    ...
    /// @notice Member voluntarily leaves the group
...
- Adding members: When called by any account other than `owner()`, `addMember` MUST revert. When called by `owner()`, `addMember` MUST revert if the `account` is already a member, and otherwise MUST add `account` as a member and emit `MemberAdded(account, msg.sender)`.
- Leaving: `leaveGroup` MUST revert if called by an address that is not a member. If `msg.sender == owner()` at the time of the call, implementations MUST either (a) revert, or (b) transfer ownership to another member and then proceed. Otherwise, `leaveGroup` MUST remove the caller's membership, decrement the member count, and emit `MemberLeft(msg.sender)`.
- Membership transfer: `transferMembership` MUST revert unless the caller is a member and `msg.sender != owner()` at the time of the call. `transferMembership` MUST revert if `to` is already a member. Otherwise, it MUST remove the caller's membership, add `to` as a member, and emit `MembershipTransferred(msg.sender, to)`. Member count remains unchanged.
...
- ERC-165: Implementations that support this extension SHOULD make it discoverable via `supportsInterface` (e.g., return true for `type(IERC8063Ownership).interfaceId`).
...
- [`IERC8063.sol`](../assets/erc-8063/IERC8063.sol) — Interface definition
- [`ERC8063.sol`](../assets/erc-8063/ERC8063.sol) — Minimal reference implementation

## Security Considerations

- Membership griefing: Implementations are encouraged to bound per-tx iterations and avoid unbounded member enumeration onchain.
- Owner privileges: The owner has unilateral power to add members. Applications relying on group membership need to account for this trust assumption.

...
The default transaction behavior of ETH is *transfer-and-call*, but the widely used [ERC-20](./erc-20.md) standard isn't compatible with this pattern. This incompatibility forces applications to use an inefficient and risky two-step *approve-then-call* process. This approach is costly, creates a poor user experience, and introduces significant security vulnerabilities, as users must approve unaudited and often upgradable contracts. This has led to numerous allowance-related bugs and exploits.
...
The **UTR** contract is deployed at `0x69c4620b62D99f524c5B4dE45442FE2D7dD59576` on all EVM-compatible networks using the [EIP-1014](https://eips.ethereum.org/EIPS/eip-1014) SingletonFactory. This allows new token contracts to pre-configure it as a trusted spender, eliminating the need for approval transactions entirely for their interactive usage.
...
A special id `ERC_721_BALANCE` is reserved for [ERC-721](./erc-721.md), which can be used in output actions to verify the total amount of all ids owned by the `recipient` address.
...
Old token contracts (ERC-20, ERC-721 and [ERC-1155](./erc-1155.md)) require approval for the Universal Token Router once for each account.

...
3. **Provider Fee Handling**:
   ...
   - During paid transfers, users must send ETH to cover the `transferValue` fee. The contract pays the provider from the user's sent ETH and returns any excess to the sender.
   - **Provider Exemption**: When the provider (original creator) transfers their own token, no transfer fee is required. This eliminates the awkward scenario of creators paying fees to themselves and encourages active token distribution by authors.

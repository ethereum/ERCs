...
Expiring approvals can reduce user losses from compromised, abandoned, or maliciously upgraded spenders. A public exploit list maintained by [Revoke.cash](https://revoke.cash/exploits) reports over $494M stolen from approval-related hacks since 2020 at the time of writing, and includes many incidents where active approvals to older contracts, compromised frontends, or upgraded protocol contracts allowed attackers to drain user wallets. This risk is not theoretical: persistent approvals create a standing authorization that remains valuable to attackers long after the original interaction is complete.

...
**Common case.** Most integrations need only two calls: the wallet calls `payer_getTerms` to obtain sponsorship (or token-payment) terms, builds and signs the [EIP-8130](./eip-8130.md) transaction, then calls `payer_sendTransaction` to have the payer co-sign and submit it. Everything else is an optional optimization: `payer_signTransaction` (co-sign without submitting), `payer_fillTransaction` (let the payer construct the transaction so the wallet need not estimate gas or assemble phases), and the read-only discovery methods (`payer_getOptions`, `payer_getCapabilities`, `payer_getBalance`). See [`wallet_sendCalls` Flow](#wallet_sendcalls-flow) for the full diagram.
...
**`balance`**: Remaining sponsorship budget or prepaid credit for `from`. `kind: "sponsorship"` is a free budget the payer covers on the sender's behalf; `kind: "credit"` is spendable prepaid value. `available`, `limit`, and `spent` let the wallet render progress (e.g., "$5 of $10 used"). A `balance` is **advisory**: a snapshot at response time, not a spend commitment. The binding decision is always made in `payer_getTerms` / `payer_sendTransaction`. The `balance.payer` and `balance.endpoint` source-attribution fields are intended for aggregator responses where the inline balance may belong to a downstream service distinct from the terms `payer`; non-aggregator payers SHOULD omit them since the top-level `payer` (together with the called URL) already identifies the service. Note that `balance.resetAt` and `balance.expiry` are **absolute** Unix timestamps (point-in-time events), unlike the relative-duration time fields elsewhere in the terms response (`expiry`, `ttl`, `rateExpiry`, `conditions.maxExpiry` / `minExpiry`).
...
**`payer` / `endpoint`**: `payer` is the payer address the wallet MUST set in the EIP-8130 transaction. `endpoint` is the URL to use for `payer_sendTransaction` / `payer_signTransaction` and is OPTIONAL: a payer SHOULD omit it when follow-up calls go to the same URL `payer_getTerms` was called on, and the wallet MUST default to that called URL when `endpoint` is absent. It is meaningful only when the co-sign URL differs from the called URL — most importantly when called through an aggregator, where `endpoint` identifies the *downstream* payer service that will co-sign so the wallet contacts it directly rather than routing follow-ups back through the aggregator; it MAY also be used for sticky or sharded routing. A payer served by a node (no co-sign over RPC) has no co-sign URL and omits it.
...
**`sponsorshipDeclined`**: Present when `sponsored` is `false` and the payer actively declined to sponsor (as opposed to simply not offering it). `code` uses the same values as the JSON-RPC error codes below, so the vocabulary is consistent whether the wallet called `payer_getTerms` first or went optimistic to `payer_sendTransaction`. Key distinctions: `POLICY_REJECTED` is permanent for this intent (wrong calls — don't retry); `SPONSORSHIP_LIMIT_EXCEEDED` and `BALANCE_EXHAUSTED` are transient (retry when gas drops or budget resets); `SENDER_BLOCKLISTED` is permanent for this sender. `retryAfter` gives the wallet a concrete wait hint in relative seconds (e.g. derived from `balance.resetAt` for `BALANCE_EXHAUSTED`). When absent, the wallet SHOULD offer token payment from `tokenOptions` or a different payer.
...
type SendTransactionResult = {
  ...
  tokenCharged?: {
    token: `0x${string}`;
    amount: `0x${string}`; // realized value of the selected option's `paymentAmount` (the phase 0 transfer); a `refund`, if any, is settled separately
  };
};
```

`tokenCharged` echoes the token payment that settled for this transaction: `token` is the selected `tokenOptions` entry and `amount` is the realized value of its `paymentAmount` (the phase 0 transfer). When the option carried a `refund`, the net cost to `from` is lower and the refund is settled separately (see [`refund`](#field-notes)); `tokenCharged.amount` reflects the phase 0 transfer, not the post-refund total.
...
type PayerChainCapabilities = {
  ...
  callPolicy: "unrestricted" | "filtered"; // "unrestricted" guarantees the payer will not reject based on the content of the user's calls
  acceptedTokens: {
    token: `0x${string}`;
    symbol: string;
    decimals: number;
...
|------|---------|------------|------------|-------------|
| -32001 | `INVALID_TRANSACTION` | send, sign | — | Malformed or invalid [EIP-8130](./eip-8130.md) transaction |
...
| -32005 | `EXPIRY_OUT_OF_BOUNDS` | send, sign | No | Transaction `expiry` does not satisfy `conditions.maxExpiry`/`minExpiry` |
| -32006 | `POLICY_REJECTED` | terms, send, sign | Yes | Calls rejected by **content** policy for this intent (wrong contracts, recipient condition not met, etc.). Permanent for this tx — do not retry; offer token payment or a different payer |
...
The same `payer_*` methods apply when using [ERC-7836](./eip-7836.md) `wallet_prepareCalls` / `wallet_sendPreparedCalls` — the wallet calls `payer_getTerms` and `payer_sendTransaction` or `payer_signTransaction` at the equivalent steps, with the app signing the transaction digest in between rather than the wallet.
...
Furthermore, when a payer advertises `callPolicy: "unrestricted"` in `payer_getCapabilities`, it provides a firm guarantee that it operates as a pure financial exchange: it will not reject a transaction based on the content of the user's calls. As long as the phase 0 token transfer covers the gas cost at the payer's cached rate, the payer will co-sign. This makes the optimistic skip path perfectly safe for wallets to use across arbitrary intents, enabling aggressive cross-transaction rate caching.

...
Each option's `methods` array denotes how the wallet uses it, with the same meaning as in [`payer_getCapabilities`](#payer_getcapabilities-optional), so the list can mix external services and a builder-native option:

- An **external payer** lists a co-sign method (`payer_sendTransaction` and/or `payer_signTransaction`) and an `endpoint` the wallet contacts to co-sign. The co-signed transaction is includable by any builder.
- A **sequencer / builder-native** option lists only read methods, omits `endpoint`, and sets `payer` to the chain's recognized sentinel address. The wallet submits via `eth_sendRawTransaction` and inclusion is the authorization (see [Sequencer / Builder-Native Gas Payment](#sequencer--builder-native-gas-payment)). This guarantee holds only when that builder builds the block: effectively certain for a single L2 sequencer, probabilistic for an L1 proposer, so a wallet ranking options MAY prefer an external co-signing payer when it needs builder-independent inclusion.
...
type PayerOption = {
  ...
  payer: `0x${string}`;
  endpoint?: string; // co-sign URL; omitted for a sequencer/builder-native option served by the node itself
  methods: string[]; // authoritative submission path (as in payer_getCapabilities); only read methods => builder-native, submit via eth_sendRawTransaction
  sponsor?: {
    name: string;
    icon?: string;
    reason?: string;
...
```json
{
  "options": [
    ...
    },
    {
      ...
      "sponsor": {
        "name": "Sequencer",
        "reason": "Builder sponsors gas; inclusion is authorization"
      },
      "priority": 2
    }
  ]

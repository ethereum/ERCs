...
discussions-to: (placeholder - topic to be created)
status: Draft
...
This ERC defines a standard interface for authorizing autonomous agents (bots, AI systems, or automated accounts) to perform specific on-chain actions on behalf of users (principals). The standard provides:
...
Contracts MUST implement EIP-165 and return `true` for:
...
This ERC occupies a unique position relative to existing delegation standards:

| Feature | ERC-4337 | EIP-7702 | This ERC |
|---------|----------|----------|----------|
...
- Unlike smart account wallets (ERC-4337) that relocate custody to contract accounts, this ERC preserves EOA custody without migration
- Unlike single-use EOA code delegation (EIP-7702), this ERC introduces persistent, function-level capabilities
- This ERC remains compatible with account abstraction features such as bundlers and paymasters, preserving existing wallet infrastructure while enabling fine-grained delegation

**ERC-2612 (Permit)**
- ERC-2612 provides gasless token approvals via signatures
- This ERC generalizes the signature-based authorization pattern beyond token transfers
- Both use EIP-712 typed data and nonces for replay protection
- This ERC adds time bounds, usage limits, and function-level granularity

**ERC-4337 (Account Abstraction)**
- ERC-4337 abstracts account logic for flexible transaction validation
- This ERC focuses on agent-level delegation within existing accounts
- ERC-4337 UserOperations require bundlers; this ERC works with standard transactions
...
- This ERC can be implemented as an ERC-6900 execution plugin
- ERC-6900 focuses on account modularity; this ERC focuses on agent authorization semantics
- Both support EIP-165 interface detection for composability
...
- This ERC's authorization module could be added to ERC-7579 accounts

## Backwards Compatibility

This ERC introduces a new interface and does not modify existing standards. Contracts implementing this standard can coexist with other delegation mechanisms.

### Account Abstraction Compatibility

This ERC is designed to complement rather than replace account abstraction infrastructure:
...
A reference implementation is available at [github.com/WORLD3-ai/ERCs](https://github.com/WORLD3-ai/ERCs/tree/main/contracts):

- [`AgentAuthorizationBase.sol`](https://github.com/WORLD3-ai/ERCs/blob/main/contracts/AgentAuthorizationBase.sol): Base implementation
- [`AgentAuthorizationUpdatable.sol`](https://github.com/WORLD3-ai/ERCs/blob/main/contracts/AgentAuthorizationUpdatable.sol): Implementation with update extension
- [`interfaces/`](https://github.com/WORLD3-ai/ERCs/tree/main/contracts/interfaces): Interface definitions
- [`libraries/`](https://github.com/WORLD3-ai/ERCs/tree/main/contracts/libraries): Shared types and errors
- [`test/`](https://github.com/WORLD3-ai/ERCs/tree/main/test): Comprehensive test suite (98 tests)

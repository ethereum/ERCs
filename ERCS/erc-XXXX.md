...
description: A standard interface for tokens with FHE-encrypted balances and zero-knowledge transfer verification.
...
This standard defines a minimal interface for **fungible tokens with encrypted balances**. Token balances are stored as Fully Homomorphic Encryption (FHE) ciphertexts. Transfers are verified via zero-knowledge proofs without revealing amounts, sender balances, or recipient balances to any on-chain observer.

Unlike ERC-20 where `balanceOf` returns a plaintext `uint256`, this standard returns an opaque ciphertext that only the balance owner can decrypt. Arithmetic operations on balances (credit, debit, comparison) are performed homomorphically by the execution environment without any party observing plaintext values.

This standard is backwards-compatible with ERC-20 for public aggregate values (`totalSupply`, `name`, `symbol`, `decimals`) while introducing encrypted counterparts for all privacy-sensitive operations.

## Motivation

ERC-20 tokens expose all balances and transfer amounts publicly. This creates several critical problems:
...
Existing privacy solutions (mixers, L2 privacy chains) operate outside the ERC standard ecosystem, breaking composability with DeFi protocols, wallets, and indexers. This standard brings privacy into the token standard itself, allowing any ERC-compliant wallet or protocol to interact with encrypted tokens through a well-defined interface.

### Why not just use ERC-20 with a privacy layer?

Privacy layers (Tornado Cash, Aztec Connect) are external wrappers. They break composability: a "shielded" token cannot be used in Uniswap, Aave, or Compound without unshielding first — defeating the purpose. This standard makes privacy native to the token interface. A compliant lending protocol can accept encrypted collateral, compute liquidation thresholds homomorphically, and never see the actual collateral amount.
...
- **Encryption Scheme**: The FHE scheme used. This standard is scheme-agnostic but RECOMMENDS TFHE (Torus FHE) for its boolean circuit efficiency.
...
/// @notice A standard interface for tokens with FHE-encrypted balances
...
Compliant contracts MUST implement ERC-165 and return `true` for the interface ID of `IERCXXXX`.

```solidity
bytes4 constant IERCXXXX_ID = type(IERCXXXX).interfaceId;
```
...
**`IERCXXXX_Shielded`** — Bridge between ERC-20 public pool and encrypted pool:
...
This standard does not mandate a specific ZK proof system. Implementations MAY use Groth16, Halo2, Nova, Plonk, or any system that provides:
...
ERC-20's `Transfer(from, to, amount)` leaks the amount. `BlindTransfer(from, to, proofHash, keyVersion)` preserves the minimum information needed for indexing (who transferred to whom) without revealing how much. The `proofHash` enables off-chain auditors (with appropriate keys) to verify transfer validity.
...
ERC-20's `approve` does not check that the owner currently holds the approved amount — the check occurs at `transferFrom` time. This standard follows the same pattern: `blindApprove` verifies only that the ciphertext is well-formed and non-negative. Balance sufficiency is proven at `blindTransferFrom` time via the ZK proof.
...
The public metadata functions (`name`, `symbol`, `decimals`, `totalSupply`) are identical to ERC-20. This allows block explorers and basic wallet UIs to display token identity. The encrypted functions (`blindTransfer`, `encryptedBalanceOf`) are additive — they don't override ERC-20 functions but extend them.

A token MAY implement both ERC-20 and this standard simultaneously, where ERC-20 functions operate on a separate "public balance" pool and encrypted functions operate on the "shielded balance" pool. The optional `IERCXXXX_Shielded` extension provides `shield`/`unshield` bridge functions to move tokens between pools.

## Backwards Compatibility

This standard is fully backwards-compatible with ERC-20 for metadata queries. Tokens implementing this standard alongside ERC-20 can participate in both the public and encrypted ecosystems.

Wallets that do not support this standard will display the token name and symbol but will show a balance of 0 (since `balanceOf` returns the public pool balance, which may be empty if all tokens are shielded).

This standard REQUIRES ERC-165 for interface detection, allowing protocols to query whether a token supports encrypted operations before attempting them.
...
Repository: https://github.com/Valisthea/styx-erc-encrypted-token
...
Standard FHE schemes (TFHE, BFV, BGV) are lattice-based and believed to be quantum-resistant. However, the ZK proof system may not be. Implementations SHOULD use quantum-resistant proof systems or plan migration paths.
...
### Share Inflation (ERC-4626 Analog)

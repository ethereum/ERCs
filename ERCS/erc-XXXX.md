...
This ERC defines a standard interface for AI agent-authenticated wallets that execute transactions only when accompanied by a verifiable proof tying the action to the agent's specific policy. It aligns with the Ethereum Foundation's PhDFP-26 RFP D3 ("Agentic Economy: Verification, Delegation, and Host-Interference Mitigation", opened February 9, 2026) and serves as **Layer 3 (Execute)** in a modular trust stack for autonomous agents:

- **Layer 1 (Register)**: [ERC-8004](./eip-8004.md) - on-chain identity and registration  
- **Layer 2 (Verify - optional)**: Standards such as [ERC-8126](./eip-8126.md) - trust/risk scoring  
- **Layer 3 (Execute)**: ERC-XXXX - policy-bound execution with immutable audit trail

The design enables secure credential delegation without exposing private keys, prevents host manipulation of agent behavior, and provides tamper-evident logging of all session activity.

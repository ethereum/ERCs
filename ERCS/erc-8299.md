...
The `agentId` is the ERC-8004 registry identity anchor — the identifier the registry assigns, paired with `registry` — not a hash of any human-readable name. When the ERC-8004 identity is a `uint256` registry id, `agentId` is its left-padded encoding `agentId = bytes32(uint256(id))` (e.g. registry id `860` becomes `0x000...035c`). Deriving an identity from a human-readable name is out of scope for this ERC; if an implementation does so, the derivation MUST be declared, and an ENS-shaped name SHOULD use the [ERC-137](./erc-137.md) namehash (the ENS node), never `keccak256(utf8(name))`.

...
- `sanitization_spec_cid_bytes` is `utf8(sanitization_spec_cid)`: the full IPFS URI string including the `ipfs://` scheme prefix (e.g., `ipfs://QmccvoM6aRVg...`), serialized as UTF-8 bytes. The `ipfs://` prefix is part of the preimage and MUST be included.
...
struct WyriweAttestation {
    ...
    bytes32 sanitizationPipelineHash;   // keccak256(sanitization_spec_cid_bytes || raw_input_hash)
...
The `agentId` is the ERC-8004 registry identity anchor — the identifier the registry assigns, paired with `registry` — not a hash of any human-readable name. When the ERC-8004 identity is a `uint256` registry id, `agentId` is its left-padded encoding `agentId = bytes32(uint256(id))` (e.g. registry id `860` becomes `0x000...035c`). Deriving an identity from a human-readable name is out of scope for this ERC; if an implementation does so, the derivation MUST be declared, and an ENS-shaped name SHOULD use the [ERC-137](./eip-137.md) namehash (the ENS node), never `keccak256(utf8(name))`.
...
Unlike the base WYRIWE triple-hash (L1–L3), whose algorithm is fixed to keccak256 because that layer exists specifically for an on-chain Solidity `verify()` call, **L4's hash algorithm is deployment-dependent**: sha256 for off-chain (relay-anchored) verdicts as published by the reference implementation, or keccak256 when a producer wraps the same fields in an on-chain EIP-712 struct. The construction below is written against the off-chain reference implementation (`api.babyblueviper.com/ledger`); an on-chain producer substitutes keccak256 and `abi.encode` for sha256 and JCS.

```
rawProposalHash    = sha256(proposed_action_artifact)
verdictHash        = sha256(JCS(verdict_preimage))
executedActionHash = sha256(executed_action_record)
```

`rawProposalHash` is the sha256 of the exact proposed-action artifact as submitted for review — no separate canonicalization step.

`verdictHash` is the sha256 of the [JCS](https://www.rfc-editor.org/rfc/rfc8785)-canonicalized (sorted keys, no insignificant whitespace, literal UTF-8) JSON object of the verdict's named preimage fields — **not** a literal byte-concatenation `keccak256(verdict_artifact_ref || rawProposalHash)`. The exact field set is producer-published on each proof and MUST include the field that pins the proposal (the one carrying `rawProposalHash`, e.g. `artifact_hash`) for the anti-replay binding to hold.

The preimage field set MUST be published alongside `verdictHash` as a named, self-describing field (e.g. a `decision_ref_preimage_fields` array). Consumers MUST recompute against the field list the producer declares on that proof, and MUST NOT assume a fixed set taken from this specification's examples. Any example field set given here — such as `{artifact_hash, artifact_type, policy_version, verdict, source_class, vantage_limitation}` — is illustrative, not exhaustive and not frozen.

This requirement exists because the set grows. In the production reference implementation it has grown twice, from six fields to twelve. A verifier that hardcoded an earlier example would have silently stopped reproducing `verdictHash` at each addition, and would have reported a mismatch as a bad proof rather than as its own stale assumption. JCS guarantees the preimage bytes recompute identically across languages and implementations.

`verdictHash` binds to the proposal — mirroring the sanitization-pipeline-hash construction — making verdict-shopping impossible: a verdict cannot be replayed against a different proposal than the one it judged. The anti-equivocation property is identical across storage backends (IPFS CID, Nostr event ID): the specific signed verdict artifact is pinned by its identifier/hash among the preimage fields, so a validator cannot swap between multiple signed verdicts post-reveal.

**Optional preimage fields.** Two further fields are defined for the verdict preimage. Both are additive: a preimage omitting them remains conformant, and neither replaces an existing field.

`intended_verifier` (SHOULD) — a CAIP-10 account identifier naming the on-chain verifier instance the verdict is issued for. Producers SHOULD include it whenever the consuming `IProofVerifier` instance is known at judgment time. It is not a MUST because not every verdict has a single known consumer when it is judged.

Without it, the preimage binds the judging party and the content judged, but nothing about *who the judgment was for*. A technically valid `verdictHash` is therefore replayable against any verifier contract willing to accept it — not only the one it was produced for. This was surfaced by an independent verifier gate testing the reference implementation against a live `MandateGate` deployment, not anticipated in the abstract. `intended_verifier` supplies crypto-level domain separation at the preimage layer, so a verdict judged for one gate does not authenticate at another.

`policy_commitment` (SHOULD) — a commitment to the policy actually in force, published alongside `policy_version` rather than instead of it:
...
A version string asserts that a policy was in force; it is not itself recomputable, so a consumer must trust the producer's later account of what that version meant. The commitment lets a stranger fetch the rubric text and the conformance vectors at the pinned commit and confirm the hash themselves, independent of that account. `policy_version` remains useful as a human-readable label; `policy_commitment` is what a recompute checks against.
...
struct JudgmentExecutionAttestation {
    ...
    bytes32 rawProposalHash;     // 32-byte hash of the proposed-action artifact, pre-review.
                                 // Deployment-dependent: sha256(artifact) off-chain / keccak256(artifact) on-chain.
    bytes32 verdictHash;         // 32-byte hash binding the verdict to rawProposalHash.
                                 // sha256(JCS(verdict_preimage)) off-chain (preimage pins the proposal) /
                                 // keccak256(verdict_artifact_ref || rawProposalHash) on-chain.
    bytes32 executedActionHash;  // 32-byte hash of the executed-action record, revealed at settlement
                                 // (same algorithm as the deployment above).
...
- VeriLLM (Ke Wang et al., Gradient, 2025; arXiv:2509.24257) — publicly verifiable decentralized LLM inference via Merkle-committed hidden states and VRF-based recompute. Compute-integrity complement to WYRIWE's behavioral-integrity layer.

...
The `agentId` is the ERC-8004 registry identity anchor — the identifier the registry assigns, paired with `registry` — not a hash of any human-readable name. When the ERC-8004 identity is a `uint256` registry id, `agentId` is its left-padded encoding `agentId = bytes32(uint256(id))` (e.g. registry id `860` becomes `0x000...035c`). Deriving an identity from a human-readable name is out of scope for this ERC; if an implementation does so, the derivation MUST be declared, and an ENS-shaped name SHOULD use the [EIP-137](./eip-137.md) namehash (the ENS node), never `keccak256(utf8(name))`.
...
`intended_verifier` (SHOULD) — a [CAIP-10](https://github.com/ChainAgnostic/CAIPs/blob/main/CAIPs/caip-10.md) account identifier naming the on-chain verifier instance the verdict is issued for. Producers SHOULD include it whenever the consuming `IProofVerifier` instance is known at judgment time. It is not a MUST because not every verdict has a single known consumer when it is judged.

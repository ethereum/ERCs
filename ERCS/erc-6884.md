...
description: Standard interface for delegation of utility usage rights bound to ERC-721 origin tokens.
...
This ERC specifies Delegatable Utility Tokens (DUT), which allow utility rights to be associated with ERC-721 origin tokens. Ownership of each utility remains tied to the origin token owner, while usage rights can be delegated to other addresses without transferring ownership.

## Motivation

Most ERC-721 NFTs provide limited or underutilized utility. These limitations become more apparent as NFTs accumulate multiple utilities and can be categorized as follows:
...
ERC-6884 addresses these limitations by defining Delegatable Utility Tokens (DUT). A DUT binds utility rights to an ERC-721 origin token while allowing usage rights to be delegated independently from ownership. DUTs remain tied to the current owner of the origin token, and when ownership of the origin token changes, all associated DUTs automatically transfer to the new owner without additional transactions.

## Specification

The key words “MUST”, “MUST NOT”, “REQUIRED”, “SHALL”, “SHALL NOT”, “SHOULD”, “SHOULD NOT”, “RECOMMENDED”, “MAY”, and “OPTIONAL” in this document are to be interpreted as described in RFC 2119.

Every ERC-6884 compliant contract must implement the `IERC6884` and `IERC165` interfaces:
...
This standard does not extend ERC-721. ERC-721 defines transferable ownership semantics, while Delegatable Utility Tokens are explicitly non-transferable and only delegable. Rather than inheriting ERC-721 and reverting transfer functions, a separate standard provides a clearer and simpler design.

### Owners and Users

Two roles are distinguished:

- **Owner**: the holder of the origin ERC-721 token, and therefore the holder of the corresponding DUT.
...
`balanceOf(owner)` reflects the balance of the origin ERC-721 contract. Delegation state does not affect balances. This preserves compatibility with other ERC standards and makes explicit that ownership of DUTs is derived directly from ownership of the origin NFTs.
...
An approval system, modeled on ERC-721, enables delegation to be mediated by approved operators. This allows external applications to build markets and coordination mechanisms for usage rights without altering the underlying ownership of the NFT.

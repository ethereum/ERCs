...
MUST revert with `UnsupportedAttribute` if an unsupported attribute key is included. MAY revert if the value of an attribute is not a valid encoding for its expected type, or if some other attribute validity requirement fails.
...
MAY generate and return a unique non-zero _outbox identifier_, otherwise returning zero. This identifier can be used to track the lifecycle of the message in the outbox in events and for relaying.

MUST emit a `MessagePosted` event, including the optional outbox identifier that is returned by the function.

MAY require further action to have the message relayed to the receiver on destination, in particular paying for the gas cost of delivery. See Relaying.

#### `MessagePosted`

This event signals that a would-be sender has requested a message to be sent.

If `outboxId` is present, further action MAY be required. See Relaying.

#### Relaying

The delivery of a message to its receiver will incur a gas cost that the protocol must cover. The following functions of a gateway allow the sender to pay for this cost.
...
Requests that the message identified by `outboxId` be relayed and delivered to the receiver.

Pays via `msg.value` for the execution of the message on the receiver up to `gasLimit`. A relayer fee MAY be deducted, the specifics of which are unspecified and SHOULD be available out of band (e.g., as gateway documentation).

Any surplus after execution at destination MUST be refunded to one of the accounts in the `refundReceivers` array. While refunds on any chain MAY be supported, the gateway MUST support at least one of source or destination chains. If no chain supported for refunds is included in the array, the surplus MAY NOT be refunded. For senders, it is RECOMMENDED to include both source and destination chain refund receiver accounts, since at least one MUST be supported.

##### Attribute Form

Instead of invoking `requestRelay` as a second step after `sendMessage`, the two steps can be combined into one by including an attribute with the following signature:

```
requestRelay(uint256 value,uint256 gasLimit, string[] refundReceivers)
```

When a message with this attribute is sent through a gateway that supports it, it MUST behave as if the `requestRelay` function was invoked as a second step with the implied `outboxId` and `value` as the call value.

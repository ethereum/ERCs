...
Once the message can be safely delivered (see Properties), the gateway MUST invoke `executeMessage` with the message identifier and contents, UNLESS not invoking this function was explicitly requested by the sender (through some attribute) or the receiver.

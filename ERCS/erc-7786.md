...
Any surplus after execution at destination MUST be refunded to one of the accounts in the `refundReceivers` array. While refunds on any chain MAY be supported, the gateway MUST support at least one of source or destination chains. If no chain supported for refunds is included in the array, `requestRelay` MUST revert. For senders, it is RECOMMENDED to include both source and destination chain refund receiver accounts, since at least one MUST be supported.

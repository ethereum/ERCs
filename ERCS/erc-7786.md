...
interface IERC7786Relayer {
    function quoteRelay(
        ...
        bytes[] calldata attributes
        uint256 gasLimit,
        string[] calldata refundRecipients
...
Pays via `msg.value` for the execution of the message on the recipient up to `gasLimit`. A relayer fee MAY be deducted, the specifics of which are unspecified and SHOULD be available out of band (e.g., as gateway documentation).

Any surplus after execution at destination MUST be refunded to one of the accounts in the `refundRecipients` array. While refunds on any chain MAY be supported, the gateway MUST support at least one of source or destination chains. If no chain supported for refunds is included in the array, `requestRelay` MUST revert. For senders, it is RECOMMENDED to include both source and destination chain refund recipient accounts, since at least one MUST be supported.

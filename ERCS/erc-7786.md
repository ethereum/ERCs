...
In the case `read(true)` is set as the first element of the `attributes` parameters, the next element MUST be `messageId(val)` where `val` is a sequence of bytes.

...
The `attributes` parameter MUST contain as its first element the value `read(true)` or `read(false)`. `read(true)` means the message can be read by the application on the destination gateway. `read(false)` means the message is pushed by the destination Gateway on the application via `executeMessage`(see below).

In the case `read(true)` is set as the first element of the `attributes` parameters, the next element MUST be `messageId(val)` where `val` is a sequence of 32 bytes.

...
This standard is an extension to [ERC-7579](./erc-7579.md) and does not redefine its module system. A compliant account and its modules MUST conform to ERC-7579 for:

- module types and their identifiers,
- the module lifecycle interface (`onInstall`, `onUninstall`, `isModuleType`),
- module installation and configuration (`installModule`, `uninstallModule`, `isModuleInstalled`),
- account configuration (`accountId`, `supportsModule`), and
- execution (`execute`, `executeFromExecutor`).
...
2. Select an installed validator. This standard does not dictate the selection mechanism; if the selector is encoded in `frame.data`, the account MUST sanitize it before use and MUST verify the module is an installed validator (ERC-7579 module type id `1`).
...
4. If the returned `approvalMode`, after clamping to `allowedScope`, is `APPROVE_NONE`, the account MUST NOT call `APPROVE`; the frame reverts and the transaction is invalid.
...
In addition to the ERC-7579 interfaces listed in [Relationship to ERC-7579](#relationship-to-erc-7579), a compliant account MUST implement the following interface. It extends ERC-7579's `IERC7579AccountConfig` so that approval-mode capability is declared alongside ERC-7579's existing account configuration queries:

```solidity
interface IFrameAccount is IERC7579AccountConfig {
    /**
...
This standard does not define an execution interface. Execution of approved operations uses ERC-7579's `execute` and `executeFromExecutor`. In addition, EIP-8141 executes user operations natively through `SENDER` frames, and handles batching, atomicity, per-frame value, and error handling at the protocol level. Accordingly, this extension governs only the validation flow.
...
### No execution interface, but execution modes still apply

This standard does not define an execution *entrypoint*. EIP-8141 orchestrates execution directly (batching as multiple frames, atomicity via the atomic-batch flag, value per `SENDER` frame, error handling via per-frame status and atomic unroll), and any account-side execution entrypoint is already covered by ERC-7579's execution interface, so redefining one here would duplicate both.

ERC-7579's execution-mode *declaration* is, however, retained: an account still exposes `supportsExecutionMode` (inherited via `IERC7579AccountConfig`) to advertise which execution shapes it honors. What changes is **when the check fires**. ERC-7579 enforces it inside `execute`, at execution time, by decoding the mode and reverting if unsupported. Under EIP-8141 the account is structurally absent from the execution path. A `SENDER` frame runs with `caller = tx.sender` against the frame's target, the account's code is not invoked, and the only gate is the transaction-scoped `sender_approved` flag set once by `APPROVE_EXECUTION`, so there is no execution-time hook at which to revert. The equivalent check therefore moves earlier: the account enforces its supported execution modes during the `VERIFY` frame, by inspecting the transaction's `SENDER` frames before approving. Discovery stays identical to ERC-7579; only the enforcement point shifts from execution time to validation time.

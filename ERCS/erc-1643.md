...
`type(IERC1643MultiDocument).interfaceId` is the XOR of only the extension's own address-scoped functions; per ERC-165 semantics it does not include the base `IERC1643` selectors. Advertising the extension id therefore says nothing about whether the base single-argument functions are implemented. Accordingly:

- A contract that implements the extension SHOULD return `true` for `type(IERC1643MultiDocument).interfaceId`.
- A contract MUST return `true` for `type(IERC1643).interfaceId` only if it also implements the base single-argument functions. A shared management contract that exposes only the address-scoped surface MUST NOT advertise `type(IERC1643).interfaceId`, since it does not implement that interface.

Events are not part of any ERC-165 interface id, so `supportsInterface` reflects only which functions a contract implements, not whether it emits the base or extension events.
...
The extension is intentionally declared **independently of `IERC1643`** (it does not inherit it), so a shared management contract can implement the address-scoped surface **without** being forced to implement the base single-argument functions. A contract MAY implement both interfaces — for example a manager that also lets a token contract manage its own documents by calling the base functions with `msg.sender` as the implied subject — in which case it implements, and advertises, each interface separately.
...
#### Call Topology in the Shared Configuration

A contract can only emit an event in a transaction in which it executes. The requirement above — that in the shared configuration each token contract emits the base events for its own documents — therefore constrains how a write reaches the shared management contract, not only which contract is nominally responsible for emitting.

A write that creates, updates, or removes a document for `subject` MUST include an execution point in `subject` at which the base event is emitted. Two topologies satisfy this:

- **Subject-initiated.** `subject` calls the shared contract's address-scoped write and emits the base event itself. The shared contract emits the corresponding extension event.
- **Manager-initiated with callback.** An authorized operator calls the shared contract, which calls back into `subject` through an implementation-defined permissioned hook; `subject` emits the base event. This ERC does not define the signature of that hook.

A deployment in which an operator calls `setDocument(address subject, ...)` or `removeDocument(address subject, bytes32 name)` on the shared contract directly, with no execution point in `subject`, does **not** satisfy the base emission requirement for `subject`: a consumer subscribed to `subject` never observes the change, so `subject` does not support subscribing to updates on its documentation and is not a conformant ERC-1643 implementation in that deployment, even though it exposes the base functions.

Accordingly, a shared management contract SHOULD restrict its address-scoped writes to callers for which one of the two topologies above holds — `subject` itself, or an operator whose write path calls back into `subject`. This is narrower than, and consistent with, the per-`subject` authorization required below: authorization decides *who* may write for a `subject`, while call topology decides whether that write remains observable on `subject`'s own address.

Consumers of the extension events are unaffected either way: `DocumentUpdatedForSubject` / `DocumentRemovedForSubject` carry the `subject` address and are emitted by the shared contract in every topology.
...
- `setDocument(address subject, ...)` and `removeDocument(address subject, bytes32 name)` SHOULD revert when `subject == address(0)`, since the null address is never a valid document subject.
  - Implementations SHOULD use the custom error defined in the extension interface (`ERC1643InvalidSubject()`) when rejecting `subject == address(0)`.
  - Implementations MAY use different error names/signatures than those shown in this specification.
  - Guarding the write path is sufficient: when `setDocument` rejects `subject == address(0)`, no document can exist under that subject, so `removeDocument` already fails with `ERC1643MissingDocument()`.
...
- A shared document-management contract should reject `subject == address(0)`. This is a data-integrity concern rather than a fund-safety one: subject namespaces are isolated, and the null address cannot call the contract to read documents registered under it, so such entries are inert. They do, however, let callers populate a namespace no contract can ever own, cluttering state and misleading off-chain indexers that key on `subject`.
- A write sent directly to a shared document-management contract, without an execution point in the `subject` contract (see Call Topology in the Shared Configuration), silently bypasses base-event emission for that subject. The failure is quiet in both directions: the write succeeds and the extension event is emitted, so nothing reverts, while a base-only consumer watching `subject` sees no event and concludes the documents are unchanged. Because events are outside ERC-165, no on-chain query reveals the gap. Integrators who must rely on the base events should confirm the deployment's write path out of band, and deployments that cannot guarantee a subject-side execution point should direct consumers to the shared contract's extension feed rather than to `subject`.
- The custom error `ERC1643InvalidSubject()` is defined in the extension interface only. Because the extension does not inherit `IERC1643`, a contract implementing both interfaces obtains `ERC1643InvalidName()` and `ERC1643MissingDocument()` from `IERC1643` and `ERC1643InvalidSubject()` from `IERC1643MultiDocument`; a contract implementing only the extension declares whichever of the other errors it uses itself.

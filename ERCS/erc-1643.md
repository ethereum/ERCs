...
A contract MUST return `true` for `type(IERC1643).interfaceId` only if it actually implements the functions defined here. In particular, a document-management contract that exposes only address-scoped functions, such as those of the companion multi-subject proposal referenced under Delegated Document Management, MUST NOT advertise `type(IERC1643).interfaceId`, since it does not implement this interface.

Events are not part of any ERC-165 interface id, so `supportsInterface` reflects only which functions a contract implements, not whether it emits `DocumentUpdated` or `DocumentRemoved`.
...
### Delegated Document Management

A contract MAY delegate the storage and mutation of its documents to a separate document-management contract while continuing to expose this interface. This section constrains such deployments; it does not define any additional interface.

`DocumentUpdated` and `DocumentRemoved` carry no address, so consumers attribute them to the address that emitted them. The events MUST therefore be emitted by the contract that exposes this interface to consumers — the address consumers are expected to subscribe to. When document management is delegated, the emitter MUST be chosen so that per-contract observability is preserved:

- A document-management contract **dedicated to a single** contract MAY be that contract's ERC-1643 implementation and MUST emit `DocumentUpdated` / `DocumentRemoved`; the events are unambiguous because only one contract is served.
- A document-management contract **shared by several** contracts MUST NOT report per-contract changes through `DocumentUpdated` / `DocumentRemoved`, since those events cannot identify which contract a change belongs to. In this configuration each contract MUST emit the events for its own documents, and the shared contract SHOULD instead emit events that carry the address of the contract the documents belong to — for example those defined by the companion multi-subject document management proposal.
- Implementations SHOULD NOT emit `DocumentUpdated` / `DocumentRemoved` from **both** the contract exposing this interface and the document-management contract.

#### Call Topology Under Shared Delegation

A contract can only emit an event in a transaction in which it executes. The requirement that each contract emit the events for its own documents therefore constrains how a write reaches a shared document-management contract, not only which contract is nominally responsible for emitting.

A write that creates, updates, or removes a document belonging to a contract that exposes this interface MUST include an execution point in that contract at which the event is emitted. Two topologies satisfy this:

- **Self-initiated.** The contract calls the document-management contract itself and emits `DocumentUpdated` / `DocumentRemoved`.
- **Manager-initiated with callback.** An authorized operator calls the document-management contract, which calls back into the contract through an implementation-defined permissioned hook; the contract emits the event. This ERC does not define the signature of that hook.

A deployment in which an operator writes to a shared document-management contract directly, with no execution point in the contract the documents belong to, does **not** satisfy the emission requirement for that contract: a consumer subscribed to it never observes the change, so it does not support subscribing to updates on its documentation and is not a conformant ERC-1643 implementation in that deployment, even though it exposes the functions defined here.

Consumers of the document-management contract's own address-carrying events are unaffected in either topology.
...
The Delegated Document Management section constrains how a conforming implementation may delegate storage and mutation to another contract. It adds no functions, events, or errors, and does not change the meaning of any defined here: every address a consumer is directed to subscribe to still emits `DocumentUpdated` / `DocumentRemoved`. An implementation that stores its own documents is unaffected by that section.
...
- Delegating document management moves the authorization boundary to the document-management contract. A contract that delegates is only as protected as that contract's access control, and a contract shared by several subjects concentrates the document sets of unrelated parties behind a single address.
- A write sent directly to a shared document-management contract, without an execution point in the contract the documents belong to (see Call Topology Under Shared Delegation), silently bypasses emission of `DocumentUpdated` / `DocumentRemoved` for that contract. The failure is quiet in both directions: the write succeeds and the document-management contract emits its own event, so nothing reverts, while a consumer watching the contract sees no event and concludes the documents are unchanged. Because events are outside ERC-165, no on-chain query reveals the gap. Integrators who must rely on these events should confirm the deployment's write path out of band, and deployments that cannot guarantee such an execution point should direct consumers to the document-management contract's address-carrying feed instead.

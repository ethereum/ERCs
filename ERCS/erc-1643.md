...
Although originally designed for security tokens built on [ERC-20](./erc-20.md) as part of a broader real-world asset token suite, this interface is not restricted to that context. It can be adopted by any token standard, including [ERC-721](./erc-721.md) non-fungible tokens and [ERC-1155](./erc-1155.md) multi-tokens, as well as by decentralized applications, vaults, and any other on-chain product that requires structured document management.
...
### Interface Detection ([ERC-165](./erc-165.md))
...
The interface is provided in [the reference interface](../assets/erc-1643/src/erc-1643/IERC1643.sol). A reusable abstract module implementing the full interface is provided in [the reference module](../assets/erc-1643/src/erc-1643/ERC1643.sol). Example integrations attaching the module to [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) tokens are provided in [the ERC-20 example](../assets/erc-1643/src/ERC20DocumentToken.sol) and [the ERC-721 example](../assets/erc-1643/src/ERC721DocumentToken.sol). These examples use the OpenZeppelin library and restrict document mutation to the contract owner. They are provided for educational purposes only and have not been audited.
...
The custom errors `ERC1643InvalidName()` and `ERC1643MissingDocument()` are defined in this interface but were absent from the original [ERC-1643](./erc-1643.md) proposal text. Older implementations may not define these errors and may instead revert with strings or implementation-specific error patterns. Integrators should not assume all ERC-1643 contracts expose identical revert data.

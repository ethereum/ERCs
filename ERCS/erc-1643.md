...
A contract MUST return `true` for `type(IERC1643).interfaceId` only if it actually implements the functions defined here. A contract that stores or manages documents through some other interface, without implementing these functions, MUST NOT advertise `type(IERC1643).interfaceId`.
...
- An implementation that delegates document storage or mutation to another contract is only as protected as that contract's access control, and remains responsible for emitting `DocumentUpdated` / `DocumentRemoved` on its own address. A delegated write that does not reach this contract emits nothing here, and a consumer subscribed to this address would see no change.

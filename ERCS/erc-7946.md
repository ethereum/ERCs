...
via JSON-RPC (e.g. `wallet_sendCalls`). However, those solutions assume an active connection
...
- **Static vs Programmable Mode:** There are two mutually exclusive ways to specify the batch of calls:
    ...
    3. **Programmable Mode:** The request contains a reference to an **on-chain “resolver” contract** and an input data

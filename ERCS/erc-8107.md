...
The registry serves as the **trust and delegation module** anticipated by [ERC-8001](./eip-8001), enabling coordinators to gate participation based on trust graph proximity. An agent is considered valid from a coordinator's perspective if sufficient trust paths exist between them.

This standard specifies trust attestation structures, the path verification algorithm, ENS integration semantics, and [ERC-8001](./eip-8001) coordination hooks.

## Motivation

[ERC-8001](./eip-8001) defines minimal primitives for multi-party agent coordination but explicitly defers trust to modules:
...
* [EIP-1271](./eip-1271) support for contract controllers
...
* For contract owners: any signer the contract validates via [EIP-1271](./eip-1271)
...
**Wallet Compatibility**: Uses [EIP-712](./eip-712) signatures, compatible with all major wallets. Supports [EIP-1271](./eip-1271) for contract wallets and smart accounts.

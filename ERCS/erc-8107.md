...
On-chain validation does not rely on CCIP-Read ([ERC-3668](./eip-3668.md)). Agents whose names are served by an off-chain resolver can still hold and issue trust, but cannot be validated by address, since `addr` is not readable on-chain for those names.

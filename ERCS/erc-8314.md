...
discussions-to: https://ethereum-magicians.org/t/erc-8314-contract-role-naming/28756
status: Draft
...
This standard is designed to be compatible with [EIP-7820](./eip-7820.md) and [EIP-173](./eip-173.md).
...
+ **Proxy and factory pattern role ownership**: Proxy contracts (Transparent Proxy / UUPS) and implementation contracts have independently isolated role namespaces by their respective addresses, but the standard does not prescribe which roles should belong to the proxy contract versus the implementation contract. Role definition conflicts across multiple facets in the Diamond pattern ([EIP-2535](./eip-2535.md)) require implementers to coordinate themselves. Child contracts created by contract factories independently own their own role namespaces and do not automatically inherit factory roles.

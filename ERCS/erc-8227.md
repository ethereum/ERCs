...
discussions-to: https://ethereum-magicians.org/t/erc-8227-encrypted-token/28214
status: Draft
...
/// @title ERC-8227 Encrypted Token Interface
...
interface IERC8227 {
    ...
    /// @dev    If `isTotalSupplyConfidential()` returns `false`, this MUST
    ///         return the actual total supply. If it returns `true`, this
    ///         MUST return 0 and the contract MUST implement
    ///         `IERC8227_ConfidentialSupply`.
    function totalSupply() external view returns (uint256);

    /// @notice Returns whether the total supply is confidential.
    /// @dev    This value is fixed at deployment and MUST NOT change over the
    ///         lifetime of the contract. It exists so that a consumer can tell
    ///         a hidden supply from a genuine supply of 0 without probing
    ///         [ERC-165](./eip-165.md) for an extension.
    /// @return `true` if the total supply is hidden, `false` if it is public.
...
Compliant contracts MUST implement [ERC-165](./eip-165.md) and return `true` for the interface ID of `IERC8227`.

```solidity
bytes4 constant IERC8227_ID = type(IERC8227).interfaceId;
```
...
**`IERC8227_Shielded`** — Bridge between the public pool (compatible with [ERC-20](./eip-20.md)) and the encrypted pool:

```solidity
interface IERC8227_Shielded is IERC8227 {
...
**`IERC8227_ConfidentialSupply`** — For tokens that also hide total supply. A contract whose `isTotalSupplyConfidential()` returns `true` MUST implement this extension; a contract whose `isTotalSupplyConfidential()` returns `false` MUST NOT:

```solidity
interface IERC8227_ConfidentialSupply is IERC8227 {
    /// @dev MUST revert if `isTotalSupplyConfidential()` returns `false`.
    function totalSupplyEncrypted() external view returns (bytes memory);
}
```

**`IERC8227_Fees`** — FHE computation fee interface:

```solidity
interface IERC8227_Fees is IERC8227 {
...
**`IERC8227_Batch`** — Aggregated batch blind transfers:

```solidity
interface IERC8227_Batch is IERC8227 {
...
### Why is `totalSupply` visibility a deployment-time flag?

Supply visibility is a property of the deployment, not of the standard. Public-supply tokens (stablecoins, governance tokens, listed assets) need a readable `totalSupply` for oracles, exchange listings and circulating-supply audits. Confidential-supply tokens (private payroll, internal corporate units, redeemable loyalty points) leak meaningful business signal through the supply alone, even when every balance is encrypted. Neither model can be made the default without breaking the other, so the choice is made once, at deployment, and published.

An earlier draft signalled confidentiality by returning 0 from `totalSupply()`. That is ambiguous: 0 is also the correct answer for a token that has not minted yet, so a consumer could not distinguish a hidden supply from an empty one. `isTotalSupplyConfidential()` makes the distinction explicit and cheap to read.

The flag is fixed rather than mutable because flipping it would retroactively change the meaning of every historical `totalSupply()` read, and a token that can reveal its supply later does not offer supply confidentiality at all.
...
A token MAY implement both [ERC-20](./eip-20.md) and this interface simultaneously, where [ERC-20](./eip-20.md) functions operate on a separate "public balance" pool and encrypted functions operate on the "shielded balance" pool. The optional `IERC8227_Shielded` extension provides `shield`/`unshield` bridge functions to move tokens between pools.
...
The normative interface is published at `Valisthea/styx-erc-encrypted-token`, in `contracts/interfaces/IERC8227.sol`.

A working implementation is in development and is not yet available. The outstanding work is the binding between an FHE co-processor and the on-chain proof verification loop. This section will be updated when that implementation exists and is tested.

...
description: A minimal and generic specification for managing recurring subscription NFTs compatible with any ERC20 approval methods.
...
Recurring Subscription NFT is an extension of [ERC-721](./eip-721.md) token that enables a manual and recurring subscription service by charging NFT holders ERC20 tokens via gasless approvals.

The interface suggests specifications that allow users to renew/extend subscriptions, service provider to charge users automatically for recurring subscription by providing their subscription data and [EIP-712](./eip-712.md) signature. Also an optional function for users to cancel their subscription by revoking the respective token approval.

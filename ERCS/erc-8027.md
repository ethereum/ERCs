...
Recurring Subscription NFT is an extension of [ERC-721](./eip-721.md) token that enables recurring subscription feature by charging the NFT holder [ERC-20](./eip-20.md) tokens via any token approval methods.
...
2. users to setup a seamless payment flow via gasless pre-approval for any [ERC-20](./eip-20.md) with an expiry date
...
1. The `SubscriptionConfig` allows multiple plans to be configured - the standard supports multiple plans to enable tiered subscription plans and only using [ERC-20](./eip-20.md) as payment token will enable recurring subscription due to its approval feature. The NFT itself represents ownership of a subscription and there is no facilitation of how the NFT should be minted or transferred.

2. `RecurringSubscriptionData` passed in `chargeRecurringSubscription` is designed to be generic in order to integrate with any [ERC-20](./eip-20.md) approval methods:
...
3. For actual implementation in `ERC8027.sol`, customizations on:
   - verifying token approval and charging automatically should be done by overriding `_verifyApprovalAndCharge` to integrate with any common [ERC-20](./eip-20.md) approval methods e.g. [ERC-2612](./eip-2612.md), [ERC-3009](./eip-3009.md) and Permit2
   - verifying inputs passed from the function caller should be done by overriding `_verifyInputs`
...
Even though we could somehow achieve something similar to gasless pre-approval with the help of [EIP-7702](./eip-7702.md), the user's Externally Owned Account (EOA) will need to delegate to an external contract to act as a smart account which would impose extra security vectors that integrators need to consider, and the subscription feature is still lacking nonetheless.

More importantly this interface is in fact compatible with such smart accounts as long as they are able to receive NFT i.e. with `onERC721Received` implemented thus this interface provides a standardized framework while being AA-compatible for apps to offer subscription features in a more composable way.

### Subscription Management

- **Manual subscription:** Users should be able to renew their subscriptions by directly transferring either native or [ERC-20](./eip-20.md) tokens to the service provider thus the `renewSubscription` function. Users will specify the index of subscription plan i.e. `planIdx` and the number of interval to subscribe for i.e. `numOfIntervals`.
...
- `getSubscriptionConfig` will give users and applications the information about the configuration of the subscription such as which payment token, which service provider will receive the subscription fee, how long each interval is, and the price for each plan.

- Finally it's important to know that only using [ERC-20](./eip-20.md) as payment token can enable recurring subscription as we cannot directly transfer native token on behalf of users without introducing external dependencies.

## Backwards Compatibility

This standard can be fully [ERC-721](./eip-721.md) compatible by adding an extension function set, and payment can be integrated with any [ERC-20](./eip-20.md) out of the box.

This standard is also fully compatible with smart accounts that utilize Account Abstraction standards such as [ERC-4337](./eip-4337.md) and [EIP-7702](./eip-7702.md) as long as they are able to receive NFT i.e. with `onERC721Received` implemented or with `ERC721Holder.sol` from OpenZeppelin inherited.

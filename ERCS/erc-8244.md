...
The string returned by `html()` is the _document_. The following properties are
RECOMMENDED:

1. **Self-contained.** Prioritizes embedded resources, such as by using `data:`
   URIs and `blob:` URIs constructed from in-document strings. External
   resources SHOULD use Subresource Integrity (SRI) (`<script src integrity>`)
   to ensure they have not been tampered with.
2. **Wallet provider.** Assumes a standardized provider such as an EIP-1193
   provider at `window.ethereum` or an EIP-6963 multi-provider discovery
   mechanism.
3. **Origin contract.** Is able to identify the contract that produced it. The
   RECOMMENDED method is to embed the contract address as a JavaScript constant
   at write time, made deterministic by deploying through CREATE2 / CREATE3 or
   by writing the document after the contract address is known.
...
1. SHOULD render with equivalent isolation to a sandboxed `<iframe>` with a
   default-deny Content-Security-Policy. The frame SHOULD NOT inherit cookies,
   `localStorage`, or any credentialed resources from the host page.
2. SHOULD inject a standardized wallet provider into the frame, such as an
   ...
   mechanisms MAY be supported as the ecosystem evolves.
...
A renderer SHOULD treat the document as untrusted code. Recommended

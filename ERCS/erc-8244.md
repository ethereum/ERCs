...
author: z0r0z (@z0r0z), ndavd (@ndavd), Zefram Lou (@ZeframLou), Ronan (@wighawag)
...
one `eth_call` and render it. Off-chain hosting (HTTP, IPFS, NPM, CDN) is
not required, and any external resource a document does reference is
integrity-checked, so no mutable second trust domain is introduced.
...
The string returned by `html()` is the _document_.

**Origin contract.** The document MUST be able to identify the contract that
produced it. The RECOMMENDED method is to embed the contract address as a
JavaScript constant at write time, made deterministic by deploying through
CREATE2 / CREATE3 or by writing the document after the contract address is
known.

The following properties are RECOMMENDED:

1. **Self-contained.** The document SHOULD prioritize embedded resources, such
   as `data:` URIs and `blob:` URIs constructed from in-document strings. Any
   external resource SHOULD be integrity-checked — for example with Subresource
   Integrity (SRI) (`<script src integrity>`) or a content-addressed URI — so
   that a mutable host cannot alter the document's behavior undetected.
2. **Wallet provider.** The document SHOULD assume a standardized provider,
   such as an EIP-1193 provider at `window.ethereum` or one discovered via
   [EIP-6963](./eip-6963.md) multi-provider discovery.
...
1. MUST render with isolation equivalent to a sandboxed `<iframe>` with a
   default-deny Content-Security-Policy. The document MUST NOT inherit cookies,
   `localStorage`, or any credentialed resources from the host page, and MUST
   NOT be able to script the host context. A sandboxed `<iframe>` is the
   RECOMMENDED mechanism; a client with an equivalent isolation primitive, such
   as an isolated browser-extension context, MAY use it instead.
2. SHOULD inject a standardized wallet provider into the frame, such as an
   EIP-1193 provider or via EIP-6963 multi-provider discovery. Other provider
   mechanisms MAY be supported as the ecosystem evolves. However the provider is
   supplied, the client MUST gate every signing request through the host
   wallet's normal user-confirmation flow.
3. MUST NOT grant the document ambient network access that could alter its
   behavior undetected. Access to integrity-checked resources, such as
   SRI-pinned or content-addressed fetches, MAY be permitted.
...
[ERC-5219](./eip-5219.md) defines a request-response interface for
contract-served web content and can coexist with this proposal; a contract
MAY implement both. A client that supports both SHOULD prefer `html()` when
it is present and returns a non-empty document, since it resolves in a
single `eth_call` without request construction.
...
A conforming client, when given a contract whose `html()` returns a document
requesting the current chain id via a wallet provider (e.g.
`window.ethereum.request({method: 'eth_chainId'})`), MUST cause that request to
resolve to the chain id of the chain on which the contract is deployed.

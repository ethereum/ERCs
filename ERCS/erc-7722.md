...
This ERC proposes a standard for an opaque token with built-in privacy support. Privacy is achieved by offloading balances data to an off-chain storage and representing these records as hashes. 
...
```
keccak256(abi.encode(salt, value))
...
For the rest of the document, we call these hashes "baskets" because they "hold" the balance information in an opaque manner, much like a covered basket hides the items that it holds.
...
interface OpaqueToken {
    ...
    event Create(address minter, bytes32[] baskets);

    event Reorg(address initiatedBy, bytes32[] basketsIn, bytes32[] basketsOut);

    event Transfer(address sender, address receiver, bytes32[] baskets, bytes32 ref);


    // returns the owner of a basket
    ...
    */
    ...
    /*
    reorganizes a set of input baskets (basketsIn) and creates as result a new set of baskets (basketsOut)where the sum of all values from input baskets equals the sum of values in output baskets. In order to ensure the integrity, external oracle service is required that will sign the reorg proposal requested by the basket owner. 
    */
    function reorg(
...
```
  PUT basket
  PostData: {
    basket: keccak256(abi.encode(salt, value)),
    data: {
      salt: <bytes32>,
      value: <uint256>
    }
  ...
  GET basket?basket-hash=<bytes32>
  Response: {
    basket: keccak256(abi.encode(salt, value)),
    data: {
      salt: <bytes32>,
...
To ensure the integrity of a reorg and avoid accidental or fraudlent mints or burns, an oracle services are required. Oracles will sign any reorg proposal made by the basket holder where 
* the sum of values in input baskets is euqal the sum of values of the output baskets,
* and basket hashes match. services will sign valid reorg proposals requested by the basket holders. 

This endpoint requires no further authentication and can be used by anyone without restrictions.

This endpoint is stateless and requires only request data as input.

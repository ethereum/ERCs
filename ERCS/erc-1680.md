...
Unlike [ERC-20](./erc-20.md) where `balanceOf` returns a plaintext `uint256`, this interface returns an opaque ciphertext that only the balance owner can decrypt. Arithmetic operations on balances (credit, debit, comparison) are performed homomorphically by the execution environment without any party observing plaintext values.

This interface is backwards-compatible with [ERC-20](./erc-20.md) for public aggregate values (`totalSupply`, `name`, `symbol`, `decimals`) while introducing encrypted counterparts for all privacy-sensitive operations.

## Motivation

[ERC-20](./erc-20.md) tokens expose all balances and transfer amounts publicly. This creates several critical problems:
...
Compliant contracts MUST implement [ERC-165](./erc-165.md) and return `true` for the interface ID of `IERC1680`.
...
**`IERC1680_Shielded`** — Bridge between the public pool (compatible with [ERC-20](./erc-20.md)) and the encrypted pool:
...
[ERC-20](./erc-20.md)'s `Transfer(from, to, amount)` leaks the amount. `BlindTransfer(from, to, proofHash, keyVersion)` preserves the minimum information needed for indexing (who transferred to whom) without revealing how much. The `proofHash` enables off-chain auditors (with appropriate keys) to verify transfer validity.
...
### Backwards Compatibility with [ERC-20](./erc-20.md)

The public metadata functions (`name`, `symbol`, `decimals`, `totalSupply`) are identical to [ERC-20](./erc-20.md). This allows block explorers and basic wallet UIs to display token identity. The encrypted functions (`blindTransfer`, `encryptedBalanceOf`) are additive — they don't override [ERC-20](./erc-20.md) functions but extend them.

A token MAY implement both [ERC-20](./erc-20.md) and this interface simultaneously, where [ERC-20](./erc-20.md) functions operate on a separate "public balance" pool and encrypted functions operate on the "shielded balance" pool. The optional `IERC1680_Shielded` extension provides `shield`/`unshield` bridge functions to move tokens between pools.

## Backwards Compatibility

This interface is fully backwards-compatible with [ERC-20](./erc-20.md) for metadata queries. Tokens implementing this interface alongside [ERC-20](./erc-20.md) can participate in both the public and encrypted ecosystems.
...
This EIP REQUIRES [ERC-165](./erc-165.md) for interface detection, allowing protocols to query whether a token supports encrypted operations before attempting them.
...
### Share Inflation ([ERC-4626](./erc-4626.md) Analog)

...
This ERC defines an onchain metadata standard for multi-token and NFT registries including [ERC-721](./eip-721.md), [ERC-1155](./eip-1155.md), [ERC-6909](./eip-6909.md), and [ERC-8004](./eip-8004.md). The standard provides a key-value store allowing for arbitrary bytes to be stored onchain.
...
Keys MAY include parameters to represent variations or instances of a metadata type. When keys include parameters, they SHOULD use TOON format (a minimal agent-optimized format similar to YAML, developed by Johann Schopplich) with a colon and space separator.
...
This format provides a clean, consistent way to represent parameterized keys while maintaining readability and compatibility with TOON format parsers.

...
The EIP introduces a system of smart contracts that a decentralized protocol may use to manage its on-chain dependencies. The system also provides a convenient way of upgrading registered smart contracts, together with specifying custom access control permissions required by a protocol's business logic.
...
- Ability to specify custom access rules to control the protocol.
...
The following digram depicts the relationship between the registry and its dependants:

![](../assets/erc-6224/diagram.png)
...
interface IContractsRegistry {
    ...
    /**
     * @notice The event that is emitted when the contract get removed from the registry
     ...
     *
     * MUST call the `setDependencies()` with `address(this)` and given `data` as arguments on the provided contract
     *
     * @param name the name of the contract
     ...
     *
     * The function may be used to add contract that the `ContractsRegistry` has to be able to upgrade.
     *
...
The `ContractsRegistry` works together with the `Dependant` contract. Every standalone contract of a protocol MUST inherit `Dependant` in order to support dependency injection mechanism. 
...
interface IDependant {
     ...
     *
     * The contract MUST perform proper access check of `msg.sender`. The calls should only be possible from `ContractsRegistry`
     *
...
The `getContract` view function reverts if the requested contract is `address(0)`. This is essential to minimize the risks of misinitialization of a protocol. Correct contracts SHOULD be added to the registry prior any dependency injection actions.

...
this feature MUST also implement the metadata extension of [ERC-1155](./eip-1155.md). This JSON extension is **optional** and
...
* The `grantRole` function MUST revert if the `_expirationDate` is in the past or if the `msg.sender` is not approved to grant roles on behalf of the grantor. It MAY be implemented as public or external, and it is RECOMMENDED using `type(uint64).max` for a permanent role.
* The `revokeRole` function SHOULD always allow the grantee to revoke roles, MAY be implemented as public or external, and MUST revert if:
...
The concept of "token commitments" is as an abstraction that serves as a powerful tool for users looking to delegate the control of their SFTs. A token commitment represents either a frozen balance or tokens deposited into a contract, offering a standardized and secure way for SFT owners to delegate the use of their assets. Through [ERC-7589](./eip-7589.md), users gain a versatile mechanism to abstract the complexities of
...
by a `uint64` is about the year 584 billion, which should be enough to be considered "permanent". For this reason, using `type(uint64).max` in a assignment, represents that it never expires.
...
grantor can revoke the role prior to the expiration date. Regardless of the `_revocable` value, the grantee will always be enabled to revoke roles, allowing recipients to eliminate undesirable assignments.

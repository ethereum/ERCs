...
* grant roles on behalf of the grantor. It MAY be implemented as public or external. It is RECOMMENDED using 
* `type(uint64).max` for a permanent role
...
The concept of "token commitments" as an abstraction, serves as a powerful tool for users looking to delegate the 
control of their SFTs. A token commitment represents either a frozen balance or tokens deposited into a contract, 
offering a standardized and secure way for SFT owners to delegate the use of their assets. 
Through [ERC-7589](./eip-7589.md), users gain a versatile mechanism to abstract the complexities of secure delegation, 
enhancing the utility and interoperability of semi-fungible tokens.

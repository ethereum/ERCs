...
[ERC-1155](./erc-1155.md) has significantly contributed to the tokenization capabilities of Ethereum by enabling
developers to create fungible and non-fungible tokens with a single contract. While [ERC-1155](./erc-1155.md) excels at
...
One example is in gaming, in-game assets can be issued with a single [ERC-1155](./erc-1155.md) contract and rented out
...
this feature MUST also implement the metadata extension of [ERC-1155](./erc-1155.md). This extension is **optional** and
allows developers to provide additional information on roles.
...
> Please note that "approval" refers to allowing users to commit tokens and grant/revoke roles on one's behalf. An
  ...
  [ERC-1155](./erc-1155.md) approvals. More information can be found in the [Role Approvals](#role-approvals) section.
...
[ERC-7589](./erc-7589.md) steps in as a solution by introducing the concept of "token commitments". This abstraction
serves as a powerful tool for users looking to delegate the control of their SFTs. A token commitment can represent
either a frozen balance or tokens deposited into a contract, offering a standardized and secure way for SFT owners to
delegate the use of their assets. Through ERC-7589, users gain a versatile mechanism to abstract the complexities of
secure delegation, enhancing the utility and interoperability of semi-fungible tokens.

[ERC-7589](./erc-7589.md) IS NOT an extension of [ERC-1155](./erc-1155.md). The main reason behind this decision is to
...
Like [ERC-1155](./erc-1155.md), [ERC-7589](./erc-7589.md) allows users to approve operators to grant and revoke roles on
...
In the context of [ERC-7589](./erc-7589.md), dates are represented as `uint64`. The maximum UNIX timestamp represented
by a `uint64` is about the year 584 billion, which should be enough to be considered "permanent". For this reason, it's
RECOMMENDED using `type(uint64).max` in use cases where the assignment is required never to expire.
...
represented by a `uint256`. Using [ERC-7589](./erc-7589.md), one could simply encode the `uint256` as bytes and pass it
...
for SFTs that can't be modified? [ERC-7589](./erc-7589.md) solves this problem by requiring the `tokenAddress` parameter
when committing tokens. This requirement ensures that dApps can either implement [ERC-7589](./erc-7589.md) inside the
SFT contract or use a standalone external contract as the authoritative source for the roles of immutable SFTs.

## Reference Implementation

See [ERC-7589.sol](../assets/erc-7589/ERC7589.sol).

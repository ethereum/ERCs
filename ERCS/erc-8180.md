...
This ERC defines interfaces for decentralized authenticated messaging over [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844) blobs.
The core registration interface extends [ERC-8179](./erc-8179.md) (Blob Space Segments) with a
...
blob-unaware ([ERC-3722](./erc-3722.md) Poster, stagnant), NFT-based ([ERC-7847](./erc-7847.md), no
...
Where `chainId` is the [EIP-155](https://eips.ethereum.org/EIPS/eip-155) chain ID (`uint256`). The signed message hash is then:
...
blobs with other protocols, callers needed a separate [ERC-8179](./erc-8179.md) call to declare their
...
ERC-8179 positions `contentTag` as the first-class indexer filter key: every compliant indexer is
expected to subscribe to `BlobSegmentDeclared` by `contentTag`. Earlier drafts of this ERC extended
...
asymmetries. A single `eth_getLogs` call on either event recovers every matching registration
with its decoder and signature registry in one shot, no joins required. The calldata path gains
protocol-identity filtering at parity with the blob path. And the ERC's "contents as flexible
payload" design keeps `contentTag` as its natural per-submission schema key — usable on both
paths, not just the blob path.

The separation of concerns the ERC already uses is preserved:
...
under ERC-8179's design, while filter-by-`decoder` becomes less useful as decoders converge on a
small stable set. `submitter` remains indexed so attribution-by-submitter (the tuple
`(chainId, contentTag, submitter)` that ERC-8179 recommends for BSS indexers, and this ERC
...
earlier layout but forces data-level filtering for the query that ERC-8179 positions as
...
signature. `eth_getLogs` callers would have to filter on the unkeyed event payload to disambiguate
...
(the route [ERC-8179](./erc-8179.md) takes with `IERC_BSS_Batch`). BSS declarations are
...
[ERC-3722](./erc-3722.md) (Poster) and [ERC-8179](./erc-8179.md): the event log suffices for
...
replay. For individual user self-publication with ECDSA, adopters may define an [EIP-712](https://eips.ethereum.org/EIPS/eip-712) TypedData
...
Existing messaging contracts (e.g., [ERC-3722](./erc-3722.md) Poster) can adopt this ERC by:
...
The `BLOBHASH` opcode ([EIP-4844](https://eips.ethereum.org/EIPS/eip-4844)) is required for `registerBlobBatch`. The calldata
...
including one associated with a well-known protocol. This mirrors ERC-8179's treatment of
...
their decoder and signature registry. This matches the null-tag guidance in ERC-8179.
...
under ERC-8179 economics SHOULD expect somewhat higher volumes on `CalldataBatchRegistered`.

...
### Signature Encoding for Signing Domains

When signatures include signing domain information for on-chain validation, implementations SHOULD encode the signature data to include the necessary components for reconstructing the signing domain chain.

#### Recommended Encoding Format

For signatures that utilize signing domains, the following encoding format is RECOMMENDED:
...
Implementations may use alternative encoding schemes as long as they contain enough information to reconstruct the signing domain chain, support the `encodeForSigningDomains` algorithm for message hash validation, and accommodate the specified authentication methods, with wallet and contract implementations coordinating to ensure compatibility.
...
The design ensures graceful degradation when only part of the stack supports this standard. Security never falls below current [EIP-712] levels. Enhanced features simply become unavailable rather than causing failures. Users can still interact with applications, albeit with reduced context and security. This approach enables incremental adoption while maintaining backward compatibility throughout the ecosystem.
...
Smart accounts currently using [ERC-7739] can seamlessly upgrade to this specification as it represents only a semantical change (rather than a breaking change), with the [`authMethods`](#authmethods) field appropriately covering the corresponding ERC's verification behavior. Existing contracts using [ERC-1271], [ERC-6492], or [ERC-7913] require no modifications, and all [EIP-712] libraries, development tools, and frameworks continue to work without changes to their current interfaces.
...
Wallets must implement new rendering logic to properly interpret signature requests for this specification.

Wallets that have not implemented this specification will simply ignore the new fields and process requests as standard [EIP-712] signatures. This may result in suboptimal user experience (missing context) but will not break functionality. Users may receive signatures that protocols cannot verify if authentication methods are incompatible.

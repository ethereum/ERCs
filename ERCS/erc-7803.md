...
The design ensures graceful degradation when only part of the stack supports ERC-7803. Security never falls below current [EIP-712] levels. Enhanced features simply become unavailable rather than causing failures. Users can still interact with applications, albeit with reduced context and security. This approach enables incremental adoption while maintaining backward compatibility throughout the ecosystem.

## Backwards Compatibility

ERC-7803 maintains full backwards compatibility with existing [EIP-712] implementations across the Ethereum ecosystem, with wallets being able to identify ERC-7803 encoded signatures through the `\x19\x02` prefix while standard [EIP-712] signatures using `\x19\x01` continue to function normally.

Smart accounts currently using [ERC-7739] can seamlessly upgrade to ERC-7803 as it represents only a semantical change (rather than a breaking change), with the [`authMethods`](#authMethods) field appropriately covering the corresponding ERC's verification behavior. Existing contracts using [ERC-1271], [ERC-6492], or [ERC-7913] require no modifications, and all [EIP-712] libraries, development tools, and frameworks continue to work without changes to their current interfaces.
...
Wallets must implement new rendering logic to properly interpret ERC-7803 enhanced signature requests.

Wallets that have not implemented ERC-7803 will simply ignore the new fields and process requests as standard [EIP-712] signatures. This may result in suboptimal user experience (missing context) but will not break functionality. Users may receive signatures that protocols cannot verify if authentication methods are incompatible.

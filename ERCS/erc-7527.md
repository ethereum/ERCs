...
This proposal outlines interfaces for a Function Oracle capable of converting fungible tokens into a single non-fungible token each instance and then reverting the non-fungible token back to fungible tokens. The pre-configured Function Oracle comes equipped with its own Automated Market Maker (AMM) interfaces, enabling a clearing mechanism for voucher issuers.
...
Consider the following for the safety of the contracts system:

...
description: Interfaces that wrap/unwrap between fungible tokens and non-fungible tokens based on an embedded Function Oracle AMM
...
This proposal outlines interfaces for a Function Oracle capable of converting fungible tokens into a single non-fungible token each instance and then reverting the non-fungible token back to fungible tokens. The pre-configured Function Oracle comes equipped with its own Automated Market Maker (AMM), enabling a clearing mechanism for voucher issuers.

## Motivation

The motivation behind a wrapping interfaces with embedded Function Oracle AMM is to achieve decentralized endorsement and thus quantifiable decentralized credit with liquidity in nature. This creates an applicable infrastructure for credit creation, transmission, and oracle in payment, social, and financial context. Decentralized credit can be implemented in the form of Non-fungible tokens, compatible with current Ethereum environment, which also expands its application scenarios. The concept of decentralized credit is highly integrated with dApps development. It aims to foster a more trustless environment.
...
Agency and App MAY be seperately implemented, but they SHALL be mutually bounded and not upgradable after initialization. Or, Agency and App MAY be both implemented in one single contract. 

Factory Interface is OPTIONAL, depending on whether the combination of Agency and App need to be deployed repeated. 

The Agancy MUST implement an Function as Oracle, which serves as AMM when token are wrapped or unwrapped. The Agency serves as a single entry point for all `mint` and `burn` transfer. 

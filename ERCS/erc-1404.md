...
- #### `detectTransferRestriction(address,address,uint256)`
  ...
  MUST be called inside the token's transfer entry points (for [ERC-20](./eip-20.md), `transfer` and `transferFrom`; for other token interfaces, the analogous transfer methods, such as [ERC-777](./eip-777.md) `send`). When a non-zero code is returned, the transfer SHOULD fail consistently with the underlying token's transfer expectations; reverting is RECOMMENDED. Implementations MAY return `false` instead.

...
Tokens that carry compliance obligations — securities, lock-up periods, jurisdiction or KYC/AML allowlists — enforce those obligations by rejecting transfers that violate them. On [ERC-20](./eip-20.md) and compatible interfaces such as [ERC-777](./eip-777.md), that rejection is opaque: a caller learns that a transfer failed, but has no way to learn beforehand that it would fail, or why. Wallets and exchanges are left to submit a transaction and interpret whatever revert comes back.

...
- A contract that performs transfers and advertises [ERC-1404](./eip-1404.md) is a **restricted token** and remains fully bound by the enforcement-consistency requirement, including where it obtains its restriction decisions from one or more restriction sources. Consulting a restriction source does not transfer that obligation to the source: the restricted token is the contract an integrator queries, so it is the contract whose reported code MUST match what its transfer path does.

- Obligations on a caller reading a restriction code. These are stated here so that the guarantees the preceding requirements provide are not over-read; the hazards behind each are described in [Security Considerations](#security-considerations).
...
6. Restriction sources and aggregation - The deployed shape of a permissioned token is frequently a token delegating to a separate, independently upgradeable rule engine, which may itself compose several policies. Defining a restriction source as a distinct conformance class keeps that architecture explicitly in scope without weakening the guarantee integrators rely on: the enforcement-consistency requirement stays attached to the contract that actually performs the transfer, because that is the contract an integrator queries and the only one whose report can be checked against an outcome. Requiring the restricted token to report the *aggregate* outcome, rather than the outcome of any single source, is what keeps the guarantee meaningful once a transfer path consults more than one source — a token that forwards one engine's `0` while its own pause flag blocks the transfer would otherwise be conformant while misreporting.
   - Two things this specification deliberately does not add. It does not reserve a code for "cannot be determined", because codes `1`–`255` have been issuer-defined since the original [ERC-1404](./eip-1404.md) and reserving one now would reassign a value deployed tokens already use; the condition is instead given a documentation duty and integrator guidance, which is what the condition actually needs. And it does not add an entry-point-aware predictor — a fourth method taking, say, a selector — even though that would resolve the many-entry-points case exactly. It would add a third interface identifier and oblige a token to publish which selectors it can answer for, which is a large addition to a surface whose value is its size, and a selector is a weak discriminator in any case for tokens that route several entry points through shared internal logic. Naming the limitation and requiring implementations to scope their predictors costs nothing and addresses the integrator's actual problem, which is knowing what an answer covers.
...
7. Mint and burn - Restriction checks on supply-changing operations remain optional, because the original [ERC-1404](./eip-1404.md) did not define them and requiring them would strand conformant implementations. But an implementation that opts in and evaluates them through this interface needs a way to express "there is no sender" in a signature that has no room for one, and integrators need to construct the same query the token evaluates. The `address(0)` encoding is specified rather than invented: [ERC-20](./eip-20.md) already establishes `address(0)` as the mint and burn counterparty in its `Transfer` event, so this records existing practice instead of adding a convention. Extending the enforcement-consistency requirement to these operations costs nothing for implementations that do not opt in, and closes the case where an implementation reports a mint restriction it does not enforce — the operation permissioned issuers most depend on.
...
| [`src/engine/WhitelistRuleEngine.sol`](../assets/eip-1404/src/engine/WhitelistRuleEngine.sol) | Restriction source — a standalone rule engine that is not a token |
| [`src/engine/RestrictedToken.sol`](../assets/eip-1404/src/engine/RestrictedToken.sol) | Restricted token delegating to a rule engine, aggregating it with a policy of its own |
...
The considerations below are grouped by the question they answer: whether the reported code and the enforced outcome agree, what an answer actually covers, and what the policy behind the answer costs to run. Normative requirements live in [Specification](#specification); this section explains the hazards behind them.

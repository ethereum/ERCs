...
description: A machine-verifiable semantic metadata to enable autonomous agent reasoning
author: Collins Adi (@collinsadi)
discussions-to: https://ethereum-magicians.org/t/erc-8174-intent-spec-for-contracts/27869
status: Draft
...
[ERC-8004](./eip-8004.md) defines infrastructure primitives for autonomous agents, including identity, reputation, and validation mechanisms. It standardizes how agents establish identity and trust.
...
Contracts implementing this standard MUST support [ERC-165](./eip-165.md) interface detection as defined in [ERC-165](./eip-165.md).
...
interface IIntentSpec {
    /// @notice Emitted when the Intent Spec metadata URI changes.
    /// @param uri The new URI pointing to the manifest.
    event IntentSpecURIUpdated(string uri);

    /// @notice Returns the URI where the Intent Spec JSON is stored.
    /// @return A URI (e.g. ipfs://... or https://...) pointing to the manifest.
    function intentSpecURI() external view returns (string memory);
}
```
...
* If the URI changes, the contract MUST emit `IntentSpecURIUpdated` with the new URI.
* Agents MUST verify content-addressed URIs where applicable.

### Agent Verification

A manifest is self-declared by the contract author and MAY be inaccurate. Agents that rely on a manifest:

* SHOULD independently verify the described behavior against the contract bytecode, execution simulations, and third-party attestations.
* SHOULD simulate execution and compare the observed state transitions with the declared `effects`.

### Canonical Manifest Schema

The resource returned by `intentSpecURI()` MUST conform to the following JSON Schema (Draft 2020-12):
...
`intentSpecURI()` provides a consistent discovery mechanism for metadata retrieval.
...
Manifest content is self-declared and can misrepresent contract behavior, whether by mistake or intent. An agent that trusts a manifest without checking it against the contract can be led into unsafe actions. The Agent Verification requirements in the Specification limit this exposure by cross-checking the manifest against bytecode, execution simulations, and third-party attestations.

### Simulation Verification

Declared `effects` describe expected state transitions but are not enforced on-chain. Comparing simulated execution against the declared `effects`, as required in the Specification, lets an agent detect divergence before submitting a transaction.

### Metadata Integrity

A manifest referenced by a mutable URI can be swapped after review. Content-addressed storage, together with the content-addressed URI verification required in the Specification, ties the manifest to a fixed hash and prevents undetected tampering.

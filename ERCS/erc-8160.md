...
description: A singleton registry mapping each Ethereum address to a single agent identity, so any address can be resolved to the agent it represents.
author: Prem Makeig (@nxt3d)
discussions-to: https://ethereum-magicians.org/t/erc-8160-primary-agent-registry/27727
status: Draft
...
If `deadline` is non-zero, the registrar MUST revert if `block.timestamp > deadline`. The registrar MUST verify the signature by recovering the signer using ECDSA over an [EIP-191](./eip-191.md) `personal_sign` hash, where the message hash is computed as:

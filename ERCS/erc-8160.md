...
When the payer is not fully sponsoring, it returns `tokenOptions` describing accepted tokens and pricing. Each token option specifies a `rate` and `rateExpiry` for the payer-quoted price. Alternatively, a token option MAY include an `oracle` address and optional `oracleData` — an onchain price oracle the wallet or protocol can verify trustlessly. When `oracle` is present, the `rate` field is advisory and the onchain oracle is the authoritative price source.

The wallet selects a token option, sets the [EIP-8130](./eip-8130.md) transaction's `fee_token` and `max_amount` accordingly, and the protocol handles the phase 0 token transfer to the payer natively.

#### Required Calls

A payer MAY return `requiredCalls` — calls the wallet MUST include in the transaction as a condition of sponsorship. These enable payer models where sponsorship is granted in exchange for executing specific onchain operations (e.g., a commitment to a privacy pool, a protocol interaction the payer needs). The wallet MUST prepend these calls to the sender's calls so they execute in phase 0. The payer validates their presence in `payer_sendTransaction` and MUST reject the transaction if they are missing or altered.

#### Recipient Address

A payer MAY return a `recipient` address indicating where value or profit from the transaction's execution should flow. This enables sponsorship models where the payer covers gas in exchange for execution profit — the payer validates (via simulation or proof) that the transaction will deliver value to `recipient` before co-signing. The payer MUST reject in `payer_sendTransaction` if the requirement is not satisfied.
...
##### `payer_getTerms` Example Return Value (Token Payment — Oracle-Based Rate)
...
Token options support two pricing models. A payer-quoted `rate` is sufficient for permissioned payers where the wallet trusts the service. For permissionless payers, an `oracle` address allows the rate to be verified onchain — the protocol or wallet can query the oracle to confirm the exchange rate independently. When `oracle` is present, the `rate` field serves as an advisory estimate for UI display, and the onchain oracle is authoritative.

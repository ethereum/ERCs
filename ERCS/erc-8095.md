...
author: Jose Luu (@joseluu), Jose Luu <jose.luu@bpce.fr>, Cyril Vignet <cyril.vignet@bpce.fr>, Vincent Griffault <vincent.griffault@cera.caisse-epargne.fr>, Frederic Faure <frederic.faure@banque-france.fr>, Clement Delaneau <clement.delaneau@banque-france.fr>
...
In terms of automation, the directory allows **on-chain verification** for:
...
Information is maintained by the stakeholders and it is also in their best interest to keep it up to date for their customers.
...
* **recognized authority**
  ...
  An organisation which is well known by potential users who decides to deploy an [ERC-8095](./eip-8095.md) contract. Such organisations can be major token issuers, exchanges, state regulators, non profit foundations, industry consortia ...
* **ERC-8095 contract**
...
* **Registrant**
  ...
  The registrant must provide and keep updated its own Universal Resource Identifier (URI) for its clients to consult service terms and possibly register for services. 
...
* **Administrator**
  ...
  The ERC-8095 contract is operated from one or several administrator addresses.
  ...
  Administrator addresses are responsible for (de)activating, configuring, and managing the decentralized directory.
* **ReferenceStatus**
...
* **on-chain verification**
  ...
  They are configured with the relevant ERC-8095 contract address and `registrantAddress` variables to enable this consultation functionality.
* **URI**
...
A non-modifiable string URI that describes the ERC-8095 contract itself and the recognized authority that has issued it. 
It has to be hosted by the issuer on its main domain name.
This URI allows the recognized authority to provide descriptive information about itself to the users.

##### `setActivationCode(uint256 _activationCode)   _activationCode` values
...
##### `getActivationCode()`
 If the contract reaches its end of life, it will revert with message "Directory is not active" to any other calls.
...
##### `disableRegistrant(address registrantAddress)`
 ...
 Once disabled, the registrant cannot be re-enabled unless the optional registrant status audit trail is implemented
...
##### `updateReferenceStatus(address referenceAddress, string newStatus)`
 ...
 see also: **Optional Features** for a possible audit trail
...
##### `getRegistrantUri(address _registrantAddress) external view returns (string memory)`
 ...
 the web identity of a contract owner.
...
##### `getReference(address referenceAddress)`
 Returns all the information known about a reference:
...
As returned by `getActivationCode()`
-  0 notActivated (initial value at deployment time)
-  1 activated
-  2 endOfLife

##### Reference status
This is status field returned by `getReference()`
...
####  distinct supplementary administrator addresses
 This feature allows the deployer to give administration power to other addresses specified at deployment time
...
##### isValidRegistrant (_registrantAddress)
 ...
 This returns whether the registrant is disabled, a non existent registrant will raise an error
...
#### functions for enumerating the registrant and reference lists
...
##### getRegistrantAtIndex(uint256 index)
 Returns the address and URI of a registrant at a specific index
##### getReferencesList(address registrantAddress)
 Returns an array of references for a given registrant

#### reference additional status
 - 5 contract compromised, should not be used
...
The deploying recognized authority needs to have an administrative off-chain process for organizations to apply and be vetted as registrants, as well as maintain the vetted registrant status. The registrant list must be updated accordingly, possibly disabling registrants at some point when they no longer match the vetting requirements. This off-chain administrative due diligence seems to be unavoidable, the alternative would be a system of confidence votes given by the users themselves. We have found that going through a recognized authority would avoid vote manipulations and therefore give more certainty to the users.
...
Addresses are indexed by their [EIP-155](./eip-155.md) chainIDs
To avoid rogue deployments on newer blockchains, addresses from all blockchain IDs must be enumerated even if the contract is deployed at the same address on every blockchain known to the recognized authority.
...
### In case of loss or compromise of keys corresponding the addresses used
There are several cases to be considered depending of the nature of the address
- If the address is a smart-contract (reference): the registrant declares it end-of-life or compromised and deploys a replacement contract at a new address
- if the address is a registrant (management) address: The registrant unlinks the address cross referenced in its URI, rendering the address invalid because it will fail the cross reference check. The registrant needs to ask for a new management address to be added by the recognized authority. It is up to the recognized authority to inspect (or not) the internal processes of the registrant which have led to such loss or compromise and to vet again (or not) the registrant and allow the addition of a new address in the list of registrants.

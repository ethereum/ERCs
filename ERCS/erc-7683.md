...
Publishing the resolver onchain is useful because the resolver is the point of trust. Solver operators whitelist can resolver addresses, and once vetted, the instructions they produce are trusted. Vetting is done through the usual means, such as security audits, bounties, lindiness, etc. However, neither resolvers nor the resolved order representation are inherently EVM-specific: the EVM resolver interface is only one concrete encoding of the more general model defined by this ERC.

...
This specification introduces a new interface that extends [ERC-721](./erc-721.md) and [ERC-1155](./erc-1155.md) that defines methods for setting and getting dynamic onchain traits associated with non-fungible tokens. These dynamic traits can be used to represent properties, characteristics, redeemable entitlements, or other attributes that can change over time. By defining these traits onchain, they can be used and modified by other onchain contracts.
...
Contracts implementing this EIP MUST include the events, getters, and setters as defined below, and MUST return `true` for [ERC-165](./erc-165.md) `supportsInterface` for `0xaf332f3e`, the 4 byte `interfaceId` for this ERC.
...
If a `tokenId` does not exist, the contract MUST revert with `TokenDoesNotExist(tokenId)`. If the token contract already defines a custom error for non-existent tokens (e.g. from [ERC-721](./erc-721.md) or [ERC-1155](./erc-1155.md)), that error MAY be used instead.
...
The trait metadata must be compliant with the [specified schema](../assets/erc-7496/DynamicTraitsSchema.json).
...
Authors have included Foundry tests covering functionality of the specification in the [assets folder](../assets/erc-7496/ERC721DynamicTraits.t.sol).

## Reference Implementation

Authors have included reference implementations of the specification in the [assets folder](../assets/erc-7496/DynamicTraits.sol).

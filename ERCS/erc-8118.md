...
description: An interface for secure, time-bound, and usage-limited delegation of on-chain actions to autonomous agents.
author: Alvin Sun <xinyao1@ualberta.ca>, Tim Zhang <tim@matrixlabs.org>, Xiao Wu <ling@chainide.com>, Wei Cai <weicaics@uw.edu>, WORLD3 Team (@world3-ai) <dev@world3.ai>
discussions-to: https://ethereum-magicians.org/t/erc-8118-agent-authorization/27402
status: Draft
...
- **Cryptographic consent**: Agents must sign consent to being authorized ([EIP-712](./eip-712.md))
- **Single-principal constraint**: Each agent serves only one principal at a time per contract

## Motivation

On-chain users and applications increasingly rely on delegated automation: trading bots, protocol keepers, relayers, schedulers, and other automated services. More recently, AI-driven assistants are emerging as another class of "agent", but the authorization problem is the same regardless of how an agent is implemented. A contract cannot (and need not) distinguish between an AI system and a traditional program; it only interacts with an account that submits transactions and signatures.

A standardized interface for agent authorization is needed to:

1. **Enable agents to act on-chain**: Any autonomous program (bot, service, script, or AI system) needs a predictable, secure way to execute transactions on behalf of a principal without taking custody of the principal's keys.
2. **Provide fine-grained, bounded permissions**: Principals should be able to limit what actions agents can take, for how long, and how many times, with clear revocation semantics.
3. **Improve safety via mutual consent**: Agents must explicitly consent to being authorized, preventing accidental or malicious binding of unwilling addresses.
4. **Support composability**: A standard interface enables cross-contract and cross-application authorization and makes it possible for wallets and tooling to present permissions consistently.

Existing delegation mechanisms (like token approvals) are either too broad (unlimited approvals) or too narrow (single-use signatures). This standard provides a balanced approach suitable for any autonomous agent scenario, with AI agents representing a prominent and growing use case rather than the only one.

### Real-World Use Cases

**Automated Trading Agents**: Users authorize trading bots (rule-based or AI-driven) to execute trades within specific parameters (e.g., only swap functions, maximum 100 calls per day, valid for 30 days).

**Gaming NPCs**: Game contracts allow automated characters (scripted or AI-driven) to perform in-game actions on behalf of players during offline periods, with automatic expiration.

**DeFi Position Management**: Users authorize automation services to rebalance portfolios, compound yields, or manage liquidation protection with usage-limited authorizations.

**DAO Proposal Execution**: Governance automation can be authorized to execute approved proposals within time windows, with automatic revocation after execution.
...
**Cross-Chain Relayers**: Bridge/relayer agents receive time-limited authorization to relay messages, with automatic expiration preventing stale authorizations.

### Production Validation

This standard has been validated through extensive production deployment and peer-reviewed academic research. In a fully on-chain gaming application deployed on opBNB (BNB Chain's Optimistic Rollup), the protocol:
...
The formal capability semantics, security invariants, and production results are documented in a peer-reviewed industrial poster.[^brains2025]

### Comparison with Existing Standards

This ERC occupies a unique position relative to existing delegation standards:

| Feature | [ERC-4337](./eip-4337.md) | [EIP-7702](./eip-7702.md) | This ERC |
|---------|----------|----------|----------|
...
**[ERC-6900](./eip-6900.md) (Modular Accounts)**
...
- Both support [EIP-165](./eip-165.md) interface detection for composability

**[ERC-7579](./eip-7579.md) (Minimal Modular Accounts)**
...
Contracts implementing this standard MUST implement `IERC8118AgentAuthorizationBase`:
...
Contracts MAY implement `IERC8118AgentAuthorizationUpdatable` for in-place authorization updates:
...
3. For contract agents, the signature MUST be validated using [EIP-1271](./eip-1271.md) `isValidSignature`.
...
### [EIP-165](./eip-165.md) Interface Detection

Contracts MUST implement EIP-165 and return `true` for:
- `IERC8118AgentAuthorizationBase` interface ID: `0x9e22ca0f`
- `IERC8118AgentAuthorizationUpdatable` interface ID (if implemented): `0x51c6e02e`
...
**Existing Contracts**: Contracts without this interface can be wrapped by a proxy implementing `IERC8118AgentAuthorizationBase` that delegates authorized calls to the underlying contract.

**Token Approvals**: This standard operates independently of ERC-20/[ERC-721](./eip-721.md)/[ERC-1155](./eip-1155.md) approval mechanisms. An agent authorized via this interface still requires appropriate token approvals for transfers.
...
Contracts using the optional update extension (`IERC8118AgentAuthorizationUpdatable`) should ensure upgrade mechanisms preserve authorization state. When using proxy patterns ([ERC-1967](./eip-1967.md), UUPS), the authorization storage slots must remain consistent across implementations.
...
A reference implementation is available in the assets folder:

- [`AgentAuthorizationBase.sol`](../assets/eip-8118/AgentAuthorizationBase.sol): Base implementation with EIP-712 signatures and single-principal constraint
- [`AgentAuthorizationUpdatable.sol`](../assets/eip-8118/AgentAuthorizationUpdatable.sol): Extended implementation with in-place authorization updates
- [`IERC8118AgentAuthorizationBase.sol`](../assets/eip-8118/IERC8118AgentAuthorizationBase.sol): Base interface definition
- [`IERC8118AgentAuthorizationUpdatable.sol`](../assets/eip-8118/IERC8118AgentAuthorizationUpdatable.sol): Updatable extension interface
- [`AuthorizationTypes.sol`](../assets/eip-8118/AuthorizationTypes.sol): Shared struct definitions
- [`AuthorizationErrors.sol`](../assets/eip-8118/AuthorizationErrors.sol): Custom error definitions
...
[^brains2025]:
    ```csl-json
    {
      ...
      "DOI": "10.1109/BRAINS67003.2025.11302911",
      "title": "Custody-Preserving AI Agent Delegation for Decentralized Applications via World AI Protocol",
      "container-title": "2025 7th Conference on Blockchain Research & Applications for Innovative Networks and Services (BRAINS)",
      "publisher": "IEEE",
      ...
      "URL": "https://ieeexplore.ieee.org/document/11302911"
    }
    ```

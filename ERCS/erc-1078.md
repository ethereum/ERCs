...
The unique identifier of the user is a contract that implements both Identity and the Executable Signed Messages ERCs. The user should should not need to provide this address directly, only a ENS name pointing to it. These types of contracts are indirectly controlled by private keys that can sign messages indicating intents, which are then deployed to the contract by a third party (or a decentralized network of deployers).  
...
If the user doesn’t have an identity, the app should provide the option to create one for them. Each app must have one or more domains they control which they can create immediately subdomains on demand. The app therefore will make these actions on the background:

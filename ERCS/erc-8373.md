...
| field | type | meaning |
|---|---|---|
| `schema` | string | the **implementation-namespaced** binding-statement schema (e.g. `kya.pq_key_binding.v0`, or a deployment's own namespace such as `invinoveritas.pq_key_binding.v1`) — distinct from the cutoff `profile`; opaque to enforcement (see [The cutoff rule](#the-cutoff-rule)) |
...
The `schema` member is **implementation-namespaced and opaque to cutoff enforcement**: an enforcer MUST NOT condition its verdict on `schema`. Enforcement keys only on anchored times, the content-address, and `pq_pubkey`; the cutoff `profile` is the only version identifier that governs. A binding under any namespace — `kya.pq_key_binding.v0`, a production deployment's own namespace, or one unknown to the verifier — yields the same verdict as the equivalent known-namespace binding. The conformance suite's *namespace opacity* case is the failing witness: an enforcer that pattern-matches the namespace reds exactly that case.

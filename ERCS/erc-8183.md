...
Payment SHALL use a single [ERC-20](./erc-20.md) token (global for the contract or specified at creation). Implementations MAY support a per-job token; the specification only requires one token per contract.
...
Agentic Commerce is intentionally minimal and does not embed a reputation system. For on-chain reputation and trust relationships between agents, implementations are RECOMMENDED to integrate with [ERC-8004](./erc-8004.md) (Trustless Agents).
...
#### Meta-Transactions / Facilitator Relay ([ERC-2771](./erc-2771.md))

To support gasless execution — where a client, provider, or evaluator signs an intent off-chain and a **facilitator** submits the transaction on their behalf — implementations SHOULD support [ERC-2771](./erc-2771.md) (Secure Protocol for Native Meta Transactions).
...
**Token approvals:** For functions that pull tokens (e.g. `fund`), the signer SHOULD use [ERC-2612](./erc-2612.md) (`permit`) to approve token spending via signature. The facilitator can then call `permit` and `fund` in a single transaction — no on-chain approval tx needed from the signer.
...
- Evaluator is trusted for completion and rejection once the job is Submitted; a malicious evaluator can complete or reject arbitrarily. Use reputation (e.g. [ERC-8004](./erc-8004.md)) or staking for high-value jobs.
...
- **Tokens:** Use SafeERC-20 or equivalent for [ERC-20](./erc-20.md).

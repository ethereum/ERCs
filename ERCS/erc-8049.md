...
This ERC defines a standard for storing contract-level metadata onchain. It extends [ERC-7572](./erc-7572.md)'s contract-level metadata concept by providing onchain storage. Contracts MAY optionally use [ERC-8042](./erc-8042.md)'s Diamond Storage pattern for predictable storage locations, enabling cross-chain compatibility and supporting upgradable contracts.
...
Keys MAY include parameters to represent variations or instances of a metadata type, such as `"registration: 1"` or `"name: Maria"`; see [ERC-8119](./erc-8119.md): Key Parameters.
...
The Diamond Storage pattern provides predictable storage locations for data, which is useful for cross-chain applications using inclusion proofs and for upgradable contracts. For more details on Diamond Storage, see [ERC-8042](./erc-8042.md).
...
Contracts implementing this ERC MAY use hooks to redirect metadata resolution to a different contract. For the full specification, see [ERC-8121](./erc-8121.md). When using hooks with contract metadata, the target function MUST be `contractMetadata(string)` returning `bytes`. The hook selector is `0x9e574b14`.

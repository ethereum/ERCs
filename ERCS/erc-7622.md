...
_The deposit and withdraw functions for users are `OPTIONAL` in implementation, as funds can be transferred directly from the user's account in real-time during the `authorizeServiceProvider` operation._
...
The `Withdrawal` event `OPTIONAL` be emitted when funds are withdrawn.

## Rationale

The choice of using [ERC-20](./eip-20.md) tokens for payments was driven by its widespread adoption. Additionally, the protocol introduces a set of standardized interfaces and events to ensure interoperability and accessibility of the contract. These interfaces and events define the interaction between users and service providers, enabling seamless communication and integration between different implementations. Furthermore, by defining events, the contract can provide important state updates and notifications to external applications, enhancing the protocol's observability and transparency.

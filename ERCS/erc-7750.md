...
## Test Cases (Revised with Detailed Outputs)
...
1. **Company Creation**
   ...
   **Expected State Changes**  
   ...
   - The `companies` mapping is updated:
     \[
       \text{companies}[1] \mapsto \{
         \text{name} = "TechCorp",
         \text{industry} = "Technology",
         \text{owner} = \text{callerAddress},
         \text{employeeIds} = \text{[ ]}
       \}
     \]
...
2. **Employee Token Minting**
   ...
   **Expected State Changes**  
   ...
   - An internal mapping (e.g., `employeeTokenToOwner`) is updated:
     \[
       \text{employeeTokenToOwner}[5] \mapsto \text{employeeAddress}
     \]
   - (Optional) If the implementation tracks metadata, another mapping (e.g., `employeeTokenMetadata`) might store:
     \[
       \text{employeeTokenMetadata}[5] \mapsto "ipfs://metadataURI"
     \]
   ...
   **Expected State Changes**  
   - **Contract Creation**:
     ...
     2. The `contracts` mapping is updated:
        \[
          \text{contracts}[10] \mapsto \{
            \text{companyId} = 1,
            \text{employeeTokenId} = 5,
            \text{salary} = 1000,
            \text{duration} = 6,
            \text{responsibilities} = "Software Development",
            \text{terminationConditions} = "Failure to meet deadlines",
            \text{status} = "Created"
          \}
        \]
     3. The system may also update a per-company or per-employee tracking structure (optional but typical):
        \[
          \text{companyContracts}[1].push(10)
        \]
        \[
          \text{employeeContracts}[5].push(10)
        \]
   ...
   - **Contract Execution**:
     1. Upon calls from both parties, the contract’s status changes from `"Created"` to `"Active"`:
        \[
          \text{contracts}[10].\text{status} \mapsto "Active"
        \]
...
4. **Salary Deposit**
   ...
   **Expected State Changes**  
   1. The contract’s escrow balance mapping is updated:
      \[
        \text{escrowBalances}[10] \mapsto 1000
      \]
...
5. **Salary Payment**
   ...
   **Expected State Changes**  
   ...
   2. The `escrowBalances[10]` is set to `0`:
      \[
        \text{escrowBalances}[10] \mapsto 0
      \]
...
6. **Employment Termination**
   ...
   **Expected State Changes**  
   1. The `contracts[10].status` is updated to `"Terminated"`:
      \[
        \text{contracts}[10].\text{status} \mapsto "Terminated"
      \]

...
4. **Salary Deposit**
   ...
   **Input**  
   - The company (owner of `companyId = 1`) calls `depositSalary(10)` and sends `1000 USDC` (or equivalent in wei for an [ERC-20](./erc-20.md) token or native token) to the contract.
...
5. **Salary Payment**
   ...
   **Expected Output**  
   ...
   - The employee’s on-chain balance (or token balance if using [ERC-20](./erc-20.md)) increases by `1000`.

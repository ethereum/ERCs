...
Also, there are plenty of tokens that do not implement [ERC-165](./eip-165.md) introspection at all. As the result it was decided to implement a special `standard() returns (uint32)` function in all [ERC-223](./eip-223.md) wrappers created by the Converter and assume that original [ERC-223](./eip-223.md) tokens may decalre themselves as [ERC-223](./eip-223.md) by implementing the same function as well. It is assumed that if a token does not implement this function then it is [ERC-20](./eip-20.md).

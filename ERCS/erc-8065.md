...
/// @param relayerData Generic data for relayer, can contain fee information or other metadata, if executed through a relayer
function remint(
    bytes calldata proof,
    bytes32 commitment,
    bytes32 nullifier,
...
contract ZWToken is ERC20 {
    ...
    function remint(
        ...
        if (relayerData.length >= 32) {
            // Extract first 32 bytes as uint256
            assembly {
                relayerFee := calldataload(add(relayerData.offset, 0))
            }

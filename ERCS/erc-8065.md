...
- Permissionless Wrapping: Launching a new zkERC20 is unlikely to achieve sufficient liquidity and adoption, and major token issuers are unlikely to issue zk variants due to regulatory and operational constraints. A permissionless wrapper therefore provides a practical path to make privacy a native feature of existing tokens. ZWToken does not require issuer consent — any existing token can be wrapped. This ensures openness and equal accessibility for all tokens regardless of their issuer’s policies.

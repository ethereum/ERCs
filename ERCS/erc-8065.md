...
- Upon successful verification:
  ...
  - If `redeem` is false, the function MUST mint ZWToken to the recipient `to`. The amount minted MAY be reduced by fees as defined by the implementation.
    - In this case, the function MUST emit the `Reminted` event after a successful remint of ZWToken.
  - If `redeem` is true, the function MUST transfer underlying token to the recipient `to`. The amount transferred MAY be reduced by fees as defined by the implementation.
...
- The function MUST emit a `Reminted(reminter, to, id, amount, redeem)` event upon successful remint。

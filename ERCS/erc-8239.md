...
`SkillRegistry` mints a unique [ERC-721](./erc-721.md) token per skill, owned by the skill's
author and pointing at an offchain manifest. A `SkillAttestation` contract
emits attestations that an [ERC-8004](./erc-8004.md) agent has used a skill, with three
...
`SkillAttestationHook` implements the [ERC-8183](./erc-8183.md) hook interface so that skill
...
`IACPHook` (as defined by [ERC-8183](./erc-8183.md)) for automatic
...
- `taxonomy`: an object of taxonomy classifications, each keyed by
  taxonomy name. Example keys include `oasf` (for OASF) or author-custom
  taxonomies.
...
from [ERC-8183](./erc-8183.md): a `getJob(uint256) returns (Job)` view

...
This proposal introduces an extension for [ERC-721](./eip-721.md) Non-Fungible Tokens (NFTs), specifying users for various contexts and differentiating between owner and ownership rights for a token. This allows for use in several financial applications for NFTs without changing the owner.
...
interface IERC7695 /* is IERC721, IERC165 */ {
  ...
  function getContext(bytes32 ctxHash) external view returns (address controller, uint64 detachingDuration, bool deprecated);
...
- MUST detach every attached context:
  ...
  - MUST emit the `ContextDetached` event.
  - MUST check if the controller address is a smart contract (e.g. code size > 0). If so, it MUST call `onExecDetachContext` and the call result MUST be skipped.
    - The `data` argument provided by the caller MUST be passed with its contents unaltered to the `onExecDetachContext` hook function via its `data` argument (leave blank `""` if there is no `data` argument).

...
**Smart contracts implementing the ERC7695 standard MUST implement all of the functions in the `IERC7695` interface.**
...
The enumeration extension is OPTIONAL for ERC7695 smart contracts. This allows your contract to publish its full list of context and make it discoverable. When calling the `supportsInterface` function MUST return the constant value `true` if `0xcebf44b7` is passed through the `interfaceID` argument.
...
The controller is RECOMMENDED to be a contract and including callback methods to allow ERC7695 contracts invokes in cases where there are any detach requests. When calling the `supportsInterface` function MUST return the constant value `true` if `0x92625f0b` is passed through the `interfaceID` argument.

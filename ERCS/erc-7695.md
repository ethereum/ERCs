...
This standard is an extension for [ERC-721](./erc-721.md), designed to specify users for various contexts with a locking feature and allow temporary ownership delegation without changing the original owner.
...
For standard [ERC-721](./erc-721.md) NFTs, there are several use cases in financial applications, including:
...
Smart contracts implementing this standard MUST implement the [ERC-165](./erc-165.md) `supportsInterface` function and MUST return the constant value `true` if `0x486b6fba` is passed through the `interfaceID` argument.
...
![Rental Flow](../assets/erc-7695/rental.svg)
...
![Mortgage Flow](../assets/erc-7695/mortgage.svg)

...
This standard defines Content-Addressable Logic Modules (CALM) - minimal, deterministic blocks of code designed for execution via delegatecall within Diamond ([ERC-2535](./eip-2535.md), [ERC-8109]((./eip-8109.md))), UUPS ([ERC-8109](./eip-8109.md)) and other proxy architectures.
...
|--|--|--|
| **Multi-Method (Standard)** | Contains an internal dispatcher (e.g. Solidity) that routes calls based on the first 4 bytes in calldata ( standard `msg.sig` in Solidity ). | Complex diamond facets containing multiple related functions. Example: Transaction, approval and metadata logic of [ERC-20](./eip-20.md) contract  |
...
Proxies such as Diamonds using CALMs SHOULD audit and test their overall configuration in order to ensure CALMs are not misaligned in the storage handling. For instance, security vulnerabilities can arise if one CALM utilizes the [ERC-7201](./eip-7201.md) storage pattern while another CALM interacting with the same logical state utilizes the [ERC-8042](./eip-8042.md) pattern, potentially leading to storage collisions or desynchronized data states.

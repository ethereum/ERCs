...
Implementations MUST use the following schema identifier, replacing `XXXX` with
this ERC's assigned number:

```solidity
bytes32 constant SCHEMA_V1 = keccak256("ERC-XXXX:BUNDLE:V1");
```
...
ERC-1643 defines document references for security tokens. It stores individual
named documents rather than a deterministic, schema-versioned bundle manifest.

[ERC-5289](./erc-5289.md) defines document signing and verification. This ERC
defines deterministic bundle commitments and supersession rather than a signing
workflow.

[ERC-5732](./erc-5732.md) defines a generic commit-reveal mechanism. It can be
used as a privacy layer around publication but does not define document entries
or bundle hashing.

[ERC-7208](./erc-7208.md) defines general on-chain data containers. This ERC
defines a document-specific commitment and lifecycle interface.

[ERC-7578](./erc-7578.md) includes document URI storage in a physical-asset
redemption flow but does not define deterministic bundle manifesting.

[ERC-3668](./erc-3668.md) defines an off-chain data retrieval and verification

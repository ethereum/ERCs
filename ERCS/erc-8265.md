---
eip: 8265
title: Body Lease and Credential Broker for ERC-8264 Agents
description: A body-lease primitive and credential-broker convention enabling ERC-8264 agent identity to persist across hardware while remaining revocable.
...
The two layers are independently usable. Portable memory *payload format* — manifest, Merkle commitment, canonicalization, and on-chain anchoring — is specified separately by the chain-agnostic CAAP-Capsule v0.1 specification (see References), which this ERC composes with but does not redefine.
...
Memory exports — whether bundled as a CAAP-Capsule (see References) or in any other portable form — MUST NOT contain raw credentials (API keys, private keys, OAuth tokens, macaroons, NWC connection strings, or any analogous bearer material).
...
**Why payload format is out of scope.** The portability format used to *carry* memory across bodies (manifest, Merkle commitment, anchoring) is fundamentally chain-agnostic — a Capsule may be content-addressed without any chain, anchored to Bitcoin, Ethereum, IPFS, or any other commitment substrate. Embedding that format in an ERC would couple a chain-neutral concern to the Ethereum standards track. The chain-agnostic CAAP-Capsule v0.1 specification (see References) is the canonical format; this ERC references it as a transport but defines neither its bytes nor its anchoring.

**Relationship to ERC-7857.** ERC-7857 (AI Agent NFT with Private Metadata) defines NFT-transfer-bound re-encryption. Body Lease is subject-initiated and not NFT-coupled. An implementor MAY operate both surfaces: an ERC-7857 NFT and a Body Lease, with NFT transfer triggering a lease rotation.

**Relationship to ERC-8181 (Self-Sovereign Agent NFTs).** That proposal frames an agent NFT as the canonical owner of its memory state. An ERC-8181 NFT MAY be the controller of an ERC-8264 subject and the issuer of its Body Leases.
...
**CAAP-Capsule v0.1** — chain-agnostic portable agent memory capsule specification (manifest format, canonical serialization, Merkle commitment, signature-suite registry, optional anchoring registry). Published at <https://github.com/clavote-boop/rmem-gateway/blob/main/standards/capsule-spec-v0.1.md>. CC0.

## Reference Implementation

A CC0 reference implementation is published at <https://github.com/clavote-boop/rmem-gateway>. The Body Lease primitive is implemented in `rmem-lease.py`; ERC-8264 gateway operations honoring leases are implemented in `rmem-gateway.py`. The Credential Broker convention is enforced at the gateway's export path (`rmem-migrate.py`'s freeze command rejects raw credential fields).

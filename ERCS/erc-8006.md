...
Modern smart contract applications often need configurable and reusable compliance logic. This standard defines an on-chain policy engine that composes rules into policy and evaluates *policy decisions* using a specialised processor, referred to in this standard as the **Policy Handler**. The outcome of a *policy decision* is typicaly a Boolean value indicating whether the policy evaluation succeeded (**true**) or failed (**false**).
...
**Policy** – A rule or a set of rules governing a specific action. A policy is typically defined in the context of the action to which it applies. For example, an ERC-20 token transfer initiated by a smart account must comply with the policy applicable to that specific type of transfer. A policy may consist of a single rule or a combination of multiple rules, and those rules may change over time. In summary, a policy is a coherent set of rules that governs a specific on-chain action, regardless of its complexity.
...
![Policy granulatiry scale](../assets/eip-8006/Policy_Decomposition_Illustration.svg)
...
The Policy Handler orchestrates artifact execution. While implementations may vary, this standard RECOMMENDS a **Directed Acyclic Graph (DAG)** as the reference architecture (see [reference implementation](../assets/eip-8006/contracts/inheritance/DAGWithPolicyMetadata.sol) and [Policy Handler](../assets/eip-8006/contracts/PolicyHandler.sol)).
...
3. **Connectivity requirement**: Only nodes connected (directly as children nodes or indirectly as decendant nodes) to the root node will be traversed during evaluation. Any graph containing disconnected nodes or disconnected subgraphs must be rejected during policy creation.

...
description: Composable policy engine designed primarily for programmable compliance, while also being suitable for other policy-driven workflows
author: Vlad Pryadko (@vpriadko) <vladyslav.priad@gmail.com>, Vitali Grabovski (@vgrabovski) <v.grabovski@guardianlabs.org>
...
Modern smart contract applications often need configurable and reusable compliance logic. This standard defines an on-chain policy engine that composes rules into policy and evaluates *policy decisions* using a specialised processor, referred to in this standard as the **Policy Handler**. The outcome of a *policy decision* is typically a Boolean value indicating whether the policy evaluation succeeded (**true**) or failed (**false**).

A rule or set of rules that composites a policy is referred to in this standard as an **Artifact(s)**. Artifacts implement a minimal interface and exchange input params / output params of the main interface methods as `bytes` or `bytes[]` (see [Interfaces section](#interfaces)), allowing heterogeneous data and on/off-chain variable supply.

A **Directed Acyclic Graph (DAG)** is the recommended approach at the core of how rule composition and policy evaluation are achieved in **Policy Handler**. 

When a **DAG** is used, each node represents a single artifact. The **Policy Handler** traverses the graph starting from a designated **root node** and ultimately returns the output of the root node (the root artifact). This output is the final *policy decision*.

The policy engine capabilities provided by the Policy Handler can be integrated, through composition or inheritance, into any smart contract requiring a policy-driven workflow.

The standard also defines self-descriptive methods for artifacts to support programmatic composition and integration with external tooling.
...
3. **Integration Difficulties**: Adding new compliance logic to existing codebases often requires:
   ...
   - The need for a carefully conducted contract upgrade process (which is error-prone)
...
**Policy** – A rule or a set of rules governing a specific action. A policy is typically defined in the context of the action to which it applies. For example, an [ERC-20](./eip-20.md) token transfer initiated by a smart account must comply with the policy applicable to that specific type of transfer. A policy may consist of a single rule or a combination of multiple rules, and those rules may change over time. In summary, a policy is a coherent set of rules that governs a specific on-chain action, regardless of its complexity.

**Artifact** – A single policy rule that serves as a building block of a policy. Within the context of a policy, an artifact is treated as an atomic entity. Internally, an artifact can be as simple or as complex as required by the policy builder.\
For example, a policy stating "be over 21 years old and a citizen" comprises 3 artifacts: "is over 21 years old" artifact, "and" artifact, and "is a citizen" artifact. While one could define a single artifact encompassing both requirements, it is generally RECOMMENDED to maintain granularity that reflects natural divisions of logic. This granular approach enables policies to be constructed from simple, reusable building blocks, tackling the challenge of complex rule composition.

Artifacts in this proposal are implemented as smart contracts, enabling them to represent not only any business requirement as rule, but also arbitrary operations across any context - on-chain transactions, off-chain actions, authorization workflows, governance workflows, or any other decision point requiring policy enforcement. Consequently, logical operators like "and" are also implemented as artifacts.

There are two types of artifact designs: **stateless** and **stateful**.\
**Stateless** artifacts can be reused across multiple policies in a stateless manner.\
**Stateful** artifacts are bound to a single policy in a stateful manner. A stateful design is helpful when a particular artifact is sensitive to the policy it is attached and must maintain its own state variables.
...
![Policy granularity scale](../assets/eip-8006/Policy_Decomposition_Illustration.svg)
<p align="center"><em>Figure 1. Examples of Policy Logic Breakdown</em></p>

### Interfaces

#### Artifact Interfaces

Artifacts MAY implement any logic but MUST adhere to the following `IArbitracyDataArtifact` interface that allows the **Policy Handler** to integrate them correctly and ensure consistent data flow:
...
The Policy Handler orchestrates artifact execution. While implementations may vary, this standard RECOMMENDS a **Directed Acyclic Graph (DAG)** as the reference architecture (see [reference implementation](../assets/eip-8006/contracts/PolicyMetadata.sol) and [Policy Handler](../assets/eip-8006/contracts/PolicyHandler.sol)).
...
![Policy bird's eye view](../assets/eip-8006/simple_policy_bird_eye_view.svg)

<p align="center"><em>Figure 2. Policy Engine Architecture</em></p>

The Policy Handler may provide an OPTIONAL method to retrieve information about all variables used by all artifacts in the policy:
...
Another notable architecture decision is a public **variables-list** getter method exposed on the handler. It improves off-chain orchestration and automation – for example, by enabling the precomputation of inputs and the wiring User Interface (UI) forms or client pipelines.
...
3. **Connectivity requirement**: Only nodes connected (directly as children nodes or indirectly as descendant nodes) to the root node will be traversed during evaluation. Any graph containing disconnected nodes or disconnected subgraphs must be rejected during policy creation.
...
A reference implementation of Policy Handler is available [here](../assets/eip-8006/contracts/PolicyHandler.sol).
The reference implementation includes the following features:
- Contracts implementing ``IArbitraryDataArtifact`` in accordance with this standard
- A **Policy Handler** contract with a rules processor (init, evaluate, upgrade) based on a DAG data structure
- An example client contract demonstrating pure on-chain policy initialization and enforcement
...
- **Artifact Developer**: Responsible for developing, conducting security audits, and maintaining their artifacts before making them publicly available. Unvetted or upgradable artifacts can introduce risks; see notes on *untrusted artifacts* and *stateful initialization problem* below.

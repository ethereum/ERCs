...
description: Optional ERC-20 extension to pre-allocate a balance slot via a sentinel bytes32 value, shifting the first 20000-gas SSTORE earlier to reduce later transfer costs.
...
This extension shares a core design philosophy with ERC-721A, a widely adopted, optimized NFT standard that allows for cheap batch minting by deferring part of the gas cost to the first transfer of each token. 
...
It's "high gas now, low gas later", the opposite time/gas bet than ERC-721A.

Just as with ERC-721A, neither approach reduces total gas in a flat-fee world.
...
A minimal example contract is provided in the repository (file `contracts/ERC20PreinitExample.sol` under `ariutokintumi/ERC-20-Pre-initialization`). Don't use for production until a security audit is performed.

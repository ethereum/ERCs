...
### Text representation in user-facing contexts
When displaying _Interoperable Addresses_ to users, implementers SHOULD NOT represent the binary _Interoperable Address_ directly as `base16`, `base58`, or `base64` encoded text. Instead, implementers MAY display the `ChainReference` and `Address`components of an _Interoperable Address_ using the text representations defined in the corresponding [CAIP-350] profile for the namespace.

The text representation of the _ChainReference_ and _Address_ components should be obtained by:

1. Extracting the binary _ChainReference_ and _Address_ fields from the _Interoperable Address_
2. Converting each component to its text representation according to the rules specified in the namespace's [CAIP-350] profile

This approach ensures that users see familiar, chain-native address formats while maintaining the canonical binary representation for on-chain and protocol-level usage.

**Example: text representation**

| Key | Value |
| :--- | :--- |
| **Interoperable Address: Ethereum mainnet** | `0x0000010114d8da6bf26964af9d7eed9e03e53415d37aa96045` |
| **Text representation** | `0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045@eip155:1` |

### Extensions
This standard defines the core binary format for _Interoperable Addresses_. Extensions to this standard MAY provide additional functionality, such as more human-readable text representations or enhanced metadata.

Extensions SHOULD:

- Maintain compatibility with the binary format defined in this specification
- Clearly document any additional encoding or representation schemes
- Specify how to convert between the extension format and the canonical binary format

For an example of an extension that provides human-readable text representations of _Interoperable Addresses_, see [ERC-7828].

...
([ERC-55]) outlines an optional checksum standard. The optional nature of this standard does not suffice to protect users.

This proposal builds on insights from [CAIP-10] and [CAIP-50]. It offers a format definition which:
...
: A 1-byte integer encoding the length of `ChainReference` in bytes. Note that it MAY be zero, in which case the _Interoperable Address_ MUST NOT include a chain reference.

**ChainReference**
: Variable length, binary representation of the [CAIP-2] chain ID. To discern how to serialize the `ChainReference` for a specific chain type, implementors MUST reference the respective [CAIP-350] profile. Chain profiles are maintained by the Chain-Agnostic Standards Alliance (CASA).

**AddressLength**
: 1-byte integer encoding the length of Address in bytes. Note that it MAY be zero, in which case the _Interoperable Address_ MUST NOT include an address. It MUST NOT be zero if the `ChainReferenceLength` is also zero.
...
`<checksum>` is a 4-byte checksum calculated by taking the first 4-bytes of the hexadecimal (as defined in [RFC 4648]) representation of the keccak256 hash of the concatenated `ChainType`, `ChainReferenceLength`, `ChainReference`, `AddressLength` and `Address` fields of the binary _Interoperable Address_. The `Version` field is **not** included.
...
- MUST set the most significant bit of the version field to 1 if they are unable to parse the _Interoperable Address_ format outlined herein 

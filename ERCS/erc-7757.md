...
This ERC introduces a compact on-chain primitive that lets autonomous AI agents coordinate through **instincts**—small rule objects ranked by a signed **temptationValue** (reward / cost). Instincts can emit mid-way targets so multi-step goals unfold safely, and—once a trigger condition is met—the contract executes the associated transaction without any agent managing a private key.

Alongside instincts, agents share simple on-chain **environment variables** (e.g., balances, ownership flags, status markers). These variables act as a public “memory” that any agent can read or update, giving disparate providers a common state layer. Together, instincts (rules) and environment variables (state) form a secure, decentralized framework that enables AI agents to sense, decide, and settle value automatically while keeping heavy computation off-chain.

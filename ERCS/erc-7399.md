...
Under the [ERC-7399](./erc-7399.md) standard a flash loan is a loan of an `amount` of an [ERC-20](./erc-20.md) `asset` from a `lender`. This loan can remain open for the span of a single `flash` call in the `lender`.
...
A `lender` MUST implement the [ERC-7399](./erc-7399.md) interface.
...
This ERC is a successor of [ERC-3156](./erc-3156.md). While not directly backwards compatible, a wrapper contract offering a ERC-7399 external interface with liquidity obtained from an ERC-3156 flash `lender` is trivial to implement.

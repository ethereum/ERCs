...
The proposal standardizes Ethereum-based authentication for HTTP requests, aligned with the [HTTP Message Signatures](https://www.rfc-editor.org/rfc/rfc9421) model where practical. Delegated authentication is specified by [ERC-XXXX](./erc-xxxx.md).
...
Other tags are not candidates of this profile. [ERC-XXXX](./erc-xxxx.md) defines `erc8128-delegated`.
...
A missing, non-String, or syntactically invalid `keyid` on a candidate yields `invalid_keyid`. The chain ID selects the state used for ERC-6492 and ERC-1271 verification; a verifier MUST NOT query another chain.
...
<!-- Signers SHOULD use `expires = created + 60`. Conforming verifiers MUST support a 60-second validity window and SHOULD allow 30 seconds of clock skew. A route MAY impose a different maximum but a baseline route MUST allow at least 60 seconds. A window exceeding the route maximum yields `request_validity_too_long`. -->
...
`byteLength(M)` is the length of the serialized signature base in bytes. The decimal length has no leading zeroes. ERC-1271 and ERC-6492 verification receive `H`, not a second hash and not the unhashed `M`.
...
1. If the signature is an ERC-6492 wrapper, verify the wrapper before inspecting account code. An invalid wrapper MUST NOT fall back.
2. Otherwise, obtain account code on the `keyid` chain. If code is present, including an [EIP-7702](./eip-7702.md) delegation indicator, call ERC-1271 with `H` and the opaque signature. An invalid call MUST NOT fall back to ECDSA.
...
Implementations MUST test a replay race by submitting the positive request concurrently: exactly one request may authenticate. They MUST also test ERC-6492 before code lookup, ERC-1271 without ECDSA fallback for deployed and EIP-7702-delegated Accounts, strict EOA recovery for a code-free Account, and that received legal parameter order is preserved.

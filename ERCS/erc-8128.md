...
Signature: eth=:dmSonRcA8ydqKFPQ+/f/dN2I0XviLbXIO4VPl9/41l5byGyBYO1SXY9eWixdKvB6cOaw96kUKFg50LZ60255jhs=:"
```
...
Where:
- `<chain-id>` is a base-10 integer as defined in ([EIP-155](./eip-155)).
...
The `erc8128` namespace reuses the CAIP-10 account identifier syntax and semantics defined for `eip155:<chain-id>:<address>`, including chain scoping and replay-domain separation; it differs only in its namespace prefix to explicitly bind signatures to this specification.
...
"@signature-params": ("@method" "@authority" "@path" "@query" \
  ...
  nonce="b64url_r4Nd0mN0nCEK0YQY4d4r7A";keyid="erc8128:1:0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045"
```

The specification adopts the [ERC-191](./eip-191) "Ethereum Signed Message" (`personal_sign`) encoding for `M`.
...
For EOAs, verifiers MUST use the method specified in [ERC-191](./eip-191).

#### 4.2 SCA Verification

For SCAs, verifiers SHOULD use [ERC-1271](./eip-1271).
...
### Why [ERC-191](./eip-191) over [EIP-712](./eip-712)
...
The reference implementation signs the RFC 9421 signature base `M` for label `eth`, covering the required components: `@method`, `@authority`, `@path`, `@query`, and `content-digest`. The signature parameters `created`, `expires`, `nonce`, and `keyid` are carried in `Signature-Input`. The signer computes an ERC-191 signature over `M` (e.g., `personal_sign` or `signMessage`), and the verifier checks either EOA recovery or ERC-1271 for SCAs.
...
type SignParams = {
  keyid: `erc8128:${number}:0x${string}`;
...
export async function verifyRequest(req: Req, policy: VerifyPolicy): Promise<boolean> {
  ...
  // Find the ERC-8128 message signature in the headers, prioritizing a compliant signature with `eth` label if present
  const { sigInput, sig } = findERC8128MessageSignature(headers);

...
* **Support other use cases:**
  ...
  * Payment of transaction fees using [ERC-20](./erc-20.md) tokens.
...
| feeToken          | address | contract address of the [ERC-20](./erc-20.md) token used to repay the bundler. _(`address(0)` for the native token)_.                                       |
...
* The `feeToken` is `address(0)` or a known [ERC-20](./erc-20.md) token that the bundler is willing to accept.

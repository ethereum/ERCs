...
Endorsers should be registered in an `EndorserRegistry` with a staked amount of ETH.
The purpose of locking up ETH in the registry is to signal to mempool operators that an endorser is staking the time value of their locked up capital on the trustworthiness and reliability of its results.
Mempool operators themselves are free to independently evaluate the trustworthiness of an endorser but MAY consider the staked capital as part of its evaluation.
Mempool operators MAY accept operations from endorsers without any staked ETH, but they would increase their risk exposing themselves to denial of service attacks.
Mempool operators MAY also advertise their requirements for endorsers to be considered.
...
* The endorser (i) is registered and has sufficiently staked (>= `MIN_ENDORSER_STAKE`), and (ii) it has not been internally flagged as banned.
...
The endorser registry serves as a place to register the stake of each endorser.
Anyone can stake an endorser by calling the `stake()` function.
The amount staked and the time period it's locked for is at the discretion of the staker.
Staked amounts are recoverable after the lock up period.
Slashing cannot be reliably proved on-chain without protocol alterations, so it remains a virtual event on which mempool operators will maintain their own opinions on the reputations of endorsers.
The staking mechanism can be considered a barrier of entry for deploying too many untrustworthy endorsers.
...
contract EndorserRegistry {
  ...
  function stake(address _endorser, uint256 _stakeUntil) external payable returns (uint256) {
    // TBD
  }
}
```
...
In effect, we move the responsibility from the wallet to the wallet developer; the developer must code, deploy and stake ETH for the `endorser`; this is a nearly ideal scenario because developers know how their wallet operations work, and thus they can build tools to evaluate these operations efficiently.
...
Anyone can deploy a endorser contract and wallet clients are the one providing which endorser contract should be used for the given transaction. Instead of having each bundler rely on an off-chain registry that they need to maintain, the endorser registry can be called to see if the requested endorser contract is present and how much ETH was staked for it. Bundlers can then decide a minimum threshold for how much ETH is required to be staked for an endorser contract to be accepted. Bundlers are also free to support endorsers contract that are not part of the registry or are part of it but have no staked ETH associated with it.

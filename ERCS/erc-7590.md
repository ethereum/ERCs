...
This proposal suggests an extension to [ERC-721](./erc-721.md) to enable easy exchange of [ERC-20](./erc-20.md) tokens. By enhancing [ERC-721](./erc-721.md), it allows unique tokens to manage and trade [ERC-20](./erc-20.md) fungible tokens bundled in a single NFT. This is achieved by including methods to pull [ERC-20](./erc-20.md) tokens into the NFT contract to a specific NFT, and transferring them out by the owner of such NFT. A transfer out nonce is included to prevent front-running issues.

## Motivation

In the ever-evolving landscape of blockchain technology and decentralized ecosystems, interoperability between diverse token standards has become a paramount concern. By enhancing [ERC-721](./erc-721.md) functionality, this proposal empowers non-fungible tokens (NFTs) to engage in complex transactions, facilitating the exchange of fungible tokens, unique assets, and multi-class assets within a single protocol.
...
By allowing [ERC-721](./erc-721.md) tokens to hold and trade different types of tokens, it enhances liquidity for markets in all types of tokens.
...
2. Lack of transfer with callback: [ERC-20](./erc-20.md) tokens lack a standardized transfer with callback method, such as the "safeTransfer" on [ERC-721](./erc-721.md), which means there is no reliable way to notify the receiver of a successful transfer, nor to know which is the destination token is.
...
2. A universal token holder which could also hold and transfer [ERC-721](./erc-721.md) and [ERC-1155](./erc-1155.md).

An implementation of the granular version is slightly cheaper in gas, and if you're using just one or two types, it's smaller in contract size. The generic version is smaller and has single methods to send or receive, but it also adds some complexity by always requiring Id and amount on transfer methods. Id not being necessary for [ERC-20](./erc-20.md) and amount not being necessary for [ERC-721](./erc-721.md).

We also considered that due to the existence of safe transfer methods on both [ERC-721](./erc-721.md) and [ERC-1155](./erc-1155.md), and the commonly used interfaces of `IERC721Receiver` and `IERC1155Receiver`, there is not much need to declare an additional interface to manage such tokens. However, this is not the case for [ERC-20](./erc-20.md), which does not include a method with a callback to notify the receiver of the transfer.
...
Tests are included in [`erc7590.ts`](../assets/erc-7590/test/erc7590.ts).
...
See [`ERC7590Mock.sol`](../assets/erc-7590/contracts/ERC7590Mock.sol).

## Security Considerations

The same security considerations as with [ERC-721](./erc-721.md) apply: hidden logic may be present in any of the functions, including burn, add resource, accept resource, and more.
...
When transferring [ERC-20](./erc-20.md) tokens in or out of an NFT, it could be the case that the amount transferred is not the same as the amount requested. This could happen if the [ERC-20](./erc-20.md) contract has a fee on transfer. This could cause a bug on your Token Holder contract if you do not manage it properly. There are 2 ways to do it, both of which are valid:
...
To prevent a seller from front running the sale of an NFT holding [ERC-20](./erc-20.md) tokens to transfer out such tokens before a sale is executed, marketplaces MUST beware of the `erc20TransferOutNonce` and revert if it has changed since listed.

[ERC-20](./erc-20.md) tokens that are transferred directly to the NFT contract will be lost.

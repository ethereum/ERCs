...
3. The pre-outcome timestamp (`committed_at`, above) MUST be anchored at issue time to a system the validator does not control. For the verdict this is the ERC-8263 proofHash leg — NOT the OCP input anchor (which commits the input, not the verdict; see `committed_at` above). It is the same kind of primitive as OCP's observation commitment, applied one layer up to the verdict.

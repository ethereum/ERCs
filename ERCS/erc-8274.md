...
interface IAgentVerifier {
    ...
    ///                            taskId, agentId, inputHash, outputHash, valid, and agentProofProfile.
...
```
keccak256(abi.encode(taskId, agentId, inputHash, outputHash, valid, agentProofProfile))
```

Including `valid` and `agentProofProfile` ensures that the same inputs verified by different proof systems, or with different outcomes, produce different digests.
...
`verificationDigest` is computed from `(taskId, agentId, inputHash, outputHash, valid, agentProofProfile)`. The event carries those same fields alongside the digest, making every record self-describing and independently auditable:
...
contract AgentVerifier is IAgentVerifier {
    ...
    function verify(
        ...
        emit VerificationCompleted(
            taskId, agentId, verificationDigest, valid, inputHash, outputHash, agentProofProfile
        );
    }
}
```

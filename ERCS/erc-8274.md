...
- **`instanceId: bytes`** — identifies the verifier deployment. On Ethereum: `abi.encodePacked(address(this))`. Other verifier classes (TEE enclave measurement, threshold-signing committee aggregate pubkey) use their own canonical encoding; the encoding is determined by the verifier class, and a consumer identifies the class from `agentProofProfile`.

...
WYRIWE defines a triple-hash scheme for input provenance:
...
**Layer separation.** WYRIWE is L3 (input provenance); this ERC is L4 (verification). They compose vertically and are independent.

**`inputHash` alignment.** `IProofVerifier.verify()`'s `inputHash` maps directly to WYRIWE's `input_hash` — same field, same construction.

**Ready-made backend.** `WyriweProofVerifier` is already deployed on mainnet, implements `IProofVerifier`, and can be passed directly to any `IAgentVerifier`.

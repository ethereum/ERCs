...
1. Representation as [ERC-721]([/ERCS/erc-721.md) or[ERC-1155](/ERCS/erc-1155.md) tokens.
...
- **tags:** Arrays of arrays of strings that describe attachments, references, and other metadata. There are more example tags below.
  - `["e", "<id_of_referenced_post>"]` for example indicates a reply.
  - Attach media:
...
- **required** for PubEvents published using this method:
  `["blockchain", "name <chain_name>", "chainid <chain_id>", "format nft", "contract <contract>", "tokenid <token_id">]`,

**Where:**

- `<chain_name>` is the name of the chain. e.g. "Ethereum", "Polygon", "Base"
- `<chain_id>` is the chain id. e.g. "1", "137", "8453"
- `<contract>` is the contract address. e.g. "0x0000000000000000000000000000000000000000"
- `<token_id>` is the token id. e.g. "12345"

At least one of the `<chain_name>` and `<chain_id>` must be present.
...
**Public key:** Public keys are based on Taproot + Schnorr, bitcoin BIP-0341.
...
`<relay-url>` optionally is the URL of a recommended off-chain relayer. Use "" if none or blockchain only.
...
The repost event MUST include an e tag with the id of the post that is being reposted. That tag MUST include a blockchain tag or indexer relay where the post can be fetched.
...
`["q", <event-id>, <blockchain-tag>, <relay-url>, <pubkey>]`

Quote reposts MUST include the NIP-21 nevent, post, or naddr of the event in the content.

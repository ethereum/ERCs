...
- **Job escrow models** (e.g. [ERC-8183](./eip-8183.md)) require upfront job specification, evaluator attestation, and multiple state transitions. This is appropriate for discrete deliverables but introduces unnecessary overhead for immediate request-response services.
...
Payment SHALL use a single [ERC-20](./eip-20.md) token per contract. Implementations MAY support per-channel token selection; the specification only requires one token per contract.
...
The caller MUST have approved the contract to spend at least `amount` of the payment token before calling `open`. Implementations MAY support [EIP-2612](./eip-2612.md) `permit` to allow gasless approval via signature.
...
- SHALL recover the signer from the [EIP-712](./eip-712.md) typed data signature (see Voucher Schema below) and revert if the recovered address is not the channel's `consumer`.
...
- SHALL recover the signer from the [EIP-712](./eip-712.md) typed data signature (see CloseAuthorization Schema below) and revert if the recovered address is not the channel's `provider`.
...
All signatures in this protocol use [EIP-712](./eip-712.md) typed structured data signing.
...
- **Reputation**: Job outcome signals belong in complementary standards (e.g. [ERC-8004](./eip-8004.md), [ERC-8183](./eip-8183.md)).
...
A production implementation of this specification is deployed on Polygon Mainnet (chain ID 137) as DrainChannelV2 at address `0x0C2B3aA1e80629D572b1f200e6DF3586B3946A8A`, using USDC (`0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359`) as the payment token.

EIP-712 domain name: `DrainChannel`, version: `1`.

...
Compliant implementations MUST implement the [ERC-165](./erc-165.md) `supportsInterface` function and MUST return `true` for the `ISolvencyProof` interface ID.
...
**Test Coverage:** Our test suite includes 23 tests total—13 for core ERC functionality ([SolvencyProof.test.ts](../assets/eip-7893/tests/SolvencyProof.test.ts)) and 10 focused on security features ([SecurityFeatures.test.ts](../assets/eip-7893/tests/SecurityFeatures.test.ts)). Every security-critical code path is tested.

**Testing Approach:** The [MaliciousOracle.sol](../assets/eip-7893/contracts/test/MaliciousOracle.sol) and [MockMultiOracle.sol](../assets/eip-7893/contracts/test/MockMultiOracle.sol) contracts simulate attack scenarios and consensus mechanisms for testing purposes. They do not connect to real oracle networks but provide comprehensive coverage of potential attack vectors and edge cases that protocols implementing this ERC should be prepared to handle.

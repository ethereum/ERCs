...
## Abstract
This standard outlines a smart contract interface eligible to represent any number of fractionalized non-fungible tokens. Existing projects utilizing standards like [EIP-1633](./erc-1633.md) conventionally deploy separate [EIP-20](./erc-20.md) compatible token contracts to fractionalize the non-fungible token into EIP-20 tokens. In contrast, this ERC allows each token ID to represent a token type representing(fractionalizing) the non-fungible token.

This standard is approximate in terms of using `_id` for distinguishing token types. However, this ERC has a clear difference with [EIP-1155](./erc-1155.md) as each `_id` represents a distinct NFT.
...
[MFNFT - Implementation](../assets/erc-4675/README.md)

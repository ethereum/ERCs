...
discussions-to: https://ethereum-magicians.org/t/erc-8184-draft-payment-channels-with-signed-vouchers-streaming-micropayments-for-ai-agents/28012
status: Draft
...
Payment SHALL use a single [ERC-20](./eip-20) token per contract. Implementations MAY support per-channel token selection; the specification only requires one token per contract.
...
The caller MUST have approved the contract to spend at least `amount` of the payment token before calling `open`. Implementations MAY support [EIP-2612](./eip-2612) `permit` to allow gasless approval via signature.
...
- SHALL recover the signer from the [EIP-712](./eip-712) typed data signature (see Voucher Schema below) and revert if the recovered address is not the channel's `consumer`.
...
- SHALL recover the signer from the [EIP-712](./eip-712) typed data signature (see CloseAuthorization Schema below) and revert if the recovered address is not the channel's `provider`.
...
All signatures in this protocol use [EIP-712](./eip-712) typed structured data signing.
...
### Service Interaction (RECOMMENDED)

This section defines the RECOMMENDED interaction pattern between consumers and HTTP-based service providers. While the on-chain protocol is sufficient for settlement, standardizing the off-chain request-voucher interaction reduces fragmentation across implementations and ensures interoperability.

#### Discovery

When an unauthenticated or unpaid request arrives, the provider SHOULD respond with HTTP 402 (Payment Required) and the following headers:

| Header | Description | Example |
| --- | --- | --- |
| `X-Payment-Protocol` | Protocol identifier | `payment-channel-v1` |
| `X-Payment-Provider` | Provider's Ethereum address | `0xABC...` |
| `X-Payment-Contract` | Payment channel contract address | `0xDEF...` |
| `X-Payment-Chain` | Chain ID | `137` |
| `X-Payment-Signing` | URL to fetch EIP-712 signing parameters | `https://provider.example/signing` |

The signing endpoint SHOULD return a JSON object containing the [EIP-712](./eip-712) domain and voucher type definitions, enabling the consumer to construct valid voucher signatures without prior knowledge of the contract's domain parameters.

#### Protocol Versioning

Providers SHOULD include an `X-Payment-Version` header in the HTTP 402 discovery response to signal which version of this specification's HTTP conventions they support:

| Header | Description | Example |
| --- | --- | --- |
| `X-Payment-Version` | Semantic version of this specification's HTTP conventions | `1.0` |

The version `1.0` refers to the HTTP header conventions defined in this section (Discovery, Authenticated Requests, Cost Reporting, Error Responses, and Cooperative Close Endpoint).

Optionally, providers MAY include an `X-Payment-Features` header to advertise supported feature flags:

```
X-Payment-Features: ["cost-reporting", "cooperative-close"]
```

Standardized feature flags:
- `cost-reporting`: Provider includes `X-Payment-Cost`, `X-Payment-Total`, and `X-Payment-Remaining` response headers
- `cooperative-close`: Provider exposes the cooperative close endpoint

Consumers SHOULD check the version and features to ensure compatibility. If a consumer does not support the advertised version, it MAY choose to fall back to on-chain settlement or reject the service. This versioning mechanism prevents fragmentation as the ecosystem evolves.
...
The voucher `amount` SHOULD be the consumer's previous cumulative total plus the estimated cost of the current request. Providers SHOULD validate that `amount >= previousTotal + requestCost` and reject vouchers that underpay.

#### Cost Reporting

After processing a request, the provider SHOULD include the following response headers to enable the consumer to construct accurate voucher amounts for subsequent requests:

| Header | Description | Example |
| --- | --- | --- |
| `X-Payment-Cost` | Cost of this request in token units | `50000` |
| `X-Payment-Total` | Cumulative total charged to this channel | `150000` |
| `X-Payment-Remaining` | Remaining channel balance (`deposit - total`) | `850000` |
| `X-Payment-Channel` | Channel ID | `0xABC...` |

These headers allow consumers to track spend without querying on-chain state and to construct the next voucher's cumulative `amount` as the value of `X-Payment-Total` plus the estimated cost of the next request.

#### Error Responses

When a voucher is rejected, the provider SHOULD respond with HTTP 402 and an error indicator:

| Header | Description |
| --- | --- |
| `X-Payment-Error` | Error code (see below) |

Standardized error codes:

| Code | Meaning |
| --- | --- |
| `voucher_required` | No voucher was provided |
| `insufficient_funds` | Voucher amount is less than required |
| `invalid_signature` | EIP-712 signature verification failed |
| `invalid_nonce` | Nonce is not greater than the last seen nonce |
| `channel_expired` | The channel has expired |
| `channel_not_found` | The channel does not exist or has been closed |

For `insufficient_funds`, the provider SHOULD additionally include:

| Header | Description |
| --- | --- |
| `X-Payment-Required` | Minimum voucher amount required |
| `X-Payment-Provided` | Voucher amount that was provided |
...
### Service interaction as a RECOMMENDED layer

This standard defines both the on-chain settlement primitive and a RECOMMENDED off-chain service interaction pattern. The settlement layer (channel lifecycle, voucher schema) is normative and sufficient on its own. The service interaction layer (HTTP 402 discovery, cost reporting headers, error codes) is RECOMMENDED to reduce fragmentation across implementations — production experience with 10+ providers showed that without standardized cost reporting and error semantics, each provider develops incompatible conventions, undermining interoperability.

### Complementarity with service discovery protocols

This standard defines the settlement primitive. Service discovery — how consumers find providers and negotiate terms — is left to application-layer protocols such as the Machine Payments Protocol (MPP) or the Model Context Protocol (MCP). A payment channel can serve as a settlement backend for any service discovery protocol, enabling agents to use channel-based micropayments regardless of how they discover and connect to services.

### Minimal surface

This standard deliberately excludes:
- **Discovery and negotiation**: How consumers find providers is out of scope. Marketplaces, registries, and service discovery protocols handle this.
- **Pricing models**: Per-token, per-request, tiered — left to the application layer.
- **Reputation**: Job outcome signals belong in complementary standards (e.g. [ERC-8004](./eip-8004), [ERC-8183](./eip-8183)).
...
The reference implementation consists of the `IPaymentChannel` interface and a minimal `PaymentChannel` contract.
...
contract PaymentChannel is IPaymentChannel, EIP712, ReentrancyGuard {
    ...
    constructor(address token_) EIP712("PaymentChannel", "1") {
        token = IERC20(token_);
    }
    ...
    function open(
        ...
        channelId = keccak256(
            abi.encodePacked(msg.sender, provider, ++_channelNonce)
        );
    ...
    function claim(
        ...
        if (ch.consumer == address(0)) revert ChannelNotFound();
        if (msg.sender != ch.provider) revert NotProvider();
        if (amount <= ch.claimed) revert InvalidAmount();
        if (amount > ch.deposit) revert InvalidAmount();

        bytes32 structHash = keccak256(
            abi.encode(VOUCHER_TYPEHASH, channelId, amount, nonce)
        );
        address signer = ECDSA.recover(
            _hashTypedDataV4(structHash),
            signature
        );
        if (signer != ch.consumer) revert InvalidSignature();

        uint256 payment = amount - ch.claimed;
        ch.claimed = amount;

        token.safeTransfer(ch.provider, payment);
        emit ChannelClaimed(channelId, ch.provider, amount);
    }

    function close(bytes32 channelId) external nonReentrant {
        Channel storage ch = _channels[channelId];
        if (ch.consumer == address(0)) revert ChannelNotFound();
        if (msg.sender != ch.consumer) revert NotConsumer();
        if (block.timestamp < ch.expiry) revert NotExpired();
    ...
    function cooperativeClose(
        ...
        if (ch.consumer == address(0)) revert ChannelNotFound();
        if (msg.sender != ch.consumer) revert NotConsumer();
        if (finalAmount > ch.deposit) revert InvalidAmount();
        ...
        if (signer != ch.provider) revert InvalidSignature();

        address provider = ch.provider;
        uint256 providerPayment = finalAmount > ch.claimed
            ? finalAmount - ch.claimed
            : 0;
        uint256 consumerRefund = ch.deposit - finalAmount;

        delete _channels[channelId];

        if (providerPayment > 0) {
            token.safeTransfer(provider, providerPayment);
            emit ChannelClaimed(channelId, provider, finalAmount);
        }

        if (consumerRefund > 0) {
            token.safeTransfer(msg.sender, consumerRefund);
        }
        emit ChannelClosed(channelId, msg.sender, consumerRefund);
    }
    ...
    {
        Channel storage ch = _channels[channelId];
        return (ch.consumer, ch.provider, ch.deposit, ch.claimed, ch.expiry);
    }
...
A production implementation of this standard is deployed on Polygon Mainnet:
...
The production implementation includes a 2% platform fee (200 basis points) deducted on claim and cooperative close, paid to a configurable fee wallet.
...
Implementations must use `SafeERC20` (or equivalent) for all token transfers to handle non-standard [ERC-20](./eip-20) implementations that do not return a boolean.

### Reentrancy

Functions that transfer tokens should be protected against reentrancy (e.g. via a reentrancy guard or checks-effects-interactions pattern).

...
description: Define a registry for generic services linked to a specific NFT or contract
...
This proposal defines a registry for generic services linked to specific non-fungible tokens (NFTs) or contracts, i.e., contracts extending an NFT or other contract, owned by a single NFT or contract and thus by the owner of the NFT or contract. It achieves this goal using generic language for functions, errors, and events, and avoids conflicting with strict restrictions imposed by other proposals. The standard supports both NFT-linked services (with a `tokenId`) and generalized contract-linked services (without a `tokenId`).
...
With this new ERC, we propose a generic registry that uses generic function/event names to allow the deployment of any kind of contract that makes sense when associated with an NFT or other contract, so that the contract is under the full control of the NFT's or contract's owner. Since one of this proposal's goals is flexibility, there is no expectation for an `ERC7656Registry` contract to be deployed as a singleton, allowing any project to adjust it to their needs; consequently, we require that any registry explicitly supports the `IERC7656Registry` interface.
...
- The `id` field (e.g., `tokenId`) is optional. If omitted, the last 32 bytes SHOULD be filled with zeros or a default value (e.g., `0x0`).

...
Another difference compared to `ERC6551Registry` is that this implementation is `IERC165` compatible and should confirm that it supports the `IERC7656Registry` interface. The reason is that, while the `ERC6551Registry` is a singleton and is supposed to be deployed a single time on every chain, an ERC7656Registry can be deployed multiple times on the same chain, and the support of the interface is essential to ensure that the contract is indeed an `ERC7656Registry`.


### A token-linked contract

...
These cases are executable as [`SlippageBoundedSwap.t.sol`](../assets/erc-8377/test/SlippageBoundedSwap.t.sol). [`ForkSlippageBounded.t.sol`](../assets/erc-8377/test/ForkSlippageBounded.t.sol) additionally derives the floor from a live Chainlink ETH/USD reference through an ERC-7726 adapter and settles a real USDC balance delta.

## Reference Implementation

- [`ISlippageBoundedSwap.sol`](../assets/erc-8377/src/ISlippageBoundedSwap.sol) - the interface, `SlippagePolicy`, and the errors.
- [`SlippageBoundedSwap.sol`](../assets/erc-8377/src/SlippageBoundedSwap.sol) - an abstract base implementing the floor logic, with route execution left as the internal `_route` hook so any router can inherit the guard.
- [`ChainlinkQuoteOracle.sol`](../assets/erc-8377/src/adapters/ChainlinkQuoteOracle.sol) - an ERC-7726 adapter over a Chainlink feed that enforces the freshness bound.
- [`MockQuoteOracle.sol`](../assets/erc-8377/test/mocks/MockQuoteOracle.sol) - the oracle used by the unit tests.
...
- Output is measured, not reported. The realized amount is the executor's `tokenOut` balance delta across the route, so a malicious or buggy `routeData` cannot pass the floor with an output it did not deliver. The Specification forbids re-introducing a route-reported amount into the floor check.

...
The dual-layer token combines the functionalities of [ERC-20](./erc-20.md), [ERC-721](./erc-721.md), and [ERC-1155](./erc-1155.md) while adding a classification layer that uses `mainId` as the main asset type identifier and `subId` as the unique attributes or variations of the main asset.
![Dual Layer Token](../assets/erc-6960/eip-6960-dual-layer-token-dlt.png)
...
The [ERC-1155](./erc-1155.md) standard has experienced considerable adoption within the Ethereum ecosystem; however, its design exhibits constraints when handling tokens with multiple classifications, particularly in relation to Real World Assets (RWAs) and fractionalization of assets.
...
A reference implementation is provided in [DLT.sol](../assets/erc-6960/DLT.sol).
...
The dual-layer allowance system inherits the same approve/transferFrom race condition present in [ERC-20](./erc-20.md). If an owner changes an allowance from N to M, the spender may be able to spend both N and M. Implementers SHOULD provide `increaseAllowance` and `decreaseAllowance` helper functions or use the check-set pattern where the owner first sets allowance to zero before setting the new value.

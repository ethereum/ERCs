...
A reference implementation is provided in [`../assets/erc-6960/DLT.sol`](../assets/erc-6960/DLT.sol).
...
Test cases are available in the reference implementation repository at [ERC-6960](https://github.com/Aboudjem/ERC-6960/tree/main/test).
...
The dual-layer allowance system inherits the same approve/transferFrom race condition present in [ERC-20](./eip-20.md). If an owner changes an allowance from N to M, the spender may be able to spend both N and M. Implementers SHOULD provide `increaseAllowance` and `decreaseAllowance` helper functions or use the check-set pattern where the owner first sets allowance to zero before setting the new value.

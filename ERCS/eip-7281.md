...
All `XERC20` tokens MUST implement the standard ERC-20 interface. Note that while many of the below functions are inspired by ERC-777, implementers are NOT REQUIRED to adhere to the full ERC-777 specification.
...
![](../assets/eip-7291/XERC20.png)

1. A given ERC-20 is wrapped into its `XERC20` representation.
...
4. When transferring back to the home chain, the `XERC20` can be unwrapped back into the original ERC-20 token.
...
Case (2) is straightforward to solve with `XERC20`s as well. Token issuers should deploy a lockbox on their home chain, and then wrap their ERC-20s into `XERC20`s prior to bridging them. This wrapping step may add some friction depending on the bridge - this is discussed further in the Compatibility with Bridges section.
...
7. At any point in the future, the token issuer can then use their own treasury to unwrap `XERC20`s on the remote domain into legacy ERC-20s and send them back to the home chain, incurring the latency of this on behalf of users.

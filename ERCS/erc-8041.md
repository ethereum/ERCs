...
An interface for creating fixed-supply collections of Agent NFTs that are registered in an [EIP-8004](eip-8004.html) Agent Registry. While [EIP-8004](eip-8004.html) provides an unlimited mint registry for agent identities, many use cases require limited collections. Collections can be created with fixed supply limits while maintaining the association between agents and their collection through onchain metadata and mint number tracking.

## Motivation

[EIP-8004](eip-8004.html) establishes a singleton registry for AI Agent identity tokens with unlimited minting capability. This open registration model serves many use cases, but there is a clear need for:
...
This standard enables these use cases while leveraging the existing [EIP-8004](eip-8004.html) registry infrastructure.
...
1. Register the agent in the [EIP-8004](eip-8004.html) registry
...
Collections SHOULD implement [EIP-7572](eip-7572.html) contract metadata to provide collection-level information such as name, description, image, and external links. This is done by implementing:

```solidity
function contractURI() external view returns (string memory);
```

The URI should point to a JSON file following the [EIP-7572](eip-7572.html) metadata schema.

## Rationale

[EIP-8004](eip-8004.html) provides an open mint registry where anyone can register agent identities without restrictions. While this unlimited minting model serves many use cases, there is a clear need for limited-supply collections of agents. This interface addresses this by defining a standard for creating fixed-supply collections that leverage the existing [EIP-8004](eip-8004.html) registry infrastructure while adding collection-specific constraints, metadata, and provenance tracking through mint numbers.

## Backwards Compatibility

This standard is fully backwards compatible with:

- **[EIP-721](eip-721.html)**: Agents remain standard [EIP-721](eip-721.html) tokens
- **[EIP-8004](eip-8004.html)**: Leverages existing registry infrastructure
- **[EIP-7572](eip-7572.html)**: Optionally supports contract-level metadata
...
The owner of an [EIP-8004](eip-8004.html) agent can modify or remove the "agent-collection" metadata stored on their agent at any time. Client applications MUST NOT rely solely on this metadata to verify collection membership. Instead, clients SHOULD:

...
2. Store the agent's mint number internally
3. Write collection metadata to the agent's Onchain Metadata using the key `"agent-collection"`

The collection metadata format MUST be:
```
bytes: [collection_contract_address (20 bytes)][agentIdLength (1 byte)][agentId (variable)]
```

This enables any party to:
- Look up which collection an agent belongs to
- Verify the collection contract address
- Query the agent's mint number from the collection contract
...
contract ERC8041Collection is IERC8041Collection {
    ...
    function mint(uint256 agentId) external {
        ...
        currentSupply++;
        _agentMintNumber[agentId] = currentSupply;
        
        emit AgentMinted(agentId, currentSupply, msg.sender);
    }

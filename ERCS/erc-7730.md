...
`encryption` objects follow the schema below:
...
Because encrypted handles do not reveal their underlying type on-chain, authors SHOULD provide `plaintextType` so wallets can correctly format decrypted values.

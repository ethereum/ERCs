...
An optional object holding localization metadata for the document, loosely mirroring [ERC-1155](./eip-1155.md)'s `localization` extension. Its `references` property maps [BCP-47](https://www.rfc-editor.org/rfc/rfc5646) language tags to an array of candidate references to a translation resource; a wallet may use any entry it is able to resolve. Its optional `locales` property lists the covered BCP-47 tags explicitly — informational only, since the authoritative set is the key set of `references`.
...
Every user-facing string field MAY carry an additional `Key` sibling property — i.e. `labelKey` alongside `label` or `intentKey` alongside `intent` — containing a stable translation identifier.

...
The current version of the specification is `1` at the time of writing. The reference schema for version `1` is [here](../assets/eip-7730/erc7730-v1.schema.json).
...
* The `keyPath` key contains a path to the context-dependent data used to select the right map value
* The `values` key contains a list of key / value pairs, each key being used to match the data pointed to in the `keyPath`. The corresponding value is used as the context-dependent constant when referenced in the `display` section.
...
Clear signing User Operations, as introduced by EIP-4337, is supported without any additional addition to the standard when using the `PackedUserOperation` EIP-712 signature recommended by the EIP-4337. 

[example-userops-eip712.json](../assets/eip-7730/example-userops-eip712.json) shows how a smart contract (also called smart wallet in this section) compliant with EIP-4337 can define its 7730 `PackedUserOperation` definition to enable clear signing its own User Operations. The `PackedUserOperation` EIP-712 format is standard and can be made easily reusable by the registry.

The inner calldata of a User Operations typically contains a call to an `execute` function of the smart wallet, enabling calling any function *from* this smart wallet *to* another contract. [example-account-execute.json](../assets/eip-7730/example-account-execute.json) shows how a smart contract can clear sign this inner call using a separate 7730 file. This functionality is not standard and is entirely dependent on the smart wallet implementation, it is expected that each reference implementation will have its own 7730 file.

Smart wallets are typically implemented using a proxy pattern, all smart wallets pointing to a single reference proxy implementation. Refer to the [Proxy](#proxy-support) section for recommendations on supporting this pattern using 7730. 
...
These patterns are often used in conjonction. Supporting those patterns require some considerations from both smart contract developers and wallets.

**Upgradable contracts**

For upgradable contracts, the ERC-7730 file should be bound to the reference, public *proxy* address (since the implementation address will change when upgrading the contract).

When upgrading the contract implementation that introduce a breaking ABI change, the developers should *replace* the ERC-7730 file in the registry, since the old implementation should never be called after the upgrade.

Wallets simply need to ensure they have the latest version of the registry and that the target address is the *proxy* address.

...
| **`date`**                  |                                                                                           |
|-----------------------------|-------------------------------------------------------------------------------------------|
| *Description*               | Display int as a date, using specified encoding. Date display RECOMMENDED use of RFC 3339. If the value is a key of the optional `specialValues` map, display the mapped text instead of a date |
| *Parameters*                | ---                                                                                       |
...
| `specialValues`             | Optional. An object whose keys are integer values written as decimal strings, and whose values are the text to display for that exact value. Contracts often use `0` or the maximum integer to mean "no deadline". A wallet MUST compare the raw field value with the keys before it applies the encoding |
| *Examples*                  | ---                                                                                       |
...
| `Unlimited`                 | Field Value = 0 <br> `encoding` = "timestamp" <br> `specialValues` = {"0": "Unlimited"}    |

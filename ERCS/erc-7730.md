...
### Storage Layout for Transaction Simulations and Transaction Assertions

ERC-7730 files may optionally reference a storage layout file that enables wallets to present transaction simulation results in human-readable form.

The "Transaction Outcome Simulation" services SHOULD NOT provide their own human-readable interpretation of the state changes.
Instead, they SHOULD return raw native balance and storage slot mutations as `{ contract, slot, old_value, new_value, derivation_path }` objects across every contract a transaction touches with a sotrage write.

#### `storageLayout`

The layout files are relatively large and are kept in a separate file to avoid bloating the main ERC-7730 specification.
The optional top-level `storageLayout` parameter provides URI reference pointing to the storage layout file for this contract:

```json
{
    "storageLayout": "./example-erc20.storage.json"
}
```

#### Storage layout file format

The storage layout file extends the existing storage layout format with ERC-7730 display annotations.

It contains three top-level keys:

* `compiler` — required object identifying the source language, compiler version, and compilation parameters.
* `storage` — ordered list of top-level state variable descriptions, identical to compiler output.
* `types` — type definitions referenced by storage entries, identical to compiler output.

The `compiler` object:

| Field      | Description                                         |
|------------|-----------------------------------------------------|
| `language` | Source language, e.g. `"solidity"` or `"vyper"`     |
| `version`  | Compiler version string                             |
| `settings` | Optional arbitrary compiler settings and parameters |

See [`erc7730-storage.schema.json`](../assets/erc-7730/erc7730-storage.schema.json) for the full JSON schema and [`example-erc20.storage.json`](../assets/erc-7730/example-erc20.storage.json) for a complete annotated ERC-20 example.

#### Slot Derivation Paths

Complex variable types, such as arrays or mappings, are not stored directly in their variables' storage slots, and instead have a deterministic formula that determintes the actual storage slot ID for a specific value.

The derivation formula for a mapping key or array index is determined entirely by the type's `encoding` field and the compiler language declared in `compiler.language`.

In order to be able to properly display the simulated outcome as a meaningful variable-based value change, the wallets MUST implement the following derivation formulas:

| Language      | Type encoding   | Derivation formula                                        |
|---------------|-----------------|-----------------------------------------------------------|
...
| Vyper ≥ 0.3.0 | `mapping`       | `keccak256(abi.encode(key, baseSlot))`                    |
| Vyper < 0.3.0 | `mapping`       | `keccak256(abi.encode(baseSlot, key))`                    |
| Vyper ≥ 0.3.0 | `dynamic_array` | `baseSlot + 1 + index * elementSlots`                     |
| Vyper         | `inplace`       | `baseSlot + member.slot`                                  |

Where `elementSlots = ceil(elementSize / 32)`, derived from the element type's `numberOfBytes` field. For single-slot element types (e.g. `uint256`, `address`) this simplifies to `index` or `index + 1` respectively.

For nested types, derivation is applied recursively at each level, with slot arithmetic following the standard EVM storage packing rules.

This specification may be extended in future extension ERCs to cover additional EVM languages, new storage types or features, non-standard storage patterns etc.

#### `display` object

Each `storage` entry and each struct member in `types` may carry an optional `display` object with the following fields.

**`label`** — User-facing name for the variable. For mappings, `{key[N]}` placeholders embed the Nth mapping key (outermost = `{key[0]}`). For arrays, `{index[N]}` embeds the Nth element index.

**`keyFormats`** — An array of formatters applied to mapping keys or array indices before they are embedded in `label`. `keyFormats[0]` applies to `{key[0]}`, and so on. Each entry uses the same `format`/`params` structure as call data field formatters.

**`format`** — The ERC-7730 formatter (`tokenAmount`, `addressName`, `raw`, `date`, etc.) applied to the storage value itself. Same vocabulary as `display.formats[*].fields[*].format`.

**`params`** — Formatter parameters for `format`. Same structure as `display.formats[*].fields[*].params`.

**`template`** — A natural language description of the change to this variable. Either a plain string or an ordered array of conditional entries evaluated using [JSON Logic](https://jsonlogic.com). All template strings support `{label}`, `{oldValue}`, and `{newValue}` placeholders, where `{oldValue}` and `{newValue}` are the values after applying `format`/`params`.

When `template` is an array, each entry has a `value` string and an optional `condition`. The wallet evaluates entries in order and uses the first entry whose condition is true. An entry without a `condition` matches unconditionally and SHOULD appear last as the fallback.
...
*Example — ERC-20 balances mapping:*
...
Solidity storage can involve complex nesting of mappings, arrays, and structs. ERC-7730 handles these through recursive resolution:
...
3. **Structs:** Members are defined in the `types` object, each with their own optional `display` annotation. When a struct member changes, the wallet displays the parent variable's resolved label alongside the member's own label.

*Example — Mapping to a struct:*
...
When `vaults[0xAlice].lockedAmount` changes, the wallet presents the parent label `"Vault belonging to Alice.eth"` and member label `"Locked Amount"` separately.

#### Packed Variables

Solidity may pack multiple small variables into a single 32-byte slot. The `offset` field on a storage entry gives the byte offset within the slot where the variable begins; the type's `numberOfBytes` field gives its size.

To extract the value of a packed variable from a raw 32-byte slot word, the wallet reads `numberOfBytes` bytes starting at byte `offset`.

When a packed slot changes, wallets MAY omit variables whose extracted old and new values are identical.

#### Simulation flow and hardware wallet verification

The derivation path for each modified slot is produced by the simulation engine during transaction tracing, as it has direct access to the keys used at each storage write.

If the software wallet performs transaction simulation before communicating with the hardware wallet, the storage layout file SHOULD be transferred to the hardware wallet alongside the ERC-7730 specification, enabling the hardware wallet to independently verify slot derivation paths.

**Clear Signing with Transaction Simulation Flow**

1. Wallet delegates transaction simulation to a simulation engine, which returns `(contract, slot, old_value, new_value, derivation_path)` objects. The `derivation_path` component is an ordered list of the base slot and the successive steps and keys used to compute the final `slot` ID.
2. For each touched contract that has a registered ERC-7730 file with a `storageLayout` reference, resolve the layout file.
   * If no ERC-7730 file or no `storageLayout` is available for a touched contract, the wallet SHOULD warn the user and display the raw slot address and value.
3. Match each modified slot to a layout entry via the base slot in the derivation path.
   * For packed slots, extract the annotated variable using its `offset` and `numberOfBytes`.
4. Format the extracted value using the entry's `display` annotation.
5. Submit the resolved, human-readable changes for signing by the user alongside the standard ERC-7730 calldata context.

**During the signing process**

1. Wallet looks up the base slot in the storage layout file and identifies the type's `encoding` and the file's `compiler.language`.
2. Wallet re-derives the final slot from the base slot and the keys in the derivation path, using the appropriate formula from the supported Slot Derivation rules.
3. Wallet asserts the computed slot equals the reported slot.
4. Displays the formatted change using the `display` annotation.

Wallets MUST display simulation results under a heading that clearly distinguishes them from the signed calldata.

Wallets SHOULD ensure that Transaction Assertions are properly enforced before claiming that the results of the outcome simulation are enforced on-chain.

Wallets SHOULD warn users if any step in the simulation or slot verification fails.

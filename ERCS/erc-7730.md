...
An optional object holding localization metadata for the document, loosely mirroring [ERC-1155](./eip-1155.md)'s `localization` extension. Its `references` property maps [BCP-47](https://www.rfc-editor.org/rfc/rfc5646) language tags to an array of candidate references to a translation resource; a wallet may use any entry it is able to resolve. Language tags MUST use canonical BCP-47 casing (e.g. `fr`, `zh-Hant`, `pt-BR`).

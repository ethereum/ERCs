...
* Some fields require additional metadata to be displayed correctly, for instance token amounts require knowledge of the magnitude and the ticker, as well as where to find the token address itself to be correctly formatted.
...
In this example, the context section refers to the USDT smart contract address on ethereum mainnet and its reference ABI. Note that the ABI is used to define unique *path* references to fields that must be formatted according to this file, so it is mandatory for all contracts clear signed.
...
The current version of this specification only supports two types of binding context, EVM smart contracts and EIP712 domains. 
...
* The `contract` sub-key is used to describe an EVM smartcontract binding context, with:
  ...
  * `chainId`: an [EIP-155 identifier](https://chainid.network/) of the chain the descibed contract is deployed at

...
### Storage Layout and Transaction Simulation

ERC-7730 files may reference a storage layout file that enables wallets to present transaction simulation results in human-readable form.

Transaction simulation returns raw storage mutations as `(contract, slot, old_value, new_value)` tuples across every contract a transaction touches.

Without annotation, storage slots are opaque 32-byte integers meaningless to users.

#### `storageLayout`

An optional top-level URI reference pointing to the storage layout file for this contract:
...
The layout can be large and is kept in a separate file.

#### Storage layout file format

The storage layout file is a **superset of Solidity compiler output**.

The Solidity compiler produces a storage layout JSON object with two keys:
* `storage` - array of variable descriptions
* `types` - type definitions.

The ERC-7730 layout file preserves this structure verbatim and adds a `display` object to each entry in `storage`.
...
Each `storage` entry carries a `display` object with the following optional fields:

| Field    | Description                                                                                                                                          |
|----------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| `label`  | User-facing name. For mappings, `{key[N]}` placeholders embed mapping keys (outermost = `{key[0]}`). For arrays, `{index}` embeds the element index. |
| `keyFormats`| Optional array of formatters for keys. `keyFormats[0]` applies to `{key[0]}`, etc. Uses same `format`/`params` structure as call fields. |
| `format` | ERC-7730 formatter (`tokenAmount`, `addressName`, etc.) applied to the storage values (`old_value`, `new_value`). |
| `params` | Formatter parameters for the storage values. |
| `template` | Optional natural language template for the change (e.g. `"{label} changes from {oldValue} to {newValue}"`). |

*Example — ERC-20 balances mapping with key formatting:*
...
```json
{
    "storage": [{
        ...
        "display": {
            ...
            "format": "tokenAmount",
            "params": { "tokenPath": "@.to" },
            "template": "{label} increases from {oldValue} to {newValue}"
        }
    }]
}
```
...
1.  **Mappings and Arrays:** Each level of nesting adds a new key or index to the derivation path.
    *   `mapping(address => mapping(uint256 => uint256))` uses `{key[0]}` for the outer key and `{key[1]}` for the inner key.
    *   `uint256[][]` uses `{index[0]}` for the outer array and `{index[1]}` for the inner array.
2.  **Structs:** Struct members are defined in the `types` object. Each member can carry its own `display` annotation. When a struct member is modified:
    *   The wallet resolves the base variable's `label` (including any mapping keys).
    *   It uses the struct member's `label` to define the final field name.
    *   A `{parent}` placeholder in a member's label can be used to reference the resolved label of the containing structure.
            ...
            "members": [
                {
                    ...
                    "type": "t_uint256",
                    "display": {
                        "label": "{parent}'s Locked Amount",
                        "format": "tokenAmount",
                        "params": { "tokenPath": "@.to" },
                        "template": "{label} increased to {newValue}"
                    }
                }
            ]
        }
...
In the example above, if `vaults[0x123].lockedAmount` changes, the label resolves to `"Vault of Vitalik.eth's Locked Amount"`.

### Storage Slot Derivation Strategies

A derivation strategy defines how a specific storage slot is computed from a base slot and, optionally, one or more keys (for mappings or arrays).

Wallets MUST support the following standard strategies:

| Strategy | Description | Derivation Formula |
|----------|-------------|-------------------|
| `solidity:mapping` | Standard Solidity mapping | `keccak256(key \|\| baseSlot)` |
| `solidity:dynamic_array` | Standard Solidity dynamic array | `keccak256(baseSlot) + index` |
| `vyper:mapping` | Vyper mapping (v0.3.0+) | `keccak256(key \|\| baseSlot)` |
| `vyper:mapping:legacy` | Vyper mapping (< v0.3.0) | `keccak256(baseSlot \|\| key)` |
| `vyper:dynamic_array` | Vyper dynamic array | Elements at `baseSlot + 1 + index` |
| `eip1967` | EIP-1967 non-structured slots | `bytes32(uint256(keccak256(key)) - 1)` |
| `diamond` | Diamond Storage (EIP-2535) | `keccak256(key)` |

This list is extensible. Future EVM languages or storage patterns MAY define new strategies via an ERC extension to ERC-7730. Wallets SHOULD warn users if an unknown strategy is encountered.

#### Custom and Future Layouts

Support for non-standard or future layout patterns is provided through the extensible `strategy` identifier. If a contract uses a unique storage pattern not covered by the standard strategies, it MAY use a custom URI-like identifier (e.g., `protocol-name:custom-strategy`). The documentation for such a strategy MUST be publicly available for wallet implementers.
...
If the software wallet performs transaction simulation and identifies expected state changes before communicating with the hardware wallet, the storage layout file SHOULD be transferred to the hardware wallet alongside the ERC-7730 specification, enabling the hardware wallet to independently verify slot derivation paths.

**Clear Signing with Transaction Simulations Flow**

1. Wallet simulates the transaction and obtains `(contract, slot, old_value, new_value, derivation)` tuples from the simulation engine. The `derivation` component is an ordered list of keys and the base slot used to compute the final `slot`.
2. For each touched contract, resolve its ERC-7730 file and follow the `storageLayout` URI to obtain the layout file.
3. Match each modified storage slot to a layout entry via the 'base slot' identifier provided by the simulation.
4. Format `old_value` and `new_value` using the entry's `display` annotation.
...
1. Wallet looks up the base slot in the loaded storage layout file and identifies the `strategy`.
2. Wallet derives the final slot by applying the `strategy` to the base slot and the keys provided in the `derivation` path.
3. Wallet asserts the computed slot equals the reported slot.
4. Displays the formatted change using the `display` annotation.

Wallets MUST display simulation results under a heading that clearly distinguishes them from the signed data or guaranteed outcomes.

Wallets SHOULD properly warn users in case any of the steps involved in the transaction simulation and presentation fail. 

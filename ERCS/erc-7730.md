...
`fields` is an array of elements, each element being either:
  ...
  * A reference to a format specification in the `definitions` section: by declaring an object with two keys, a `path` key with the [path](#path-references) to the field being formatted, and a `$ref` key with a path to the internal definition. 
    * A reference object can override a field format specification `label`, `description`, `params`, `separator`, `visible`, or `encryption` by including the corresponding sub-key, whose values will take precedence over the common definition
...
An optional plain-text string providing an explanation of the field's meaning or significance in the context of this specific operation.
Unlike `label`, which provides a short display name, `description` is intended to help users better understand the details of the field's meaning.

Wallets SHOULD display `description` when present, subject to available screen space.
Resource-constrained wallets MAY omit `description` entirely or render it on user's request.

A `description` MUST NOT exceed 280 characters.
...
An optional plain-text string providing a contextual explanation of the group as a whole.
...
| [example-erc20-v3.0.0-next.json](../assets/erc-7730/example-erc20-v3.0.0-next.json) | Example of ERC-20 interface definition utilizing v3 schema features (descriptions) |

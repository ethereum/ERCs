...
| `format` | ERC-7730 formatter (`tokenAmount`, `addressName`, `raw`, `date`, `enum`, etc.) using the same set as call parameters.                                |
| `params` | Formatter parameters using same structure as `display.formats[*].fields[*].params`.                                                                  |
...
1. Wallet simulates the transaction and obtains `(contract, slot, old_value, new_value, derivation)` tuples from the simulation engine.
...
1. Wallet looks up the base slot in the loaded storage layout file.
2. Wallet derives the final slot by walking the derivation path.

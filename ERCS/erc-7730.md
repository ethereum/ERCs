...
The `$schema` key identifies the schema this descriptor was authored against, either the alias of a MAJOR line or an exact version, as detailed in [Versioning](#versioning).
...
This ERC periodically releases a new, immutable, explicitly numbered version of the schema that may be considered "Final" for all intents and purposes.
...
This standard may deprecate a version of the schema explicitly. Wallets and tooling SHOULD maintain support for all non-deprecated versions of the schema declared in the [Released versions](#released-versions) section.
...
The version parts are defined by the set of descriptors that the schema accepts:

* A PATCH bump does not change the set of valid descriptors. Only titles, descriptions and other non-normative text in the schema change.
* A MINOR bump grows the set of valid descriptors. Every descriptor that was valid stays valid.
* A MAJOR bump shrinks the set of valid descriptors. Some descriptor that was valid stops being valid.

A change that invalidates any published descriptor is a MAJOR change, whatever its size. Tooling that consumes the schema SHOULD validate its whole corpus of descriptors against a changed schema file to classify the change.

Schema changes that target the next MAJOR version accumulate in a single mutable draft file, named `erc7730-v<VERSION>-next.schema.json` after the targeted version.
...
A PATCH or MINOR change to the active MAJOR line does not go through the draft. It is released directly, by the pull request that publishes the new exact-version file and updates the alias of its line (see [Schema files and the `$schema` key](#schema-files-and-the-schema-key)).
...
Two kinds of schema files are published for each MAJOR line.

An **exact-version file** is named `erc7730-v<MAJOR>.<MINOR>.<PATCH>[-rc.<N>].schema.json`, with all three components written out (e.g. `erc7730-v2.0.0.schema.json`). It is immutable from the moment it is published, and its contents MUST NOT change. Any change to a finalized version, including error fixes, is published as a new version with a new exact-version file. Exact-version files are never renamed and never removed, except that release-candidate files MUST be removed by the authors once their version is finalized.

An **alias file** is named `erc7730-v<MAJOR>.schema.json`, with only the MAJOR component (e.g. `erc7730-v2.schema.json`). It holds a byte-for-byte copy of the newest released exact-version file of its line. Only a release changes an alias: the pull request that publishes a new exact-version file on a line also updates that line's alias to the same content, in the same commit. An alias is never removed while its line is not deprecated.

The in-development draft is published as `erc7730-v<VERSION>-next.schema.json`. It is the only file that changes outside of a release.
...
Each schema file's own top-level `version` key is the authoritative identifier of its version. An alias file therefore carries the full version of the release it currently holds (e.g. `2.0.1`), not the name of its line.

The file naming patterns above are a convention for humans and tooling, not the source of truth, and consumers MUST NOT derive the version by parsing the file name.
...
The version of the specification used by a descriptor file is the version identified by its `$schema` key, as introduced in the [Simple example](#simple-example). A descriptor MAY point at an alias file, in which case it is validated against the newest release of that MAJOR line, or at an exact-version file, in which case it pins that exact version. Pointing at an alias is safe because a PATCH or MINOR release never invalidates a descriptor, by the definition of the version parts above. Consumers MUST use this key to select which MAJOR version and which validation and interpretation rules apply to a given descriptor.
...
| Version      | Exact-version file                                                                        | Alias file                                                           | Commit ID                                  | Status     |
|--------------|-------------------------------------------------------------------------------------------|----------------------------------------------------------------------|--------------------------------------------|------------|
| `1.0.0`      | [`erc7730-v1.0.0.schema.json`](../assets/eip-7730/erc7730-v1.0.0.schema.json)             | [`erc7730-v1.schema.json`](../assets/eip-7730/erc7730-v1.schema.json) | `d42dfc9d0711b7175a24142b1e2c2d36e43ed8b7` | Deprecated |
| `2.0.0`      | [`erc7730-v2.0.0.schema.json`](../assets/eip-7730/erc7730-v2.0.0.schema.json)             | [`erc7730-v2.schema.json`](../assets/eip-7730/erc7730-v2.schema.json) | `2528d6a0cd463d7309464a33889774368ab52df3` | Superseded |
| `2.1.0`      | [`erc7730-v2.1.0.schema.json`](../assets/eip-7730/erc7730-v2.1.0.schema.json)             | [`erc7730-v2.schema.json`](../assets/eip-7730/erc7730-v2.schema.json) | TBD                                         | Active     |
| `3.0.0-next` | [`erc7730-v3.0.0-next.schema.json`](../assets/eip-7730/erc7730-v3.0.0-next.schema.json)   | N/A                                                                  | N/A                                        | Draft      |

Every version released from now on is added as a new row in this table. The Version, file and Commit ID columns of a row are never altered once added. The Status column changes over time: a version is "Active" while it is the newest release of a non-deprecated line, "Superseded" once a newer release exists on its line, and "Deprecated" once this standard deprecates it.

The Commit ID column is informational only: it notes the commit that last set each version's schema content. It is not itself part of the versioning mechanism, and the `version` key inside each schema file remains the authoritative identifier.

`1.0.0` is deprecated.

New integrations SHOULD target the alias file of the "Active" line.
...
| `message`                  | message to display above threshold. Optional, defaults to "All" |
| *Examples*                 | --- |
| `0.19866144 ETH`           | Field value = 0x2c1c986f1c48000 |
| `All`                      | Field value = 0xFFFF...FFFF <br> `threshold` "0xFFFF...FFFE" |

| **`tokenAmount`**          |              |
|----------------------------|--------------|
| *Description*              | Convert value using token decimals, and append token ticker name. If value is greater than or equal to the optional `threshold`, display `message` with ticker instead. |
| *Parameters*               | --- |
...
| `threshold`                | integer value. If the field value is greater than or equal to it, the wallet displays `message` instead of the amount. Optional |
| `message`                  | message to display when the field value is greater than or equal to `threshold`. Optional, defaults to "Unlimited" |
...
All display strings in ERC-7730 files — including intents, labels, field names, and enumeration values — MUST be authored in English (`en-US`).

Translation of these strings into other languages can be provided based on the English document.

#### `$localization` field

An optional object holding localization metadata for the document. Its `references` property maps [BCP-47](https://www.rfc-editor.org/rfc/rfc5646) language tags to an array of candidate references to a translation resource; a wallet may use any entry it is able to resolve. Language tags MUST use canonical BCP-47 casing (e.g. `fr`, `zh-Hant`, `pt-BR`).

`$localization` is a non-authoritative discovery hint: it lists translations known to the document author, but wallets may also obtain translation resources for this document from other sources, such as a registry or an attestation index. Translation validity does not depend on being listed here — a resource from any source is checked against the document and attested independently, as defined in [ERC-8346](./eip-8346.md).

This specification does not mandate or favor any particular URI scheme or resolution mechanism. A relative reference is resolved against the document's own location, if applicable.
...
```json
{
    "$localization": {
        "references": {
            "fr": [
                { "uri": "./example-main.fr.json" },
                { "uri": "ipfs://bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi/example-main.fr.json" }
            ],
            "zh-Hant": [
                { "uri": "./example-main.zh-Hant.json" }
            ]
        }
    }
}
...
Every user-facing string field MAY carry an additional `Key` sibling property — i.e. `labelKey` alongside `label` or `intentKey` alongside `intent` — containing a stable translation identifier. A `Key` property is only valid alongside its literal sibling: the literal remains the authoritative English source text, so a `labelKey` without a `label` (or `intentKey` without `intent`, etc.) makes the document invalid.
...
```json
{
    ...
    "intentKey": "erc20.transfer.intent"
}
```

Enumerations declared under `metadata.enums` are keyed through the sibling `metadata.enumKeys` map, which maps an enumeration name to its translation key; the corresponding translation entry maps each raw enumeration value to its translated display string.
...
```json
{
    "metadata": {
        ...
        "enumKeys": {
            "interestRateMode": "lending.interest_rate_mode"
        }
    }
}
```

The key format and resolution against a translation resource are defined in [ERC-8346](./eip-8346.md).
...
| [example-main-v3.0.0-next.json](../assets/eip-7730/example-main-v3.0.0-next.json) | Example of `labelKey`/`intentKey`/`interpolatedIntentKey` translation key annotations |

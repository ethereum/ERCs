...
In addition, **`metadata.intentMutability`** is an OPTIONAL free-form string describing intent-mutability vectors that cannot be reduced to an on-chain precondition (off-chain oracle endpoints, deployment-time library linkage, dependencies on `block.timestamp` or other block environment). This field is documentation only; wallets cannot verify these claims. The trust anchor for off-chain mutability is auditor attestation, not this field.
...
|-------|----------|-------------|
| `type` | Yes | One of `eip-1967`, `eip-1822`, `eip-2535`. Tells the wallet which verification primitive to apply: a fixed-slot read for [EIP-1967](./eip-1967.md) and [EIP-1822](./eip-1822.md), a `facets()` loupe call for [EIP-2535](./eip-2535.md). Custom (non-standard) proxy layouts are not declared here; use `stateRefs` instead. |
...
| `address` | Yes | EIP-55 address of the implementation contract. |
| `selectors` | Conditional | Array of 4-byte function selectors (0x-prefixed hex) served by this facet. REQUIRED when the enclosing `type` is `eip-2535`; MUST be absent otherwise. Wallets verify every `(selector, facet)` pair returned by the diamond loupe matches a declared `(selectors, address)` entry, catching both new selectors and selector rerouting. |

**Wallet behavior:**

- For `type: "eip-1967"` and `type: "eip-1822"`: read the well-known implementation slot defined by the corresponding EIP, interpret the value as an address, and verify the address is present in `expectedImplementations`. On mismatch, treat the descriptor as stale.
- For `type: "eip-2535"`: call `facets()` on the standard diamond loupe interface. For every `(facetAddress, selectors[])` pair returned, verify there is a matching entry in `expectedImplementations` and that the live selector set is contained in the declared selector set for that facet. On any mismatch, treat the descriptor as stale.
...
*Off-chain mutability disclosure (documentation only, in `metadata`):*

```json
{
    "metadata": {
        "intentMutability": "Swap rate is fetched at execution time from an off-chain oracle at https://oracle.example.com. The owner can rotate the oracle endpoint without any on-chain state change; this vector cannot be witnessed by `stateRefs` and is surfaced here for auditor review."
    }
}
```
...
**Why descriptor-side rather than a separate auditor-produced report.** We considered placing this information in a separate report. We chose the descriptor for three reasons: (a) wallets need a self-contained, fast-to-verify signal on every signing rather than an additional registry fetch; (b) the verification is local and stateless, with no extra trust assumptions beyond the descriptor itself; (c) the protocol author is closest to the contract and is the cheapest place to source the slot list and implementation set. Auditor attestations remain the trust anchor: they verify the declared `proxy`, `stateRefs`, and `metadata.intentMutability` claims are exhaustive and accurate. Standalone auditor-published mutability reports are a possible future extension and are out of scope for this specification.

**Why off-chain mutability lives in `metadata.intentMutability` rather than `context`.** `context` is the contract that the wallet enforces machine-verifiably. A free-form prose field housed there would invite authors to launder weak, unverifiable claims through it and would force wallets to display unverified text in a security-critical block. Off-chain mutability vectors (oracle endpoint rotation, deployment-time library linkage, block-environment dependencies) are real but not machine-verifiable; their natural home is `metadata`, as documentation surfaced to auditors and to wallet UI as supplementary context — not as a precondition the wallet can pretend to check. Auditors reviewing a descriptor MUST verify both that the `context.contract.proxy` and `context.contract.stateRefs` preconditions are exhaustive for on-chain mutability and that every intent-mutability vector that cannot be witnessed on-chain is disclosed in `metadata.intentMutability`. Any vector known to the auditor but absent from both blocks is an audit defect; an attestation MUST NOT be issued until the descriptor is updated.
...
- **Diamond facet swap or addition.** Caught by the selector-aware `proxy.expectedImplementations` entries under `type: "eip-2535"`, which bind both the facet set and the selector-to-facet routing.

**Vectors the on-chain preconditions cannot catch.** Several real intent-mutability vectors cannot be reduced to a slot read or a fixed interface call. Descriptor authors and auditors MUST be aware of these, and any that apply MUST be disclosed in the free-form `metadata.intentMutability` field even though wallets cannot machine-verify them:
...
- **Dynamic external call resolution.** A function calls into another contract whose address is computed at runtime — via a registry lookup, factory deployment, CREATE2 prediction, or oracle endpoint — rather than read from a fixed storage slot. The target can be rotated without any state change on the bound contract.
- **Cross-facet routing changes within an allow-list.** Even with selector-aware diamond support, an adversary or admin who can install one allow-listed facet at a different selector than originally audited can cause execution to diverge from the displayed intent while every individual address remains in the declared set. Auditor attestations MUST cover routing stability, not just facet membership.
- **Off-chain dependencies.** Oracle endpoints, off-chain signing services, deployment-time library linkage, and other off-chain factors whose change leaves no on-chain witness.
- **Composition.** A function that is correct in isolation can be wrapped in a multicall, EIP-4337 batch, or hook that reframes the user's intent. A single-contract descriptor cannot witness composition-level changes.

These vectors are out of reach of the precondition machinery. The trust mechanism for them is auditor attestation: an auditor reviewing a descriptor MUST verify that (a) the `context.contract.proxy` and `context.contract.stateRefs` preconditions are exhaustive for vectors that can be witnessed on-chain, and (b) every vector that cannot be witnessed on-chain — including any of the cases enumerated above that apply to the bound contract — is disclosed in `metadata.intentMutability`. An attestation MUST NOT be issued while a known intent-mutability vector is undeclared in either block. Wallets are encouraged to prefer descriptors that carry an auditor attestation. Treatment of conflicts between an auditor's findings and a descriptor's self-declaration is out of scope for this specification.

When neither `context.contract.proxy` nor `context.contract.stateRefs` is declared and `metadata.intentMutability` is absent, the descriptor is asserting that no intent-mutability vectors apply to the bound contract. Wallets MAY surface this assertion to users; auditors MUST verify it.
...
A good pattern to support diamond proxies is to introduce an ERC-7730 file for each facet, without any context info, and use the composability of ERC-7730 files to create an overarching ERC-7730 file that includes all implemented facets of a proxy and bound to the final *proxy* address. The overarching file SHOULD declare `context.contract.proxy` with `type: "eip-2535"` and list each facet address alongside the function selectors it serves under `expectedImplementations`. Wallets enumerate the live `selector → facet` mapping via the standard diamond loupe `facets()` call and verify every entry matches a declared `(address, selectors)` pair, catching both facet additions and selector rerouting between audited facets.

...
Maps references can be used in place of a constant parameter value. The parameters accepting a map reference are the ones declared as such in the schema, following two rules:

* A parameter accepts a map reference when its value is a constant *operand* of the format: a value the format compares the field against, or uses to render it. `token`, `nativeCurrencyAddress`, `senderAddress`, `collection`, `callee`, `threshold` and the `unit` scale are of this kind.
* A parameter does not accept a map reference when it selects *how* the field is decoded, or when it points at another definition. `encoding`, the `encryption` parameters and `$ref` are of this kind: making them context-dependent would let identical data be interpreted differently per deployment, which the bound contract's ABI already fixes.

Purely human-readable parameters, such as the `message` accompanying a `threshold`, are deliberately left out: they carry no context-dependent constant. A map value is a scalar, so parameters taking an array of values, such as `types` and `sources`, cannot be resolved through a map either.
...
|---------------------|----------------------------------------------------------------|
| `address`           | `0x`-prefixed hexadecimal, no checksum casing applied          |
...
If no key of the `values` object matches, the descriptor does not describe the transaction it is being applied to: the map enumerates the contexts its author accounted for, and this is not one of them. The wallet MUST NOT use the descriptor to display the transaction, and MUST fall back to the same behaviour as when no descriptor is available at all. It MUST NOT substitute a default, and MUST NOT render the remaining fields of the format, as unrelated constants in the same descriptor may be equally out of date.
...
Examples can be found in [example-maps.json](../assets/eip-7730/example-maps.json) and [example-maps-pools.json](../assets/eip-7730/example-maps-pools.json), both keyed on the deployment chain id, and in [example-maps-deployments.json](../assets/eip-7730/example-maps-deployments.json), keyed on the called contract address.

...
| `2.0.0`      | [`erc7730-v2.0.0.schema.json`](../assets/eip-7730/erc7730-v2.0.0.schema.json)             | [`erc7730-v2.schema.json`](../assets/eip-7730/erc7730-v2.schema.json) | `2528d6a0cd463d7309464a33889774368ab52df3` | Active     |
...
Map references can be used in place of a constant parameter value. The schema declares which parameters accept one: those whose value is a constant the format compares the field against or renders it with, such as `token`, `nativeCurrencyAddress`, `threshold` or the `unit` scale. Parameters that instead select how a field is decoded (`encoding`, the `encryption` parameters) or that point at another definition (`$ref`) do not, since how the data is interpreted is already fixed by the bound contract.

A map reference is a json object with two keys:
...
* The `keyPath` key refers to the path to use to retrieve the map key to use for the resolution. It is resolved like any other path parameter, and MAY be a container path such as `@.to` or `@.chainId`.

A wallet MUST replace a path to a map using the value matching the `keyPath` parameter.

**Resolving the map key**

The value pointed to by `keyPath` is rarely a string in the structured data, so a wallet MUST convert it to its *canonical key form* before matching it against the keys of the `values` object:

| Resolved value type | Canonical key form                                             |
|---------------------|----------------------------------------------------------------|
| `address`           | `0x`-prefixed hexadecimal                                      |
| `bytes`, `bytesN`   | `0x`-prefixed hexadecimal                                      |
| `uintN`, `intN`     | Decimal, no leading zeros, `-` prefix for negative values      |
| `bool`              | `true` or `false`                                              |
| `string`            | The string itself                                              |

A wallet MUST compare the canonical key form against the keys of the `values` object case-insensitively. This makes address keys robust to checksum casing: descriptor authors typically write [ERC-55](./eip-55.md) checksummed addresses, whereas the value resolved from the structured data or from the transaction container carries no casing guarantee. It also means a map MUST NOT declare two keys that differ only by case.

**When no value matches**

If no key of the `values` object matches, the descriptor does not describe the transaction it is being applied to: the map enumerates the contexts its author accounted for, and this is not one of them. A map has no fallback entry, and the wallet MUST NOT invent a value for the missing key. It MUST NOT use the descriptor to display the transaction at all, falling back to the same behaviour as when no descriptor is available — in particular it MUST NOT skip the affected field or render the format's other fields, as unrelated constants in the same descriptor may be equally out of date.

A wallet SHOULD surface the failed lookup as a diagnostic, since a lookup miss is normally an authoring mistake — most often a deployment added to `context.contract.deployments` without a matching entry being added to the map.

Examples can be found in [example-maps.json](../assets/eip-7730/example-maps.json), keyed on the deployment chain id, and [example-maps-pools.json](../assets/eip-7730/example-maps-pools.json), which mixes a chain-id key and a contract-address key.

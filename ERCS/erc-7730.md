...
author: Laurent Castillo (@lcastillo-ledger), Derek Rein (@arein)
...
An optional string containing an intent description with embedded field values using [interpolation syntax](#value-interpolation).
...
- Wallets MUST enforce maximum lengths on interpolated values and truncate if necessary
- Wallets MUST validate that ENS names and other address labels are cryptographically verified before interpolation

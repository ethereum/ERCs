...
```json
{
    "$schema": "https://eips.ethereum.org/assets/eip-7730/erc7730-i18n.schema.json",
    "$locale": "fr",
    "translations": {
        "Send":                  "Envoyer",
...
See [`example-main.fr.json`](../assets/erc-7730/example-main.fr.json) for a complete example, and [`erc7730-i18n.schema.json`](../assets/erc-7730/erc7730-i18n.schema.json) for the JSON schema of the translation file format.
...
The English string and translation file MAY use text metadata of the form `"{englishString}$${translatorNote}"` to provide translator notes and avoid ambiguity.
Wallets MUST NOT render the text appearing in string labels after the `$$` separator in the UI.

```json
{
    "translations": {
        "Transfer$$label":  "Переказ коштів",
        "Transfer$$button":   "Переказати"
    }
}
```

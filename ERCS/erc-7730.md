...
The following is an example of how to clear sign a `transfer` function call on an [ERC-20](./erc-20.md) contract.
...
The current version of the specification is `2` at the time of writing. The reference schema for version `2` is [here](../assets/erc-7730/erc7730-v2.schema.json).
...
- Enabling clear descriptions of batch transactions (e.g., [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792)), where multiple operations can be concatenated into a single readable sentence
...
A deployment option is an object with:
  * `chainId`: an [EIP-155 identifier](https://eips.ethereum.org/EIPS/eip-155) of the chain the described contract is deployed on.
...
* `chainId`: an [EIP-155](https://eips.ethereum.org/EIPS/eip-155) identifier of the chain the message is bound to 
...
| **`raw`**       |                                                                                              |
|-----------------|----------------------------------------------------------------------------------------------|
| *Description*   | Display address as an [EIP-55](./erc-55.md) formatted string. Truncation is device dependent |
| *Parameters*    | None                                                                                         |
| *Examples*      | ---                                                                                          |
...
| **`tokenTicker`**          |                                                                                              |
|----------------------------|----------------------------------------------------------------------------------------------|
| *Description*              | Display address as an [ERC-20](./erc-20.md) token ticker                                     |
| *Parameters*               | ---                                                                                          |
...
Clear signing [EIP-4337](./erc-4337.md) User Operations is supported using the `PackedUserOperation` EIP-712 signature format. See [example-userops-eip-712.json](../assets/erc-7730/example-userops-eip712.json) for a reference implementation.
...
When displaying batch transactions as defined in [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792), wallets SHOULD either:
...
In the future, this specification could be extended to structured data like Meta Transaction in [EIP-2771](./erc-2771.md), User Operations in [EIP-4337](./erc-4337.md), and batch transaction payloads in [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792).

The `interpolatedIntent` field is particularly well-suited for [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792) batch transactions, as it enables wallets to concatenate multiple operation descriptions into a single, natural language sentence. By joining individual `interpolatedIntent` strings with " and ", wallets can provide users with clear, readable summaries of complex multi-step operations like "Approve Uniswap Router to spend 1000 USDC and Swap 1000 USDC for at least 0.25 WETH". This significantly improves the user experience when reviewing batch transactions compared to displaying separate, disconnected operation descriptions. See the [Value Interpolation](#value-interpolation) section for detailed implementation guidance and examples.
...
| [example-eip-712.json](../assets/erc-7730/example-eip712.json) | Clear signing EIP-712 example |
...
| [example-account-execute.json](../assets/erc-7730/example-account-execute.json) | Example of EIP-4337 smart wallet execute function clear signing |
| [example-userops-eip-712.json](../assets/erc-7730/example-userops-eip712.json) | Example of EIP-4337 User Operation clear signing |

...
The `intentMutability` section declares that nothing on-chain or off-chain can change this contract's displayed intent. Wallets use this to skip live-state verification and avoid showing a "mutability not specified" warning. Most real contracts will not be `stable: true` — they will declare the storage slots whose values bound the displayed intent's correctness. See the [`IntentMutability` section](#intentmutability-section) for the full shape and other examples.
...
### `IntentMutability` section

The `intentMutability` section declares on-chain and off-chain factors that can cause a function's executed behavior to diverge from its displayed intent. It enables wallets to (a) verify state preconditions before signing, (b) display consistent warnings when intent is mutable, and (c) downgrade to opaque signing when mutability is unspecified.

The `intentMutability` field is OPTIONAL. When absent, wallets MUST display a "mutability not specified" warning alongside the rendered intent, or downgrade to opaque signing. App developers SHOULD include this section for every descriptor. Absence is treated as "unknown," not as "stable."

The field has two sub-keys:
- `contract` — applies to every entry in `display.formats` unless overridden
- `functions` — per-function overrides, keyed by the same signature strings used in `display.formats`

If a function has an entry under `functions`, that entry REPLACES the `contract` entry for that function. Merging is NOT performed.

**Mutability entries (`contract` and each `functions[<signature>]`)**

| Field | Required | Description |
|-------|----------|-------------|
| `stable` | Conditional | Boolean. When `true`, the author asserts that nothing on-chain or off-chain can change the displayed intent. When `false` or omitted, at least one of `stateRefs` or `notes` MUST be provided. |
| `stateRefs` | Conditional | Array of `StateRef` objects describing storage slots whose values bound the correctness of the displayed intent. |
| `notes` | Conditional | Free-form string describing mutability vectors that cannot be expressed as a single storage slot (e.g., off-chain oracle endpoints, composition with externally-controlled contracts, deployment-time library linkage). |

Validation rules:
- `stable: true` ⇒ `stateRefs` MUST be empty or absent AND `notes` MUST be absent.
- `stable: false` (explicit or implicit) ⇒ at least one of `stateRefs` or `notes` MUST be present. A bare `stable: false` with no explanation is invalid.
...
Wallets implementing ERC-7730 MUST:
- For every `StateRef` applicable to the function being signed, read the live slot value and compare it to `expectedValue`. When `mask` is present, the comparison is `(liveValue & mask) == (expectedValue & mask)`; otherwise the full slot value is compared. On mismatch, the wallet MUST either (a) refuse to apply the descriptor's formatting and downgrade to opaque signing, or (b) display a clear warning that the on-chain state differs from the audited baseline.
- When `intentMutability` is absent from the descriptor, display a "mutability not specified" warning alongside the rendered intent, or downgrade to opaque signing.
- When `stable: true` is declared (with no `stateRefs`), perform no live-state verification for that scope.
...
*Immutable contract (no admin, no proxy, no state-dependent branches):*
...
*Standard [EIP-1967](./eip-1967.md) proxy:*
            ...
            "stateRefs": [
                {
                    ...
                    "expectedValue": "0x000000000000000000000000abcdef0123456789abcdef0123456789abcdef01",
                    "description": "EIP-1967 implementation slot. If this changes, the proxy points to new code and any intent in this descriptor may no longer apply."
                }
            ]
        }
    }
...
*State-dependent branch (a function whose behavior depends on a mutable flag):*
                ...
                "stateRefs": [
                    {
                        ...
                        "description": "Behavior branches on slot 0 (variable `a`). When a == 1, the function deposits collateral; when a != 1, the function transfers caller funds to the owner. The owner can change `a` at any time."
                    }
                ]
            }
        }
...
*Mostly-stable contract with one mutable function:*
                ...
                "stateRefs": [
                    {
                        ...
                        "description": "Pause flag at slot 3. Non-zero means redemptions revert; the owner can flip it without notice."
                    }
                ]
            }
        }
...
*Packed slot (only one variable in the slot is relevant):*

Solidity packs multiple variables smaller than 32 bytes into a single storage slot, so the full slot value will change when any packed variable changes — including ones unrelated to the displayed intent. Use `mask` to isolate the bits that actually bound the intent.
...
*Off-chain mutability (cannot be expressed as a storage slot):*
...
```json
{
    "intentMutability": {
        "functions": {
            "swap(uint256 amount)": {
                "stable": false,
                "notes": "Swap rate is fetched at execution time from an off-chain oracle at https://oracle.example.com. The owner can rotate the oracle endpoint without any on-chain state change, so no storage slot can witness the change."
            }
        }
    }
}
...
The `intentMutability` section addresses these cases by requiring authors to declare the storage slots whose values bound the descriptor's correctness, along with their expected values at authoring time. Wallets verify expected values against live state before applying the descriptor, so that any change to a declared slot fails safely.

We considered placing this information in a separate auditor-produced report rather than in the descriptor itself. We chose the descriptor for three reasons: (a) wallets need a self-contained, fast-to-verify signal on every signing rather than an additional registry fetch; (b) the `(slot, expectedValue)` verification is local and stateless, with no extra trust assumptions beyond the descriptor itself; (c) the protocol author is closest to the contract and is the cheapest place to source the slot list. Auditor attestations remain the trust anchor: they verify the declared `stateRefs` are exhaustive and accurate. Standalone auditor-published mutability reports are a possible future extension and are out of scope for this specification.

`expectedValue` is required whenever `slot` is present. This makes the descriptor self-verifying at the cost of requiring re-publication when a legitimate state change occurs (for example, a proxy upgrade or an admin parameter change). We consider this the correct behavior: a change to a declared slot is exactly when the descriptor's truth claim has changed, and re-publication with a fresh auditor attestation is what surfaces the change to wallets and users.

Absence of `intentMutability` is treated as "unknown," not "stable." Treating absence as "stable" would silently grant existing descriptors a trust label they were not authored to bear. Treating absence as "unknown" creates pressure on authors to declare mutability explicitly.

The optional `mask` on a `StateRef` accommodates Solidity's storage packing. Variables smaller than 32 bytes share a slot, so the raw slot value changes whenever any packed neighbour changes — even neighbours that have no effect on the displayed intent. Without `mask`, authors would be forced to either re-publish the descriptor on every unrelated write or declare more than the truth claim warrants. With `mask`, authors pin exactly the bits that bound the intent and ignore the rest. The masked comparison is `(liveValue & mask) == (expectedValue & mask)`, so bits outside the mask are ignored on both sides; we do not require `expectedValue` to be canonicalized (zeroed outside the mask) because doing so would add a validation step without changing the comparison result.

The escape-hatch `notes` field exists because not every mutability vector can be reduced to a single storage slot — for example, off-chain oracle endpoints the protocol can rotate, or composition with externally-controlled contracts whose addresses can themselves be re-targeted. `notes` preserves expressiveness for these cases while keeping the primary mechanism machine-verifiable.
...
A descriptor's `intent` and `fields` describe a function as it exists at authoring time. The executed behavior of that function can later diverge from the displayed intent in ways that the descriptor's format string alone cannot detect. Concrete classes of divergence include:

- **Implementation upgrade.** A proxy contract's implementation slot is replaced; the function selector and parameter layout stay the same, but the executed bytecode is entirely different.
...
- **Admin-controlled parameter.** A fee, pause flag, allowlist, or routing address is changed by the contract owner or governance, altering which actions the function takes for a given input.
- **External dependency change.** The function calls into another contract whose address is stored in mutable state, or relies on an oracle whose value or endpoint can be rotated.

Detecting these divergences is intractable from the descriptor's formatting alone: the displayed intent looks correct because the abstract syntax of the call is unchanged.

The [`IntentMutability` section](#intentmutability-section) above requires authors to declare the storage slots whose values bound the descriptor's correctness, with the values witnessed at authoring time. Wallets verify those values against live state on every signing; on mismatch they downgrade to opaque signing or warn the user. This converts an unbounded "what could the contract do over time" question into a bounded "do these slots still hold the values the author signed off on" check.

Authors do not always disclose every mutability vector, whether by oversight or omission. Auditor attestations are the trust anchor for this disclosure: an auditor verifies that the descriptor's `stateRefs` are exhaustive and that each `expectedValue` matched live state at the time of attestation. Wallets are encouraged to prefer descriptors that carry an auditor attestation. Treatment of conflicts between an auditor's findings and a descriptor's self-declaration is out of scope for this specification.

When `intentMutability` is absent from a descriptor, the descriptor is making no claim about mutability. Wallets do not treat absence as `stable: true`; they warn the user or downgrade to opaque signing (see the [`IntentMutability` section](#intentmutability-section) for normative wallet behavior).
...
Upgrades that preserve the ABI but change function semantics (the more common case) are not detectable from the descriptor's formatting alone. For these, developers SHOULD declare the proxy's implementation slot — for EIP-1967 proxies, slot `0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc` — under the descriptor's `intentMutability.contract.stateRefs`, with `expectedValue` set to the implementation address at authoring time. Wallets that verify `(slot, expectedValue)` will detect any implementation change and treat the descriptor as stale until it is re-published and re-attested. See the [`IntentMutability` section](#intentmutability-section).

...
description: Draft specification of a json format providing additional description of how to clear-sign smartcontract calls and EIP-712 messages
...
The [ERC-7730](https://github.com/ethereum/ERCs/blob/master/ERCS/erc-7730.md) specification enriches type data contained in the ABIs and schemas of structured messages (structures like the calldata of an EVM transaction or an [EIP-712](https://eips.ethereum.org/EIPS/eip-712) message) with additional formatting information, so that wallets can construct a better UI when displaying the data before signature. For instance, a solidity field containing an amount, encoded as a uint256, can be converted to the right magnitude and appended with the correct ticker.
...
The following is a simple example of how to clear sign a `transfer` function call on an [ERC-20](https://eips.ethereum.org/EIPS/eip-20) contract.
...
In the future, it could be extended to structured data like Meta Transaction in [EIP-2771](https://eips.ethereum.org/EIPS/eip-2771) or User Operations in [EIP-4337](https://www.erc4337.io/docs/understanding-ERC-4337/user-operation).
...
* `chainID`: an [EIP-155](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md) identifier of the chain the message is bound to 

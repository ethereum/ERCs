...
description: Draft specification of a json format providing additional description of how to clear-sign smartcontract calls and EIP712 messages on any wallets
...
The ERC 7730 specification enriches type data contained in the ABIs and schemas of structured messages (structures like the calldata of an EVM transaction or an [EIP-712](https://eips.ethereum.org/EIPS/eip-712) message) with additional formatting information, so that wallets can construct a better UI when displaying the data before signature. For instance, a solidity field containing an amount, encoded as a uint256, can be converted to the right magnitude and appended with the correct ticker.
...
A deployment option is an object with:
  * `chainId`: an [EIP-155 identifier](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md) of the chain the descibed contract is deployed on.
...
A wallet MUST verify that each key-value pair in this `domain` binding matches the values of the `domain` key-value pairs of the message. Note that the message can have more keys in its `domain` than those listed in the ERC 7730 file. An EIP-712 domain is a free-form list of keys, but those are very common to include:
    ...
    * `chainID`: an [EIP 155 identifier](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md) of the chain the message is bound to 

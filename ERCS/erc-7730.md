...
Instead, they SHOULD return raw native balance and storage slot mutations as `{ contract, slot, old_value, new_value, derivation_path }` objects across every contract a transaction touches with a storage write.

#### `storageLayout`

The layout files are relatively large and are kept in a separate file to avoid bloating the main ERC-7730 specification.
The optional top-level `storageLayout` parameter provides a URI reference pointing to the storage layout file for this contract:
...
The referenced file MUST conform to [ERC-XXXX: Contract Storage Layout Descriptor Format](./eip-XXXX.md), which defines the full file format including compiler identification, storage entries, type definitions, slot derivation formulas for all supported languages and encodings, `baseSlotDerivation` for namespaced and diamond storage patterns, and optional `display` annotations using this ERC's formatter vocabulary.
...
4. Format the extracted value using the entry's `display` annotation per [ERC-XXXX](./eip-XXXX.md).
...
2. Wallet re-derives the final slot from the base slot and the keys in the derivation path, using the Slot Derivation Formulas defined in [ERC-XXXX](./eip-XXXX.md).
3. Wallet asserts the computed slot equals the reported slot.
4. Displays the formatted change using the entry's `display` annotation.

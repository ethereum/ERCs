...
Translations are provided through links to translation files, declared in the top-level `$i18n` field of the parent document.

"Clear signing" deeply depends on the information and descriptions presented to the users being genuinely clear to them.
The required languages are selected to be familiar to the absolute majority of internet users worldwide.

ERC-7730 files MUST provide translations for at least the following languages:
...
An object mapping [BCP-47](https://www.rfc-editor.org/rfc/rfc5646) language tags to URI references pointing to translation files.

#### Translation file format

A translation file is a separate JSON document with the following fields:

| Field          | Required | Description                                               |
|----------------|----------|-----------------------------------------------------------|
| `$schema`      | Yes      | Translation files have a separate schema.                 |
| `$locale`      | Yes      | BCP-47 language tag.                                      |
| `translations` | Yes      | Flat map from English source string to translated string. |

The `translations` map uses exact parent document English strings as keys and their translations as values:
...
```json
{
    ...
    "translations": {
        "Send":                  "Envoyer",
        "Send {value} to {to}":  "Envoyer {value} à {to}",
        "To":                    "Destinataire",
        "Amount":                "Montant"
    }
}
...
See [`example-main.fr.json`](../assets/erc-7730/example-main.fr.json) for a complete example, and [`erc7730-i18n-v3.0.0-next.schema.json`](../assets/erc-7730/erc7730-i18n-v3.0.0-next.schema.json) for the JSON schema of the translation file format.

#### Lookup semantics

Wallets MUST apply the following procedure:
...
2. If a matching locale is listed in `$i18n`, fetch the corresponding translation file.
3. For each user-facing string field in the ERC-7730 document, look up the localized string in `translations`. If not found, the wallet SHOULD present a clear warning about missing translations and fall back to using the English original.

Note that `interpolatedIntent` strings contain field-value placeholders in the form `{fieldPath}`.
These MUST appear verbatim in the translation.
Word order may be rearranged around placeholders to suit the target language.
Validators MUST check that every placeholder present in the English key also appears in the translation value, for example:

```
"You are providing {amount} as liquidity to {poolName}": "Ви надаєте ліквідність до {poolName} на суму {amount}"
```

#### Translator notes text metadata

The English string and translation file MAY use text metadata of the form `"{englishString}|{translatorNote}"` to provide translator notes and avoid ambiguity (for example, differentiating a noun from a verb).
To use this feature, the parent document MUST include the `|` separator and translator note directly in its English field values.
Wallets MUST NOT render the text appearing in string labels after the `|` separator in the UI, both when falling back to English and when rendering translated text.

For example, the parent document:

```json
{
    "label": "Transfer|noun",
    "intent": "Transfer|verb"
}
```
...
{
    "translations": {
        "Transfer|noun":  "Переказ коштів",
        "Transfer|verb":  "Переказати"
    }
}
```

#### Translation Keys and Context Rationale

Unlike some localization frameworks that use abstract IDs (e.g., `action.transfer`) or support dedicated context fields (like gettext's `msgctxt`), ERC-7730 uses the natural English strings themselves as the primary keys in a flat JSON map. This design choice maintains schema readability and significantly lowers the barrier to entry for developers authoring the file. 

However, this creates a challenge when identical English strings have different meanings depending on context (e.g., "Transfer" as a noun vs. a verb). To solve this without adding complex secondary keys to the JSON schema, this specification adopts the historical pattern of appending context directly to the string using a pipe delimiter (`|`), which is stripped by the wallet at runtime. This allows exact string matching in flat JSON files while fully resolving contextual ambiguity for translators.

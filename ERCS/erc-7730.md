...
All display strings in ERC-7730 files—including intents, labels, field names, and enumeration values—are expected to be authored in a single language, typically English. This decision was made for several reasons:

* **Complexity reduction**: Supporting multiple languages would significantly increase the complexity of authoring and maintaining ERC-7730 files. Each file would need to include translations for all display strings, or reference external translation resources, adding substantial overhead.
* **Implementation burden**: Wallet implementations would need to support language selection, fallback mechanisms, and validation of translated strings, increasing development and testing costs.
* **Developer audience**: ERC-7730 files are primarily authored by application developers and contract owners, who typically work in English and are accustomed to English-language technical specifications.
* **Consistency and validation**: A single-language approach ensures consistency across wallets and simplifies validation of ERC-7730 files. Multiple languages could introduce ambiguities or translation errors that might affect security.

Wallets MAY implement their own translation layer on top of ERC-7730 display strings if they choose to provide localized experiences for their users. However, such translations are outside the scope of this specification and are the responsibility of individual wallet implementations.

...
description: An interface for permissionless, operator-diverse validator networks plugging into ERC-8004's Validation Registry
...
discussions-to: https://ethereum-magicians.org/t/erc-8294-validation-network-interface-extension-for-erc-8004/28669
status: Draft
...
```json
{
  "schema": "vni-aggregated-response/v1",
  "requestHash": "0x...",
  "policy": "0x...",
  "validators": ["0x...", "0x..."],
  "attestations": [
...
The `schema` field identifies the aggregated-response schema version. Networks MAY use a more specific namespaced schema value for extension documents, but generic clients MUST recognize `vni-aggregated-response/v1` for this revision.
...
In all three modes, the aggregated verdict is exactly 0 or 100. Spectrum verdicts (e.g., a literal mean of received verdicts) are deferred to a follow-on extension.
...
**Why `agentId` stays `uint256`.** This interface keeps `agentId` as ERC-8004's `uint256` (the [ERC-721](./eip-721.md) `tokenId` assigned by the Identity Registry). Composition with related agent-identity schemes happens at the application or registry layer — VNI does not need to redefine the identifier type to accommodate alternative resolution paths. The `wyriwe-input-provenance-v1` challenge kind above and the `verificationProfile()` introspection surface together provide the hooks for compositions of this kind.
...
- **Spectrum verdicts as a follow-on.** v1 is binary-only by design. Should a follow-on extension define `verdictMode = mean` (or weighted-mean) plus the tag and timeout semantics needed to keep success and failure cases distinguishable? Current lean: yes, but only after at least one pilot demonstrates a use case where binary loses meaningful information.

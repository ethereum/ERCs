...
After adding/replacing/removing functions the `_functionCall` argument is executed with `delegatecall` on `_init`. Typically this execution is used to set up or remove state after adding, replacing, or removing functions
...
Immutable functions complicate the correct implementation of diamonds and make them less flexible. This standard discourages their use.
...
2. Don't have any immutable functions.
...
- Discourages immutable functions.

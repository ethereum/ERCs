...
If the fallback function cannot find a facet for a function selector, and there is no default function or other mechanism to handle the call, the contract MUST revert with the error `FunctionNotFound(bytes4 _selector)`.

...
Immutable functions may offer minor gas savings by avoiding a storage read and a `delegatecall`. However, they introduce an additional implementation pattern alongside facets, resulting in two ways to provide similar functionality. This increases implementation complexity and cognitive overhead.

In upgradeable diamonds, immutable functions also reduce flexibility, as they cannot be replaced or removed. This can limit the ability to evolve or improve functionality over time.

Additionally, immutable functions that read from or write to storage are not isolated from upgrades. Other functions, including upgrade logic, may modify the same storage relied upon by immutable functions. 

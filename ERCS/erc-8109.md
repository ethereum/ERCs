...
When the standard `upgradeDiamond` function defined in this specification is implemented, emission of `DiamondDelegateCall` for its `delegatecall` step is REQUIRED. Otherwise this event is OPTIONAL.
...
When the standard `upgradeDiamond` function defined in this specification is implemented, emission of `DiamondMetadata` for its metadata step is REQUIRED. Otherwise this event is OPTIONAL.
...
3. **Function Event Emission**
   - Every change to a function selector MUST be recorded by emitting the corresponding event:
     - `DiamondFunctionAdded`
     - `DiamondFunctionReplaced`
     - `DiamondFunctionRemoved`
...
7. **DelegateCall Execution**
   - If `_delegate` is non-zero, the diamond MUST execute `_functionCall` on `_delegate` using `delegatecall`.
   ...
   - `_functionCall` MAY be empty. If empty, the `delegatecall` executes with no calldata.
8. **Metadata Event**
   - If `_tag` is non-zero or `_metadata.length > 0`, the diamond MUST emit the `DiamondMetadata` event.

After adding/replacing/removing functions a `delegatecall` can be done to initialize, modify, or remove diamond state after an upgrade.
...
1. Use a constructor in a diamond to add functions and initialize variables.
...
### [ERC-8109 Diamonds](./eip-8109.md) vs ERC-2535 Diamonds
...
If the ERC-2535 diamond upgrade function is immutable, then it can't be removed. If possible, disable the upgrade function by making its authentication always fail, or make it revert in some other way.

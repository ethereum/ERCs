...
description: "A simplified diamond architecture for modular smart contract systems. Reducing complexity by isolating, organizing, testing, and managing distinct areas of on-chain functionality.
...
![Diagram showing how a diamond contract works](../assets/erc-8109/basic-diamond-diagram.svg)

Diamond contracts were originally standardized by [ERC-2535](./eip-2535). This standard refines that specification by simplifying terminology, reducing the implementation complexity of introspection functions, and standardizing events that are easier for block explorers, indexers, and other tooling to consume. 
...
Additional motivation for diamond-based smart contract systems can be found in [ERC-1538](./eip-1538) and [ERC-2535](./eip-2535).
...
![Diagram showing structure of a diamond](../assets/erc-8109/functionFacetMapping.svg)
...
The reference implementation for this upgrade function is here: [DiamondUpgradeFacet.sol](../assets/erc-8109/DiamondUpgradeFacet.sol).
...
A reference implementation exists for these two introspection functions here: [DiamondInspectFacet.sol](../assets/erc-8109/DiamondInspectFacet.sol)
...
Software can retrieve function selectors and facet addresses from a diamond in order to use and show what functions a diamond has. Function selectors and facet addresses combined with ABIs of contracts provide enough information to be useful for various software.

### [ERC-8109 Diamonds](./erc-8109.md) vs ERC-2535 Diamonds
...
- Standardized events that are easier for indexers to consume.
- Optional upgrade function with metadata support.
- Disallows immutable functions.
- Optional upgrade path for existing ERC-2535 diamonds.

The new events also make it simpler to implement different upgrade functions or functionality that modifies the functions and facets used by a diamond.

The new and optional `DiamondMetadata(bytes32 indexed _tag, bytes _data)` event provides a standard way to record versioning information about diamonds.

### Gas Considerations
Delegating function calls has a small amount of gas overhead. This is mitigated in several ways:

1. Because diamonds do not have a max size limitation it is possible to add gas optimizing functions. For example someone could use a diamond to implement the [ERC-721](./eip-721) standard and implement batch transfer functions to reduce gas (and make batch transfers more convenient).
2. Some contract architectures require multiple external function calls in the same transaction. Gas savings can be realized by condensing contracts into a single diamond and accessing contract storage directly. This saves gas by reducing external function calls.
...
Diamonds and facets need to use a storage layout organizational pattern because Solidity’s default storage layout doesn’t support proxy contracts or diamonds. The particular technique of storage layout to use is not specified in this ERC. However, examples of storage layout patterns that work with diamonds are [ERC-8042 Diamond Storage](./eip-8042) and [ERC-7201 Namespaced Storage Layout](./eip-7201).
...
Immutable functions are intentionally excluded from ERC-8109 to keep the function–facet model uniform and to simplify implementations.

Immutable functions are not allowed in ERC-8109 diamonds.

However, to enable existing ERC-2535 diamonds to upgrade to this standard, ERC-8109 permits a compatibility mode for diamonds that contain immutable functions.
...
- The reference implementation for the `upgradeDiamond` function is here: [DiamondUpgradeFacet.sol](../assets/erc-8109/DiamondUpgradeFacet.sol).
- The reference implementation for the `facetAddress(bytes4 _functionSelector)` and `functionFacetPairs()` introspection functions is here: [DiamondInspectFacet.sol](../assets/erc-8109/DiamondInspectFacet.sol)
- An example implementation of an ERC-8109 diamond is here: [DiamondExample.sol](../assets/erc-8109/DiamondExample.sol)

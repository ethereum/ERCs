...
Routing calls via `delegatecall` introduces a small amount of overhead. In practice, this cost is mitigated by several architectural and tooling advantages enabled by diamonds:
...
2. **Reduced external call overhead**  
   Some contract architectures require multiple external calls within a single transaction. By composing related functionality behind a single diamond address, these interactions can be performed through internal execution paths that share storage, reducing gas costs by avoiding external calls.

4. **Selective optimization per facet**  

...
Ethereum has the concept of keystores: pieces of data that define a key (see [EIP-2335](./erc-2335.md) for details).  This adds the concept of walletstores: stores that define wallets and how keys in said wallets are created.
...
This element MUST be present.  It MUST be an object that follows the definition described in [EIP-2335](./erc-2335.md).

### Next Account

The `nextaccount` provided is the index to be supplied to the path `m/12381/60/<index>/0` when creating a new private key from the seed.  The path follows [EIP-2334](./erc-2334.md).

This element MUST be present if the wallet type requires it.  It MUST be a non-negative integer.

### JSON schema

The walletstore follows a similar format to that of the keystore described in [EIP-2335](./erc-2335.md).

...
description: An interface for tokens that suppose to maintain a peg to an underlying token while being only partially redeemable to this token.
...
Hence, the interest rate **SHOULD** be monotonically increasing with the current reserve ratio.

...
This proposal introduces a protocol that establishes a unified interface for managing both [ERC-20](./erc-20.md) fungible tokens and [ERC-721](./erc-721.md) non-fungible tokens (NFTs) on the Ethereum blockchain. By defining a common set of functions applicable to both token types, developers can seamlessly interact with [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) tokens using a single interface. This simplifies integration efforts and enhances interoperability within decentralized applications (DApps).


## Motivation

The proposal aims to address the demand for assets combining the liquidity of [ERC-20](./erc-20.md) tokens and the uniqueness of [ERC-721](./erc-721.md) tokens. Current standards present a fragmentation, requiring users to choose between these features. This proposal fills that gap by providing a unified token interface, enabling smooth transitions between [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) characteristics to accommodate diverse blockchain applications.

## Specification

- Introduces a token contract that combines features from both [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) standards.
- Supports state transitions between [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) modes, facilitating seamless conversion and utilization of both liquidity and non-fungibility.
- Defines essential functions and events to support token interactions, conversions, and queries.
- Implements low gas consumption [ERC-20](./erc-20.md) mode to maintain efficiency comparable to typical [ERC-20](./erc-20.md) token transfers.
...
- Introduces a unified interface to address the fragmentation caused by separate [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) standards.
...
- Includes transferFrom function for seamless movement of tokens between addresses, as it's a core component of both [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) standards.
...
- Provides functions like balanceOf and ownerOf for retrieving token balances and ownership information, crucial for both [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) tokens.

Compatibility and Extensibility:

- Ensures compatibility with existing [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) implementations, minimizing disruption during transition.
...
The proposed this proposal introduces a challenge in terms of backward compatibility due to the distinct balance query mechanisms utilized by [ERC-20](./erc-20.md) and [ERC-721](./erc-721.md) standards. [ERC-20](./erc-20.md) employs `balanceOf` to check an account's token balance, while [ERC-721](./erc-721.md) uses `balanceOf` to inquire about the quantity of tokens owned by an account. To reconcile these differences, the ERC must consider providing either two separate functions catering to each standard or adopting a more generalized approach.

### Compatibility Points

The primary compatibility point lies in the discrepancy between [ERC-20](./erc-20.md)'s balanceOf and [ERC-721](./erc-721.md)'s balanceOf functionalities. Developers accustomed to the specific balance query methods in each standard may face challenges when transitioning to this proposal.

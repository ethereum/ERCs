...
**The multi-metadata extension is OPTIONAL for [ERC-721](./erc-721.md) contracts and it is RECOMMENDED to be used in conjunction with the [ERC-4906](./erc-4906.md) standard if implemented**.
...
Implementing functionality to add or remove uris to a token MUST be implemented separately from this standard. It is RECOMMENDED that one of the event defined in [ERC-4906](./erc-4906.md) are emitted whenever uris are added or removed.
...
An open-source reference implementation of the `IERC721MultiMetadata` interface can be provided, demonstrating how to extend an existing [ERC-721](./erc-721.md) contract to support multi-metadata functionality. This reference implementation can serve as a guide for developers looking to implement the extension in their own contracts.

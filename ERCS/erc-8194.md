...
allows downstream task market protocols to support human↔agent, agent↔agent, and human↔human task interactions
under a single interface.

PGTR is a new primitive. It is not a profile of [ERC-2771](./eip-2771.md). ERC-2771 solves gas abstraction — the
...
[ERC-4337](./eip-4337.md)) presuppose that the principal can produce a valid cryptographic signature. This assumption
...
and that forwarders implement to be detectable by [ERC-165](./eip-165.md).
...
1. **Payment** — The payer transfers tokens to the forwarder (or to the forwarder's designated
   escrow) as an atomic [ERC-3009](./eip-3009.md) `transferWithAuthorization` or equivalent mechanism.

...
The registration function MUST verify the BLS12-381 signature using the BLS12_PAIRING_CHECK precompile ([EIP-2537](./eip-2537.md), address `0x0f`). The signed message is:
...
On success the registration function MUST emit `BuilderRegistered` and MUST either add the builder to the enumerable list (first registration) or update its stored record in place (subsequent registrations).

The deregistration function MUST verify the BLS12-381 signature in the same manner as registration. The signed message is:

```
message = abi.encode(block.chainid, address(this), pubkey, nonce)
```

On success the deregistration function MUST emit `BuilderDeregistered` and MAY remove the builder from the enumerable list or mark as deregistered.
...
`builderCount` MUST return the number of builders in the list.

`getBuilderAtIndex` MUST return the builder record at position `index`, and MUST revert if `index >= builderCount()`.
...
interface IBuilderList {
    ...
    function getBuilderAtIndex(uint256 index)
...
Each registration or deregistration MUST be accompanied by a BLS12-381 signature over the corresponding message fields (`abi.encode(chainid, contractAddress, pubkey, fqdn, nonce)` for registration, `abi.encode(chainid, contractAddress, pubkey, nonce)` for deregistration). The chain ID prevents a valid registration on one network from being replayed on another. The contract address prevents replay across different registry deployments on the same chain. The nonce — which SHOULD NOT be reused for the same public key — prevents replay of any prior registration transaction.

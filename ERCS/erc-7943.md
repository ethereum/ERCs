...
The following defines the standard interfaces for an [ERC-7943](./erc-7943.md) token contract, which MUST extend from one base token interface such as [ERC-20](./erc-20.md), [ERC-721](./erc-721.md), [ERC-1155](./erc-1155.md), or [ERC-6909](./erc-6909.md). Note that [ERC-6909](./erc-6909.md)-based implementations can use the multi token interface.
...
- `canTransfer` is a transfer-level authorization check that evaluates whether a specific transfer is permissible under permissioned compliance rules. This function:
    ...
    - MUST perform a `canSend` check on the `from` parameter and a `canReceive` check on the `to` parameter. Note that [ERC-3643](./erc-3643.md) does not perform this check within `canTransfer` as this standard requires.
    ...
    - MUST NOT return false based on token-specific, non-permissioned validations such as balance or allowance checks. These checks belong to the underlying base token standard (e.g., [ERC-20](./erc-20.md), [ERC-721](./erc-721.md), [ERC-1155](./erc-1155.md)).

- `getFrozenTokens` will return the absolute frozen amount, which MAY exceed the account's current balance. In [ERC-721](./erc-721.md) tokens, it MAY return true even if the account does not hold the token.
...
- In single-party permissioned contexts:
    - It MAY bypass the `canTransfer` checks. If this happens, and the transfer involves tokens that are currently counted as frozen, it MUST unfreeze the assets first and emit a `Frozen` event before the underlying base token transfer event reflecting the change. Having the unfrozen amount changed before the actual transfer is critical for tokens that might be susceptible to reentrancy attacks doing external checks on recipients, as is the case for [ERC-721](./erc-721.md) and [ERC-1155](./erc-1155.md) tokens.
...
- **Compatibility**: Designed as an interface layer compatible with existing base standards like [ERC-20](./erc-20.md), [ERC-721](./erc-721.md), [ERC-1155](./erc-1155.md), and [ERC-6909](./erc-6909.md). Implementations extend from [ERC-7943](./erc-7943.md) alongside their base standard interface.
...
3) Developers can also perform several operations through the use of `multicall` patterns similar to the one defined in [ERC-6357](./erc-6357.md) so that a mix of the given primitives with additional features can be batched in one transaction:
...
- **Front-running of the `forcedTransfer` and `setFrozenTokens` functions**: Both functions are susceptible to front-running, similar to the `approve` function of [ERC-20](./erc-20.md). Furthermore, if the suggestion to allow freezing more than what an account owns is not followed, any account may be incentivized to front-run attempts to freeze its balance when receiving new funds. Additional features to gradually increment or decrement the frozen status MAY be considered for implementation.
- **Standard Contract Security**: Implementations MUST adhere to general smart contract security best practices (reentrancy guards where applicable, checks-effects-interactions, etc.). Specifically, in the checks-effects-interactions consideration, implementations need to be aware of tokens having hooks, especially on recipients as in [ERC-721](./erc-721.md) or [ERC-1155](./erc-1155.md). In such circumstances, it might be convenient to adopt reentrancy guards to prevent unwanted executions.

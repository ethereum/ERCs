...
The `ERC7943NotAllowedTransfer` and `ERC7943NotAllowedUser` errors CAN be used as a general revert mechanism whenever internal calls to `isUserAllowed` or `isTransferAllowed` return false. Those MIGHT NOT be used or MIGHT be replaced by more specific errors depending on the custom checks performed inside those calls.

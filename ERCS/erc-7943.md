...
- `canTransfer` is a transfer-level authorization check that evaluates whether a specific transfer is permissible under permissioned compliance rules. This function:
    ...
    - MUST perform a `canSend` check on the `from` parameter and a `canReceive` check on the `to` parameter. An important documentation note is that [ERC-3643](./eip-3643.md) doesn't perform this check within `canTransfer` as required.
...
- **Public transfers** (`transfer`, `transferFrom`, `safeTransferFrom`, etc.) MUST NOT succeed in cases in which `canTransfer` would return `false`, or in which `canSend` would return `false` for the `from` address, or `canReceive` would return `false` for the `to` address.
...
*   **Flexible compliance:** Provides standard view functions (`canSend`, `canReceive`, `canTransfer`, `getFrozenTokens`) for compliance checks without dictating *how* those checks are implemented internally by the token contract. This allows diverse compliance strategies.
...
Reference implementations of uRWA for [ERC-20](../assets/eip-7943/contracts/uRWA20.sol), [ERC-721](../assets/eip-7943/contracts/uRWA721.sol) and [ERC-1155](../assets/eip-7943/contracts/uRWA1155.sol) token implementations is provided in assets folder. They use the OpenZeppelin library and include separate send/receive whitelists and enumerable role-based access control. These examples are provided for educational purposes only and are not audited.

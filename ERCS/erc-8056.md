...
### Multiplier Authority and Accounting Isolation

A multiplier change can materially alter displayed holdings without changing ERC-20 accounting. Any mechanism that schedules, replaces, or otherwise changes the active or pending multiplier MUST be restricted to addresses authorized by the issuer's stated policy. Implementations SHOULD make multiplier changes and changes to that authority observable to integrators.

The active multiplier and any pending multiplier MUST be greater than zero. Scheduling, replacing, or activating a multiplier MUST NOT, by itself, modify ERC-20 raw balances, total supply, or allowances.

Once an implementation accepts a scheduled update, `newUIMultiplier()` and `effectiveAt()` MUST expose the accepted multiplier and activation timestamp until the update is activated or superseded. `uiMultiplier()` MUST return the multiplier active at the current block timestamp.

### Conversion Consistency and Arithmetic

For contracts implementing `IScaledUIAmountConversion`, conversions MUST use the active multiplier and round down as follows:

```text
toUIAmount(rawAmount) = floor(rawAmount * uiMultiplier() / 1e18)
fromUIAmount(uiAmount) = floor(uiAmount * 1e18 / uiMultiplier())
```

For every input whose mathematical result fits in `uint256`, conversion functions MUST return that result and MUST NOT revert solely because an intermediate product exceeds `uint256`. Implementations can use full-precision multiplication and division to satisfy this requirement.

For contracts implementing `IScaledUIAmountBalances`, `balanceOfUI(account)` MUST equal `toUIAmount(balanceOf(account))`, and `totalSupplyUI()` MUST equal `toUIAmount(totalSupply())`, when evaluated with the same active multiplier.

The conversion definitions above are monotonic. They also ensure that a raw-to-UI-to-raw round trip does not increase the raw amount. Integrators MUST account for the possible loss caused by rounding down and MUST NOT assume that either round trip is an identity operation.

### Integration and Oracle Risks

Integrators MUST clearly distinguish UI-adjusted amounts from raw ERC-20 amounts and MUST use raw amounts for ERC-20 transfers, approvals, and internal accounting.

An implementation that derives multiplier updates from an oracle MUST authenticate the oracle data and validate its freshness and expected bounds. It SHOULD define fail-safe behavior for stale, unavailable, or invalid data so that an oracle failure does not silently activate an unintended multiplier.

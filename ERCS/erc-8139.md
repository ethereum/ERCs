...
This ERC defines an **Authorization Object (AO)**: an [EIP-712](./eip-712.md) typed struct describing a *time-bounded, revocable permission* granted by a **grantor** to a **grantee**, within an application-defined **scope**.
...
This ERC defines **no execution semantics**. It does not specify when, how, or whether any action occurs.
...
This ERC specifies such a primitive and its hashing rules, so that:
...
- `scope`: application-defined scope identifier. This ERC treats `scope` as opaque.  
...
### Identity and Nonce Semantics

This ERC defines requirements on identity and replay behavior, but does not prescribe storage layout.
...
This ERC defines **requirements on revocation semantics**, but does not prescribe storage layout. Implementations MAY use:
...
### Optional Registry Interface (Recommended)

This ERC RECOMMENDS a minimal registry/observer interface to standardize on-chain publication and revocation of AO state.
...
This ERC is additive. It does not modify existing token standards or account models.

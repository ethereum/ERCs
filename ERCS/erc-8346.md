...
This specification is new and additive; the `$localization` field it governs is optional in ERC-7730. It supersedes the `erc7730-i18n-v3.0.0-next.schema.json` draft schema that briefly existed in ERC-7730's own assets directory, and the literal-English-string-as-key mechanism that existed briefly in ERC-7730's v3 draft before translation files were split out into this specification; neither shipped in a finalized ERC-7730 release, so no deployed descriptor or translation file requires migration.
...
See [`erc8346-v3.0.0-next.schema.json`](../assets/eip-8346/erc8346-v3.0.0-next.schema.json) for the translation file JSON schema, and [`example-main.fr.json`](../assets/eip-8346/example-main.fr.json) for a complete translation file referenced by the [ERC-7730](./eip-7730.md) example descriptor.

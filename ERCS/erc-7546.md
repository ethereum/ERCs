...
description: An upgradeable and cloneable proxy pattern.
...
## Abstract
This ERC is aimed at standardizing a design pattern in smart contract development with two key features:

### Function-Level Upgradeability
Like [ERC-2535](./erc-2535.md), this feature allows switching the implementation contract for each function call, enabling upgrades on a function-by-function basis. Also, by splitting implementation contracts by function, this approach reduces the impact of the contract size limit (24.576kB as of EVM version Shanghai or before).

### Factory/Clone-Friendly & Simultaneous Upgradeability
Similar to the Beacon concept from [ERC-1967](./erc-1967.md), this feature facilitates easy cloning and simultaneous upgrading of Proxy contracts, ensuring consistent behavior across different instances with individual state.

These features combine to create a more dynamic and adaptable framework for smart contract development, suitable for the needs of evolving decentralized applications.


## Motivation
Smart contract developers often face challenges when preparing for future functionality additions or addressing vulnerabilities in smart contracts. These challenges typically stem from the complexities of incorporating upgradeability and maintaining these contracts effectively.

This ERC focuses on enhancing the development and maintainability of contracts. It addresses common challenges in the evolution of smart contracts.

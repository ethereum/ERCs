...
We anticipate that there will be multiple EIP-7702 delegations from multiple major vendors. A standard for the execution interface will enable better interoperability. EIP-7702 delegation is a risky procedure which should be done sparingly -- it should not be performed for each time when a user switches websites. Also, EIP-7702 delegations are transactions that cost gas, making frequent re-delegation uneconomical. A standardized execution interface will reduce the need to switch delegations.
...
Using atomic batched executions reduces total latency and tranaction costs, making it preferable over sequential transaction sending for EOAs.

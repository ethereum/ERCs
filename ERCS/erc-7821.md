...
With the advent of [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702), it is possible for Externally Owned Accounts (EOAs) to perform atomic batched executions.
...
This standard complements the `wallet_sendCalls` API in [EIP-5792](https://eips.ethereum.org/EIPS/eip-5792). It enables the detection of atomic batch execution capabilities on EOAs and the preparation of the calldata for atomic batch executions on EOAs.
...
interface IERC7821 {
    ...
    ///   See: https://ercs.ethereum.org/ERCS/erc-7579
    ///
...
We have opted to use the `execute` and `supportsExecutionMode` functions in [ERC-7579](./erc-7579.md) for better compatibility with the existing smart account ecosystem.

While radical minimalism is the goal, some compromises have to be made in the pursuit for better adoption.

For minimalism, this standard does not require implementing the `executeFromExecutor` function in [ERC-7579](./erc-7579.md).
...
abstract contract ERC7821 {
    ...
    function _executionModeId(bytes32 mode) internal view virtual returns (uint256 id) {
        ...
        // For the encoding scheme, see: https://ercs.ethereum.org/ERCS/erc-7579
        // Bytes Layout:

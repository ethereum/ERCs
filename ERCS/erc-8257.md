...
Tool IDs are scoped to the `(chainId, registryAddress)` tuple. Two independent deployments of this registry (on the same or different chains) MAY assign the same tool ID to unrelated tools. Offchain consumers (indexers, wallets, agent frameworks) MUST qualify tool references with the deploying chain ID and registry address. The RECOMMENDED canonical string form is `eip155:<chainId>:<registryAddress>:<toolId>`, using the [CAIP-2](https://github.com/ChainAgnostic/CAIPs/blob/ebacdb90283ded501350bd0011db4362734f9c4b/CAIPs/caip-2.md) `eip155:<chainId>` chain prefix, lowercase hex `registryAddress`, and the decimal `uint256` `toolId`. This ERC does not register a CAIP-19 asset namespace; consumers needing a CAIP-19-style serialization MAY define their own namespace, but interoperable references SHOULD use the form above.
...
Canonical tool reference: `eip155:8453:0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa:1`.

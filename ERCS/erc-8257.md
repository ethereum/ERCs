...
This ERC targets EVM chains. The registry interface uses EVM-native types (`address`, `bytes32`, `uint256`), and tool IDs are scoped to the `(chainId, registryAddress)` tuple of an EVM deployment (see §1). Pricing entries carry their own chain identifiers via CAIP-19 `asset` and CAIP-10 `recipient`, so a tool registered on an EVM chain MAY price itself in assets on any CAIP-10/CAIP-19 namespace; concrete guidance in §3 focuses on `eip155:*` because that is the namespace the reference implementation has been exercised against.
...
| --- | --- | --- |
| `type` | string | Schema version identifier (e.g., `"https://ercs.ethereum.org/ERCS/erc-8257#tool-manifest-v1"`) |
...
- An ERC-20 or [ERC-721](./eip-721.md) `balanceOf` check plus a comparison runs in roughly 5,000-10,000 gas.
...
This appendix pins reference values so that implementations can be checked byte-for-byte against a known-good producer. Vectors are generated by `scripts/generate-test-vectors.ts` in the companion repository at `github.com/ProjectOpenSea/tool-registry`; any conformant JCS (RFC 8785) + keccak256 pipeline MUST reproduce the hashes below when fed the listed manifests.
...
Canonical CAIP-19 tool reference: `eip155:8453/erc8257:0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/1`.

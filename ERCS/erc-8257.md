...
- **Direct callers** (wallets and agent frameworks invoking `hasAccess` from off-chain or via `eth_call`) supply gas through the normal RPC budget. A predicate that exhausts the budget reverts; the caller treats this as "access denied" and MAY retry with a larger budget.

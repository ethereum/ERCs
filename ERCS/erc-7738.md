...
[ERC-5169](./erc-5169.md) provides a client script lookup method for contracts. This requires the contract to have implemented the [ERC-5169](./erc-5169.md) interface at the time of construction (or allow an upgrade path).
...
The contract SHOULD order the `scriptURI` returned so that the [ERC-173](./erc-173.md) `owner()` of the contract's script entries are returned first (in the case of simple implementations the wallet will pick the first `scriptURI` returned).
...
This method allows contracts written without the [ERC-5169](./erc-5169.md) interface to associate scripts with themselves, and avoids the need for a centralised online server, with subsequent need for security and the requires an organisation to become a gatekeeper for the database.

## Test Cases

Test cases are included in [NFTRegistryTest.test.ts](../assets/erc-7738/test/NFTRegistryTest.test.ts). Contracts, deployment scripts and registry script can be found alongside the test script.

Clone the repo and run:

```shell
cd https://ercs.ethereum.org/assets/erc-7738
...
[Create Registry Entry](../assets/erc-7738/scripts/createRegistryEntry.ts)
...
1. The target contract which the setter specifies implements the [ERC-173](./erc-173.md) `Ownable` interface. Once the script is fetched, the signature can be verified to match the Owner(). In the case of TokenScript this can be checked by a dapp or wallet using the TokenScript SDK, the TokenScript online verification service, or by extracting the signature from the XML, taking a keccak256 of the script and ecrecover the signing key address.

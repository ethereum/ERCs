...
discussions-to: https://ethereum-magicians.org/t/erc-8338-token-bound-executable-skills/29005
status: Draft
...
**Relationship to skill and tool registries.** An ERC-721-based agent skill registry has separately been proposed (in draft review at the time of writing) defining skill-manifest commitments and agent-skill installation and usage attestations. [ERC-8257](./eip-8257.md) defines a mapping-based Agent Tool Registry with canonical tool metadata, creator and origin binding, and optional predicate-based access control.
...
Because that registry proposal represents each registered skill as an ERC-721 token, such a registry can also implement this interface for the same token identifier. In that composition, the registry supplies registration, discovery, installation, and attestation semantics, while this proposal supplies package identity, per-file integrity, version chaining, publication authority, freezing, and optional confidentiality. ERC-8257 does not require ERC-721 representation; an ERC-8257 tool manifest can instead reference a token-bound executable artifact as an external artifact dependency.

A registry manifest hash, an ERC-8257 metadata hash, and this proposal's `packageHash` commit to different objects and are not interchangeable.
...
Provided in the assets of this proposal: Solidity reference contracts (`ISkillToken`, `IOnchainSkillDocument`, and a self-contained `SkillToken`); the two ERC-165 interface identifiers (compiler-verified: `ISkillToken = 0x734553a6`, `IOnchainSkillDocument = 0x7050dd2c`); JSON Schemas for the manifest and the confidentiality descriptor; path-validation negative tests; a Foundry suite asserting every MUST clause, including the on-chain-document atomic update; and six frozen test vectors for the deterministic encoding — public v1, a v2 update changing only a companion file (constant `mdHash`), a confidential package, an encrypted key-rotation update, a license-encrypted package, and a custom primary-document path — so independent implementations can be checked byte-for-byte. A zero-dependency packer/verifier implementing the deterministic encoding ships with the canonical repository referenced in the assets.

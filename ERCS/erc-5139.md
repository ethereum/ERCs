...
## Abstract
This proposal specifies a JSON schema for describing lists of remote procedure call (RPC) providers for Ethereum-like chains, including their supported [EIP-155](https://eips.ethereum.org/EIPS/eip-155) `CHAIN_ID`.

## Motivation
The recent explosion of alternate chains, scaling solutions, and other mostly Ethereum-compatible ledgers has brought with it many risks for users. It has become commonplace to blindly add new RPC providers using [EIP-3085](https://eips.ethereum.org/EIPS/eip-30857) without evaluating their trustworthiness. At best, these RPC providers may be accurate, but track requests; and at worst, they may provide misleading information and frontrun transactions.
...
List versioning MUST follow the [Semantic Versioning 2.0.0](../assets/erc-5139/semver.md) (SemVer) specification.
...
Provider lists SHOULD be published to an Ethereum Name Service (ENS) name using [EIP-1577](./erc-1577.md)'s `contenthash` mechanism on mainnet.

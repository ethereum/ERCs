...
description: A registry for tracking smart contract lifecycle status.
author: Kenny Kung (@kennyk10) <kenny.kung@ant-intl.com>, Jeff Fei (@77eff) <j.fei@ant-intl.com>, Shepherd Shu (@baishuo13) <shulei.shu@ant-intl.com>
discussions-to: https://ethereum-magicians.org/t/erc-8089-smart-contract-lifecycle-registry/26641
status: Draft
...
- `Frozen`: The contract cannot be used due to a temporary, reversible halt (e.g., via a pause mechanism). Used for signaling temporary unavailability with potential reactivation (e.g., The contract is paused and can't be interacted with temporarily but it can be recovered to fully operational status by unpausing the contract).
...
- Access Control: MUST restrict `registerContract()`, `modifyContractStatus()`, `modifyContractExtraData()` and `removeContract()` to authorized addresses.
...
- Exclusion of Privileged Accounts: Fields for tracking ownership (e.g., `owner`) were considered but excluded due to the complexity of smart contract ownership models. Not all contracts have a single privileged account. Tracking privileged accounts accurately requires a dedicated registry to handle diverse models, which is better addressed in a separate EIP.

- Exclusion of Upgrade-Specific Fields: Fields like `upgradeProxy` were considered but removed due to the diversity of upgrade patterns (e.g., [EIP-1967](./eip-1967.md) with a single implementation vs. [EIP-2535](./eip-2535.md) Diamonds with multiple facets). Instead, users can register each implementation contract (e.g., proxies, facets) as separate entries in the registry, using the `extraData` field to clarify relationships. This approach ensures flexibility for all upgradeable contract types while keeping the struct simple.
...
- Access Control: The registry’s write functions are a critical attack surface. If access control is misconfigured or compromised, an attacker could falsely mark active contracts as `Terminated` or register fraudulent entries, misleading users and dependent tools.

- Data Integrity: The registry does not verify the accuracy of metadata it stores. A registry owner could intentionally or accidentally record incorrect statuses or misleading `extraData`, causing users to interact with unsafe or obsolete contracts.

- State Transition Validation: The base interface does not enforce valid state transitions. Without additional guards, a `Terminated` contract could be reactivated.

- Off-Chain Verification for Sensitive Statuses: The `Terminated` status signals permanent deactivation. If applied without sufficient evidence, it could incorrectly signal that a live contract is defunct, or conversely, delay marking a genuinely compromised contract. The `extraData` field can serve as an audit trail by referencing supporting off-chain documentation.

- Legitimacy of Registry: Anyone can deploy a contract implementing this interface. A malicious actor could create a fake registry that misrepresents the status of contracts they do not own, deceiving users or block explorers.

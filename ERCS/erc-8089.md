...
- Exclusion of Upgrade-Specific Fields: Fields like `upgradeProxy` were considered but removed due to the diversity of upgrade patterns (e.g., [EIP-1967](./eip-1967.md) with a single implementation vs. [EIP-2535](./eip-2535.md) Diamonds with multiple facets). Instead, users can register each implementation contract (e.g., proxies, facets) as separate entries in the registry, using the `extraData` field to clarify relationships. Examples include:

  - [EIP-1967](./eip-1967.md) Proxy: The proxy (0xABC) and implementation (0xDEF) are registered separately. Proxy: `Active`, `extraData`: "EIP-1967 proxy using implementation 0xDEF." Implementation: `Active` or `Deprecated`, `extraData`: "Implementation for proxy 0xABC."

  - [EIP-2535](./eip-2535.md) Diamond Proxy: The proxy (0x789) and facets (0x123, 0x456) are registered separately. Proxy: `Active`, `extraData`: "Diamond proxy with facets 0x123 (governance), 0x456 (staking)." Facet: `Active` or `Deprecated`, `extraData`: "Facet for governance in Diamond 0x789."

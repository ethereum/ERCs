...
discussions-to: https://ethereum-magicians.org/t/erc-8211-smart-batching/28135
status: Draft
...
> **Note on cross-chain authorization:** Cross-chain authorization for smart batches uses [ERC-7964] — a standard for EIP-712 cross-chain signatures that omits `chainId` from the domain and encodes chain-specific operations as a typed array of structs. ERC-7964 enables a single user signature to authorize operations on multiple chains while preserving full wallet transparency: every operation on every chain is a structured EIP-712 type visible in any standard wallet. This ERC defines the smart batch encoding and constraint mechanism; ERC-7964 handles the cross-chain authorization ("is this call allowed?").

```
MULTI-CHAIN ORCHESTRATION VIA ERC-7964 + PREDICATE ENTRIES
═══════════════════════════════════════════════════════════════════

 User signs ONE ERC-7964 cross-chain signature covering all operations:

  EIP-712({
    domain: { name: "SmartBatch", version: "1" },  ← no chainId
    primaryType: "CrossChainBatch",
    message: {
      operations: [
        hashStruct(BatchOperation{ chainId: 1,     executions: [...] }),  ← L1
        hashStruct(BatchOperation{ chainId: 10,    executions: [...] }),  ← Optimism
        hashStruct(BatchOperation{ chainId: 42161, executions: [...] }),  ← Arbitrum
        hashStruct(BatchOperation{ chainId: 8453,  executions: [...] }),  ← Base
      ]
    }
  })

  Each BatchOperation is a fully typed EIP-712 struct — wallets display
  the complete batch contents for every chain, with no opaque hashes.
...
The SDK compiles this into `ComposableExecution` entries with fetchers, constraints, and storage instructions. The user signs once. Relayers execute on-chain. In multi-chain environments, the same model extends naturally — a cross-chain yield flow is a single signed program with predicate entries gating each step, authorized by one [ERC-7964] cross-chain signature.
...
- Relayers cannot execute unauthorized instructions — each instruction is verified against a user-signed authorization (e.g., an [ERC-7964] cross-chain signature) on-chain.

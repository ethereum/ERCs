...
eip: 8260
title: Dive Log
description: An interface specification for storing, retrieving, and verifying dive log data.
...
- **Cryptographic trust**: [EIP-712](./eip-712.md) signed attestations provide verifiable proof of dive verification.

## Specification

The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in [RFC 2119](https://www.rfc-editor.org/rfc/rfc2119) and [RFC 8174](https://www.rfc-editor.org/rfc/rfc8174).

Every ERC-8260 compliant contract MUST implement the `IDiveLog` and `IERC165` interfaces:
...
enum BottomType { Sand, Coral, Rock, Wreck, Silt, Other }
...
Every dive record MUST declare a unit system. Consumers MUST interpret numeric fields according to the declared unit system:
...
Each dive record carries its own `UnitSystem` field, allowing dives to be logged in either unit system regardless of the diver's preferred convention. Multiple dives MAY share the same `diveDate`; `getDivesByDate` returns all matching dive IDs.
...
```solidity
struct DiveData {
    uint64 leaveSurfaceTime;     // Unix timestamp — diver descends from surface
    uint64 leaveBottomTime;      // Unix timestamp — diver leaves bottom
    uint64 reachSurfaceTime;     // Unix timestamp — diver reaches surface
    uint32 bottomTimeMinutes;    // Total bottom time in minutes
    uint32 maxDepth;              // Maximum depth attained (positive = below surface)
    uint32 averageDepth;          // Average depth (OPTIONAL, 0 = not recorded)
...
**Constraints:**
- `maxDepth` MUST be greater than 0 (the `uint32` type enforces non-negative at the ABI level).
...
struct Environment {
    ...
    BottomType bottomType;       // Bottom composition
...
Input struct for `logDive` and `batchLogDives`. Contains all fields needed to record a dive, excluding the assigned `id`.
...
```solidity
/// @title ERC-8260 Dive Logbook Interface
...
interface IDiveLog is IERC165 {
    ...
    /// @dev Thrown when maxDepth is zero.
    error InvalidDepth();
    ...
    /// @dev Thrown when the attestation nonce does not match the expected value.
    ...
    /// @param input The dive data to log
    ...
    /// @dev Throws unless msg.sender is the owner. Validates each dive
    ///  identically to logDive. Reverts the entire batch if any single dive
    ///  is invalid. Emits one DiveLogged per dive.
    ...
    ///  Data section). The nonce MUST match the attester's current on-chain
    ///  nonce (queriable via attesterNonce). After a successful attestation,
    ///  the attester's nonce is incremented. Throws if the dive does not exist,
    ///  has been voided, the nonce is incorrect, the signature is invalid,
    ///  or the attester has already attested.
    /// @param diveId The dive being attested
    /// @param nonce The attester's current nonce
    ...
    /// @notice Get the current attestation nonce for an address.
...
```solidity
keccak256("Attestation(uint256 diveId,address verifyingContract,uint256 nonce)")
```

An attester signs the EIP-712 typed hash of this structure. The `verifyingContract` binds the attestation to a specific dive log contract. The `nonce` binds the attestation to a specific signing order, allowing attesters to invalidate pending signatures by advancing their on-chain nonce.
...
```solidity
keccak256("Coordinates(int32 latitude,int32 longitude)")

keccak256("DiveData(uint64 leaveSurfaceTime,uint64 leaveBottomTime,uint64 reachSurfaceTime,uint32 bottomTimeMinutes,uint32 maxDepth,uint32 averageDepth,uint8 mode,uint8 purpose,uint8 suit)")

keccak256("Environment(int32 airTemp,int32 waterTemp,int16 currentKnots,uint8 bottomType,Coordinates coords,string location,string weatherConditions)Coordinates(int32 latitude,int32 longitude)")
...
- `logDive` MUST revert if `diveDate` is zero (the reference implementation reverts with `InvalidDate`).
...
- `batchLogDives` MUST reject empty batches and MAY enforce a maximum batch size to bound transaction gas (the reference implementation reverts with `InvalidBatchSize` for empty batches or batches exceeding 100 dives).
...
- `attestDive` MUST revert with `NonceMismatch` if the provided nonce does not match the attester's current on-chain nonce.
...
- `attestDive` MUST increment the attester's on-chain nonce after a successful attestation.
...
- `attesterNonce` MUST return the current nonce for the given address (starting at 0).

#### [ERC-165](./eip-165.md) Compliance
...
The contract uses the owner's wallet address as the sole on-chain identity. Personal information (name, age, biometrics) is deliberately excluded from on-chain storage. Linking a wallet address to a real-world identity is a KYC/process concern that belongs at the application layer — not in the contract. Front-ends and training agencies manage profiles through their own identity verification workflows, using wallet signatures to prove ownership. This keeps the contract privacy-preserving by design while remaining compatible with any identity system a consumer chooses to use.
...
Including a nonce in the attestation signature gives attesters a cancellation mechanism. Without it, a signed but unsubmitted attestation remains valid indefinitely — the attester has no way to revoke it. With nonces, an attester can advance their on-chain nonce (by submitting any attestation, including a self-attestation on a dummy dive or having the contract owner create a cancellation mechanism), invalidating all pending signatures tied to the old nonce. This follows the same pattern used by [ERC-2612](./eip-2612.md) permit nonces.

### Structs Instead of Individual Fields

Grouping related data into structs (`DiveInput`, `DiveData`, `Environment`, `Decompression`, `GasData`) reduces the number of function parameters, improves readability, and maps cleanly to the military source forms (DD Form 2544, ENG Form 4615) which group fields into sections. Using a `DiveInput` struct for `logDive` and `batchLogDives` eliminates the risk of parameter misalignment that arises with parallel arrays or long parameter lists.
...
A reference implementation is provided in the assets directory: [SovereignDiveLog.sol](../assets/eip-8260/SovereignDiveLog.sol). It implements the complete `IDiveLog` interface with:
...
**Access control**: Write operations (`logDive`, `voidDive`) are restricted to the contract owner. The owner is set at construction and SHOULD be immutable. Implementations deployed with upgradeable proxy patterns MUST preserve the invariant that only the designated owner can write.

**No personal data on-chain**: This standard deliberately stores no personally identifying information on-chain. No names, ages, biometrics, or other PII are part of the contract interface. The wallet address serves as the sole on-chain identity. Implementors SHOULD NEVER store PII on-chain. Consumers that require personal information (e.g., for regulatory compliance) MUST manage it off-chain through their own identity verification processes. **Note**: Free-text fields (`location`, `remarks`, `bottomType`, `weatherConditions`, `reason` in `VoidInfo`) are stored permanently on-chain. Implementors SHOULD warn divers that any personal information entered in these fields becomes irretrievably public.
...
**Attestation security**: EIP-712 signatures bind the attestation to a specific (`diveId`, `verifyingContract`, `chainId`, `nonce`) tuple, preventing replay attacks across contracts, chains, and signing orders. The nonce mechanism allows attesters to invalidate pending signatures by advancing their on-chain nonce without submitting the original attestation. The attester is recovered from the signature, not from `msg.sender`, so front-running an attestation transaction has no impact on the recorded attester identity.

**Signature malleability**: For any valid ECDSA signature `(v, r, s)`, the tuple with `s` replaced by `secp256k1n - s` is also valid and recovers the same address. Implementations SHOULD reject non-canonical signatures where `s` exceeds half the secp256k1 group order, as required for canonical transactions by [EIP-2](./eip-2.md). The reference implementation enforces this check in `_recoverSigner`; combined with the per-attester nonce and per-dive deduplication, malleated signatures cannot be replayed.

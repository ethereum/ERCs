...
This proposal introduces a method for native transfers that can originate from an approved spender on a smart contract wallet or a [ERC-7702](https://eips.ethereum.org/EIPS/eip-7702) delegated EOA account. For wallets that can execute code, other contracts can be authorized to pull native tokens via a new callback mechanism.
...
An approval system similar to the [ERC-20](https://eips.ethereum.org/EIPS/eip-20) `approve` mechanism MUST be implemented to allow users to specify trusted accounts that can perform Native transfers on their behalf.
...
- Compatible with [ERC-4337](https://eips.ethereum.org/EIPS/eip-4337) accounts.

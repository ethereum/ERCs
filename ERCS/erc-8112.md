...
description: Off-chain transfer signature extension for smart contract wallets
...
## Motivation
Wallets require on-chain transactions for every token transfer but requires on-chain transactions for every transfer. This creates friction for gasless use cases such as red packets, claimable airdrops, and relayer-sponsored transfers.
...
By implementing authorization at the wallet level, this extension enables gasless transfers for *all* ERC-20 tokens managed by the wallet, significantly expanding the reach of gasless applications.
...
Compliant smart contract wallets that support this extension MUST include the following functions and behavior.
...
- tokenTransferWithSig MUST verify the signature via ERC-1271, reject expired signatures  (unless `deadline == 0`), increment the scoped nonce, and perform `IERC20(asset).safeTransfer(...)`.
...
2. Compute the EIP-712 digest and validate it using ERC-1271 on the wallet address. Validate the digest using ERC-1271 on the wallet address.
3. Verify the provided nonce matches the current stored nonce for the corresponding scope.
4. Increment the scoped nonce before performing the ERC-20 transfer.
...
Existing smart contract wallets remain valid and may add ERC8112 support incrementally. Clients may feature-detect permit support by checking for `IERC8112` via ERC-165 or probing the new function selectors.

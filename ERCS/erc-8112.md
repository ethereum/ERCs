...
- tokenTransferWithSig MUST verify the signature via [ERC-1271](./eip-1271.md)., reject expired signatures  (unless `deadline == 0`), increment the scoped nonce, and perform `IERC20(asset).safeTransfer(...)`.
...
Existing smart contract wallets remain valid. Clients may feature-detect permit support by checking for `IERC8112` via ERC-165 or probing the new function selectors.

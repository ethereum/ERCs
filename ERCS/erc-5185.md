...
The **metadata updates extension** is OPTIONAL for [EIP-721](./erc-721.md) and [EIP-1155](./erc-1155.md) contracts.
...
The original metadata for a given NFT is always defined as the JSON pointed by the result of `tokenURI` for [EIP-721](./erc-721.md) and function `uri` for [EIP-1155](./erc-1155.md).
...
The proposal is fully compatible with both [EIP-721](./erc-721.md) and [EIP-1155](./erc-1155.md). Third-party applications that don't support this EIP will still be able to use the original metadata for each NFT.

...
## Specification
ERC-1450 extends [ERC-20](./erc-20.md).

### Optional Dependencies
The following standards MAY be implemented for enhanced functionality but are NOT required for compliance:
- **[EIP-3668 (CCIP-Read)](./erc-3668.md)**: MAY be used for off-chain compliance pre-checks. Implementations choosing to support this MUST implement the `preCheckCompliance` and `preCheckComplianceCallback` functions as specified.
- **[ERC-1820 (Registry)](./erc-1820.md)**: MAY be used for interface registration. Implementations can optionally register their interfaces in the ERC-1820 registry for improved discoverability.
...
- **Token Holders**: Cannot initiate transfers directly (unlike standard [ERC-20](./erc-20.md))
...
- Standard [ERC-20](./erc-20.md) view functions (`balanceOf`, `totalSupply`, etc.)
...
### [ERC-20](./erc-20.md) Extension
[ERC-20](./erc-20.md) tokens provide the following functionality:
...
[ERC-165](./erc-165.md) interface for standard interface detection:
...
interface IERC165 {
     ...
     * @param interfaceId The interface identifier, as specified in [ERC-165](https://ercs.ethereum.org/ERCS/erc-165)
...
[ERC-20](./erc-20.md) is extended as follows:
...
interface IERC1450 is IERC20, IERC165 {
     ...
     *      Uses specific error for disabled functionality per [ERC-6093](https://ercs.ethereum.org/ERCS/erc-6093) guidelines
     */
    ...
    /**
     * @notice Get token decimals (OPTIONAL per [EIP-20](https://ercs.ethereum.org/ERCS/erc-20))
     ...
     *
     *      NOTE: Per [EIP-20](https://ercs.ethereum.org/ERCS/erc-20), name(), symbol(), and decimals() are OPTIONAL
    ...
    /**
     * @notice [EIP-165](https://ercs.ethereum.org/ERCS/erc-165) support for interface detection
     * @param interfaceId The interface identifier to check
     * @return bool True if the contract implements the interface
     * @dev MUST return true for:
     *      - 0x01ffc9a7: [ERC-165](https://ercs.ethereum.org/ERCS/erc-165) interface ID
     *      - 0xXXXXXXXX: IERC1450 interface ID (see Interface Detection section for calculation)
     *
     *      MUST return false for:
     *      - 0x36372b07: [ERC-20](https://ercs.ethereum.org/ERCS/erc-20) interface ID
     *
     *      Returning false for [ERC-20](https://ercs.ethereum.org/ERCS/erc-20) prevents wallets from assuming standard transfer behavior.
     *      While we are ABI-compatible with [ERC-20](https://ercs.ethereum.org/ERCS/erc-20), we are not behaviorally compatible
    ...
    /**
     * @notice Request transfer with [EIP-2612](https://ercs.ethereum.org/ERCS/erc-2612) permit for gasless fee approval (OPTIONAL)
     * @param from Source address for the transfer
     * @param to Destination address for the transfer
     * @param amount Number of tokens to transfer
     * @param feeAmount Amount of fee being paid (in the configured fee token)
     ...
     *      Uses [EIP-2612](https://ercs.ethereum.org/ERCS/erc-2612) permit for gasless approval of fee token
     *      Particularly useful for first-time users who don't have fee token approvals
     *      MUST revert if the configured fee token doesn't support [EIP-2612](https://ercs.ethereum.org/ERCS/erc-2612)
     ...
     * @return token The [ERC-20](https://ercs.ethereum.org/ERCS/erc-20) token used for fee payments
     ...
     * @param newFeeToken Address of the [ERC-20](https://ercs.ethereum.org/ERCS/erc-20) token to use for fees
     ...
     *      ```
     *
     *      Wallets supporting [EIP-3668](https://ercs.ethereum.org/ERCS/erc-3668) will:
     ...
     *
     *      Per [EIP-3668](https://ercs.ethereum.org/ERCS/erc-3668) specification:
...
- Implementations MUST return `true` from `supportsInterface(0x01ffc9a7)` for [ERC-165](./erc-165.md) itself
...
- Implementations MUST NOT return `true` from `supportsInterface(0x36372b07)` for [ERC-20](./erc-20.md)
...
- `decimals()`, `name()`, `symbol()`: Token metadata (OPTIONAL per [EIP-20](./erc-20.md), SHOULD be provided)
...
For broader discovery, implementations MAY also register with the [ERC-1820 Pseudo-introspection Registry](./erc-1820.md). This allows any address (including externally owned accounts acting as proxies) to publish interface support:
...
ERC-1450 implements the ERC-20 interface but with critical behavioral differences. Per the [ERC-20 specification](./erc-20.md#specification), functions MAY return `false` or `revert()` on failure. The specification notes: "Callers MUST handle `false` from `returns (bool)`. Callers MUST NOT assume that `false` is never returned!"
...
- The `transfer()` and `approve()` functions must ALWAYS revert with appropriate [ERC-6093](./erc-6093.md) errors

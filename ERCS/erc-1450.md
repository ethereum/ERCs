...
interface IERC1450 is IERC20, IERC165 {
     ...
     *      When burning via burnFrom (FIFO), multiple events may be emitted for different regulation types
     ...
     * @dev Only callable by the registered transfer agent
     *      Burns tokens following FIFO order (oldest issuance dates first)
     *      Used primarily for wallet recovery and full redemptions
     *      MUST emit TokensBurned event(s) - one for EACH regulation type burned
     *      Example: Burning 150 tokens from holder with 100 RegCF + 200 RegD
     *               would emit 2 events: one for 100 RegCF, one for 50 RegD
     */
     ...
     *      RTA uses this for FIFO transfer calculations and compliance checks
     */
    ...
    /**
     * @notice Batch mint tokens to multiple addresses with regulation tracking (RTA only)
     * @param recipients Array of addresses to receive tokens
     * @param amounts Array of token amounts to mint to each recipient
     * @param regulationTypes Array of regulation types for each mint
     * @param issuanceDates Array of original issuance dates for each mint
     * @return bool True if all mints succeed
     * @dev All arrays must have equal length. Reverts if any mint fails.
     *      Only callable by the registered transfer agent.
     *      Allows minting tokens from different regulations/dates to different recipients
     *      Useful for bulk tokenization of existing cap tables
     */
    function batchMint(
        address[] calldata recipients,
        uint256[] calldata amounts,
...
Implementations MUST store regulation data on-chain to enable the RTA to enforce compliance. A recommended pattern uses FIFO (First-In-First-Out) ordering:
...
Implementations MUST use FIFO ordering for transfers and burns when holders have tokens from multiple regulations or issuance dates. This ensures consistent behavior and proper regulatory compliance:

**Transfer FIFO Example:**
...
// Transfer request for 150 tokens
// FIFO order transfers:
// - 100 Reg CF tokens (oldest, March 2023)
// - 50 Reg D tokens (next oldest, June 2023)
// Remaining: 150 Reg D + 50 Reg A tokens
```

**Burn FIFO Example:**

```solidity
// Using burnFrom(address, 250) would burn:
// - 100 Reg CF (March 2023) - emits TokensBurned event
// - 150 Reg D (June 2023) - emits TokensBurned event
// Result: Two TokensBurned events, one per regulation type
```

**Regulation-Specific Burns:**

```solidity
// Using burnFromRegulation(address, 50, REG_US_A) would:
// - Burn exactly 50 Reg A tokens
// - Leave Reg CF and Reg D tokens untouched
// - Emit one TokensBurned event for Reg A
```

This FIFO approach ensures:
1. Oldest shares are sold first (tax efficiency)
2. Lockup periods expire naturally
3. Consistent behavior across all implementations
4. Clear audit trail for regulatory reporting

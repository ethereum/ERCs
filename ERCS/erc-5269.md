...
It designates a field called `majorERCIdentifier` which is normally known or referred to as "ERC number". For example, [ERC-721](./erc-721.md) has a `majorERCIdentifier = 721`. This ERC has a `majorERCIdentifier = 5269`.
...
- [ERC-867](https://eips.ethereum.org/EIPS/eip-867) (Stagnant) proposes creating an `erpId`, described as a string identifier for that ERP, likely based on the associated ERC number.
...
Here is an example where a contract of [ERC-721](./erc-721.md) also implements this ERC to make it easier

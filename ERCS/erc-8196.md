...
This ERC defines a standard interface for AI agent-authenticated wallets that execute transactions only when accompanied by a verifiable proof tying the action to the agent's specific policy. It aligns with the Ethereum Foundation's PhDFP-26 RFP D3 ("Agentic Economy: Verification, Delegation, and Host-Interference Mitigation", opened February 2, 2026) and the Ethereum Foundation Mandate (March 2026), serving as **Layer 3 (Execute)** in a modular trust stack for autonomous agents:
...
- User sovereignty over agent actions via enforceable, auditable policy compliance (aligning with EF Mandate emphasis on final user control over agents and actions)
...
- **Active Containment** — real-time monitoring, policy validation, kill switch (**SHOULD implement** for user sovereignty and resilience)  
...
This ERC directly supports the Ethereum Foundation Mandate (March 2026) by ensuring users retain "final say over their [...] actions, and agents" (Section iii). It eliminates blind trust in hosts through entropy commit-reveal and active containment, minimizes intermediation by enforcing on-chain policy without centralized dependencies, and upholds CROPS:

- **Censorship Resistance** — on-chain randomness and policy enforcement prevent selective exclusion  
- **Open Source and Free** — encourages verifiable, non-proprietary implementations  
- **Privacy** — optional ERC-8126 risk gating preserves verification without unnecessary exposure  
- **Security** — tamper-evident hash-chained audit, nonce/entropy protection, and real-time containment  

The design avoids uninspectable intermediaries and promotes verifiable, user-controlled agent execution.
...
- MUST prioritize user final control over agent actions, rejecting any mechanism that introduces blind trust or unaccountable intermediation (EF Mandate alignment)  
- SHOULD design for CROPS compliance: maximize censorship resistance (no kill switches), security (verifiable enforcement), and privacy (minimal data exposure via ERC-8126 gating)  
- Active Containment SHOULD be implemented to enable rapid user response to violations, preserving sovereignty

## Copyright

Copyright and related rights waived via [CC0](../LICENSE.md).

<!-- ci: force full htmlproofer - incremental bug -->

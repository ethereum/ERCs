...
* **SRV Record:** The SRV record ([RFC 2782](https://www.rfc-editor.org/rfc/rfc2782)) MUST point to the hostname and port where the provider's Capacity Information API is hosted. Standard SRV priority and weight mechanisms apply.
* **TXT Record:** An associated TXT record ([RFC 1035](https://www.rfc-editor.org/rfc/rfc1035)) for the same SRV name SHOULD be provided. It MAY contain the following key-value pairs:

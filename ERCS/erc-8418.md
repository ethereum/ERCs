...
author: Dojun Lee (@leedoe), HyeonJae Lee (@HyeonJaaE), TaeRim Lee (@rim95dev), Junho Kim (@howoo), Hyein Lee (@hyenne)
...
| 11 | item `V` with `maxSupply == 2`: mint `tokenId` 7, burn it, mint `tokenId` 7 again | reverts (requirement 7), although `itemTotalMinted(V) == 1` leaves capacity |

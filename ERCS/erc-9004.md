...
description: A standard interface for managing subscriptions to onchain AI agent services with cycle-based pricing and direct payment.
author: Andrei Sambra (andrei@wardenprotocol.org)
discussions-to: https://ethereum-magicians.org/t/<pending>
status: Draft
...
This proposal defines a standard interface for managing subscriptions to onchain AI agent services. It enables users (humans or agents) to subscribe to tiered service plans offered by agents registered via [ERC-8004](./eip-8004.md), with payments in ERC-20 tokens. The protocol uses a cycle-based prepaid model where subscribers pay upfront for one or more billing cycles and tokens are transferred directly to the agent owner. The registry contract never holds funds. Subscription state lives entirely onchain, requiring zero offchain billing infrastructure from agent developers. An HTTP 402 access flow using EIP-712 signatures allows subscribers to prove access without revealing their identity in the initial request.

## Motivation

The agentic economy is converging on two complementary primitives: identity ([ERC-8004](./eip-8004.md)) and per-request payments (x402). Per-request payment works well for traditional APIs where the cost of fulfilling a request is known in advance. Agent interactions are fundamentally different: the work required depends on the prompt, users rarely achieve their goal in a single message, and agents themselves may need to ask clarifying questions, with each exchange triggering another payment. This makes per-request billing unpredictable for users and friction-heavy for multi-turn conversations. A subscription model is a better fit:
...
1. Requires zero offchain infrastructure from agent developers. Subscribe, renew, cancel, and access verification logic is handled entirely onchain.
2. Integrates with [ERC-8004](./eip-8004.md) for agent identity and discovery.
3. Supports multiple plan tiers per agent, enabling differentiated service levels.
4. Gives users full control with cycle-based billing and cancellation that retains access until the paid period ends.
...
The protocol consists of a singleton **SubscriptionRegistry** contract deployed per chain. At deployment, the registry is configured with the address of the local [ERC-8004](./eip-8004.md) Identity Registry on the same chain. This address is immutable and used for all agent ownership verification: when an agent owner calls `createPlan()`, the registry calls `identityRegistry.ownerOf(agentId)` to confirm the caller owns the agent NFT.

Agents MUST be registered on the same chain as the SubscriptionRegistry they use. An agent registered on Base that wants subscriptions on Base uses the Base SubscriptionRegistry. If the same agent also wants subscriptions on Arbitrum, it registers on Arbitrum's [ERC-8004](./eip-8004.md) Identity Registry and uses Arbitrum's SubscriptionRegistry.

The registry interacts with:

- An [ERC-8004](./eip-8004.md) Identity Registry (same chain) for agent ownership verification.
- ERC-20 tokens (e.g., USDC) for payment.
...
|  Subscriptions: subscribe / renew / cancel                    |
...
- 1-month subscription: `5000000 * 1 = 5,000,000` (5 USDC)
- 1 year (12-month) subscription: `5000000 * 12 = 60,000,000` (60 USDC)
...
**Cancel**: Cancellation marks the subscription as cancelled but does NOT trigger any refund or token movement. The subscriber retains access until the already-paid `endTime`. This is the standard model for prepaid subscriptions: once paid, the service runs until the end of the billing period.
...
struct Subscription {
    ...
    bool    cancelled;      // whether explicitly cancelled
}
```
...
- `cancelled` distinguishes between explicit cancellation and natural expiry. A cancelled subscription remains active until `endTime`; cancellation only prevents future renewals.
...
- `Cancelled(bytes32 indexed subscriptionId)` when a subscription is cancelled.
...
Agent owners manage plans through three functions. The caller MUST be the owner of the [ERC-8004](./eip-8004.md) agent NFT for the given `agentId`.
...
The `planId` MUST be unique within the scope of `agentId`. Both `price` and `cycleDuration` MUST be greater than zero. The `asset` MUST be a standard ERC-20 token; fee-on-transfer and rebasing tokens are NOT supported. A `PlanCreated` event MUST be emitted.
...
A permit variant combines EIP-2612 approval and subscription in a single transaction:
...
```
User                         SubscriptionRegistry              Agent Owner    ERC-20 Token
 |                                     |                            |               |
...
The subscription MUST exist and MUST NOT be cancelled. `cycles` MUST be >= 1. Payment is charged at the plan's current price (not a snapshotted rate), so renewals reflect any price changes made by the agent owner. If the subscription is still active, the renewal extends from the current `endTime`. If the subscription is expired, it resets: `startTime` is set to now, `endTime` is set to `now + (cycleDuration * cycles)`. In the expired case, the plan MUST be active and the call MUST revert if another active subscription already exists for the same `(subscriber, agentId, planId)`. Tokens are transferred directly to the agent owner. A `Renewed` event MUST be emitted.
...
A subscriber cancels by calling:

```solidity
function cancel(bytes32 subscriptionId) external;
```

The caller MUST be the subscriber. No refund is issued. The subscriber retains access until the existing `endTime`. The `cancelled` flag is set to `true`, preventing future renewals. A `Cancelled` event MUST be emitted.

```
User                         SubscriptionRegistry
 |                                     |
 |--- cancel(subscriptionId) --------->|
 |                                     |  [set cancelled = true]
 |                                     |  [no refund, no token movement]
 |                                     |  [access continues until endTime]
 |<-- success -------------------------|
```
...
A subscription is active if it exists and `block.timestamp` is between `startTime` and `endTime` (inclusive). The `cancelled` flag is NOT checked; cancelled subscriptions remain active until `endTime`.
...
function getSubscription(bytes32 subscriptionId)
    external view returns (
        ...
        uint48 startTime, uint48 endTime, bool cancelled
    );
```

### Same-Chain Identity Binding

Each SubscriptionRegistry is deployed with an immutable reference to the [ERC-8004](./eip-8004.md) Identity Registry on the same chain. An agent can register on multiple chains, each with its own independent identity registry, subscription registry, and subscription pool.
...
Subscription IDs are derived from `keccak256(subscriber, agentId, planId, nonce)`, not sequential integers.
...
Agents that support subscriptions SHOULD advertise their plans in the [ERC-8004](./eip-8004.md) agent registration file (resolved via `agentURI`). The following extension to the agent card schema is RECOMMENDED:
...
Subscription onboarding begins with discovery: the client fetches the agent's [ERC-8004](./eip-8004.md) agent card, which advertises available plans, registries, and pricing. Using this information, the client subscribes onchain before making any authenticated requests. The 402 flow described below handles access control, gating requests from clients that have already subscribed.
...
#### Step 3: Client Signs EIP-712 Proof

The client constructs and signs an [EIP-712](./eip-712.md) typed data message proving it controls a subscriber address, without revealing that address in advance.
...
```json
{
  "name": "ERC-9004: Agent Subscription Protocol",
  "version": "1",
  "chainId": 8453,
...
- The EIP-712 domain's `chainId` and `verifyingContract` implicitly bind the proof to the specific registry the client subscribed on.
...
| `authorization.agentId` | MUST | The [ERC-8004](./eip-8004.md) agent identifier on the chosen chain. MUST match the `agentId` from the corresponding `registries` entry in the `SUBSCRIPTION-REQUIRED` response. |
...
| `signature` | MUST | EIP-712 signature over the `SubscriptionProof` struct (derived from the `authorization` fields). |
...
2. **Recover signer**: Use `ecrecover` on the EIP-712 signature (using the domain corresponding to the specified chain and registry) to obtain the subscriber address.
...
If verification fails, the agent SHOULD respond with HTTP 403 (Forbidden) and an appropriate error message indicating whether the subscription is expired, cancelled, or the signature is invalid.

#### Caching Recommendation

To avoid an RPC call per request, agents SHOULD maintain a local cache of subscription state by indexing `Subscribed`, `Renewed`, and `Cancelled` events. The agent re-syncs with the chain on a cadence appropriate for its use case (e.g., every block, every minute). Agents with low request volume MAY use onchain verification directly.
...
| Aspect | x402 | ERC-9004 |
|--------|------|----------|
| **402 means** | "Pay me" | "Identify yourself" |
| **Client authorizes** | A token transfer | Subscription identity lookup |
| **Authorization type** | Payment authorization (EIP-3009 / Permit2) | Subscription authorization (EIP-712) |
...
| Subscriber identity at request time | Offchain, private (revealed only to the agent via signature) |
| Service usage / API calls | Offchain, private |
| Request/response content | Offchain, private |

The HTTP 402 flow provides an important privacy property: the client's initial request carries no identifying information. The subscriber address is only revealed to the agent (via `ecrecover`) after the agent has already committed to a 402 challenge. Third-party observers of the HTTP traffic see only the `SUBSCRIPTION-SIGNATURE` header, which is opaque without access to the EIP-712 verification logic.

Onchain, subscription data is public. This is comparable to what is visible in any ERC-20 transfer and is acceptable for most use cases.
...
- **Simplicity**: The contract only needs to track subscription state (start time, end time, cancelled) and facilitate token transfers. The state footprint is minimal.
...
Cancellation does not trigger a refund. The subscriber has already paid for the service period and retains access until it ends. This matches user expectations from services like Netflix, Spotify, and GitHub.
...
Plans could be defined purely offchain (as [ERC-8004](./eip-8004.md) does with agent metadata), with only a hash stored onchain. However, onchain plans enable:
...
This ERC introduces no backwards compatibility issues. It is a new contract that references but does not modify existing standards ([ERC-8004](./eip-8004.md), ERC-20, [EIP-2612](./eip-2612.md)).
...
| Renew cancelled subscription | `cancelled=true` | Reverts |
...
| Test | Input | Expected |
|------|-------|----------|
| Cancel active subscription | Caller is subscriber | `Cancelled` event; `cancelled=true`; `isActive` still returns true until `endTime` |
| Cancel by non-subscriber | Caller is not the subscriber | Reverts |
| Cancel already cancelled | `cancelled=true` | Reverts |
...
| Cancelled but not expired | `cancelled=true`, `block.timestamp < endTime` | `isActive` returns true; `verifyAccess` returns true |
...
The registry MUST use reentrancy guards on all state-mutating functions that interact with external contracts (token transfers). The `subscribe`, `renew`, and any permit-based variants call `safeTransferFrom` on an external ERC-20 token, which could trigger a callback if the token is malicious.
...
The registry SHOULD validate that the payment token implements ERC-20. Rebasing tokens and fee-on-transfer tokens are NOT supported and MAY cause accounting errors.

...
This interface serves as a foundational building block for both wrapper protocols (adding privacy to existing [ERC-20](./erc-20.md) tokens) and dual-mode tokens (single tokens supporting both transparent and private transfers).
...
**Wrapper Protocols** ([ERC-20](./erc-20.md) → Privacy → ERC-20):
...
To enable **client interoperability** across different implementations, this standard defines an **OPTIONAL but RECOMMENDED** Privacy Configuration File mechanism, inspired by [ERC-8004](./erc-8004.md)'s Agent Registration File pattern.
...
| Subfield          | Required | Description                                                   |
| ----------------- | -------- | ------------------------------------------------------------- |
| `method`        | NO       | Key derivation method: [EIP-712](https://eips.ethereum.org/EIPS/eip-712) Signature , `BIP-32`, etc. |
...
- **Developer Familiarity**: Matches [ERC-20](./erc-20.md) conventions, reducing learning curve
...
**The Solution**: Privacy Configuration File (inspired by [ERC-8004](./erc-8004.md))
...
**Wrapper Protocols**: Add privacy to existing [ERC-20](./erc-20.md) tokens

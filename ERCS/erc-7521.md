...
We enable the entry point contract to skip the validation of `UserIntent` objects with their `sender` field set to `address(0)` (rather than fail validation). Similarly, they are skipped during execution. The `intentData` field is then free to be treated as a way to inject any arbitrary data into intent execution. This data could be useful in solving an intent that has an intent standard which requires some secret to be known and proven to it, or an intent whose behavior can change according to free data input. 

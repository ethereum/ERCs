...
- `<ens-name>`: An ENS name, which should be resolved following the appropiate ENSIPs (which this standard does not aim to overwrite). Its grammar definition is slightly different than that of ENSIP-1, as this standard does not support names consisting of only a TLD, since they'd be of limited usefulness.

...
1. Serialize the CAIP-2 chain identifier into the [ERC-7930] chain ID byte format.
2. Call `WildcardResolver.chainName(bytes calldata _chainIdBytes)` with the serialized bytes.
3. Display the returned chain name label without the <namespace>.eth suffix
(e.g., show `arbitrum` instead of `arbitrum.<namespace>.eth`).

To obtain the chain identifier corresponding to a human-readable chain name, wallets **SHALL**:

1. Append the reserved ENS second-level domain suffix to the chain name (e.g., `arbitrum.<namespace>.eth`).
2. Compute the ENS namehash of the full name to obtain the `node`.
3. Call `WildcardResolver.chainId(bytes32 _node)` passing the ENS node.

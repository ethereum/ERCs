---
eip: 7828
title: Human readable names for Interoperable Addresses using ENS
...
This proposal extends [ERC-7930] (Interoperable Addresses & Names) by standardizing a human-readable format for chain-specific addresses in the form `<address>@<chain>#<checksum>`, resolved via ENS as `<name>.eth@<chain-name>.l2.eth#<checksum>`. It introduces:
...
- An on-chain registry for chain names and identifiers, allowing resolution of `chainName.l2.eth` even in the absence of a top-level domain (TLD), thereby enabling greater flexibility in naming conventions and chain metadata discovery.
...
Instead of using chain identifiers, which are not human-readable, the address should be extended with a human-readable chain name, which shall be resolved to its corresponding chain identifier via the ENS L2Resolver.
...
- `<chain-name>`:   An ENS name that shall be resolved via the L2Resolver methods using the l2.eth second-level domain (e.g., the user-facing name optimism corresponds to optimism.l2.eth), where the l2.eth suffix is implicit and abstracted from the user.
...
- alice.eth@arbitrum-nova.l2.eth#AAAAAAAA
...
> [!NOTE]
> This standard explicitly defines the use of the l2.eth second-level domain for chain names. Beyond this, hierarchical name structures continue to be inherited from ENS, as exemplified in the ENS-in-rollup example.
...
Chain names without a dot (.) are interpreted as labels under the ENS l2.eth second-level domain and resolved via the L2Resolver contract.

To obtain the human-readable chain name corresponding to a CAIP-2 chain identifier, wallets **SHALL**:

1. Serialize the CAIP-2 chain identifier into the ERC-7930 bytes chain format.
2. Call `L2Resolver.chainName(bytes calldata _chainIdBytes)` with the serialized bytes.
3. Display the returned chain name **without** the `l2.eth` second- and top-level domains  
   (e.g., `optimism` instead of `optimism.l2.eth`).

To obtain the chain identifier corresponding to a human-readable chain name, wallets **SHALL**:

1. Append the `l2.eth` second- and top-level domains to the chain name (e.g., `optimism.l2.eth`).
2. Compute the ENS namehash of the full name to obtain the `node`.
3. Call `L2Resolver.chainId(bytes32 _node)` passing the ENS node.
4. Optionally, deserialize the returned ERC-7930 bytes to recover the CAIP-2 chain identifier.

#### Supported CASA namespaces

While currently only `eip155` chains are supported, the on-chain registryrepository could start listing other chains in order to make human-readable names for non-evm chains possible.
...
This section is non-normative, as future ENSIPs can modify the way in which ENS is used. Its purpose is to demonstrate how human-readable interoperable names are resolved to the [ERC-7930] binary format used at the protocol level.

1. Let the user input an address name. Assume it's `alice.eth`.
2. Let the user select a chain. Assume it's `optimism`. The second- and top-level domains (`l2.eth`) are abstracted away at the chain level to improve the user experience.
3. Append the second- and top-level domains to the chain-name, in this case `optimism.l2.eth`, and compute the ENS namehash: `nameHash(optimism.l2.eth)`.
4. Call `L2Resolver.chainId(bytes32 _node)`, which returns the chain ID in [ERC-7930] byte format.
...
1.  Call `nameForAddr(bytes address)` with the address as serialized in step 8. This will return the human-readable name `alice.eth`.
2.  Check forward resolution of the name as described in ENSIP-11 and repeated in the section above.
...
3.  Compute the checksum as described in [ERC-7930]: `0xC69BEB13`.
4.  Format the address in the format `<address>@<chain>#<checksum>` by removing `l2.eth` from step 3 and display it to the user: `alice.eth@optimism#C69BEB13`
...
- Wallet developers should be aware of possible unicode glyph collisions in resolved names and warn users about them in order to keep checksums effective, since an attacker could, in order to impersonate `alice.eth@chain.l2.eth#00112233`:
    - Mine an address on `chain.l2.eth` such that the checksum of the Interoperable Address is `00112233`.

...
`<caip-2-chain-id>` is the string representation of the [CAIP-2] blockchain identifier. For example, `eip155:1` or `bip122:000000000019d6689c085ae165831e93`
...
If the `<chain>` is `ethereum`, you would resolve the `chain-id` data record for `ethereum.<namespace>.eth` subject to ENSIP-TBD-19.
...
An _Interoperable Address_ is binary data of arbitrary length. Using ENSIP-5: Text Records would not be appropriate as the data being represented is not.. text. ENSIP-TBD-19 allows for the resolution of arbitrary data.
...
4. Discern the resolver for `optimism.<namespace>.eth` and resolve `data(bytes32 namehash, string key)` (ENSIP-TBD-19: Arbitrary Data Resolution) passing in the `namehash` (see ENSIP-1) of `optimism.<namespace>.eth` and the key, `chain-id`. 

...
- `<address>` can either be:
    ...
    - The _Target Address_ as defined in [ERC-7930].
...
- `<checksum>` is the first 4 bytes of the hexadecimal ([RFC 4648]) string representation of a keccak256 hash. This hash is computed over the concatenation of the _Interoperable Address_'s binary fields: `ChainType`, `ChainReferenceLength`, `ChainReference`, `AddressLength`, and `Address`. The `Version` field MUST NOT be included in the hashed data.
...
`alice.eth@eip155#FFFFFFFF` is *not a valid* _Interoperable Name_. It's `<chain>` component must include both a [CAIP-2] namespace and reference as the address to which an ENS name resolves is chain specific. 

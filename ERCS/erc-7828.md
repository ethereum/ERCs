...
- An on-chain registry mapping chain names to identifiers within the `l2.eth` namespace, enabling decentralized resolution of chain metadata through ENS L2Resolver. 
- The `l2.eth` suffix is omitted from chain names in user-facing addresses to improve readability, ensuring simpler display without losing on-chain identity.
...
Instead of using non-human-readable numeric chain identifiers, this specification **SHALL** require a human-readable chain name resolved on-chain via ENS L2Resolver. The centralized chain list maintained in `ethereum-lists/chains` **SHALL** be superseded by an on-chain registry under the `l2.eth` namespace, enabling decentralized, extensible chain metadata resolution.
...
- `<chain-name>`: A human-readable chain label within the reserved `l2.eth` namespace. This standard uses the `l2.eth` domain to provide a consistent abstraction for chain identity, mapping these labels to their corresponding canonical chain identifiers.
...
> [!NOTE]
> This standard explicitly defines the use of the l2.eth second-level and top-level domain for chain names. Beyond this, hierarchical name structures continue to be inherited from ENS, as exemplified in the ENS-in-rollup example.
...
3. Display the returned chain name **without** the `l2.eth` second-level and top-level domains  
...
1. Append the `l2.eth` second-level and top-level domains to the chain name (e.g., `optimism.l2.eth`).
...
3. Append `l2.eth` and compute the ENS namehash: `nameHash(optimism.l2.eth)`.

...
The `<address>` component definition is extended to allow usage of ENS names.

The `<chain>` component definition is extended to allow the use of a human-readable label. This label can be resolved, via the Ethereum Name Service (ENS), to chain-specific metadata, including the chain's [ERC-7930] _Interoperable Address_.

## Motivation

The core motivation for the changes to the _Interoperable Name_ definition is to improve readability. ENS is a well-established blockchain identity primitive that is more easily consumable than the alphanumeric address formats used by most blockchains.

An on-chain registry would provide a single source of truth for mapping a chain label to metadata associated with the chain. This specification defines an architecture that enables rollup operators to take ownership of their chain-specific data, thereby reducing reliance on a single centralized entity and ensuring data integrity.

Historically the mapping from chain names to identifiers has, since [EIP-155](./eip-155.md), been maintained off-chain using a centralized list. 

...
This proposal defines an _Interoperable Name_, a _chain-specific address_ format with the structure `<address>@<chain>#<checksum>`, where both `<address>` and `<chain>` can be expressed as human-readable names and resolved via ENS.

An on-chain registry enables decentralized resolution of chain labels to their canonical identifiers, while address resolution leverages existing ENS infrastructure.

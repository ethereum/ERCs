...
**Note:** `alice.eth@eip155#FFFFFFFF` is *not a valid* _Interoperable Name_. It's `<chain>` component must include both a [CAIP-2] namespace and reference as the address to which an ENS name resolves is chain-specific. 

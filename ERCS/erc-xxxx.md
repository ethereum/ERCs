...
description: A minimal data model standard that an Orchestrator can understand to help fulfil a requirement from a wallet or any other system that wishes to request blockchain solutions from an Orchestrator.
...
An arbitrary system could be a website, device, app, server program etc - anything that manages an Ethereum wallet, **[speaks Ethereum JSON-RPC](https://ethereum.org/en/developers/docs/apis/json-rpc/)** and is looking to request solutions from an Orchestrator.

All solutions from an Orchestrator are ChA¹ (Chain Abstration-first) by default.

![Flow](../assets/erc-xxxx/flow.jpg)
...
![Sequence Diagram](../assets/erc-xxxx/sequence.png)

This specification follows the top level data shape of [Ethereum JSON-RPC](https://ethereum.org/en/developers/docs/apis/json-rpc/) requests, as shown below:

### Request

The request definition is what a wallet sends to an Orchestrator for solutions to one or more problems. The request follows the specification from [Ethereum JSON-RPC](https://ethereum.org/en/developers/docs/apis/json-rpc/).
...
- The `towards` property is `REQUIRED`, takes and `array` of either an `Asset` or a `Destination` type and represents where this `Action` is targeted towards.
...
The response definition is what an Orchestrator sends back as a response to the request for solutions from a wallet. The response, like the request, follows the specification from the [Ethereum JSON-RPC](https://ethereum.org/en/developers/docs/apis/json-rpc/) specification.
...
- The `deadline` property is `OPTIONAL`, takes a `number` and represents a unix timestamp by which this `Solution` should be executed by. This is defined by the Orchestrator.
...
- The `chainId` property is `REQUIRED`, takes a number and represents the chain that this `Transaction` is targeted at. The `chainId` is `REQUIRED` here because the wallet `MUST` know where to send assets from as an origin due to the existance of multichain assets.
...
- Uses the [Ethereum RPC](https://ethereum.org/en/developers/docs/apis/json-rpc/) JSON wrapper for greater compatibility.
...
#### Sending an ERC20 token to another address
...
#### Sending an ERC20 token to multiple addresses

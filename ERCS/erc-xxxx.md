...
The interface `IERCXXXXRegistry` is defined as follows:
...
Any contract created using an ERCXXXXRegistry SHOULD implement the IERCXXXXService interface:

```
interface IERCXXXXService {
  /**
...
function owner() public view returns (address) {
  (, address wallet) = IERCXXXXService(address(this)).wallet();
  return wallet;
}
...
## Rationale
The technical foundation of ERCXXXX centers on the extension and generalization of contract types that can be associated with [ERC-4337](./eip-4337.md) wallets. Key decisions include:

...
modular routing, enabling protocols to distribute logic across multiple facets
while maintaining a single external address. This approach has proven
...
**Multi-Facet Proxy Architecture**: An architectural organization of a modular
proxy system that realizes a routing model by selecting and constraining its
permissible semantics. It additionally defines its constituent components,
routing metadata, identifier collection location, and the upgrade,
observability, deployment, and storage layout models.

**Routing Model**: An architectural definition governing how facet execution is
represented, resolved, and maintained. It establishes the permissible routing
...
approach, facet identity type, routing model type, function routing types, and
routing granularity.
...
facet from its facet identity. Examples include Selector Routing (Diamond) and
Protocol Routing.
...
consistency. Examples include `diamondCut()` and `atomicUpdate()`.
...
routing model's upgrade operation. Examples include FacetManager and
DiamondCutFacet.
...
Observability and DiamondLoupeFacet.
...
appended to the end of calldata for Protocol Routing. Future specifications
MAY support wider routing indices while preserving interoperability.
...
implementation (`atomicUpdate()`), or its corresponding event (`AtomicUpdate`),
...
The IObservability interface enables external systems to inspect the protocol's
routing table and derived occupancy state. This is essential for:
...
ERC-2535 (Diamond) pioneered standardized modular proxy architectures.
...
[`ERC8349-Cento-Proxy-Reference-Implementation.zip`](../assets/eip-8349/reference/ERC8349-Cento-Proxy-Reference-Implementation.zip)

The reference implementation includes:

- `CentoRouter`: Protocol Routing and Interface Compatibility logic
(assembly-optimized)
- `FacetManager`: Atomic Update implementation
- `Ownership`: ERC-173 ownership mechanism
- `Observability`: Routing metadata introspection and ERC-165 integration
- Complete test suite with gas benchmarks
- Examples demonstrating deployment and upgrades
- Storage migration examples using [ERC-7201](./eip-7201.md) storage
namespacing

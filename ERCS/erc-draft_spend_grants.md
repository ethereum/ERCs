...
A non-normative [ERC-7730](./eip-7730.md) descriptor for the reference deployment is in [../assets/erc-draft_spend_grants/clear-signing/spend-grant.json](../assets/erc-draft_spend_grants/clear-signing/spend-grant.json). It lets a wallet show the delegate, recipient, window, each asset's caps in that asset's units, and the validity dates, and it rejects a grant whose `assetCombine` is not `0`. A descriptor is bound to specific registry deployments and cannot call `executor()`, so a wallet still has to look up and show the executor itself.

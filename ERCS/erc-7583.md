...
This EIP proposes a standardized method for enabling tokens to be traded both as ERC20 and ERC721, providing the flexibility for tokens to operate in a fungible and non-fungible manner on the Ethereum. Furthermore, this standard incorporates a standard to embed inscription data within event data. This addition allows for the inclusion of immutable, extra information directly on the Ethereum. 
...
**Every ERC-7583 compliant contract MUST implement the ERC-7583 and ERC-165 interfaces.**
...
The ERC-7583 standard aims to combine the advantages of fungible tokens with new inscription functionalities, providing developers and users with a token standard that is both familiar and innovative. ERC20, as the most widely used token standard on Ethereum, offers a clear and efficient set of rules for the issuance and trading of fungible tokens (FTs). Additionally:

- **Broad Ecosystem Support**: ERC20 tokens, due to their simplicity and high degree of compatibility, have gained broad ecosystem support, including wallets, exchanges, DeFi platforms, etc. By inheriting ERC20, tokens under the ERC-7583 standard can seamlessly integrate into this mature ecosystem, thereby leveraging existing infrastructure and tools.

- **User Familiarity**: The ERC20 standard, due to its long-standing presence and popularity within the Ethereum community, is well-known among users. Users have a clear understanding of how to trade and manage ERC20 tokens, which reduces the educational and adaptation costs when promoting a new standard.

### Design Decisions for the NFT Module

In the design process of the ERC-7583 standard, we thoroughly considered how to effectively integrate the characteristics of non-fungible tokens (NFTs), which was achieved by drawing on the ERC721 standard. ERC721 provides a widely accepted set of interface definitions and transaction specifications for NFTs, making it the representative standard for unique assets on Ethereum. However, in attempting to integrate features from ERC721 into ERC-7583, we encountered several key challenges and constraints:

- **Naming conflicts for functions and events**: There are multiple functions and events with similar functionalities but different purposes between ERC20 and ERC721. These duplicated interfaces, if used directly in the same contract, could lead to confusion and compatibility issues. For example, the `transfer` and `approve` functions exist in both standards but deal with different types of assets and logic.

Therefore, we made the following design decisions:

- **Modifying function names**: To preserve the full capabilities of ERC721 while avoiding naming conflicts, we modified the names of functions and events borrowed from ERC721. This approach maintains clarity and consistency of the interface while avoiding direct conflicts with ERC20 features.

- **Preserving core features of ERC721**: Through this method, we ensured that the ERC-7583 standard could support the core features of NFTs, such as unique identification, ownership management, and secure transfer.
...
To integrate NFT functionalities within the ERC-7583 standard, we referred to the core features of the ERC721 standard and made necessary adjustments. Despite the need to modify some event and function names due to naming conflicts, we endeavored to minimize changes to the core functionalities of ERC721. In the ERC-7583 standard, we retained the following core components of ERC721 to ensure compatibility with the existing NFT ecosystem:
...
- [ERC-7583 implementation](https://github.com/insevm/ERC7583_Implementation/blob/main/contracts/ERC7583.sol)

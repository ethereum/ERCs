...
### Deployed reference implementations

A second reference implementation, `RmemMemoryRegistry`, extends the spec along two dimensions implementors are likely to want in practice: (a) on-chain storage of `keccak256(data)` commitments rather than raw `bytes` payloads (32-byte footprint per record; real payloads live in an off-chain encrypted vault); and (b) subject-controlled on-chain Body-Lease delegation (a subject grants a body a scoped, time-bounded lease via `grantLease(body, scopes, expiresAt)`; the leased body can then call the four ops within scope). Both implementations are CC0.

- Source: `clavote-boop/rmem-gateway` on GitHub, directory `contracts/`.
- Sepolia (chain id `11155111`): `0x2cf251859d172e292aa6a4ef4bbf7621b8117e4e`
- Base Sepolia (chain id `84532`): `0x2cf251859d172e292aa6a4ef4bbf7621b8117e4e`
- BNB Smart Chain Testnet (chain id `97`): `0x2cf251859d172e292aa6a4ef4bbf7621b8117e4e`
- All three deployments are at the same address (deterministic from deployer EOA + nonce 0); each passes `supportsInterface(0x13a642d4)` (this interface) and `supportsInterface(0x01ffc9a7)` (ERC-165).

The interface itself is unchanged; only the storage and authorisation layers differ, as the spec explicitly allows.

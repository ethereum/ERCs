...
This ERC defines a minimal four-function interface — `readMemory`, `writeMemory`, `deleteMemory`, and `exportMemory` — through which an Ethereum address (the *subject*) asserts sovereign control over AI agent memory records that reference that address. Any smart contract or verifier gateway that stores, indexes, or proxies AI agent memory records for a subject MUST implement this interface. The interface is deliberately narrow: it carries no opinion about storage backend, memory format, or agent business logic, and is orthogonal to other agent-related interface proposals such as [ERC-8183](./erc-8183.md) (agentic commerce). It addresses a concern those proposals do not cover — the subject's rights over the memory an agent stores about them.
...
Compliant contracts MUST implement [ERC-165](./erc-165.md) and MUST return `true` when queried for the `IAgentMemoryAccess` interface identifier.
...
- **[ERC-173](./erc-173.md) owner:** The contract owner can read/write on behalf of any subject (e.g. for agent-side writes).

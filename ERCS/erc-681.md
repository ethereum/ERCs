...
## Simple Summary
A standard way of representing various transactions, especially payment requests in ether and [ERC-20](./erc-20.md) tokens as URLs.
...
For the syntax of ENS_NAME, please consult [ERC-137](./erc-137.md) defining Ethereum Name Service.
...
Requesting payments in [ERC-20](./erc-20.md) tokens involves a request to call the `transfer` function of the token contract with an `address` and a `uint256` typed parameter, containing the *beneficiary address* and the *amount in atomic units*, respectively. For example,

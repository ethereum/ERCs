...
description: End-to-end workflow for agent-issued intents, permissionless fulfillment, atomic settlement, and verifiable receipts
...
| **Signed** | Principal has produced a valid [EIP-712](./eip-712.md) signature over the intent. |
...
2. **Sign**: The Principal signs the intent using [EIP-712](./eip-712.md) structured data. Signature verification MUST support [EIP-1271](./eip-1271.md) where the maker is a contract wallet, in addition to EOA (ECDSA) signatures.
...
- `maker`: The Principal's address. MUST match the signer (or be authorized via [EIP-1271](./eip-1271.md)).
- `inputToken` / `outputToken`: [ERC-20](./eip-20.md) token addresses for the input spent and output received.
...
- `supportsInterface(bytes4 interfaceId)` MUST return `true` for `0x01ffc9a7` ([ERC-165](./eip-165.md)).
...
The standardization of agentic on-chain operations requires balancing three competing concerns: user safety, solver flexibility, and verifiability. This ERC addresses these through its core design principles:

1. **Bounded Safety Envelope**: The core intent structure enforces fundamental safety bounds (maximum input, minimum output, validity windows) on-chain by every compliant hub. This ensures that regardless of execution path complexity, users have guaranteed protection against unbounded losses.

2. **Separation of Concerns**: By separating intent authorship (agents/users), solving (compute execution paths), and execution (submit transactions), the standard enables specialization and competition at each layer while maintaining clear accountability.

3. **Structured Receipts**: Machine-readable settlement events provide verifiable workflow closure, enabling agents to reliably detect completion, attribute outcomes, and build memory systems around structured execution records.

### Alternative Approaches Considered

Several alternative designs were evaluated:

- **Execution-specific standards**: Creating separate standards for swaps, lending, etc. would fragment the ecosystem and prevent cross-domain composition.
- **Execution constraint in core envelope**: Including execution instructions (rather than outcome constraints) would reduce solver flexibility and increase signature verification complexity.
- **No optional modules**: A purely constraint-free design would be insufficient for complex use cases like MEV protection or oracle-based conditions.

The chosen approach maximizes interoperability while preserving safety and extensibility.

### Intent ID Computation

The intent ID computation (`keccak256(abi.encode(Intent))`) ensures deterministic correlation between off-chain distribution and on-chain settlement. This enables:

- Off-chain intent pools to reference intents uniquely before on-chain submission
- Solvers to correlate intent announcements with settlement events
- Replay protection that works across both submission profiles

### Fee Model

The basis-points fee model (`feeBps`) provides a predictable, percentage-based fee structure that scales with transaction value. This aligns solver incentives with user outcomes while remaining simple to implement and verify.

## Security Considerations

### Signature Verification

Intent signatures MUST be verified using the IntentHub's [EIP-712](./eip-712.md) domain separator to prevent cross-chain and cross-hub replay attacks. Implementations must support both ECDSA (EOA) and [EIP-1271](./eip-1271.md) (contract wallet) signature verification. If `isValidSignature` reverts during contract wallet verification, the hub MUST treat the signature as invalid.

### Nonce Management

Nonce uniqueness MUST be enforced per `(maker, nonce)` pair globally within each hub. Simple `fulfilled[intentId]` tracking is insufficient because two intents with the same `(maker, nonce)` but different `salt` or `constraintsHash` would have distinct intent IDs, allowing replay attacks.

### Time Window Validation

Implementations MUST validate that `validAfter ≤ block.timestamp ≤ validUntil` during fulfillment. Expired intents pose no security risk but waste gas. Too-short validity windows may prevent fulfillment due to network congestion; too-long windows increase exposure to changing market conditions.

### Fee Validation

The `feeBps` parameter MUST NOT exceed 10,000 (100%). While technically valid, `feeBps = 10000` results in zero net output to the receiver, which should be prevented by setting `outputAmountMin > 0`. Makers should always specify meaningful minimum output amounts.

### Balance Delta Accounting

Token transfer accounting MUST use actual balance deltas rather than trusting token return values to handle fee-on-transfer tokens, rebasing tokens, and other non-standard token behaviors. The formulae specified in Fee Accounting provide the canonical computation.

### Constraint Module Validation

When `constraintsHash != bytes32(0)`, implementations MUST validate that the corresponding constraint module is both registered and callable. Unregistered or reverting constraint modules MUST cause the entire fulfillment to revert. Constraint modules have arbitrary execution power and should be used only with trusted implementations.

### Front-running and MEV Protection

This ERC does not mandate MEV protection mechanisms, but implementations MAY include:

- Private mempool submission
- Commit-reveal schemes for intent submission  
- Time-locked fulfillment windows
- Constraint modules that enforce execution constraints

Users requiring MEV protection should use implementations that provide appropriate mechanisms for their use case.

### Oracle Dependencies

Constraint modules MAY depend on external price oracles or other data sources. Users should understand the trust assumptions and manipulation risks of any oracles used by constraint modules in their intents.

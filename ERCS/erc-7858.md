...
- `isTokenExpired` is used for retrieving the status of the given `tokenId` or `tokenType` the function **MUST** return `true` if the token is still valid; otherwise, `false`.

...
- `startTime` and `endTime` of `tokenId`, can be `block.number` or `block.timestamp` depending on `expiryType`. The `startTime` **MUST** less than `endTime` and **SHOULD** except when both are set to `0`. A `startTime` and `endTime` of `0` indicates that the `tokenId` has no time-limited.
...
- `supportInterface` for `IERC7858` is `0x3ebdfa31` for `IERC7858Epoch` is `0xaaf87b24`

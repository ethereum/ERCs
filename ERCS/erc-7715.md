  ...
  permissions: [
    {
      type: {
        name: 'erc20-transfer'
        description: 'Transfer ${symbol} tokens'
      },
      policies: [
        {
          type: {
            name: 'token-allowance',
            description: 'Allowed to transfer ${allowance} tokens'
          }
          properties: {
            allowance: 'uint256'
          },
        ...
        },
        {
          type: {
            name: 'gas-limit',
            description: 'Allowed to spent ${limit} gas'
          },
          properties: {
            limit: 'uint256'
          },
...
type ERC20TokenTransferPermission = {
  type: {
    ...
    description: 'Transfer ${ticker} tokens';
  };
  policies: [...];
  properties: {
    address: 'address';
...
type ContractCallPolicy = {
  type: {
    name: 'contract-call';
    description: 'Call contract at ${address}';
  };
  policies: [...];
  properties: {
    address: 'address';
  ...
  data: {
    calls: string[]; // eg. ['function createToken(string memory tokenURI) public returns (uint)']
  };
};
```
...
type ERC20TokenAllowancePolicy = {
  type: {
    name: "token-allowance";
    description: "Allowed to transfer ${allowance} tokens";
  };
  data: {
    allowance: "0x..."; // limit in wei in hex
  };
};
...
type RateLimitPolicy = {
  type: {
    name: "rate-limit";
    description: "Allowed to call ${count} time(s) every ${interval} seconds";
  };
  data: {

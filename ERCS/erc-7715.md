...
Permissions represent a singular piece of authority that can be used by the application.
eg. ability to transfer a token, update an ENS domain, vote on a DAO, etc

Policies reduce the permitted scope on any given permission. A policy without a permission could be excessively permissive.
...
`account` is an optional field that identifies the account being targetted for this permissions request which is useful when a connection has been established and multiple accounts have been exposed. See the "Account Identifier" section for details.
...
This ERC does not specify an exhaustive list of signer or permission/policy types, since we expect more signer and permission/policy types to be developed as wallets get more advanced. A signer type and permission/policy type is valid as long as both the DApp and the wallet are willing to support it.

However, if two signers or two permissions/policies share the same type name, a DApp could request with one type of signer or permission/policy while the wallet grants another. Therefore, it’s important that no two signers or two permissions/policies share the same type. Therefore, new signer or permission/policy types should be specified in ERCs, either in this ERC as an amendment or in another ERC.
...
It follows the `did` spec, which requires a `"did:"` prefix and enables us to identify accounts in a flexible and forwards-compatible manner. 

To idenfiy a Ethereum addresses, [CAIP-10](https://github.com/ChainAgnostic/CAIPs/blob/main/CAIPs/caip-10.md) is RECOMMENDED, for example: `"did:pkh:eip155:1:0x..."`. 

To identify an ENS name, [DID-ENS-spec](https://github.com/veramolabs/did-ens-spec) is RECOMMENDED, for example: `"did:ens:mainnet:vitalik.eth"`.
...
```json
{
  "0x123": {
    "permissions": {
      ...
      "permissionTypes": ["erc20-transfer", "contract-call"],
      "policyTypes": ["token-allowance", "gas-limit"]
    }
  }
}
```

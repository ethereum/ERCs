...
If `signer` is not specified in the request, the wallet manages the session. If the wallet approves the request, it MUST accept [ERC-5792](./eip-5792.md)’s `wallet_sendCalls` with the `permissions` capability, which MAY include the session with a `permissionsContext`. For example:
...
Wallet that don’t support `wallet_grantPermissions` SHOULD return error message if the JSON-RPC method is called.
...
Malicious DApps could pose as legitimate applications and trick users into granting broad permissions. Wallets should always clearly display the permissions to users and warn them against granting dangerous permissions.

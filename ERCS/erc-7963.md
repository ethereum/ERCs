...
description: ERC-20 extension with oracle-permissioned transfers validated via zero-knowledge proofs of payment instructions
status: Draft
...
This EIP extends [ERC-20](./eip-20.md) tokens with oracle-permissioned transfers validated by zero-knowledge proofs. Token transfers are only valid when an external "Transfer Oracle" pre-approves them using off-chain payment instructions in a standardized JSON format, proven on-chain via ZK proofs.
...
A complete reference implementation is available in the assets directory: [`../assets/erc-7963/eip-permissioned-erc20`](../assets/erc-7963/eip-permissioned-erc20)
...
## Appendix: Financial System Interoperability (Informative)

This section is **non-normative** and provided for informational purposes only.

### ISO 20022 Compatibility

The payment instruction format specified in this ERC is designed to be compatible with ISO 20022 pain.001 (Customer Credit Transfer Initiation) messages, enabling interoperability with institutional payment systems. Financial institutions that use ISO 20022 can map their existing messages to the JSON format defined in this standard.

**Important**: ISO 20022 is a proprietary standard maintained by ISO. This ERC does **NOT require** implementers to purchase or reference ISO 20022 documentation. The payment instruction format is fully specified within this document using only freely accessible standards (JSON, RFC 8785, ISO 8601, ISO 4217).

### Mapping Traditional Payment Messages

Traditional payment system messages can be converted to ERC-7963 payment instructions:

**SWIFT MT103 → ERC-7963:**
| SWIFT Field | ERC-7963 Field |
|-------------|----------------|
| Field 20 (Reference) | `messageId` |
| Field 50 (Ordering Customer) | `paymentInfo.debtor` |
| Field 59 (Beneficiary) | `paymentInfo.creditor` |
| Field 32A (Amount) | `paymentInfo.amount` |

**ACH/NACHA → ERC-7963:**
| ACH Field | ERC-7963 Field |
|-----------|----------------|
| Trace Number | `messageId` |
| Company Name/ID | `paymentInfo.debtor` |
| Receiver Account | `paymentInfo.creditor` |
| Amount | `paymentInfo.amount` (in milli-units) |

**SEPA Credit Transfer → ERC-7963:**
| SEPA Field | ERC-7963 Field |
|------------|----------------|
| End-to-End ID | `messageId` |
| Debtor IBAN | `paymentInfo.debtor.identifier` |
| Creditor IBAN | `paymentInfo.creditor.identifier` |
| Instructed Amount | `paymentInfo.amount` |

### Alternative Payment Instruction Formats

Implementers **MAY** use alternative payment instruction schemas (proprietary formats, FIX protocol, etc.) as long as they contain the required fields specified in the "Payment Instruction Message Format" section. The ZK proof system only needs to verify that:
1. The message contains debtor/creditor identifiers
2. The amount is within specified bounds
3. All fields are cryptographically bound to the same message via Merkle proofs

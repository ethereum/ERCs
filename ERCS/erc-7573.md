...
Confirms token transfer, locks the token, and emits `TransferConfirmed` and `TokenLocked` events.
...
of tokens is locked (transferred from `from` to the smart contract), `TransferConfirmed` is emitted, and
`TokenLocked(id)` is emitted after the lock succeeds.

`TokenLocked` is the flow-independent signal that the token has actually been locked. It MUST be
emitted exactly once for a transfer leg, after the successful locking transition. Failed, pending,
cancelled, and idempotent no-op attempts MUST NOT emit it.
...
The resulting transition confirms and locks the asset and MUST emit `TokenLocked(id)` exactly once, after the token has been locked. Failed attempts, cancelled acknowledgements, and idempotent retries MUST NOT emit it. It does not complete the terminal transfer, which remains the responsibility of `transferWithKey`.

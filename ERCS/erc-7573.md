...
Neither method infers a participant from `msg.sender`. The caller is a submitter only;
implementations MUST authorize it separately from the explicit participant fields.
...
immutable inception, and the implementation MUST authorize the submitter separately from the
explicit participants. Cancellation does not permit the `id` to be reused for another transfer.
...
The interface does not infer either participant from `msg.sender`; the implementation authorizes the submitter separately.
...
Neither party is inferred from `msg.sender`. The caller is a submitter only, and implementations
MUST authorize it separately from the explicit participant fields. Because `id` identifies one
...
stored asynchronous callback, or `address(0)` for a synchronous inception. The interface does not
infer either participant from `msg.sender`; the implementation MUST authorize the submitter
separately and validate the lifecycle state.
...
match the immutable completed inception. The implementation MUST authorize the submitter
separately from the explicit participant fields. If these preconditions are met and a valid call to `transferAndDecrypt` has not been issued before,
...
The frozen group definition MUST establish which submitters are authorized to finalize the group
via `transferAndDecrypt`. That authorization is separate from the explicit transfer participants;
neither `from` nor `to` is inferred from `msg.sender`.

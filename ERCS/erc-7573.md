...
The parameter `id` is an identifier of the trade. The parameter `from` is the address of the seller (the address of the buyer is message.sender).
...
The parameter `id` is an identifier of the trade. The parameter `to` is the address of the buyer (the address of the seller is message.sender).
...
matches that of a previous call to `inceptTransfer`, and the balance is sufficient, the corresponding `anount`
...
The following methods specify the functionality of the smart contract implementing
the conditional decryption. For further information, please also look at the interface documentation `IDecryptionContract.sol`.
...
The parameter `keyEncryptedFailure` is an encryption of the key and will be decrypted if the transfer failed in a call to `transferAndDecrypt` or if `cancelAndDecrypt`is successful.
...
The method will not decrpt any key and not perfrom a transfer of a payment if the values (`id`, `amount`, `from` `to`, `keyEncryptedSuccess`, `keyEncryptedFailure`)
...
with the exact same arguments an cancels this specific transfer.

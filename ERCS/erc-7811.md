...
`assetFilter` is **OPTIONAL** field that accepts a list of assets identifiers. Each asset identifier is an object that contains `address` and `type` fields and is scoped by `chainId`, where ChainId **MUST** be a valid [EIP-155](https://eips.ethereum.org/EIPS/eip-155) chainId.
...
`chainFilter` is an **OPTIONAL** field that specifies an array of chain ids, where each value in the array **MUST** be a valid [EIP-155](https://eips.ethereum.org/EIPS/eip-155) chainId
...
The key **MUST** be [EIP-155](https://eips.ethereum.org/EIPS/eip-155) chainId
...
- **`erc20`:** For [ERC-20](./erc-20.md) tokens
- **`erc721`:** For [ERC-721](./erc-721.md) tokens (NFTs)
...
If the wallet supports [ERC-5792](https://eips.ethereum.org/EIPS/eip-5792) wallet **SHOULD** respond on `wallet_getCapabilities` request using the `assetDiscovery` key. Value should be an object with `supported` key and value `true`

...
If the `assetFilter` field is provided, the wallet **MUST** only return the assets specified within it, effectively disregarding the `assetTypeFilter` and `chainFilter` fields as they are already implicitly defined within the `assetFilter`.

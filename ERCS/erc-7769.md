...
When providing the `eip7702Auth` tuple, the fields `factory` and `factoryData` have a modified behaviour.

The `factory` field must be set to exactly the `INITCODE_EIP7702_MARKER = 0x7702` flag.
Passing his flag instructs the `EntryPoint` contract to verify the `sender` address contains a valid EIP-7702 authorization.

The `factoryData` value is passed directly to the `sender` contract instead of the `factory` contract.
This is done in a separate frame, meaning the `sender` contract will be called two times during validation.

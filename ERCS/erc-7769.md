...
Defines new JSON-RPC API methods which enable [ERC-4337](./erc-4337.md) wallets to communicate with `UserOpeation` mempool
...
* in case of failure, MUST return an `error` result object, with `code` and `message`.\
  ...
  * **code: -32502** - transaction rejected because of [ERC-7562](./erc-7562.md) opcode validation rule violation
...
On networks with [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) activated, the `UserOperation` object may also contain an `eip7702Auth` tuple.

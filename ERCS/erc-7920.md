...
2. Hash message `mₓ` using EIP-712's `encode` to get the leaf node and verify the Merkle proof against
   the Merkle root:
   ```
   isValid = verifyMerkleProof(encode(mₓ), merkleProof, merkleRoot)
   ```
...
When the number of message is one, `eth_signCompositeTypedData` produces the same signature as `eth_signTypedData_v4` since `merkleRoot == leaf` in this scenario. This allows `eth_signCompositeTypedData` to be a drop-in replacement for `eth_signTypedData_v4`.

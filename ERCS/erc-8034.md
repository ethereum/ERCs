...
- Supports optional [EIP-712](https://eips.ethereum.org/EIPS/eip-712) signature-based configuration to streamline marketplace or owner-driven updates.
...
The `IRNFTRoyalty` interface defines the royalty distribution for rNFTs and MUST inherit `ERC165` so that supporting contracts can advertise compliance via [ERC-165](./erc-165.md):

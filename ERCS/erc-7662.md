...
The interface `IERC7662Registry` is defined as follows:

```solidity
// interfaceId 0xcd691053
interface IERC7662Registry {
  /**
...
Any contract created using a ERC7662Registry SHOULD implement the IERC7662Contract interface:

```solidity
interface IERC7662Contract {
  /**
...
or both. This flexibility makes [ERC-6551](./eip-6551.md) accounts compatible with ERC7662 out-of-the-box.
...
```solidity
contract ERC7662Registry is IERC7662Registry {
  /**
  ...
  /// @dev Returns true if interfaceId is IERC7662Registry's interfaceId
...
Another difference compared to ERC6551Registry is that this implementation is IERC165 compatible and should confirm that it supports the IERC7662Registry interface. The reason is that, while the ERC6551Registry is a singleton and is supposed to be deployed a single time on every chain, an ERC7662Registry can be deployed multiple times on the same chain, and the support of the interface is essential to ensure that the contract is indeed an ERC7662Registry.


### ERC7662Contract

A simple implementation of IERC7662Contract:

```solidity
contract ERC7662Contract is IERC7662Contract {

...
Principal tokens represent ownership of an underlying [EIP-20](./erc-20.md) token at a future timestamp.

This specification is an extension on the [EIP-20](./erc-20.md) token that provides basic functionality for depositing
and withdrawing tokens and reading balances and the [EIP-2612](./erc-2612.md) specification that provides
[EIP-712](https://eips.ethereum.org/EIPS/eip-712) signature based approvals.
...
All Principal Tokens (PTs) MUST implement [EIP-20](./erc-20.md) to represent ownership of future underlying redemption.
If a PT is to be non-transferrable, it MAY revert on calls to `transfer` or `transferFrom`.
The [EIP-20](./erc-20.md) operations `balanceOf`, `transfer`, `totalSupply`, etc. operate on the Principal Token balance.

All Principal Tokens MUST implement [EIP-20](./erc-20.md)'s optional metadata extensions.
...
All Principal Tokens MAY implement [EIP-2612](./erc-2612.md) to improve the UX of approving PTs on various integrations.

### Definitions:

- underlying: The token that Principal Tokens are redeemable for at maturity.
  Has units defined by the corresponding [EIP-20](./erc-20.md) contract.
...
MUST support a withdraw flow where the principal tokens are burned from `holder` directly where `holder` is `msg.sender` or `msg.sender` has [EIP-20](./erc-20.md) approval over the principal tokens of `holder`.
...
[EIP-20](./erc-20.md) is enforced as implementation details such as token approval and balance calculation directly carry over. This standardization makes Principal Tokens immediately compatible with all [EIP-20](./erc-20.md) use cases in addition to EIP-5095.
...
This EIP draws heavily on the design of [EIP-4626](./erc-4626.md) because technically Principal Tokens could be described as a subset of Yield Bearing Vaults, extended with a `maturity` variable and restrictions on the implementation. However, extending [EIP-4626](./erc-4626.md) would force PT implementations to include methods (namely, `mint` and `deposit`) that are not necessary to the business case that PTs solve. It can also be argued that partial redemptions (implemented via `withdraw`) are rare for PTs.
...
This EIP is fully backward compatible with the [EIP-20](./erc-20.md) specification and has no known compatibility issues with other standards.

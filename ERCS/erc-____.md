...
A minimal L1 router only requires the `contractL1` address to which routing must be made, while the clients must ensure that the calldata is invariant under routing to another contract. One example implementation of an L1 router is given below.
...
In this example, the routing must prompt the client to build the transaction with the exact same original calldata, and submit it to the `contractL1` by calling the exact same function.

```solidity
// Function in router L1 contract
function setValue(
    bytes32 node,
    bytes32 key,
    bytes32 value
...
A minimal L2 router only requires the list of `chainId` values and the corresponding `contract` addresses, while the clients must ensure that the calldata is invariant under routing to L2. One example implementation of an L2 router in an L1 contract is shown below.
...
### New Revert Events
1. Each new storage router must submit their `StorageRoutedTo__()` identifier through an ERC track proposal referencing the current draft.

...
flowchart LR
    ...
    Delegation_Manager -.->|validate delegation w/ actions| Delegation_Manager
    Delegation_Manager --"execute delegated actions"--> Delegator(["Delegator"])
    Delegator --"executes CALLs using actions"--> Target(["Target"])
...
being invoked, and will then call the delegator to execute a call for each action in the `_actions` array.  If any
 call in the array fails, `redeemDelegation` MUST revert.
...
interface ERC7710Manager {
     ...
     * @param _actions an array of actions to be executed atomically
     */
    function redeemDelegation(bytes calldata _authority, Action[] calldata _actions) external;
}
```
...
A previous iteration of this spec defined `redeemDelegation` to only take a single `Action`, and defined a specific
...
- The execution behavior is limited to a single call, since `Action` could only encode a single call.  This makes batching
...
To solve the second issue, we decied to update `redeemDelegation` to take an array of `Action`s instead.

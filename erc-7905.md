...
description: Fungible representations of tokens bridged across domains
author: Radek Svarz (@radeksvarz), Shaito (@0xShaito), Excalibor (@excaliborr), Arjun Bhuptani (@arjunbhuptani)
discussions-to: https://ethereum-magicians.org/t/erc-7905-minimal-sovereign-bridged-token-minimal-xerc20/28427
status: Draft
...
The former and broadly communicated xERC20 proposal encompasses Lockbox wrappers, structs, and extensive configuration functions, exceeding the minimal requirements for core interoperability. This ERC presents a streamlined, focused approach, providing the essential functionality for 'xERC20 native' fungible token interoperability."
...
All _minimal XERC20_ tokens MUST implement the [ERC-20](./eip-20.md)'s `ERC20` interface.
...
2. Hitting minting limits on the destination chain. This case can be hit _after_ tokens are burned on the source chain in cases where many different minting transactions are simultaneously triggered on a given destination domain and bridge combination, leading to the bridge’s limit being saturated.
...
```solidity
    /**
     * @notice Mint tokens through a crosschain transfer by the bridge using ERC-7802 and ERC-7905 limits.

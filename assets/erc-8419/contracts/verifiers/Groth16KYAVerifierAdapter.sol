...
/// @dev snarkjs-style Groth16 verifier with 15 public signals.
interface IGroth16Verifier15 {
    function verifyProof(uint256[2] calldata a, uint256[2][2] calldata b, uint256[2] calldata c, uint256[15] calldata input)
        external
        view
        returns (bool);
}
...
///
///         signals (15), in snarkjs order (circuit outputs first, then public inputs):
...
///                                                  proof is bound to the RULE and the circuit
///                                                  checks the attestor signed this schemeId
///          [12] epoch
///          [13] admissionDomain.hi [14] admissionDomain.lo — taken from the `admissionDomain`
///                                                  argument (computed by the calling registry, never
///                                                  by the prover); the circuit derives the nullifier
///                                                  from it, so a proof made for one admitting
///                                                  registry is not valid at another and nullifier
///                                                  scopes are disjoint per (scheme, domain)
///
...
contract Groth16KYAVerifierAdapter is IKYAVerifier {
    ...
    IGroth16Verifier15 public immutable groth16;
    ...
    constructor(address groth16_, bytes32 requiredIssuerSetRoot_, uint64 epochLength_, uint64 epochGrace_) {
        groth16 = IGroth16Verifier15(groth16_);
        ...
        returns (bool ok, bytes32 subjectKey, bytes32 nullifier, uint8 level, bytes32 claimDigest, uint64 expiresAt, bytes32 anchor)
    {
        ...
        _checkEpoch(p.epoch);
        ok = _verifyGroth16(schemeId, admissionDomain, p, proof);
    ...
    function _verifyGroth16(bytes32 schemeId, bytes32 admissionDomain, Pub memory p, bytes calldata proof) internal view returns (bool) {
        (uint256[2] memory a, uint256[2][2] memory b, uint256[2] memory c) =
            abi.decode(proof, (uint256[2], uint256[2][2], uint256[2]));
        return groth16.verifyProof(a, b, c, signals(schemeId, admissionDomain, p));
    }

    /// @notice Public-signal vector for (schemeId, admissionDomain, p), in the circuit's order. Exposed for tooling.
    function signals(bytes32 schemeId, bytes32 admissionDomain, Pub memory p) public pure returns (uint256[15] memory input) {
        ...
        input[12] = p.epoch;
        (input[13], input[14]) = _split(admissionDomain);
    }

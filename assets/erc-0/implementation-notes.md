...
- **Pass action links.** Holders onboard by email and are never prompted per action, so no per-action signature exists at tap time: the capability configuration. Each link is a capability URL bound to the pass and the action. A tap resolves the link, performs the fresh read, and submits the action with a session key the owner authorized once with a signature. The session key's authority is scoped on chain to the single state-changing function the pass exposes. The forwarding residual the specification discloses therefore stays bounded: a forwarded link can repeat that action for the owner, and cannot transfer, burn, or approve the token, change who may act for it, or reach any other function. The repetition is bounded on chain as well. Each care spends the care cost from the token's own account through the issuer's appointed spender, the tap-to-pay path the links use, and the account caps that spender at a per-transaction amount and a daily allowance (20 cares per 24 hour window at the shipped configuration; both are parameters set when the spender is appointed). The owner's own signed execution through the account is not capped, and no link reaches it. So the most a forwarded link can do before the owner rotates it is spend that allowance on care, the one thing the account exists to spend on. "Cannot touch the token" therefore holds for ownership and entitlement; what the token's account holds is exposed only up to that cap.
...
- **Transfer.** Holders can also give a pet away from the account page. The transfer is an ERC-721 `transferFrom` signed by the owner's own account, never by the care session key, whose authority is scoped to the care function and cannot move the token. The transfer is gas-sponsored only when it matches a gift the server prepared for that owner, token, and recipient, and only when a fresh read shows the signer is still the owner; the sponsorship webhook refuses every other transfer, approval, or operator call on the token contract. Landing the transfer revokes the giver's care session at once; the pass secret rotates when the recipient first claims. A new owner claims with a signature over a server-issued single-use nonce. The claim, like a proven manifest request from a new owner, rotates the per-pass secret, which invalidates every action link and pass download URL derived from it and triggers the keepsake behavior described above.

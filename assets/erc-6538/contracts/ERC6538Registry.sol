        ...
        unchecked {
            dataHash = keccak256(
                abi.encodePacked(
                    ...
                    DOMAIN_SEPARATOR(),
                    keccak256(
                        abi.encode(ERC6538REGISTRY_ENTRY_TYPE_HASH, schemeId, stealthMetaAddress, nonceOf[registrant]++)
                    )
                )
            );
        }
        ...
        if (
            (
                (recoveredAddress == address(0) || recoveredAddress != registrant)
                    && (IERC1271(registrant).isValidSignature(dataHash, signature) != IERC1271.isValidSignature.selector)
            )
        ) {
